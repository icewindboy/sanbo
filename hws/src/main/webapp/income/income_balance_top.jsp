<%--进口结算管理--%><%@page contentType="text/html; charset=UTF-8" %><%@ include file="../pub/init.jsp"%>
...
function toDetail(){
  // 转到主从明细
  parent.location.href='income_balance_edit.jsp';
}
function masterAdd(){
  // 转到主从明细
  parent.location.href='income_balance_finance.jsp';
}
function showDetail(masterRow){
  //单击主表的一行激活的操作
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='income_balance_buttom.jsp?operate=<%=In_BuyBalanceBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<%
  ...
  EngineDataSet list = In_BuyBalanceBean.getMaterTable();//主表数据集
  HtmlTableProducer table = In_BuyBalanceBean.masterProducer;//主表数据的表格打印
  String curUrl = request.getRequestURL().toString();
  RowMap m_RowInfo =In_BuyBalanceBean.getMasterRowinfo();
  //提单类型
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">进口结算管理</TD>
  </TR>
</TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
    ...
    <tr>
     ...
     <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
      ...
      <td class="td" nowrap align="right">
        ...
        <%}%>
        <%if(In_BuyBalanceBean.retuUrl!=null){String ret = "parent.location.href='"+In_BuyBalanceBean.retuUrl+"'";%>
        <input name="button22" type="button" class="button" onClick="parent.location.href='<%=In_BuyBalanceBean.retuUrl%>'" value="返回(C)" onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <td nowrap>
     ...
     String add = "sumitForm("+In_BuyBalanceBean.MASTER_ADD+",-1)";
    %>
    <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=In_BuyBalanceBean.MASTER_ADD%>,-1)" src="../images/add.gif" border="0">
    ...
    <%
     ...
     BigDecimal  t_whxje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        String loginId=In_BuyBalanceBean.loginId;
        ...
        String togo = (over)?String.valueOf(Operate.EDIT):In_BuyBalanceBean.MASTER_EDIT;

        boolean cansubmit=false;
        boolean submitType = In_BuyBalanceBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
        ...
        if(submitType&&czyid.equals(loginId))
          ...
          cansubmit=true;
        String clickstring ="";
        //if(In_BuyBalanceBean.canVerify)
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=togo%>,<%=list.getRow()%>)" onClick="<%=clickstring%>">
       <td <%=rowClass%> align="center" nowrap>
        ...
        <%}else{%><input name="image2" class="img" type="image" title='<%=loginBean.hasLimits(pageCode, op_edit) ?"修改" :"查看"%>' onClick="sumitForm(<%=In_BuyBalanceBean.MASTER_EDIT%>,<%=list.getRow()%>)" src="../images/edit.gif" border="0">
        <%}%>
       <%if(isInit&&cansubmit){%>
      ...
      <%if(isCancer){%>
            <input name="image3" class="img" type="image"  title='取消审批' onClick="sumitForm(<%=In_BuyBalanceBean.CANCER_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
     <%if((rowzt.equals("1"))&&iscanover){%>
            <input name="image3" class="img" type="image"   title='核销完成' onClick="if(confirm('完成后不能修改,确认要完成吗？'))sumitForm(<%=In_BuyBalanceBean.BALANCE_OVER%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
      </td>
...
<%
  if(!In_BuyBalanceBean.masterIsAdd()){
    int row = In_BuyBalanceBean.getSelectedRow();
    if(row >= 0)
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><input type="hidden" name="dwtxid" value='<%=In_BuyBalanceBean.getFixedQueryValue("dwtxid")%>'>
                <input type="text" name="buyerName" value='<%=In_BuyBalanceBean.getFixedQueryValue("buyerName")%>' style="width:130" class="edline" readonly>
             ...
             <td width="120" class="td">
              <%String khlx = In_BuyBalanceBean.getFixedQueryValue("khlx");%>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="rq$a" value='<%=In_BuyBalanceBean.getFixedQueryValue("rq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="rq$b" value='<%=In_BuyBalanceBean.getFixedQueryValue("rq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <td noWrap class="td"><input type="text" name="je$a" value='<%=In_BuyBalanceBean.getFixedQueryValue("je$a")%>' maxlength='100' style="width:131" class="edbox"></td>
              <td noWrap class="td" align="center">--</td>
              <td noWrap class="td"><input type="text" name="je$b" value='<%=In_BuyBalanceBean.getFixedQueryValue("je$b")%>' maxlength='100' style="width:131" class="edbox"></td>
            </TR>
            <TR>
              <td noWrap class="td" align="center">单据号</td>
              <td noWrap class="td"><input type="text" name="djh$a" value='<%=In_BuyBalanceBean.getFixedQueryValue("djh$a")%>' maxlength='100' style="width:131" class="edbox"></td>
              <td noWrap class="td" align="center">--</td>
              <td noWrap class="td"><input type="text" name="djh$b" value='<%=In_BuyBalanceBean.getFixedQueryValue("djh$b")%>' maxlength='100' style="width:131" class="edbox"></td>
            </TR>
            <TR>
              <TD class="td" nowrap>部门</TD>
              <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
              <%=deptBean.getList(In_BuyBalanceBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>单据性质</TD>
              <TD nowrap class="td">
                <pc:select name="djxz" addNull="1" style="width:130" >
                <%=In_BuyBalanceBean.listToOption(lists, opkey.indexOf(In_BuyBalanceBean.getFixedQueryValue("djxz")))%>
               </pc:select>
              </TD>
             </TR>
            <TR>
              ...
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:100">
                  <%=personBean.getList(In_BuyBalanceBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             <td noWrap class="td" align="center">&nbsp;制单人&nbsp;</td>
              <td noWrap class="td"><input type="text" name="czy" value='<%=In_BuyBalanceBean.getFixedQueryValue("czy")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
           <TR>
              <TD class="td" nowrap>状态</TD>
              <TD colspan="3" nowrap class="td">
                <%
                  String [] zt = In_BuyBalanceBean.zt;
                  String zt0="";
                  String zt9="";
                  String zt1="";
                  String zt8="";
