...
%><%
  ...
  String SYS_APPROVE_ONLY_SELF =buyInBean.SYS_APPROVE_ONLY_SELF;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
...
function showDetail(masterRow){
  ...
  parent.bottom.location.href='buy_in_bottom.jsp?operate=<%=buyInBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">采购进货单列表</TD>
  </TR></TABLE>
<%String retu = buyInBean.doService(request, response);
  ...
  EngineDataSet list = buyInBean.getMaterTable();
  HtmlTableProducer table = buyInBean.masterProducer;
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tbcontrol" width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" nowrap align="right">
         ...
         <%--
         <input name="search4" type="button" class="button" onClick="sumitForm(<%=buyInBean.THD_ADD%>,-1)" value="新增退货单(T)" onKeyDown="return getNextElement();">
          <pc:shortcut key="t" script='<%="sumitForm("+ buyInBean.THD_ADD +",-1)"%>'/>
         --%>
         <%}%>
        ...
        <%if(buyInBean.retuUrl!=null){%><input name="button22" type="button" class="button" onClick="parent.location.href='<%=buyInBean.retuUrl%>'" value=" 返回(C)" onKeyDown="return getNextElement();">
        <% String back ="parent.location.href='"+buyInBean.retuUrl+"'" ;%>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%BigDecimal t_zsl = new BigDecimal(0), t_zje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
       ...
       //isApproveOnly = isApproveOnly && loginId.equals(zdrid);//如果时只有制单人才可以提交审批。制单人等于登录人才显示
        ...
        boolean isReference = buyInBean.isReference(jhdid);//进货单是否被入库，如果入库颜色要变化
        ...
        boolean isCancel=buyInBean.hasReferenced("VW_KC_CGSFDJ","jhdid",list.getValue("jhdid"));//是否被入库单用，如果被引用，则不能取消审批，否则，可以取消审批
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
     ...
     <%if(isCancel){%><input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=buyInBean.CANCLE_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
      <%if((zt.equals("2") && hasInstore)||(zt.equals("1")&&sfrk.equals("2"))){%><input name="image3" class="img" type="image" title='入库确认' onClick="if(confirm('是否入库确认？'))sumitForm(<%=buyInBean.INSTORE_CONFIRM%>,<%=list.getRow()%>)" src="../images/edit.old.gif" border="0"><%}%>
     <%--if(isComplete){%><input name="image5" class="img" type="image" title='完成' onClick="if(confirm('是否强制完成该纪录？'))sumitForm(<%=buyInBean.COMPLETE%>,<%=list.getRow()%>)" src="../images/edit.old.gif" border="0"><%}--%></td>
...
<%if(!buyInBean.masterIsAdd()){
    int row = buyInBean.getSelectedRow();
    if(row >= 0)
  ...
  <div class="queryPop" id="fixedQuery" name="fixedQuery">
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
           <TR>
            ...
            %>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="jhdbm" name="jhdbm" value='<%=buyInBean.getFixedQueryValue("jhdbm")%>' maxlength='32' onKeyDown="return getNextElement();"></TD>
             <%-- <TD class="td" nowrap>单据类型</TD>
              <TD nowrap class="td">
                <pc:select name="djlx" addNull="1" style="width:160" >
                <%=buyInBean.listToOption(lists, opkey.indexOf(buyInBean.getFixedQueryValue("djlx")))%>
               </pc:select>
              </TD>--%>
            </TR>
            <TR>
              <TD nowrap class="td">交货日期</TD>
              <TD class="td" nowrap><INPUT class="edbox" id="jhrq$a" style="WIDTH: 130px" name="jhrq$a" value='<%=buyInBean.getFixedQueryValue("jhrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="jhrq$b" style="WIDTH: 130px" name="jhrq$b" value='<%=buyInBean.getFixedQueryValue("jhrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
                <A href="#"><IMG title=选择日期 onClick="selectDate(jhrq$b);" height=20 src="../images/seldate.gif" width=20 align=absMiddle border=0></A></TD>
            </TR>
            <TR>
            <%String dwtxid = buyInBean.getFixedQueryValue("dwtxid");
        ...
        <%--    <TR>
            <%String dwt_dwtxid = buyInBean.getFixedQueryValue("dwt_dwtxid");
           ...
           --%>
              ...
              <TD align="center" nowrap class="td">产品名称</TD>
               <td nowrap class="td" colspan="3"><input class="EDLine" style="WIDTH:330px" name="product" value='<%=buyInBean.getFixedQueryValue("product")%>' onKeyDown="return getNextElement();"readonly>
                <INPUT TYPE="HIDDEN" NAME="jhdid" value="<%=buyInBean.getFixedQueryValue("jhdid")%>"><img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProdSingleSelect('fixedQueryform','srcVar=jhdid&srcVar=product','fieldVar=cpid&fieldVar=product',fixedQueryform.jhdid.value)">
               <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="jhdid.value='';product.value='';">
            </td>
            </TR>
             <TR>
              <TD class="td" nowrap>产品编码</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="jhdid$cpbm" name="jhdid$cpbm" value='<%=buyInBean.getFixedQueryValue("jhdid$cpbm")%>' onKeyDown="return getNextElement();"></TD>
               <TD class="td" nowrap>品名规格</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="jhdid$product" name="jhdid$product" value='<%=buyInBean.getFixedQueryValue("jhdid$product")%>' onKeyDown="return getNextElement();"></TD>
            </TR>
               <TR>
             <TD class="td" nowrap>规格属性</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="jhdid$sxz" name="jhdid$sxz" value='<%=buyInBean.getFixedQueryValue("jhdid$sxz")%>' onKeyDown="return getNextElement();"></TD>
               <TD nowrap class="td">仓库</TD>
              <TD nowrap class="td"><pc:select name="storeid" addNull="1" style="width:160">
              <%=storeBean.getList(buyInBean.getFixedQueryValue("storeid"))%></pc:select>
              </TD>
              </TR>
              <TR>
              <TD class="td" nowrap>部门</TD>
              <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
         <%=deptBean.getList(buyInBean.getFixedQueryValue("deptid"))%></pc:select>
           </TD>
             </TR>
            <TR>
              <TD class="td" nowrap>状态</TD>
              <TD colspan="3" nowrap class="td">
                <%String zt = buyInBean.getFixedQueryValue("zt");%>
