...
function backList()
{
  location.href='other_payable.jsp';
}
function deptchange(){
...
<%
  ...
  engine.project.LookUp fundTypeBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_BUY_FUND_TYPE);//
  ...
  RowMap masterRow = otherPayableBean.getRowinfo();
          ...
          <tr>
            ...
            <tr>
              ...
              <tr>
                  ...
                  <td colspan="3" noWrap class="td">
                    ...
                    <input type="text" <%=edClass%> style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=corpRow.get("dwdm")%>' onchange="corpCodeSelect(this)" <%=readonly%>>
                    <input type="text" <%=edClass%> name="dwmc" onKeyDown="return getNextElement();" value='<%=corpBean.getLookupName(masterRow.get("dwtxid"))%>' style="width:200"   onchange="corpNameSelect(this)"  <%=readonly%>>
                    <%if(!isEnd){%>
                    <img style='cursor:hand' src='../images/view.gif' border=0 onClick="selectCustomer()">
                      <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value=''; sumitForm(<%=otherPayableBean.DWTXID_CHANGE%>,-1)">
                    <%}%>
                  </td>
            ...
            <tr>
                ...
                <tr>
                  ...
                  <td colspan="7" noWrap class="td"><textarea name="bz" rows="3" onKeyDown="return getNextElement();" style="width:690" <%=(zt.equals("9")||zt.equals("8")||zt.equals("1"))?"readonly":""%> ><%=masterRow.get("bz")%></textarea></td>
                </tr>
         </table>
        </td>
      </tr>
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
               <%if(!otherPayableBean.isApprove&&!otherPayableBean.isReport){%>
               <%if(!isEnd){%>
              ...
              <input type="button" class="button" value="打印(P)" onclick="location.href='../pub/pdfprint.jsp?code=shyu_otherpayable_print&operate=<%=Operate.PRINT_BILL%>&a$otherfundid=<%=masterRow.get("otherfundid")%>&src=../buy_shengyu/other_payable_edit.jsp'">
              <%}%>
              <%--if(zt.equals("1")){%>
              <input name="button" type="button" class="button" onClick="sumitForm(<%=otherPayableBean.AFTER_POST%>);" value=" 保存 "><%}--%>
...
{
  SaleProdCodeChange(document.all['prod'], obj.form.name,'srcVar=cpid&srcVar=product&srcVar=cpbm&issale=1','fieldVar=cpid&fieldVar=product&fieldVar=cpbm',obj.value,'sumitForm(<%=otherPayableBean.PRODUCT_CHANGE%>)');
}
//产品名称
function productNameSelect(obj)
{
  SaleProdNameChange(document.all['prod'], obj.form.name,'srcVar=cpid&srcVar=product&srcVar=cpbm&issale=1','fieldVar=cpid&fieldVar=product&fieldVar=cpbm',obj.value,'sumitForm(<%=otherPayableBean.PRODUCT_CHANGE%>)');
}
//选择购货单位
function selectCustomer()
{
  ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=custaddr&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=otherPayableBean.DWTXID_CHANGE%>,-1)');
}
//客户编码
function corpCodeSelect(obj)
{
  ProvideCodeChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=custaddr','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',obj.value,'sumitForm(<%=otherPayableBean.DWTXID_CHANGE%>,-1)');
}
//客户名称
function corpNameSelect(obj)
{
  ProvideNameChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=custaddr','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',obj.value,'sumitForm(<%=otherPayableBean.DWTXID_CHANGE%>,-1)');
}
</script>
<%//&#$
if(otherPayableBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>