...
<SCRIPT LANGUAGE="javascript">
  ...
  {
    var url = "buy_othertype_edit.jsp?operate="+oper+"&rownum="+rownum;
...
<%EngineDataSet list = billTypeSetBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <TABLE WIDTH="600" BORDER=0 CELLSPACING=0 CELLPADDING=0 align="center">
    <TR>
      <TD align="right">
        <%if(billTypeSetBean.retuUrl!=null){%>
        <input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=billTypeSetBean.retuUrl%>'" value=" 返回 "border="0">
        <%}%>
      </TD>
    </TR>
  </TABLE>
  <table id="tableview1" width="600" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      ...
      <td class="td" align="left" nowrap><%=list.getValue("funditemcode")%></td>
    </tr>
    <%  i++;
        list.next();
      }
