...
<form name="form1" action="<%=curUrl%>" method="POST" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table BORDER="0" cellpadding="1" cellspacing="3" align='center'>
   ...
   <%if(isProvider){%>
   <tr>
     <%           String aa=row.get("dwtxid");
     RowMap corpRow = corpBean.getLookupRow(row.get("dwtxid"));%>

    <td noWrap class="tableTitle">供应商</td>
    <td nowrap class="td">
    <input type="text" name="dwdm" value='<%=corpRow.get("dwdm")%>' style="width:100" class="<%=tableClass%>" onKeyDown="return getNextElement();" onchange="corpCodeSelect(this);" >
    <input type="text" name="dwmc" value='<%=corpBean.getLookupName(row.get("dwtxid"))%>' style="width:100" class="<%=tableClass%>" onKeyDown="return getNextElement();" onchange="corpNameSelect(this);" <%=readonly%>>
    ...
    <td noWrap class="tableTitle">原辅料</td>
      <td nowrap class="td">
      <%if(!isHsbj){%>
     ...
     <%if(!isHistory&&isProvider){%><img style='cursor:hand' title='单选物资' src='../images/view.gif' border=0 onClick="ProdSingleSelect('form1','srcVar=cpid&srcVar=cpbm&srcVar=product&srcVar=jldw','fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw',form1.cpid.value)">
    ...
    <tr>
        ...
        <td class="td" nowrap><input  class="<%=tableClass%>" name="sxz" value='<%=propertyBean.getLookupName(row.get("dmsxid"))%>' style="width:200" onchange="if(form1.cpid.value==''){alert('请先输入产品');return;}propertyNameSelect(this,form1.cpid.value)" onKeyDown="return getNextElement();" <%=readonly%>>
        <input type="hidden" id="dmsxid" name="dmsxid" value="<%=row.get("dmsxid")%>">
        <%if(isEdit){%>
        <%if(!isHistory&&isProvider){%><img style='cursor:hand' src='../images/view.gif' border=0 onClick="if(form1.cpid.value==''){alert('请先输入产品');return;}if('<%=prodRow.get("isprops")%>'=='0'){alert('该物资没有规格属性');return;}PropertySelect('form1','dmsxid','sxz',form1.cpid.value)">
  ...
  <%--      <td noWrap class="tableTitle">货物情况</td>
      <td noWrap class="td">
    <%String hyf = row.get("hyf");%>
         <input type="checkbox" name="CH" value="1" <%=hyf.equals("1")?" checked" :""%>>
         含运费
      <%String hp = row.get("hp");%>
         <input type="checkbox" name="CH" value="2" <%=hp.equals("2")?" checked" :""%>>
         含票
      <%String hg = row.get("hg");%>
         <input type="checkbox" name="CH" value="3" <%=hg.equals("3")?" checked" :""%>>
         含硅
      </td>  --%>
  </tr>
 <%-- <tr>
      <td noWrap class="tableTitle">基本质量情况</td>
      <td noWrap class="td"><textarea name="textfield" NAME="jbzlqk"  cols="30" VALUE="<%=row.get("jbzlqk")%>" onKeyDown="return getNextElement();" <%=readonly%>></textarea>
     <%-- <INPUT TYPE="textarea" NAME="gyslh" VALUE="<%=row.get("jbzlqk")%>" rows = "10" cols = "60" style="width:100" MAXLENGTH="<%=ds.getColumn("jbzlqk").getPrecision()%>" class="<%=tableClass%>" onKeyDown="return getNextElement();" <%=readonly%>>
      </td>
  </tr>--%>
    <tr>
      <td noWrap class="tableTitle">基本质量情况</td>
      <td  noWrap colspan="4" class="td"><INPUT TYPE="TEXT"  NAME="jbzlqk"  VALUE="<%=row.get("jbzlqk")%>" style="width:200" MAXLENGTH="<%=ds.getColumn("jbzlqk").getPrecision()%>" class="<%=tableClass%>" onKeyDown="return getNextElement();" <%=readonly%>>
      </td>
  </tr>

  <tr>
      <td noWrap class="tableTitle">采购报价</td>
      <td noWrap colspan="4" class="td"><INPUT TYPE="TEXT" NAME="bj" VALUE="<%=row.get("bj")%>" style="width:200" MAXLENGTH="<%=ds.getColumn("bj").getPrecision()%>" class="<%=tableClass%>" onKeyDown="return getNextElement();" <%=readonly%>>
      </td>
  </tr>
  <tr>
