...
%><%
  ...
  String pageCode = "other_instore_check";
  buyCheckBean.billType="9";
  ...
  String SYS_APPROVE_ONLY_SELF =buyCheckBean.SYS_APPROVE_ONLY_SELF;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
...
function toDetail(){
  parent.location.href='other_instore_check_edit.jsp';
}
function showDetail(masterRow){
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='other_instore_check_bottom.jsp?operate=<%=buyCheckBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">其它入库检验</TD>
  </TR></TABLE>
<%String retu = buyCheckBean.doService(request, response);
  ...
  EngineDataSet list = buyCheckBean.getMaterTable();
  HtmlTableProducer table = buyCheckBean.masterProducer;
  String curUrl = request.getRequestURL().toString();
  String loginId = buyCheckBean.loginId;
  //
%>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tbcontrol" width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" nowrap align="right"><%if(hasSearchLimit){%><input name="search2" type="button" class="button" onClick="showFixedQuery()" value=" 查询(Q)" onKeyDown="return getNextElement();">
        ...
        <%if(buyCheckBean.retuUrl!=null){%><input name="button22" type="button" class="button" onClick="parent.location.href='<%=buyCheckBean.retuUrl%>'" value=" 返回(C)" onKeyDown="return getNextElement();">
        <% String back ="parent.location.href='"+buyCheckBean.retuUrl+"'" ;%>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=buyCheckBean.CANCLE_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0">
      <input name="image5" class="img" type="image" title='完成' onClick="if(confirm('是否强制完成该纪录？'))sumitForm(<%=buyCheckBean.COMPLETE%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0">
      <input name="image4" class="img" type="image" title='作废' onClick="if(confirm('是否确定作废该纪录？'))sumitForm(<%=buyCheckBean.REPEAL%>,<%=list.getRow()%>)" src="../images/close.gif" border="0"></td>
      --%>
...
<%if(!buyCheckBean.masterIsAdd()){
    int row = buyCheckBean.getSelectedRow();
    if(row >= 0)
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              <%buyCheckBean.table.printWhereInfo(pageContext);%>
            </TR>
            <TR>
