...
<%if(!loginBean.hasLimits("bug_grade", request, response))
    return;
   // engine.erp.quality.B_QualityReason  BugBean=engine.erp.quality.B_QualityReason.getInstance(request);
...
<SCRIPT LANGUAGE="javascript">
  ...
  {
    var url = "bug_grade_edit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>缺陷等级</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = BugBean.getOneTable();
...
</td> <TD align="right"><%if(BugBean.retuUrl!=null){%><input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=BugBean.retuUrl%>'" value=" 返回 "border="0"><%}%></TD>
    </TR>
  </TABLE>
  <table id="tableview1" width=60%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      <td width="242" nowarp>缺陷编码</td>
      <td width="400" nowarp>缺陷名称</td>
      <td nowarp width="70">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      <td class="td" nowarp>
      &nbsp;<%
       String bugCode = list.getValue("bugCode");
          out.print(bugCode );
      %>
      </td>
      <td class="td" nowarp>&nbsp;<%=list.getValue("bugName")%></td>
      <td class="td" nowarp>
...
<div class="queryPop" id="detailDiv" name="detailDiv">
  ...
  <TABLE cellspacing=0 cellpadding=0 border=0>
    <TR>
      <TD><iframe id="interframe1" src="" width="210" height="150" marginWidth="0" marginHeight="0" frameBorder="0" noResize scrolling="no"></iframe>
      </TD>
    </TR>
  </TABLE>
</div>
