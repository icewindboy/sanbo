...
<%!
  ...
  String pageCode  = "adminicle_check";
%>
<html>
<head>
<title></title>
...
function toDetail(){
  location.href='adminicle_check_edit.jsp';
}
...
<%engine.erp.quality.B_AdminicleCheck  Adminicle_CheckBean = engine.erp.quality.B_AdminicleCheck.getInstance(request);
  ...
  EngineDataSet list = Adminicle_CheckBean.getOneTable();
  if(!loginBean.hasLimits("adminicle_check", request, response))
    return;
  String retu = Adminicle_CheckBean.doService(request, response);
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">辅料验收报告单</TD>
  </TR>
</TABLE>
<%
  ...
  String SYS_APPROVE_ONLY_SELF =Adminicle_CheckBean.SYS_APPROVE_ONLY_SELF;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
  boolean isApproveOnly = SYS_APPROVE_ONLY_SELF.equals("1") ? true : false;//true仅制单人可提交
  String loginID = Adminicle_CheckBean.loginID;
  ...
  String state=null;
  RowMap row = Adminicle_CheckBean.getRowinfo();
%>
...
<%String key = "ppdfsgg"; pageContext.setAttribute(key, list);
      ...
      <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
        ...
        <%if(Adminicle_CheckBean.retuUrl!=null){%><input name="button2" type="button" align="Right"
  class="button" onClick="location.href='<%=Adminicle_CheckBean.retuUrl%>'" value=" 返回 "border="0"><%}%></TD>
    </tr>
  </table>
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%if(isShow){%><input name="image3" class="img" type="image" title='提交审批'onClick="sumitForm(<%=Operate.ADD_APPROVE%>,<%=list.getRow()%>)" src="../images/approve.gif" border="0"><%}%>
     <%if(isCancelApprove){%><input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=Adminicle_CheckBean.CANCLE_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
      </div>
      </td>
      <td nowrap <%=rowClass%>><%=list.getValue("adminiclecheckNo")%></td><!--单据号-->
      <td nowrap <%=rowClass%>><%=corpBean.getLookupName(list.getValue("dwtxid"))%></td><!--往来单位ID-->
      <td nowrap <%=rowClass%>><%=productRow.get("cpbm")%></td><!--产品编码-->
      <td nowrap <%=rowClass%>><%=productBean.getLookupName(list.getValue("cpid"))%></td><!--品名规格-->
      <td nowrap <%=rowClass%>><%=propertyBean.getLookupName(list.getValue("dmsxid"))%></td><!--物资规格属性ID-->
      <td nowrap <%=rowClass%>><%=list.getValue("goods_date")%></td><!--到货日期-->
      <td nowrap <%=rowClass%>><%=list.getValue("tot_num")%></td><!--总数量-->
      <td nowrap <%=rowClass%>><%=list.getValue("facies")%></td><!--外观-->
      <td nowrap <%=rowClass%>><%=list.getValue("sample_number")%></td><!--抽样数-->
      <td nowrap <%=rowClass%>><%=list.getValue("tryout_date")%></td><!--试用日期-->
      <td nowrap <%=rowClass%>><%
      ...
      %></td><!--试用结论-->
      <td nowrap <%=rowClass%>><%=deptBean.getLookupName(list.getValue("deptid"))%></td><!--试用部门-->
     <td  nowrap <%=rowClass%> align='center'>
     <a href='javascript:'onClick="openUrlOpt1('../pub/approve_result.jsp?operate=0&project=adminicle_check&id=<%=list.getValue("adminiclecheckid")%>')">
     <%=stateval%></a> </td><%--审批状态--%>
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              <%Adminicle_CheckBean.table.printWhereInfo(pageContext);%>
            </TR>
            <TR>
