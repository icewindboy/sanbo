...
<%String retu = buyOrderBean.doService(request, response);
  ...
  //if(zt.equals("0"))
    //isCanAmend = engine.baseinfo.BasePublicClass.isRework(list,"sjjhl");//取消审批后从表实际进货量如果有一条大于零，主表不能修改。
  boolean isEnd =true;// buyOrderBean.isReport || buyOrderBean.isApprove || (!buyOrderBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  ...
  <table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="center" width="760">
    <tr valign="top">
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            <td class="activeVTab">采购合同(<%=title%>)</td>
          </tr>
        </table>
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" width="100%" bgcolor="#f0f0f0">
                ...
                <%RowMap corpRow = corpBean.getLookupRow(masterRow.get("dwtxid"));%>
				  ...
				  <input type="hidden" name="dwtxid" value='<%=masterRow.get("dwtxid")%>' >
                   <input type="text" name="dwmc" value='<%=corpBean.getLookupName(masterRow.get("dwtxid"))%>' style="width:190" <%=edClass%> onKeyDown="return getNextElement();" onchange="corpNameSelect(this);" <%=masterReadonly%>>
                    <%if(!isEnd && isCanAmend && isHasDeptLimit){%>
                    <img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=buyOrderBean.ONCHANGE%>)');"><img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';sumitForm(<%=buyOrderBean.ONCHANGE%>)">
                    <%}%>
                  </td>
                ...
                <td  noWrap class="tdTitle"><%=masterProducer.getFieldInfo("personid").getFieldname()%></td>
                  ...
                  <td  noWrap class="td">
                    <%String sumit = "if(form1.wbid.value!='"+masterRow.get("wbid")+"')sumitForm("+buyOrderBean.WB_ONCHANGE+")";%>
                    <%if(isEnd || !isCanAmend || !isHasDeptLimit) out.print("<input type='text' value='"+wbRow.get("mc")+"' style='width:85' class='edline' readonly>");
                    else {%>
                 ...
                 <td noWrap class="tdTitle">汇率</td>
                  <td noWrap class="td"><input type="text" name="hl" value='<%=masterRow.get("hl")%>' maxlength='<%=ds.getColumn("hl").getPrecision()%>' style="width:100" <%=edClass%> onKeyDown="return getNextElement();" onchange="hl_onchange();"<%=masterReadonly%>></td>
               </tr>
                <%/*打印用户自定义信息*/
                int j=0;
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      BigDecimal t_sl = new BigDecimal(0), t_je = new BigDecimal(0), t_ybje = new BigDecimal(0),t_sjjhl = new BigDecimal(0);
                      int i=0;
                      RowMap detail = null;
                      for(; i<detailRows.length; i++)   {
                        ...
                        if(zt.equals("0"))
                          isCanRework =true; //engine.baseinfo.BasePublicClass.isRevamp(list, "sjjhl", i);//合同状态在未审状态时，判断该条纪录是否能被修改
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap><input type="text" <%=detailClass_r%>  onKeyDown="return getNextElement();" id="ybje_<%=i%>" name="ybje_<%=i%>" value='<%=detail.get("ybje")%>' maxlength='<%=list.getColumn("ybje").getPrecision()%>' onchange="je_onchange(<%=i%>, true)" <%=readonly%>></td>
                  ...
                  %>
                      ...
                      <tr id="rowinfo_end">
                        ...
                        <td align="right" class="td"><input id="t_ybje" name="t_ybje" type="text" class="ednone_r" style="width:100%" value='<%=t_ybje%>' readonly></td>
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             ...
             <input name="btnback" class="button" type="button" value="引入申请单(W)" style="width:100" onClick="if(form1.dwtxid.value==''){alert('请选择往来单位'); return;}ApplyMultiSelect('form1','srcVar=importapply&dwtxid='+form1.dwtxid.value)" border="0">
             ...
             <input name="btnback" class="button" type="button" value="供应商报价(B)" style="width:100" onClick="if(form1.dwtxid.value==''){alert('请选择往来单位'); return;}goodsMultiSelect('form1','srcVar=select&dwtxid='+form1.dwtxid.value)" border="0">
             ...
             <%}%>
              ...
              <%if(isCanDelete && isCanAmend && !buyOrderBean.isReport){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);" value=" 删除(D)">
...
<%=buyOrderBean.adjustInputSize(new String[]{"cpbm", "product","sl", "dj", "je","ybje","sxz","gyszyh","bz"}, "form1", detailRows.length)%>
  ...
  if(confirm('是否删除该记录？'))
    ...
    function importApply(){
      if(form1.dwtxid.value==''){alert('请选择往来单位'); return;}ApplyMultiSelect('form1','srcVar=importapply&dwtxid='+form1.dwtxid.value);
  }
  function corpPrice(){
      if(form1.dwtxid.value==''){alert('请选择往来单位'); return;}goodsMultiSelect('form1','srcVar=select&dwtxid='+form1.dwtxid.value);
  }
    ...
    function je_onchange(i, isBigUnit)
    {
      ...
      if(isBigUnit && hlObj.value!="" && !isNaN(hlObj.value) && hlObj!="0" && slObj.value!="" && !isNaN(slObj.value) && slObj.value!="0")
        djObj.value = formatPrice(parseFloat(jeObj.value)* parseFloat(hlObj.value) / parseFloat(slObj.value));
      cal_tot('je');
      cal_tot('ybje');
    }
