...
<%if(!loginBean.hasLimits("verify_method", request, response))
    return;
  //engine.store.B_StoreBillKind storeMethodBean  =  engine.store.B_StoreBillKind.getInstance(request);
...
<SCRIPT LANGUAGE="javascript">
  ...
  {
    var url = "verify_method_edit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>检验方法管理</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = storeMethodBean.getOneTable();
...
</td> <TD align="right"><%if(storeMethodBean.retuUrl!=null){%><input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=storeMethodBean.retuUrl%>'" value=" 返回 "border="0"><%}%></TD>
    </TR>
  </TABLE>
  <table id="tableview1" width="60%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      <td width="242" nowarp>代码</td>
      <td width="400" nowarp>检验方法</td>
      <td nowarp width="60"><input name="image" class="img" type="image" title="新增(ALT+A)" onClick="showInterFrame(<%=Operate.ADD%>,-1)" src="../images/add.gif" border="0">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      <td class="td" nowarp>
      &nbsp;<%
       String code = list.getValue("code");
          out.print(code);
      %>
      </td>
      <td class="td" nowarp>&nbsp;<%=list.getValue("checkmethod")%></td>
      <td class="td" nowarp>
...
<div class="queryPop" id="detailDiv" name="detailDiv">
  ...
  <TABLE cellspacing=0 cellpadding=0 border=0>
    <TR>
      <TD><iframe id="interframe1" src="" width="220" height="150" marginWidth="0" marginHeight="0" frameBorder="0" noResize scrolling="no"></iframe>
      </TD>
    </TR>
  </TABLE>
</div>
