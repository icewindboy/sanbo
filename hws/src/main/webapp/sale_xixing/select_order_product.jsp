...
%><%
  engine.erp.sale.xixing.ImportOrderProduct importOrderProductBean = engine.erp.sale.xixing.ImportOrderProduct.getInstance(request);//创建实例
  String retu = importOrderProductBean.doService(request, response);
  ...
  EngineDataSet list = importOrderProductBean.getOneTable();
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onclick="checkTr(<%=i%>)">
     ...
     <%if(isHsbj){%>
      ...
      <%}%>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sl_<%=i%>" class="td"><%=list.getValue("zsl")%></td>
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              <TD nowrap class="td">合同编号</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="htbh" name="htbh" value='<%=importOrderProductBean.getFixedQueryValue("htbh")%>' maxlength='50' onKeyDown="return getNextElement();"></TD>
              <%if(importOrderProductBean.djlx.equals("-1")){%><TD class="td" nowrap>状态</TD>
              <TD nowrap class="td">
                <%String zt = importOrderProductBean.getFixedQueryValue("zt");%>
                <input type="radio" name="zt" value="1" <%=zt.equals("1")?" checked" :""%>>未完成
                <input type="radio" name="zt" value="8" <%=zt.equals("8")?" checked" :""%>>已完成
              </TD>
             <%}%>
            </TR>
            <TR>
              <TD nowrap class="td">合同日期</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="htrq$a" value='<%=importOrderProductBean.getFixedQueryValue("htrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="htrq$b" value='<%=importOrderProductBean.getFixedQueryValue("htrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
