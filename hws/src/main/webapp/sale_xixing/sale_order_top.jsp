...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">销售订单列表</TD>
  </TR></TABLE>
<%
  ...
  EngineDataSet anotherDetail = saleOrderBean.getAnotherDetailTable();
  ...
  <table id="tbcontrol" width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" nowrap align="right">
       <%--if(!isProductInvoke){%>
       <input name="search2" type="button" style="width:120" class="button" onClick="showInterFrame()" value="未完成订单列表(W)" onKeyDown="return getNextElement();">
      ...
      <%}--%>
       ...
       <%}%>
        <%if(!isProductInvoke&&!saleOrderBean.retuUrl.equals("")){
...
//2004.12.05 加入已部分入库、已全部入库、已部分进货、已全部进货的状态描述 wlc
      ...
      for(; i<count; i++){
        int j;
        int isSaleDone=-1,isStoreDone=-1;   //isSaleDone :开单；isStoreDone：出库 0表示‘没有’，1表示‘部分’，2表示‘全部’
        double skdsl=0.0,stsl=0.0,tempSl=0.0;
        synchronized(anotherDetail){
          saleOrderBean.openAnotherDetailTable(false);
          anotherDetail.first();
          for(j=0;j<anotherDetail.getRowCount();j++){
            ...
            skdsl = Double.parseDouble(anotherDetail.getValue("skdsl").trim().length()>0?anotherDetail.getValue("skdsl"):"0");
            if(skdsl == 0 && isSaleDone == 2){isSaleDone = 1; break;}
            if(skdsl >= tempSl && isSaleDone == 0 && skdsl != 0){isSaleDone = 1; break;}
            if(skdsl > 0 && skdsl < tempSl ){isSaleDone = 1; break;}
            if(skdsl >= tempSl && skdsl != 0){isSaleDone = 2;}
            if(skdsl == 0){isSaleDone = 0;}
            anotherDetail.next();
          }
          anotherDetail.first();
          for(j=0;j<anotherDetail.getRowCount();j++){
          ...
          if(isStoreDone == 1){
            ...
            sb.append("已全部出库");
          }else if(isSaleDone == 1){
            sb.append("已部分开单");
          }else if(isSaleDone == 2){
            sb.append("已全部开单");
          }
    ...
    <tr id="tr_<%=list.getRow()%>" <%=dbl%> onClick="showDetail(<%=list.getRow()%>)">
     ...
     <%if(!rowzt.equals("9")&&!rowzt.equals("4")&&iscanDepose&&!rowzt.equals("8")&&loginBean.hasLimits(pageCode, op_cancel)){%>
         <input name="image3" class="img" type="image"   title='作废' onClick="if(confirm('确实要做废吗？'))sumitForm(<%=saleOrderBean.SALE_CANCER%>,<%=list.getRow()%>)" src="../images/close.gif" border="0"><%}%>
     <%
       if(rowzt.equals("1")&&(loginBean.hasLimits(pageCode, op_over))){//&&(loginBean.hasLimits(pageCode, op_over))
      %>
         <input name="image3" class="img" type="image"   title='完成' onClick="if(confirm('确认要完成吗？'))sumitForm(<%=saleOrderBean.SALE_OVER%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
     <%}%>
     </td>
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="jhfhrq$a" value='<%=saleOrderBean.getFixedQueryValue("jhfhrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="jhfhrq$b" value='<%=saleOrderBean.getFixedQueryValue("jhfhrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
             ...
             <TD class="td" nowrap>
             <INPUT class="edbox" style="WIDTH:160" id="htidcpbm" name="htidcpbm" value='<%=saleOrderBean.getFixedQueryValue("htid$cpbm")%>' maxlength='16' onKeyDown="return getNextElement();">
             </TD>
             <TD class="td" nowrap>品名规格</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="htid$product" name="htid$product" value='<%=saleOrderBean.getFixedQueryValue("htid$product")%>' onKeyDown="return getNextElement();"></TD>
             </TR>
             <TR>
             <TD class="td" nowrap>规格属性</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="htid$sxz" name="htid$sxz" value='<%=saleOrderBean.getFixedQueryValue("htid$sxz")%>' onKeyDown="return getNextElement();"></TD>
             </TR>
             <TR>
