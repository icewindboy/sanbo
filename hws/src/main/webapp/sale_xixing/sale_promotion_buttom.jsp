...
<%
  ...
  synchronized(b_SalePromotionBean){
    b_SalePromotionBean.doService(request, response);
    EngineDataSet list = b_SalePromotionBean.getDetailTable();
    HtmlTableProducer detailProducer = b_SalePromotionBean.detailProducer;//从表表格生成器
    //list.first();
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      if(list.changesPending())
        b_SalePromotionBean.openDetailTable(b_SalePromotionBean.masterIsAdd());
      ...
      for(; i<list.getRowCount(); i++)   {
        ...
        if(b_SalePromotionBean.isDouble(xsje))
        ...
        if(b_SalePromotionBean.isDouble(sthssl))
          t_sthssl = t_sthssl.add(new BigDecimal(sthssl));
        String stsl = list.getValue("stsl");
        if(b_SalePromotionBean.isDouble(stsl))
