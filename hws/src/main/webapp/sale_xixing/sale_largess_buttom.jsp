...
<%
  ...
  synchronized(b_SaleLargessBean){
    b_SaleLargessBean.doService(request, response);
    EngineDataSet list = b_SaleLargessBean.getDetailTable();
    HtmlTableProducer detailProducer = b_SaleLargessBean.detailProducer;//从表表格生成器
    list.first();
...
<form name="form1" action="" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      <td nowrap width=10><%--if(false){%><INPUT class="button" onClick="openrelationwin(<%=tdid%>)" type="button" value="出库情况" name="button2" onKeyDown="return getNextElement();"><%}--%></td>
      <td  nowrap>产品代码</td>
      <td  nowrap>品名</td>
      <td nowrap>数量</td>
      <td nowrap>单位</td>
    ...
    <%
      if(list.changesPending())
        b_SaleLargessBean.openDetailTable(b_SaleLargessBean.masterIsAdd());
      ...
      for(; i<list.getRowCount(); i++)   {
        ...
        if(b_SaleLargessBean.isDouble(xsje))
