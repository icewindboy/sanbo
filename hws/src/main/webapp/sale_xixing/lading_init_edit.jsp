  ...
  <table class="editformbox" CELLSPACING=1 CELLPADDING=0 >
      ...
      <tr>
                ...
                <td  noWrap class="td">
                  ...
                  <td noWrap class="tdTitle">回款日期</td>
                      ...
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="center">
                         <iframe id="prod_<%=i%>" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                          <%if(!isEnd){%>
                          <%if(detail.get("hthwid").equals("")){%><img style='cursor:hand' src='../images/select_prod.gif' border=0 onClick="if(form1.khlx.value==''){alert('请选择客户类型');return}SaleProdSingleSelect('form1','srcVar=wzdjid_<%=i%>&srcVar=cpid_<%=i%>&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=cpid','','sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,<%=i%>)')"><%}%>
...
<script language="javascript">
  ...
  <%=b_LadingInitBean.adjustInputSize(new String[]{"cpbm","product","sxz","hssl","sl","xsj","xsje", "zk","dj","jje","bz"}, "form1", detailRows.length)%>
...
{
    SaleProdCodeChange(document.all['prod'], obj.form.name,'srcVar=wzdjid_'+i+'&srcVar=cpid_'+i+'&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=cpid',obj.value,'sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,'+i+')');
}
//产品名称
function productNameSelect(obj, i)
{
    SaleProdNameChange(document.all['prod'], obj.form.name,'srcVar=wzdjid_'+i+'&srcVar=cpid_'+i+'&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=cpid',obj.value,'sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,'+i+')');
}
</script>
<%//&#$
