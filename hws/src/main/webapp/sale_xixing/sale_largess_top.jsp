...
<%
  ...
  boolean hasSearchLimit = loginBean.hasLimits(pageCode, op_search);
    synchronized(b_SaleLargessBean){
...
function toDetail(){
  // 转到主从明细
  parent.location.href='sale_largess_edit.jsp';
  return;
}
function toTihd(){
  // 转到提货单
...
function showDetail(masterRow){
  ...
  parent.bottom.location.href='sale_largess_buttom.jsp?operate=<%=b_SaleLargessBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
  return;
}
...
function prnt(tdid)
{
  ...
  openSelectUrl('../pub/pdfprint.jsp?code=shyu_sale_largess_print&operate=<%=Operate.PRINT_BILL%>&a$tdid='+tdid, winName, winopt2);
}
</script>
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">销售赠送单</TD>
  </TR>
</TABLE>
<%
  ...
  EngineDataSet list = b_SaleLargessBean.getMaterTable();//主表数据集
  HtmlTableProducer table = b_SaleLargessBean.masterProducer;//主表数据的表格打印
  String curUrl = request.getRequestURL().toString();
  //RowMap m_RowInfo =b_SaleLargessBean.getMasterRowinfo();//主表一行数据
  //提单类型
     ...
     <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
      ...
      <td class="td" nowrap align="right">
        ...
        <%
            ...
            //if(b_SaleLargessBean.retuUrl!=null){
            String s = "parent.location.href='../pub/main.jsp'";//+b_SaleLargessBean.retuUrl+"'";
         %>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    BigDecimal t_zsl = new BigDecimal(0), t_zje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        if(b_SaleLargessBean.isDouble(zsl))
          t_zsl = t_zsl.add(new BigDecimal(zsl));
        String zje = list.getValue("zje");
        if(b_SaleLargessBean.isDouble(zje))
        ...
        boolean hasReferenced = b_SaleLargessBean.hasReferenced(list.getValue("tdid"));
        rowClass = engine.action.BaseAction.getStyleName(rowClass);
        String loginId=b_SaleLargessBean.loginId;
        String sprid=list.getValue("sprid");
        String rowzt =list.getValue("zt");
        boolean isCancer=rowzt.equals("1") && loginId.equals(sprid)&&b_SaleLargessBean.isCanCancer(list.getValue("tdid"));
        boolean isCanOver = b_SaleLargessBean.isCanOver(list.getValue("tdid"));

        boolean cansubmit=false;
        boolean submitType = b_SaleLargessBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>);" onClick="showDetail(<%=list.getRow()%>)">
      <td <%=rowClass%> align="center" nowrap>
       ...
       <%if(isCancer){%>
           <input name="image3" class="img" type="image"  title='取消审批' onClick="if(confirm('确实要取消审批吗？'))sumitForm(<%=b_SaleLargessBean.CANCER_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
     <%if((rowzt.equals("1")||rowzt.equals("0"))&&loginBean.hasLimits(pageCode, op_cancel)){%>
         <input name="image3" class="img" type="image"   title='作废' onClick="if(confirm('确实要做废吗？'))sumitForm(<%=b_SaleLargessBean.LADDING_CANCER%>,<%=list.getRow()%>)" src="../images/close.gif" border="0"><%}%>
       <%if((rowzt.equals("2")&&loginBean.hasLimits(pageCode, op_outstore))){%>
         <input name="image3" class="img" type="image" title='出库确认' onClick="if(confirm('确认完成出库吗？'))sumitForm(<%=b_SaleLargessBean.LADING_OUT%>,<%=list.getRow()%>)" src="../images/edit.old.gif" border="0"><%}%>
     </td>
    ...
    <%  list.next();
      ...
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleLargessBean.getZsl()%>' readonly></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=engine.util.Format.formatNumber(b_SaleLargessBean.getZje(),"#0.00")%>' readonly></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
...
<%
  if(!b_SaleLargessBean.masterIsAdd()){
    int row = b_SaleLargessBean.getSelectedRow();//得到所选择的行号
    if(row >= 0)
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="tdbh" name="tdbh" value='<%=b_SaleLargessBean.getFixedQueryValue("tdbh")%>' maxlength='16' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>制单人</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="czy" name="czy" value='<%=b_SaleLargessBean.getFixedQueryValue("czy")%>' maxlength='16' onKeyDown="return getNextElement();">
              </TD>
            </tr>
            <tr>
              <TD align="center" nowrap class="td">往来单位</TD>
              <td nowrap class="td" colspan=3>
              <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=b_SaleLargessBean.dwdm%>' onchange="customerCodeSelect(this)" >
              <input type="text" name="dwmc"  style="width:260" value='<%=b_SaleLargessBean.dwmc%>' class="edbox"  onchange="customerNameSelect(this)" >
              <INPUT TYPE="HIDDEN" NAME="dwtxid" value='<%=b_SaleLargessBean.getFixedQueryValue("dwtxid")%>'>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="czrq$a" value='<%=b_SaleLargessBean.getFixedQueryValue("czrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="czrq$b" value='<%=b_SaleLargessBean.getFixedQueryValue("czrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <%=deptBean.getList(b_SaleLargessBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>业务员</TD>
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:130">
                  <%=personBean.getList(b_SaleLargessBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             </TR>
            <TR>
                ...
                <input class="edbox" style="WIDTH:80" name="tdid$cpbm" value='<%=b_SaleLargessBean.getFixedQueryValue("tdid$cpbm")%>' onKeyDown="return getNextElement();" onchange="productCodeSelect(this)" >
                <INPUT class="edbox" style="WIDTH:220" id="tdid$product" name="tdid$product" value='<%=b_SaleLargessBean.getFixedQueryValue("tdid$product")%>' maxlength='10' onKeyDown="return getNextElement();"  onchange="productNameSelect(this)">
            ...
            <TR>
              ...
              <TD colspan="3" nowrap class="td">
                <%
                  String [] zt = b_SaleLargessBean.zt;
                  String zt0="";
                  String zt9="";
                  String zt1="";
                  String zt4="";
