...
%>
<%
  engine.erp.sale.dafa.OtherLadingBill otherLadingBillBean = engine.erp.sale.dafa.OtherLadingBill.getInstance(request);
  String pageCode = "lading_bill";

  boolean hasSearchLimit = true;
    synchronized(otherLadingBillBean){
  String retu = otherLadingBillBean.doService(request, response);
...
function showDetail(masterRow){
  ...
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='lading_bill_buttom.jsp?operate=<%=otherLadingBillBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<%
  ...
  EngineDataSet list = otherLadingBillBean.getMaterListTable();//主表数据集
  HtmlTableProducer table = otherLadingBillBean.masterListProducer;//主表数据的表格打印
     ...
     <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
      ...
      <td class="td" nowrap align="right">
        ...
        <%
          ...
          //if(otherLadingBillBean.retuUrl!=null){
            String s = "parent.location.href='../pub/main.jsp'";//+otherLadingBillBean.retuUrl+"'";
         %>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      ...
      for(; i<count; i++)   {
        ...
        if(otherLadingBillBean.isDouble(zsl))
          t_zsl = t_zsl.add(new BigDecimal(zsl));
        String zje = list.getValue("zje");
        if(otherLadingBillBean.isDouble(zje))
        ...
        boolean hasReferenced = otherLadingBillBean.hasReferenced(list.getValue("tdid"));
        rowClass = engine.action.BaseAction.getStyleName(rowClass);
        String loginId=otherLadingBillBean.loginId;
        String sprid=list.getValue("sprid");
        String rowzt =list.getValue("zt");
        boolean isCancer=rowzt.equals("1") && loginId.equals(sprid)&&otherLadingBillBean.isCanCancer(list.getValue("tdid"));
        boolean isCanOver = otherLadingBillBean.isCanOver(list.getValue("tdid"));
        boolean cansubmit=false;
        boolean submitType = otherLadingBillBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
    ...
    %>
    <tr id="tr_<%=list.getRow()%>" onDblClick="showDetail(<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
    ...
    <%  list.next();
      ...
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=otherLadingBillBean.getZsl()%>' readonly></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=engine.util.Format.formatNumber(otherLadingBillBean.getZje(),"#0.00")%>' readonly></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
...
<%
  if(!otherLadingBillBean.masterIsAdd()){
    int row = otherLadingBillBean.getSelectedRow();//得到所选择的行号
    if(row >= 0)
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="tdbh$a" name="tdbh$a" value='<%=otherLadingBillBean.getFixedQueryValue("tdbh$a")%>' maxlength='16' onKeyDown="return getNextElement();"></TD>
              <TD nowrap class="td">--</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="tdbh$b" name="tdbh$b" value='<%=otherLadingBillBean.getFixedQueryValue("tdbh$b")%>' maxlength='16' onKeyDown="return getNextElement();"></TD>
              </tr>
              <tr>
              <TD align="center" nowrap class="td">往来单位</TD>
              <td nowrap class="td" colspan=3>
              <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=otherLadingBillBean.dwdm%>' onchange="customerCodeSelect(this)" >
              <input type="text" name="dwmc"  style="width:260" value='<%=otherLadingBillBean.dwmc%>' class="edbox"  onchange="customerNameSelect(this)" >
              <INPUT TYPE="HIDDEN" NAME="dwtxid" value='<%=otherLadingBillBean.getFixedQueryValue("dwtxid")%>'>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="tdrq$a" value='<%=otherLadingBillBean.getFixedQueryValue("tdrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="tdrq$b" value='<%=otherLadingBillBean.getFixedQueryValue("tdrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="hkrq$a" value='<%=otherLadingBillBean.getFixedQueryValue("hkrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
                <A href="#"><IMG title=选择日期 onClick="selectDate(hkrq$a);" height=20 src="../images/seldate.gif" width=20 align=absMiddle border=0></A></TD>
              <TD align="center" nowrap class="td">--</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="hkrq$b" value='<%=otherLadingBillBean.getFixedQueryValue("hkrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <%=deptBean.getList(otherLadingBillBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>提单类型</TD>
              <TD nowrap class="td">
                <pc:select name="djlx" addNull="1" style="width:130" >
                <%=otherLadingBillBean.listToOption(lists, opkey.indexOf(otherLadingBillBean.getFixedQueryValue("djlx")))%>
               </pc:select>
              </TD>
             </TR>
              <tr>
              ...
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="czy" name="czy" value='<%=otherLadingBillBean.getFixedQueryValue("czy")%>' maxlength='16' onKeyDown="return getNextElement();">
             </TD>
              <TD class="td" nowrap>业务员</TD>
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:130">
                  <%=personBean.getList(otherLadingBillBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             </tr>
             <tr>
                ...
                <td  noWrap class="td">
                  <pc:select name="storeid" addNull="1" style="width:110">
                    <%=storeBean.getList(otherLadingBillBean.getFixedQueryValue("storeid"))%> </pc:select>
                  </td>
              <TD class="td" nowrap>码单号</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="djh" name="djh" value='<%=otherLadingBillBean.getFixedQueryValue("djh")%>' maxlength='16' onKeyDown="return getNextElement();">
             </TD>
             </tr>
            <TR>
              <TD class="td" nowrap>品名</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="tdid$pm" name="tdid$pm" value='<%=otherLadingBillBean.getFixedQueryValue("tdid$pm")%>' maxlength='10' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>规格</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="tdid$gg" name="tdid$gg" value='<%=otherLadingBillBean.getFixedQueryValue("tdid$gg")%>' maxlength='10' onKeyDown="return getNextElement();"></TD>
            </TR>
            <tr>
             <td noWrap  class="td">客户类型</td>
             <td width="120" class="td">
              <%String khlx = otherLadingBillBean.getFixedQueryValue("khlx");%>
              <pc:select name="khlx" style="width:80" addNull="1" value="<%=khlx%>" >
               <pc:option value="A">A</pc:option>
               <pc:option value="C">C</pc:option>
              </pc:select>
            </td>
             <td noWrap  class="td">数据来源</td>
             <td width="120" class="td">
              <%String isnet = otherLadingBillBean.getFixedQueryValue("isnet");%>
              <pc:select name="isnet" style="width:80" addNull="1" value="<%=isnet%>" >
               <pc:option value="0">销售合同</pc:option>
               <pc:option value="1">外加工单</pc:option>
              </pc:select>
            </td>
            </tr>
            <TR>
              ...
              <TD colspan="3" nowrap class="td">
                <%
                  String [] zt = otherLadingBillBean.zt;
                  String zt0="";
                  String zt9="";
                  String zt1="";
                  String zt4="";
