...
{
	alert('dd');
	OtherProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsbl_'+i+'&srcVar=hsdw_'+i+'&storeid=','fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsbl&fieldVar=hsdw', obj.value, 'product_change('+i+')');
}
...
<%String retu = destroyBean.doService(request, response);
  ...
  engine.project.LookUp pdhmBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_STORE_CHECK);
  ...
  boolean isEnd = (!destroyBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成2.14
  boolean isCanDelete = !isEnd && !destroyBean.masterIsAdd() 
                        && loginId.equals(zdrid);//没有结束,在修改状态,并有删除权限,2004-08-04 并且登陆人等于制单人
  isEnd = isEnd|| !loginId.equals(zdrid);//2004-08-04 新增 只有当前登陆人是制单人的时候才可以修改 yjg
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0">
                <%corpBean.regData(ds,"dwtxid");String jsr = masterRow.get("jsr");
                  ...
                  <td  noWrap class="td" colspan="3"><input type="text" name="bz" value='<%=masterRow.get("bz")%>' maxlength='<%=ds.getColumn("bz").getPrecision()%>' style="width:110" class="edbox" onKeyDown="return getNextElement();" <%=readonly%>></td>
...
<%=destroyBean.adjustInputSize(new String[]{"cpbm","product", "jldw", "ph", "hssl", "sl", "kwid", "sxz", "bz"},  "form1", (destroyBean.max-min+1), min)%>
    ...
    function sl_onchange(i, isBigUnit)
  {
    ...
    unit_Convert('<%=request.getContextPath() %>',document.all['prod_'+i], 'form1', 'srcVar=truebl_'+i, 'exp='+oldhsblObj.value, sxzObj.value, 'nsl_onchange('+i+','+isBigUnit+')');
  }
