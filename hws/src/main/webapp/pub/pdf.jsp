<%@ page contentType="text/html; charset=UTF-8" %><%@page import="engine.action.Operate,engine.dataset.sql.QueryWhereField"%><%@ include file="../pub/init.jsp"%><%
...
<script language="javascript">
  ...
  function showExcel(){
    window.open("excel.jsp", "", "location=no,menubar=no,toolbar=no,status=no,scrollbars=no");
...
<body oncontextmenu="window.event.returnValue=true">
  ...
  <table id="tbcontrol" width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <pc:navigator id="pdfNav" recordCount="<%=pdf.getRowCount()%>" pageSize="<%=String.valueOf(iPage)%>"/></td>
      <TD align="right" NOWRAP><input name="search2" type="button" accessKey="q" class="button" onClick="showFixedQuery()" value="查询(Q)" onKeyDown="return getNextElement();">
...
<%
  int min = pdfNav.getRowMin(request);
  int max = pdfNav.getRowMax(request);
