...
<%engine.erp.common.B_ProductSelect productSelect = engine.erp.common.B_ProductSelect.getInstance(request);
  ...
  if(inputName != null && fieldName != null)
  {
    ...
    for(int i=0; i< length; i++)
    {
      ...
      out.println("obj."+ (fieldName[i].equalsIgnoreCase("cpid") || fieldName[i].equalsIgnoreCase("wzlbid") ||
                           ...
                           fieldName[i].equalsIgnoreCase("sjlbsf")
                           ? "value;":"innerText;"));
    }
  }
...
<%if(loginBean.hasLimits("productinfoset","op_add")){%><input name="button12" type="button" class="button" onClick="sumitForm(<%=Operate.POST%>);" value="新物资(A)" onKeyDown="return getNextElement();">
        <pc:shortcut key="a" script='<%="sumitForm("+Operate.POST+");"%>'/><%}%>
