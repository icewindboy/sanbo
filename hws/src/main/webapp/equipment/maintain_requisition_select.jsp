<!--引设备保养申请单--><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
if(operate !=null && operate.equals(String.valueOf(Operate.INIT)))
{
  ...
  engine.erp.equipment.B_MaintainRequisitionSelect b_MaintainReqSelectBean = engine.erp.equipment.B_MaintainRequisitionSelect.getInstance(request);
  String retu = b_MaintainReqSelectBean.doService(request, response);
  ...
  EngineDataSet list = b_MaintainReqSelectBean.getOneTable();
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
     ...
     <tr onDblClick="Radio(<%=i%>);selectequipment(<%=i%>);">
      ...
      <td style="display:none" id="maintainapplyID_<%=i%>" ><%=list.getValue("maintainapplyID")%></td>
      <td style="display:none" id="equipmentid_<%=i%>" ><%=list.getValue("equipmentid")%></td>
      <td class="td" nowarp  id="maintainapplyNO_<%=i%>" onClick="Radio(<%=i%>)"><%=list.getValue("maintainapplyNO")%></td><!--单据号-->
      ...
      <td class="td" nowarp id="manufacturer_<%=i%>" onClick="Radio(<%=i%>)"><%=equipmentRow.get("manufacturer")%></td><!--制造厂-->
      <td class="td" nowarp id="buy_date_<%=i%>" onClick="Radio(<%=i%>)"><%=equipmentRow.get("buy_date")%></td><!--购买日期-->
      <td class="td" nowarp id="use_date_<%=i%>" onClick="Radio(<%=i%>)"><%=equipmentRow.get("use_date")%></td><!--开始使用日期-->
      <td style="display:none" id="apply_dept_<%=i%>" ><%=list.getValue("deptid")%></td>
      <td class="td" nowarp id="deptid_<%=i%>" onClick="Radio(<%=i%>)"><%=deptBean.getLookupName(list.getValue("deptid"))%></td><!--申请部门-->
      <td style="display:none" id="proposer_<%=i%>" ><%=list.getValue("personid")%></td>
      <td class="td" nowarp id="personid_<%=i%>" onClick="Radio(<%=i%>)"><%=personBean.getLookupName(list.getValue("personid"))%></td><!--申请人-->
      <td class="td" nowarp id="applydate_<%=i%>" onClick="Radio(<%=i%>)"><%=list.getValue("applydate")%></td><!--申请日期-->
    </tr>
          <%  list.next();
          }
