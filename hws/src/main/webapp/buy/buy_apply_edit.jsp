...
%><%
  engine.erp.buy.BuyApply buyApplyBean = engine.erp.buy.BuyApply.getInstance(request);
...
function productCodeSelect(obj, i)
{
  ...
  ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw', obj.value,'product_change('+i+')');
  //alert(obj.value);
}
              ...
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" width="100%" bgcolor="#f0f0f0">
                <tr>
                  ...
                  <td noWrap class="td">
                    ...
                    <pc:select name="deptid" addNull="1" style="width:111">
                      <%=deptBean.getList(masterRow.get("deptid"))%> </pc:select>
                    <%}%>
                  </td>
                  <td noWrap class="tdTitle"><%=masterProducer.getFieldInfo("cgyy").getFieldname()%></td>
                  <td noWrap class="td"><input type="text" name="cgyy" value='<%=masterRow.get("cgyy")%>' maxlength='<%=ds.getColumn("cgyy").getPrecision()%>' style="width:200" class="edbox" onKeyDown="return getNextElement();" <%=masterReadonly%>></td>
                </tr>

                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:100%;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%BigDecimal t_sl = new BigDecimal(0), t_je = new BigDecimal(0), t_ybje = new BigDecimal(0), t_skhtl = new BigDecimal(0);
                      int i=0;
                      RowMap detail = null;
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="center">
                          ...
                          <img style='cursor:hand' title='单选物资' src='../images/select_prod.gif' border=0 onClick="ProdSingleSelect('form1','srcVar=cpid_<%=i%>&srcVar=cpbm_<%=i%>&srcVar=product_<%=i%>&srcVar=jldw_<%=i%>','fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw',form1.cpid_<%=i%>.value,'product_change(<%=i%>)')">
                          <%}else{%>
                          <img style='cursor:hand' title='单选物资' src='../images/select_prod.gif' border=0 onClick="ProdSingleSelect('form1','srcVar=cpid_<%=i%>&srcVar=cpbm_<%=i%>&srcVar=product_<%=i%>&srcVar=hsdw_<%=i%>','fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=hsdw',form1.cpid_<%=i%>.value,'product_change(<%=i%>)')">
                          <%}
                       ...
                       <%if(!isHsbj){%><input type="text" <%=(isimport || !isCanRework) ? "class=ednone" : detailClass%>  onKeyDown="return getNextElement();" name="cpbm_<%=i%>" value='<%=prodRow.get("cpbm")%>' onchange="productCodeSelect(this,<%=i%>)"<%=isimport ? "readonly" : detailReadonly%>>
                        ...
                        <td class="td" nowrap align="center"><input type="text" <%=detailClass_r%>  onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=detail.get("sl")%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onchange="cg_onchange(<%=i%>)" <%=readonly%>></td>
                        ...
                        <img style='cursor:hand' src='../images/view.gif' title="选择采购报价中的供应商" border=0 onClick="if(form1.cpid_<%=i%>.value==''){alert('请先输入产品');return;}corpSingleSelect('form1','srcVar=dwtxid_<%=i%>&srcVar=dwmc_<%=i%>&srcVar=dj_<%=i%>','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=bj&cpid='+form1.cpid_<%=i%>.value'','cg_onchange(<%=i%>)')">
                        <img style='cursor:hand' src='../images/view.gif' title="选择往来单位供应商" border=0 onClick="if(form1.cpid_<%=i%>.value==''){alert('请先输入产品');return;}ProvideSingleSelect('form1','srcVar=dwtxid_<%=i%>&srcVar=dwmc_<%=i%>','fieldVar=dwtxid&fieldVar=dwmc&cpid='+form1.cpid_<%=i%>.value)">

                        <%--img style='cursor:hand' src='../images/view.gif' title="选择供应商" border=0 onClick="if(form1.cpid_<%=i%>.value==''){alert('请先输入产品');return;}corpSingleSelect('form1','srcVar=dwtxid_<%=i%>&srcVar=dwmc_<%=i%>&srcVar=dj_<%=i%>','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=bj&cpid='+form1.cpid_<%=i%>.value','cg_onchange(<%=i%>)')"--%>
          ...
          <tr>
            ...
            <%}--%>
             ...
             <input name="button2" type="button" class="button" onClick="sumitForm(<%=Operate.POST_CONTINUE%>);" value="保存添加(N)">
              ...
              <%if(isCanDelete && isCanAmend && isHasDeptLimit){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);" value=" 删除(D)">
...
<%=buyApplyBean.adjustInputSize(new String[]{"cpbm","product", "sl", "dwmc","bz"}, "form1", detailRows.length)%>
