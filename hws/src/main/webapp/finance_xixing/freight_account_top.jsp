<%--销售结算管理--%><%@page contentType="text/html; charset=UTF-8" %><%@ include file="../pub/init.jsp"%>
...
function toDetail(){
  // 转到主从明细
  parent.location.href='freight_account_edit.jsp';
}
function masterAdd(){
  // 转到主从明细
  parent.location.href='freight_account_finance.jsp';
}
function showDetail(masterRow){
  //单击主表的一行激活的操作
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='freight_account_buttom.jsp?operate=<%=b_SaleFreightAccountBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
{
    TransportCodeChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc',obj.value);
}
//客户名称
function customerNameSelect(obj)
{
    TransportNameChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc',obj.value);
}
...
<%
  ...
  String retu = b_SaleFreightAccountBean.doService(request, response);
    ...
    EngineDataSet list = b_SaleFreightAccountBean.getMaterTable();//主表数据集
    HtmlTableProducer masterProducer = b_SaleFreightAccountBean.masterProducer;//主表数据的表格打印
    String curUrl = request.getRequestURL().toString();
    RowMap m_RowInfo =b_SaleFreightAccountBean.getMasterRowinfo();

%>
<BODY oncontextmenu="window.event.returnValue=true">
<iframe id="prod" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">运费结算管理</TD>
  </TR>
</TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
    ...
    <tr>
     ...
     <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
      ...
      <td class="td" nowrap align="right">
        ...
        <%}%>
        <%if(b_SaleFreightAccountBean.retuUrl!=null){String ret = "parent.location.href='"+b_SaleFreightAccountBean.retuUrl+"'";%>
        <input name="button22" type="button" class="button" onClick="parent.location.href='<%=b_SaleFreightAccountBean.retuUrl%>'" value="返回(C)" onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <td nowrap>
     ...
     String add = "sumitForm("+b_SaleFreightAccountBean.MASTER_ADD+",-1)";
    %>
    <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=b_SaleFreightAccountBean.MASTER_ADD%>,-1)" src="../images/add.gif" border="0">
    ...
    <%
     ...
     BigDecimal  t_whxje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        String loginId=b_SaleFreightAccountBean.loginid;
        ...
        String togo = (zt.equals("8")||zt.equals("1"))?String.valueOf(Operate.EDIT):b_SaleFreightAccountBean.MASTER_EDIT;

        boolean cansubmit=false;
        boolean submitType = b_SaleFreightAccountBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=togo%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
       <td <%=rowClass%> align="center" nowrap>
        ...
        <%}else{%><input name="image2" class="img" type="image" title='<%=loginBean.hasLimits(pageCode, op_edit) ?"修改" :"查看"%>' onClick="sumitForm(<%=b_SaleFreightAccountBean.MASTER_EDIT%>,<%=list.getRow()%>)" src="../images/edit.gif" border="0">
        <%}%>
       <%if(isInit&&cansubmit){%>
      ...
      <%if(isCancer){%>
            <input name="image3" class="img" type="image"  title='取消审批' onClick="sumitForm(<%=b_SaleFreightAccountBean.CANCER_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
     <%if(rowzt.equals("1")&&loginBean.hasLimits(pageCode, op_over)){%>
            <input name="image3" class="img" type="image"   title='核销完成' onClick="if(confirm('完成后不能修改,确认要完成吗？'))sumitForm(<%=b_SaleFreightAccountBean.BALANCE_OVER%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
      </td>
    ...
    <%  list.next();
     ...
     %>
      ...
      <td align="right" class="td"></td>
      <td class="td" nowrap></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleFreightAccountBean.getZje()%>' readonly></td>
      <td class="td" nowrap><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleFreightAccountBean.getZhxje()%>' readonly></td>
      <td class="td" nowrap><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleFreightAccountBean.getZwhxje()%>' readonly></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              <TD align="center" nowrap class="td">承运商</TD>
              <td nowrap class="td" colspan=3>
              <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=b_SaleFreightAccountBean.dwdm%>' onchange="customerCodeSelect(this)" >
              <input type="text" name="dwmc"  style="width:260" value='<%=b_SaleFreightAccountBean.dwmc%>' class="edbox"  onchange="customerNameSelect(this)" >
              <INPUT TYPE="HIDDEN" NAME="dwtxid" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("dwtxid")%>'>
              <img style='cursor:hand' src='../images/view.gif' border=0 onClick="TransportSingleSelect('fixedQueryform','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',fixedQueryform.dwtxid.value)">
              <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';dwdm.value='';">
              </td>
            </TR>
            <TR>
              <TD nowrap class="td">日期</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="rq$a" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("rq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="rq$b" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("rq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <td noWrap class="td"><input type="text" name="je$a" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("je$a")%>' maxlength='50' style="width:131" class="edbox"></td>
              <td noWrap class="td" align="center">--</td>
              <td noWrap class="td"><input type="text" name="je$b" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("je$b")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
            <TR>
              <td noWrap class="td" align="center">单据号</td>
              <td noWrap class="td"><input type="text" name="djh$a" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("djh$a")%>' maxlength='50' style="width:131" class="edbox"></td>
              <td noWrap class="td" align="center">--</td>
              <td noWrap class="td"><input type="text" name="djh$b" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("djh$b")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
            <TR>
              <TD class="td" nowrap>部门</TD>
              <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
              <%=deptBean.getList(b_SaleFreightAccountBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
             </TR>
            <TR>
              ...
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:100">
                  <%=personBean.getList(b_SaleFreightAccountBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             <td noWrap class="td" align="center">&nbsp;制单人&nbsp;</td>
              <td noWrap class="td"><input type="text" name="czy" value='<%=b_SaleFreightAccountBean.getFixedQueryValue("czy")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
           <TR>
              <TD class="td" nowrap>状态</TD>
              <TD colspan="3" nowrap class="td">
                <%
                  String [] zt = b_SaleFreightAccountBean.zt;
                  String zt0="";
                  String zt9="";
                  String zt1="";
                  String zt8="";
        ...
        <TD><TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              <TD colspan="4" nowrap class="td"><table cellspac="0" cellpadding="0" width="100%" border="0">
                  <tr>
                    <%java.util.List fieldCodes = b_SaleFreightAccountBean.orderFieldCodes;
                      java.util.List fieldNames = b_SaleFreightAccountBean.orderFieldNames;
                      java.util.List selectedCodes = b_SaleFreightAccountBean.selectedOrders;
