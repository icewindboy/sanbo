...
if(!loginBean.hasLimits(pageCode, request, response))
  ...
  //engine.project.LookUp saleLadingBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_SALE_LADING_BILL);//得到题单编号
  engine.project.LookUp saleLadingBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_SALE_LADING_OUTTPUT);//得到出库单编号
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                <%corpBean.regData(ds,"dwtxid");%>
                  ...
                  <%}else{%><input type='text'  value='<%=(masterRow.get("djxz").equals("1"))?"销售收款":"销售退款"%>' style='width:110' class='edline' readonly><input type='hidden' name='djxz' value='<%=masterRow.get("djxz")%>' style='width:110' class='edline' readonly><%}%></td>
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                    <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <%if(!zt.equals("8")){%>
                        <input type="hidden" name="rkdmxids" value="" onchange="sumitForm(<%=b_SaleBalanceBean.DETAIL_SALE_ADD%>,-1);">
                    ...
                    <%
                      ...
                      saleLadingBean.regData(list,"rkdmxid");
                      ...
                      for(; i<detailRows.length; i++)   {
                        ...
                        RowMap tdRow = saleLadingBean.getLookupRow(detail.get("rkdmxid"));
                     ...
                     %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="center">
                          ...
                          <%}%><%}%></td>
                        <td class="td" nowrap><%=tdRow.get("sfdjdh")%></td>
                  ...
                  %>
                      ...
                      <tr id="rowinfo_end">
                        ...
                        <td class="td" align="right"><input id="t_tcj" name="t_tcj" type="text" class='ednone_r' style="width:<%=widths[0]%>" value='<%=t_tcj%>' readonly></td>
...
<script language="javascript">
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    function TdhwMultiSelect(frmName,srcVar,fieldVar,curID,methodName,notin)
       {
         ...
         paraStr = "../finance_xixing/sale_balance_import_lading_product.jsp?operate=0&multi=1&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+curID;
    ...
    function selctTDHWOfLading()
    {
      ...
      //  return;
     // }
      TdhwMultiSelect('form1','srcVar=rkdmxids','fieldVar=tdid','&dwtxid='+form1.dwtxid.value+'&khlx='+form1.khlx.value,"sumitForm(<%=b_SaleBalanceBean.DETAIL_SALE_ADD%>,-1)");
        }
    function OrderSingleSelect(frmName,srcVar,fieldVar,curID,methodName,notin)
       {
        var winopt = "location=no scrollbars=yes menubar=no status=no resizable=1 width=640 height=450  top=0 left=0";
        var winName= "SingleladingSelector";
        paraStr = "../finance_xixing/sale_balance_import_lading.jsp?operate=0&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+"&dwtxid="+curID;
        if(methodName+'' != 'undefined')
          paraStr += "&method="+methodName;
        if(notin+'' != 'undefined')
          paraStr += "&notin="+notin;
