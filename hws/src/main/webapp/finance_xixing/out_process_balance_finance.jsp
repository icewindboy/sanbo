...
{
    CustomerCodeChange('3',document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc',obj.value,'sumitForm(<%=b_OutProcessBalanceBean.DWTXID_CHANGE%>,-1)');
}
//客户名称
function customerNameChange(obj)
{
    CustomerNameChange('3',document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc',obj.value,'sumitForm(<%=b_OutProcessBalanceBean.DWTXID_CHANGE%>,-1)');
}
function deptchange(){
...
<%
  ...
  String classType =  zt.equals("1") ? "class=ednone_r" : "class=edFocused_r";
              ...
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                <%
                 ...
                 <tr>
                  ...
                  <td noWrap class="td"><input type="text" name="cjyf" value='<%=masterRow.get("cjyf")%>' maxlength='<%=ds.getColumn("cjyf").getPrecision()%>' style="width:110" <%=classType%> onKeyDown="return getNextElement();" onchange="cjyf_onchange()" <%=readonly%>></td>
