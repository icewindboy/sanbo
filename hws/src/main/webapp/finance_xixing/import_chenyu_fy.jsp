...
%>
<%
  engine.erp.finance.xixing.ImportChenyuCharge ImportChenyuChargeBean = engine.erp.finance.xixing.ImportChenyuCharge.getInstance(request);
  ...
  String retu = ImportChenyuChargeBean.doService(request, response);
  ...
  EngineDataSet list = ImportChenyuChargeBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<%
  if(inputName != null && fieldName != null)
  {
    ...
    for(int i=0; i< length; i++)
    {
      ...
      out.print(".value=");     out.println("obj."+ (fieldName[i].equals("tdcyqkid")?"value;":"innerText;"));
    }
  }
  if(methodName != null)
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">提单承运选择</TD>
  </TR>
</TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectCorp(<%=i%>);"<%}%>>
      <td class="td">
        <%if(isMultiSelect){%>
        <input type="checkbox" name="sel" value="<%=list.getValue("tdcyqkid")%>" onKeyDown="return getNextElement();">
        <%}else{%>
        <input type="radio" name="sel" onKeyDown="return getNextElement();" value="<%=i%>"<%=i==0?" checked":""%>>
        <%}%><input type="hidden" id="tdcyqkid_<%=i%>" value='<%=list.getValue("tdcyqkid")%>'>
      </td>
      ...
      <td nowrap align="center"  class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("qsd")%></td>
      <td nowrap align="center"  class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("mdd")%></td>
