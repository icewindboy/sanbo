...
function toDetail(){
  // 转到主从明细
  parent.location.href='sale_balance_edit.jsp';
}
function masterAdd(){
  // 转到主从明细
  parent.location.href='sale_balance_finance.jsp';
}
function showDetail(masterRow){
  //单击主表的一行激活的操作
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='sale_balance_buttom.jsp?operate=<%=b_SaleBalanceBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<%
  ...
  String retu = b_SaleBalanceBean.doService(request, response);
    ...
    EngineDataSet list = b_SaleBalanceBean.getMaterTable();//主表数据集
    HtmlTableProducer masterProducer = b_SaleBalanceBean.masterProducer;//主表数据的表格打印
    String curUrl = request.getRequestURL().toString();
    RowMap m_RowInfo =b_SaleBalanceBean.getMasterRowinfo();
    //提单类型
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">销售结算管理</TD>
  </TR>
</TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
    ...
    <tr>
     ...
     <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
      ...
      <td class="td" nowrap align="right">
        ...
        <%}%>
        <%if(b_SaleBalanceBean.retuUrl!=null){String ret = "parent.location.href='"+b_SaleBalanceBean.retuUrl+"'";%>
        <input name="button22" type="button" class="button" onClick="parent.location.href='<%=b_SaleBalanceBean.retuUrl%>'" value="返回(C)" onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <td nowrap>
     ...
     String add = "sumitForm("+b_SaleBalanceBean.MASTER_ADD+",-1)";
    %>
    <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=b_SaleBalanceBean.MASTER_ADD%>,-1)" src="../images/add.gif" border="0">
    ...
    <%
     ...
     BigDecimal  t_whxje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        if(b_SaleBalanceBean.isDouble(zje))
          t_zje = t_zje.add(new BigDecimal(zje).multiply(new BigDecimal(djxz)));
        String hxje = list.getValue("hxje");
        if(b_SaleBalanceBean.isDouble(hxje))
          t_hxje = t_hxje.add(new BigDecimal(hxje).multiply(new BigDecimal(djxz)));
        String whxje = list.getValue("whxje");
        if(b_SaleBalanceBean.isDouble(whxje))
          t_whxje = t_whxje.add(new BigDecimal(whxje).multiply(new BigDecimal(djxz)));
        ...
        String loginid=b_SaleBalanceBean.loginid;
        ...
        String togo = (zt.equals("8")||zt.equals("1"))?String.valueOf(Operate.EDIT):b_SaleBalanceBean.MASTER_EDIT;

        boolean cansubmit=false;
        boolean submitType = b_SaleBalanceBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
    ...
    <tr id="tr_<%=list.getRow()%>"  onDblClick="sumitForm(<%=togo%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
       <td <%=rowClass%> align="center"  nowrap>
        ...
        <%}else{%><input name="image2" class="img" type="image" title='<%=loginBean.hasLimits(pageCode, op_edit) ?"修改" :"查看"%>' onClick="sumitForm(<%=b_SaleBalanceBean.MASTER_EDIT%>,<%=list.getRow()%>)" src="../images/edit.gif" border="0">
        <%}%>
       <%if(isInit&&cansubmit){%>
      ...
      <%if(isCancer){%>
            <input name="image3" class="img" type="image"  title='取消审批' onClick="sumitForm(<%=b_SaleBalanceBean.CANCER_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
     <%if(rowzt.equals("1")&&loginBean.hasLimits(pageCode, op_over)){%>
            <input name="image3" class="img" type="image"   title='核销完成' onClick="if(confirm('完成后不能修改,确认要完成吗？'))sumitForm(<%=b_SaleBalanceBean.BALANCE_OVER%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
      </td>
    ...
    <%  list.next();
     ...
     <td class="td" nowrap></td>
      ...
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleBalanceBean.getZje()%>' readonly></td>
      <td class="td" nowrap><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleBalanceBean.getZhxje()%>' readonly></td>
      <td class="td" nowrap><input type="text" class="ednone_r" style="width:100%" value='<%=b_SaleBalanceBean.getZwhxje()%>' readonly></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
...
<%
/*
  if(!b_SaleBalanceBean.masterIsAdd()){
    int row = b_SaleBalanceBean.getSelectedRow();
    if(row >= 0)
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=b_SaleBalanceBean.dwdm%>' onchange="customerCodeSelect(this)" >
              <input type="text" name="dwmc"  style="width:260" value='<%=b_SaleBalanceBean.dwmc%>' class="edbox"  onchange="customerNameSelect(this)" >
              <INPUT TYPE="HIDDEN" NAME="dwtxid" value='<%=b_SaleBalanceBean.getFixedQueryValue("dwtxid")%>'>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="rq$a" value='<%=b_SaleBalanceBean.getFixedQueryValue("rq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="rq$b" value='<%=b_SaleBalanceBean.getFixedQueryValue("rq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <td noWrap class="td"><input type="text" name="je$a" value='<%=b_SaleBalanceBean.getFixedQueryValue("je$a")%>' maxlength='50' style="width:131" class="edbox"></td>
              <td noWrap class="td" align="center">--</td>
              <td noWrap class="td"><input type="text" name="je$b" value='<%=b_SaleBalanceBean.getFixedQueryValue("je$b")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
            <TR>
              <td noWrap class="td" align="center">单据号</td>
              <td noWrap class="td"><input type="text" name="djh$a" value='<%=b_SaleBalanceBean.getFixedQueryValue("djh$a")%>' maxlength='50' style="width:131" class="edbox"></td>
              <td noWrap class="td" align="center">--</td>
              <td noWrap class="td"><input type="text" name="djh$b" value='<%=b_SaleBalanceBean.getFixedQueryValue("djh$b")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
            <TR>
              <TD class="td" nowrap>部门</TD>
              <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
              <%=deptBean.getList(b_SaleBalanceBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>单据性质</TD>
              <TD nowrap class="td">
                <pc:select name="djxz" addNull="1" style="width:130" >
                <%=b_SaleBalanceBean.listToOption(lists, opkey.indexOf(b_SaleBalanceBean.getFixedQueryValue("djxz")))%>
               </pc:select>
              </TD>
             </TR>
            <TR>
              ...
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:100">
                  <%=personBean.getList(b_SaleBalanceBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             <td noWrap class="td" align="center">&nbsp;制单人&nbsp;</td>
              <td noWrap class="td"><input type="text" name="czy" value='<%=b_SaleBalanceBean.getFixedQueryValue("czy")%>' maxlength='50' style="width:131" class="edbox"></td>
            </TR>
            <tr>
             <td noWrap  class="td">客户类型</td>
             <td width="120" class="td">
              <%String khlx = b_SaleBalanceBean.getFixedQueryValue("khlx");%>
             ...
             <%=balanceModeBean.getList(b_SaleBalanceBean.getFixedQueryValue("jsfsid"))%>
             </pc:select>
             </td>
            </tr>
            <tr>
             <td noWrap  class="td">备注</td>
             <td  noWrap class="td">
             <input type="text" name="bz"  style="width:260" value='<%=b_SaleBalanceBean.getFixedQueryValue("bz")%>' class="edbox"  >
             </td>
            </tr>
           <TR>
              ...
              <TD colspan="3" nowrap class="td">
                <%
                  String [] zt = b_SaleBalanceBean.zt;
                  String zt0="";
                  String zt9="";
                  String zt1="";
                  String zt8="";
        ...
        <TD><TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              <TD colspan="4" nowrap class="td"><table cellspac="0" cellpadding="0" width="100%" border="0">
                  <tr>
                    <%java.util.List fieldCodes = b_SaleBalanceBean.orderFieldCodes;
                      java.util.List fieldNames = b_SaleBalanceBean.orderFieldNames;
                      java.util.List selectedCodes = b_SaleBalanceBean.selectedOrders;
