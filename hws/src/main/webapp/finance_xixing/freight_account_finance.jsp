...
function TransportDmChange(obj)
{
    TransportCodeChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dfyh&srcVar=dfzh','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=khh&fieldVar=zh',obj.value,'sumitForm(<%=b_SaleFreightAccountBean.DWTXID_CHANGE%>,-1)');
}
function TransportName(obj)
{
    TransportNameChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dfyh&srcVar=dfzh','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=khh&fieldVar=zh',obj.value,'sumitForm(<%=b_SaleFreightAccountBean.DWTXID_CHANGE%>,-1)');
}
</script>
<%
              ...
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                <%
                 ...
                 %>
                  ...
                  <td noWrap class="td">
                    <%
                      String t="sumitForm("+b_SaleFreightAccountBean.DEPT_CHANGE+",-1)";
                 ...
                 <tr>
                  ...
                  <td  noWrap colspan='3' class="td">
                    ...
                    <input type="text" <%=detailClass%> style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=corpRow.get("dwdm")%>' onchange="TransportDmChange(this)" <%=readonly%>>
                    <input type="text" <%=detailClass%> name="dwmc" value='<%=corpBean.getLookupName(masterRow.get("dwtxid"))%>'  onKeyDown="return getNextElement();"  style="width:200"  onchange="TransportName(this)" <%=readonly%>>
                    <%if(!isEnd){%>
                    <img style='cursor:hand' src='../images/view.gif' border=0 onClick="TransportSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm&srcVar=dfyh&srcVar=dfzh','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm&srcVar=khh&srcVar=zh',form1.dwtxid.value,'sumitForm(<%=b_SaleFreightAccountBean.DWTXID_CHANGE%>,-1)');">
                    <%}%>
                  </td>
                 ...
                 <%}%></td>
                  <td noWrap class="tdTitle"><%=masterProducer.getFieldInfo("dfyh").getFieldname()%></td>
                  <td noWrap class="td" >
                  <%if(isEnd)
                    {
                    out.print("<input type='text' value='"+masterRow.get("dfyh")+"' style='width:110' class='edline' readonly>");
                    }else
                    {String yh=masterRow.get("dfyh");%>
                  <pc:select name="dfyh" addNull="1" style="width:110"   onSelect="yhchange()"  combox="1" value="<%=yh%>">
                  <%=bankBean.getList()%> </pc:select>
                   <%}%></td>
                  <td noWrap class="tdTitle"><%=masterProducer.getFieldInfo("dfzh").getFieldname()%></td>
                  <td noWrap class="td" >
                  <%if(isEnd) out.print("<input type='text' value='"+masterRow.get("dfzh")+"' style='width:110' class='edline' readonly>");
                  else {String zh=masterRow.get("dfzh");%>
                  <pc:select name="dfzh" addNull="1" style="width:110" combox="1" value="<%=zh%>">
                  <%=bankaccountBean.getList(masterRow.get("dfzh"),"yhmc",masterRow.get("dfyh"))%> </pc:select>
                   <%}%>
                  </td>
                 </tr>
                 <tr>
