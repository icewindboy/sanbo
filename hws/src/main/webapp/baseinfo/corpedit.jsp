            ...
            <td>
              <table CELLSPACING="2" CELLPADDING="0" BORDER="0" width="100%" bgcolor="#f0f0f0">
                <tr>
                  ...
                  <td noWrap class="td"><input type="text" name="dwdm" value='<%=m_RowInfo.get("dwdm")%>' maxlength='<%=dsDWTX.getColumn("dwdm").getPrecision()%>' style="width:120" class="edline"  onKeyDown="return getNextElement();"></td>
                ...
                <tr>
                  ...
                  <td colspan="4" noWrap class="td"><input type="text" name="dwmc" value='<%=m_RowInfo.get("dwmc")%>' maxlength='<%=dsDWTX.getColumn("dwmc").getPrecision()%>' style="width:350" class="<%=rowClass%>"<%=readOnly%> onKeyDown="return getNextElement();"></td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td noWrap class="td">
                    ...
                    if(isSave)
                    {
                      String deptChange = "if(form1.deptid.value!='"+deptid+"') sumitForm("+corpBean.DEPT_CHANGE+");";
                    %>
                    <pc:select name="deptid" style="width:120" addNull="1" disable="<%=disable%>" className="<%=rowClass%>" onSelect="<%=deptChange%>">
                    <%=deptBean.getList(deptid)%></pc:select>
                    <%}else out.print(deptBean.getLookupName(deptid));%>
                  </td>
                  <td noWrap class="tdTitle">所属业务员</td>
                  <td noWrap class="td">
                    <%String personid = m_RowInfo.get("personid");
                    if(isSave){
                      personBean.regConditionData(dsDWTX, "deptid");
                    %>
                    <pc:select name="personid" style="width:120" addNull="1" disable="<%=disable%>" className="<%=rowClass%>">
                    <%=personBean.getList(personid, "deptid", deptid)%></pc:select>
                    <%}else out.print(personBean.getLookupName(personid));%>
                  </td>
                  <td noWrap class="tdTitle">所有制</td>
                  <td colspan="2" noWrap class="td"><input type="text" name="syz" value='<%=m_RowInfo.get("syz")%>' maxlength='<%=dsDWTX.getColumn("syz").getPrecision()%>' style="width:150" class="<%=rowClass%>"<%=readOnly%> onKeyDown="return getNextElement();"></td>
                </tr>
                <tr>
                  <td noWrap class="tdTitle">关系深度</td>
                  <%String gxsd=m_RowInfo.get("gxsd");%>
                  <td noWrap class="td"><pc:select name="gxsd" style="width:120" value="<%=gxsd%>" disable="<%=disable%>" className="<%=rowClass%>">
                    <pc:option value="1">未知型</pc:option> <pc:option value="2">接触型</pc:option>
                    <pc:option value="3">成交型</pc:option> <pc:option value="4">重要型</pc:option>
                    </pc:select></td>
                  <td noWrap class="tdTitle">资信等级</td>
                  <%String zxdj = m_RowInfo.get("zxdj");%>
                  <td noWrap class="td"><pc:select name="zxdj" style="width:120" value="<%=zxdj%>" disable="<%=disable%>" className="<%=rowClass%>">
                    <pc:option value="1">★</pc:option> <pc:option value="2">★★</pc:option>
                    <pc:option value="3">★★★</pc:option> <pc:option value="4">★★★★</pc:option>
                    <pc:option value="5">★★★★★</pc:option> </pc:select></td>
                  <td noWrap class="tdTitle">行业</td>
                  <td colspan="2" noWrap class="td"><input type="text" name="hy" value='<%=m_RowInfo.get("hy")%>' style="width:150" class="<%=rowClass%>"<%=readOnly%> onKeyDown="return getNextElement();"></td>
                </tr>
