...
<script LANGUAGE="javascript">
  ...
  function showInterFrame(oper, rownum){
    var url = "send_modeedit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>发货方式</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = B_SendModeBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
  ...
  <TABLE WIDTH="600" BORDER=0 CELLSPACING=0 CELLPADDING=0 align="center">
    <TR><TD align="right">
    <%if(loginBean.hasLimits(pageCode,op_add)) { String ret = "location.href='("+B_SendModeBean.retuUrl+",-1)";%>
    <%if(B_SendModeBean.retuUrl!=null)%>
    <input name="button2222232" type="button" align="Right" title = "返回" class="button" onClick="location.href='<%=B_SendModeBean.retuUrl%>'" value=" 返回(C) "border="0"><pc:shortcut key="c" script='<%=ret%>'/><%}%>
   </TD></TR>
  </TABLE>
  <table id="tableview1" width="600" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(isCanEdit){%>onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)"<%}%> >
      ...
      <td class="td"><%=list.getValue("SendModeCode")%></td>
      <td class="td"><%=list.getValue("SendMode")%></td>
    </tr>
    <%
      list.next();
      }
