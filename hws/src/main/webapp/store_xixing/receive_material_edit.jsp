...
{
  ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=drawprice_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=scydw_'+i+'&srcVar=hsbl_'+i+'&srcVar=scdwgs_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=jhdj&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=scydw&fieldVar=hsbl&fieldVar=scdwgs', obj.value,'product_change('+i+')');
}
function productNameSelect(obj,i)
{
  ProdNameChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=drawprice_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=scydw_'+i+'&srcVar=hsbl_'+i+'&srcVar=scdwgs_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=jhdj&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=scydw&fieldVar=hsbl&fieldVar=scdwgs', obj.value,'product_change('+i+')');
}
...
function deptChange(){
  ...
  sumitForm(<%=receiveMaterialBean.MATERIAL_ON_CHANGE%>,-1)
    }
...
<%String retu = receiveMaterialBean.doService(request, response);
  ...
  EngineDataSet p_stock = receiveMaterialBean.getProductStockData();//2004.11.12库存量 wlc
...
//  RowMap[] p_stockRows = receiveMaterialBean.getPorductStockInfos();//2004.11.12库存量 wlc
  ...
  String czyid= masterRow.get("creatorid");
  ...
  String show = isDrawType? "领料单" : "退料单";
...
if(!isEnd){
  ...
  personBean.regConditionData(ds, "deptid");
                     ...
                     // processBean.regData(list,"jgdmxid");
                      ...
                      for(i=min; i<=max && i<detailRows.length; i++){
                        ...
                        */
                        ...
                        if(receiveMaterialBean.isDouble(drawsum))
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                        ...
                        <td class="td" nowrap align="center">
                          ...
                          <img style='cursor:hand' title='单选物资' src='../images/select_prod.gif' border=0 onClick="ProdSingleSelect('form1','srcVar=cpid_<%=i%>&srcVar=cpbm_<%=i%>&srcVar=product_<%=i%>&srcVar=jldw_<%=i%>&srcVar=hsdw_<%=i%>&srcVar=hsbl_<%=i%>&srcVar=scydw_<%=i%>&srcVar=scdwgs_<%=i%>&srcVar=isprops_<%=i%>&srcVar=drawprice_<%=i%>',
                               'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=scydw&fieldVar=isprops&fieldVar=hsbl&fieldVar=scdwgs&fieldVar=jhdj','&storeid='+form1.storeid.value,'product_change(<%=i%>)')">
                          <%}if(!isEnd){%>
                      <%--   <input name="image" class="img" type="image" title="配批号" onClick="if(form1.cpid_<%=i%>.value==''){alert('请输入产品');return;}if(form1.storeid.value==''){alert('请输入产品'); return;}BatchMultiSelect('form1','srcVar=mutibatch_<%=i%>&cpid='+form1.cpid_<%=i%>.value+'&storeid='+form1.storeid.value+'&dmsxid='+form1.dmsxid_<%=i%>.value)" src="../images/edit.old.gif" border="0">--%>
                        ...
                        <input type="hidden" <%=detailClass_r%> onKeyDown="return getNextElement();" id="drawprice_<%=i%>" name="drawprice_<%=i%>" value='<%=detail.get("drawprice")%>' onchange="drawprice_onchange(<%=i%>, false)" <%=readonly%>>
                        <input type="hidden" <%=detailClass_r%> onKeyDown="return getNextElement();" id="drawsum_<%=i%>" name="drawsum_<%=i%>" value='<%=detail.get("drawsum")%>' maxlength='<%=list.getColumn("drawsum").getPrecision()%>' onchange="je_onchange(<%=i%>, false)" <%=readonly%>>
                        ...
                        <input type="hidden"  name="drawbignum_<%=i%>" value='<%=detail.get("drawbignum")%>'  readonly>
                        <input type="hidden"   name="hsdw_<%=i%>" value='<%=prodRow.get("hsdw")%>'  readonly>
                        ...
                        <%if(!detail.get("cpid").equals("")){%>
                        <input type="text" name="kcsl_<%=i%>" id="kcsl_<%=i%>" class=ednone_r onKeyDown="return getNextElement();" value='<%=productStockBean.getLookupName(new String[]{masterRow.get("filialeid"),masterRow.get("storeid"),detail.get("cpid")})%>' readonly>
                        <%}%>
                        </td>
                ...
                <tr>
                <td noWrap class="tdTitle"><%=masterProducer.getFieldInfo("memo").getFieldname()%></td>
                <td colspan="10" noWrap class="td"><textarea name="memo" style="width:690" rows="2" onKeyDown="return getNextElement();"><%=masterRow.get("memo")%></textarea></td>
                </tr>
              </table>
            </td>
          </tr>
      ...
      <td>
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            ...
            <%}%>
              <%if(isCanDelete && !receiveMaterialBean.isReport&&!isEnd){%><input name="button3" type="button" class="button" title="删除(ALT+D)" value="删除(D)" onClick="buttonEventD();" style="width:50">
...
<script language="javascript">
  initDefaultTableRow('tableview1',1);
   ...
   <%=SC_OUTSTORE_SHOW_ADD_FIELD.equals("1") ?
     receiveMaterialBean.adjustInputSize(new String[]{"cpbm","product", "jldw",  "drawnum","drawbignum","hsdw","sxz", "memo","scydw","kcsl","drawprice","drawsum"},  "form1", receiveMaterialBean.max-min+1, min)
  : receiveMaterialBean.adjustInputSize(new String[]{"cpbm","product", "jldw",  "drawnum","drawbignum","hsdw","sxz", "memo","scydw","kcsl","drawprice","drawsum"},  "form1", receiveMaterialBean.max-min+1, min)%>
    }
 ...
 var bjfs = <%=bjfs%>;
    ...
    */
      ...
      if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);
        ...
        {
      location.href='../pub/pdfprint.jsp?code=receive_material_edit_bill&operate=<%=receiveMaterialBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("drawid")%>&src=../store_xixing/receive_material_edit.jsp';
        }
        //新增
