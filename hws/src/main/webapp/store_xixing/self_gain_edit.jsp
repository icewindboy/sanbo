...
<%String retu = selfGainBean.doService(request, response);
  ...
  engine.project.LookUp shopBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_PRODUCE_SHOP_GOODS);//通过车间产能明细id得到车间产能单号
  ...
  <table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="center" width="760">
    <tr valign="top">
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            <td class="activeVTab">生产入库单(<%=title%>)
			<%--03.19 20:32 新增 新增为方便不退出此页面就可以直接打印xxx_top.jsp上列出的单据而加的上一笔,下一笔按钮 yjg--%>
             ...
             <%}
               if (!isAtLast)
              {%>
              <a href="#" title="到下一笔(ALT+X)" onClick="sumitForm(<%=selfGainBean.NEXT%>)">&gt</a>
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      shopBean.regData(list,"cjcnmxid");
                 ...
                 <%--   <input name="image" class="img" type="image" title="配批号" onClick="if(form1.cpid_<%=i%>.value==''){alert('请输入产品');return;}if(form1.storeid.value==''){alert('请输入产品'); return;}BatchMultiSelect('form1','srcVar=mutibatch_<%=i%>&cpid='+form1.cpid_<%=i%>.value+'&storeid='+form1.storeid.value+'&dmsxid='+form1.dmsxid_<%=i%>.value)" src="../images/edit.old.gif" border="0">--%>
                   ...
                   <%--        'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=scydw&fieldVar=isprops&fieldVar=hsbl&fieldVar=scdwgs&fieldVar=isbatchno','&storeid='+form1.storeid.value)">--%>
                      ...
                      <%--  <td class="td" nowrap><%=processBean.getLookupName(detail.get("jgdmxid"))%></td>--%>
                       <td class="td" nowrap><%=shopBean.getLookupName(detail.get("cjcnmxid"))%></td>
         ...
         <%--    <input type="hidden" name="importProcess" value="">
             ...
             onClick="buttonEventQ()">
              ...
              <input type="hidden" name="scanValue" value="">
               ...
               <input type="hidden" name="importshop" value="">
               <input name="btnback" type="button" class="button"  title="引车间产能表(ALT+Q)"style='width:110' value="引车间产能表(Q)"
               onClick="buttonEventC()">
              <pc:shortcut key="q" script='<%="buttonEventC()"%>'/>
              ...
              <%}%>
              <%if(isCanDelete && !selfGainBean.isReport){%><input name="button3" type="button" class="button" title="删除(ALT+D)" style='width:50' value="删除(D)" onClick="buttonEventD();">
              ...
              <%if(!selfGainBean.isApprove && !selfGainBean.isReport){%><input name="btnback" type="button" class="button" title="返回(ALT+C)" style='width:50' value="返回(C)" onClick="backList();">
                <pc:shortcut key="c" script='<%="backList()"%>'/>
              <%}%>
                <%--03.09 11:43 新增 新增关闭按钮提供给当此页面是被报表调用时使用. yjg--%>
                <%if(selfGainBean.isReport){%><input name="btnback" type="button" class="button" title="关闭(ALT+T)" value="关闭(T)"  style='width:50' onClick="window.close()">
...
<script language="javascript">initDefaultTableRow('tableview1',1);
  function onload(){
    <%=selfGainBean.adjustInputSize(new String[]{"cpbm","product","jldw","drawbignum","hsdw", "drawnum","sxz", "memo"},  "form1", selfGainBean.max-min+1, min)%>
  }
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    function propertyChange(i){
     ...
     function newsl_onchange(i, isBigUnit)
     {
       ...
       var obj = isBigUnit ? hsslObj : slObj;//07-19 因为已没有了换算数量,所以全部都变成slOj.下同
  ...
  //引车间产能表
  function ImportShopSelect(frmName,srcVar,fieldVar,curID,storeid,methodName,notin)
  {
    var winopt = "location=no scrollbars=yes menubar=no status=no resizable=1 width=700 height=600 top=0 left=0";
    var winName= "ImportShopSelector";
    paraStr = "../store/import_shop.jsp?operate=0&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+"&deptid="+curID+"&storeid="+storeid;
    if(methodName+'' != 'undefined')
      paraStr += "&method="+methodName;
    if(notin+'' != 'undefined')
      paraStr += "&notin="+notin;
  ...
  function buttonEventC()
  {
   ...
   ImportShopSelect('form1','srcVar=importshop','fieldVar=cjcnid',form1.deptid.value,form1.storeid.value,'sumitForm(<%=selfGainBean.SELF_SEL_SHOP%>)')
     }
     //复制自制收货单
  ...
  {
   location.href='../pub/pdfprint.jsp?code=self_gain_edit_bill&operate=<%=selfGainBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("receiveid")%>&src=../store_xixing/self_gain_edit.jsp'
  }
