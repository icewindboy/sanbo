...
<%String retu = storeBillBean.doService(request, response);
  ...
  boolean showdjje = loginBean.getSystemParam("KC_IN_SHOW_PRICE").equals("1");//是否显示单价与金额
  boolean isHandwork = loginBean.getSystemParam("KC_HANDIN_STOCK_BILL").equals("1");//是否允许手工输入
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                        ...
                        <td class="td" nowrap><input type="hidden" name="wjid_<%=i%>" value="<%=detail.get("wjid")%>"><%=buyOrderGoodsRow.get("jhdbm")%></td>
                        ...
                        <td class="td" nowrap><%=prodRow.get("jldw")%></td>
                         ...
                         <%if(showdjje){%>
                         <td class="td" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="dj_<%=i%>" name="dj_<%=i%>" value='<%=detail.get("dj")%>' maxlength='<%=list.getColumn("dj").getPrecision()%>' onchange="sl_onchange(<%=i%>, false)" readonly></td>
                  ...
                  %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td">&nbsp;</td><td class="td">&nbsp;</td><td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td><%if(showdjje){%><td class="td">&nbsp;</td><td class="td">&nbsp;</td><%}%>
...
<%=storeBillBean.adjustInputSize(new String[]{"cpbm", "product", "ph", "hssl", "sl", "sxz", "bz", "kwid"},  "form1", detailRows.length)%>
  ...
  function nsl_onchange(i, isBigUnit)
  {
    ...
    var bjfs = <%=bjfs%>;
    //var isHandwork = <%=loginBean.getSystemParam("KC_IN_SHOW_PRICE")%>;
  ...
  {
     location.href='../pub/pdfprint.jsp?code=constract_instore_edit_bill&operate=<%=storeBillBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("sfdjid")%>&src=../store_xixing/contract_instore_edit.jsp';
  }
       //新增空白行
