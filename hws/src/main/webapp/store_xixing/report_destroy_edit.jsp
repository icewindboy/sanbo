...
%><%
  engine.erp.store.xixing.B_Destroy destroyBean = engine.erp.store.xixing.B_Destroy.getInstance(request);
  destroyBean.djxz = 7;
...
function productCodeSelect(obj, i)
{
  ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsbl_'+i+'&srcVar=hsdw_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsbl', obj.value);//,'sumitForm(<%=destroyBean.HSBL_ONCHANGE%>,'+i+')'
}
...
<%String retu = destroyBean.doService(request, response);
  ...
  boolean isCanDelete = !isEnd && !destroyBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
                        ...
                        <input type="hidden" name="mutibatch_<%=i%>" value="" onchange="sumitForm(<%=destroyBean.SELECTBATCH%>,<%=i%>)">
                         <input type="hidden" name="singleIdInput_<%=i%>" value="">
                          <input name="image" class="img" type="image" title="单选物资" src="../images/select_prod.gif" border="0"
                            onClick="ProdSingleSelect('form1','srcVar=singleIdInput_<%=i%>','fieldVar=cpid','&storeid='+form1.storeid.value,'sumitForm(<%=destroyBean.SINGLE_PRODUCT_ADD%>,<%=i%>)')">
...
<%=destroyBean.adjustInputSize(new String[]{"cpbm","product", "jldw", "ph",  "sl", "kwid", "sxz", "bz","hssl","hsdw"},  "form1", (destroyBean.max-min+1), min)%>
    ...
    function sl_onchange(i, isBigUnit)
  {
   ...
   {
     location.href='../pub/pdfprint.jsp?code=report_destroy_edit_bill&operate=<%=destroyBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("sfdjid")%>&src=../store_xixing/report_destroy_edit.jsp';
   }
       //2004-3-30 15:44 新增 给部门下拉表加上能使经手人跟随它自己不同部门而变的js函数 yjg
   function deptChange()
   {
