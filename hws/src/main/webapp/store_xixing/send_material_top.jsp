...
%><%
  ...
  String loginId = sendMaterialBean.loginId;
  String bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");//得到登陆用户报价方式的系统参数
  String SYS_APPROVE_ONLY_SELF =sendMaterialBean.SYS_APPROVE_ONLY_SELF;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
  String SC_OUTSTORE_SHOW_ADD_FIELD =sendMaterialBean.SC_OUTSTORE_SHOW_ADD_FIELD;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
...
function toDetail(){
  parent.location.href='send_material_edit.jsp';
}
...
function showDetail(masterRow){
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='send_material_bottom.jsp?operate=<%=sendMaterialBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">发料单列表</TD>
  </TR></TABLE>
<%String retu = sendMaterialBean.doService(request, response);
  ...
  EngineDataSet list = sendMaterialBean.getMaterTable();
  HtmlTableProducer table =null;
  if(SC_OUTSTORE_SHOW_ADD_FIELD.equals("1"))
    table = sendMaterialBean.masterProducer;
  else
    table = sendMaterialBean.newmasterProducer;
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
    ...
    <tr>
      ...
      <td class="td" nowrap align="right">
       <%if(loginBean.hasLimits(pageCode, op_add)){%><input name="search3" type="button" class="button" onClick="sumitForm(<%=Operate.ADD%>,-1)" value="新增(A)" onKeyDown="return getNextElement();">
         ...
         <%--<input name="search4" type="button" class="button" onClick="sumitForm(<%=sendMaterialBean.ADD_BACK_MATERAIL%>,-1)" value="新增退料单(T)" onKeyDown="return getNextElement();">
          <pc:shortcut key="t" script='<%="sumitForm("+ sendMaterialBean.ADD_BACK_MATERAIL +",-1)"%>'/>--%>
         <%}%>
         <input name="search2" type="button" class="button" title="记账(ALT+Z)" value="记账(Z)" onClick="if(confirm('是否确定记帐'))sumitForm(<%=sendMaterialBean.RECODE_ACCOUNT%>)"  onKeyDown="return getNextElement();">
       <pc:shortcut key="z" script='<%="sumitForm("+ sendMaterialBean.RECODE_ACCOUNT +",-1)"%>'/>
      ...
      <%}%>
        <%if(sendMaterialBean.retuUrl!=null){%><input name="button22" type="button" class="button" title="返回(ALT+C)" value="返回(C)" onClick="buttonEventC();" onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      ...
      <td height='20' nowrap>发料单号</td>
      <td height='20' nowrap>发料类型</td>
      <td height='20' nowrap>发料日期</td>
      <td height='20' nowrap>仓库</td>
      <td height='20' nowrap>部门</td>
      <td height='20' nowrap>单据类别</td>
      <td height='20' nowrap>用途</td>
      <td height='20' nowrap>总数量</td>
       <td height='20' nowrap>总金额</td>
      <td height='20' nowrap>经手人</td>
      <td height='20' nowrap>状态</td>
      <td height='20' nowrap>审批人</td>
      <td height='20' nowrap>状态描述</td>
      <td height='20' nowrap>制单人</td>
    ...
    <%BigDecimal t_zsl = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        String zje = list.getValue("totalSum");
        //String zgf = list.getValue("zgf");
        //String zgf2 = list.getValue("zgf2");
        if(sendMaterialBean.isDouble(zsl))
          t_zsl = t_zsl.add(new BigDecimal(zsl));
        if(sendMaterialBean.isDouble(zje))
          t_zje = t_zje.add(new BigDecimal(zje));
        /*if(sendMaterialBean.isDouble(zgf))
           t_zgf = t_zgf.add(new BigDecimal(zgf));
        if(sendMaterialBean.isDouble(zgf2))
    ...
    <tr  id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <%if(isCancel){%><input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=sendMaterialBean.RECEIVE_CANCEL_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%></td>
 ...
 <%--      <tr>
      ...
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=sendMaterialBean.getZsl()%>' readonly></td>
       <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=sendMaterialBean.getZje()%>' readonly></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
...
<%if(!sendMaterialBean.masterIsAdd()){
    int row = sendMaterialBean.getSelectedRow();
    if(row >= 0)
...
{
  parent.location.href='<%=sendMaterialBean.retuUrl%>';
}
//关闭查询小视窗
...
<form name="fixedQueryform" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <div class="queryPop" id="fixedQuery" name="fixedQuery">
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0><%sendMaterialBean.table.printWhereInfo(pageContext);%>
            <%--TR>
              <TD class="td" nowrap>单据号</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="sfdjdh" name="sfdjdh" value='<%=sendMaterialBean.getFixedQueryValue("sfdjdh")%>' maxlength='10' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>部门</TD>
              <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
         <%=deptBean.getList(sendMaterialBean.getFixedQueryValue("deptid"))%></pc:select>
           </TD>
            <TR>
              <TD nowrap class="td">收发日期</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="sfrq$a" value='<%=sendMaterialBean.getFixedQueryValue("sfrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="sfrq$b" style="WIDTH: 130px" name="sfrq$b" value='<%=sendMaterialBean.getFixedQueryValue("sfrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <%=produceUseBean.getList(sendMaterialBean.getFixedQueryValue("ytid"))%></pc:select>
              </TD>
              <TD nowrap class="td">仓库</TD>
              <TD nowrap class="td"><pc:select name="storeid" addNull="1" style="width:160">
            <%=storeBean.getList(sendMaterialBean.getFixedQueryValue("storeid"))%></pc:select>
           </TD>
            </TR>
           </TR>
            <TR>
              <TD align="center" nowrap class="td">产品名称</TD>
               <td nowrap class="td" colspan="3"><input class="EDLine" style="WIDTH:330px" name="product" value='<%=sendMaterialBean.getFixedQueryValue("product")%>' onKeyDown="return getNextElement();"readonly>
                <INPUT TYPE="HIDDEN" NAME="sfdjid" value="<%=sendMaterialBean.getFixedQueryValue("sfdjid")%>"><img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProdSingleSelect('fixedQueryform','srcVar=sfdjid&srcVar=product','fieldVar=cpid&fieldVar=product',fixedQueryform.sfdjid.value)">
               <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="sfdjid.value='';product.value='';">
            </td>
            </tr>
             <TR>
              <TD class="td" nowrap>产品编码</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="sfdjid$cpbm" name="sfdjid$cpbm" value='<%=sendMaterialBean.getFixedQueryValue("sfdjid$cpbm")%>' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>品名规格</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="sfdjid$product" name="sfdjid$product" value='<%=sendMaterialBean.getFixedQueryValue("sfdjid$product")%>' onKeyDown="return getNextElement();"></TD>
            </TR>
            <TR>
              <TD class="td" nowrap>状态</TD>
              <TD colspan="3" nowrap class="td">
                <%String zt = sendMaterialBean.getFixedQueryValue("zt");%>
