...
<%String retu = selfGainBean.doService(request, response);
  ...
  engine.project.LookUp processBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_PRODUCE_PROCESS_GOODS2);//通过加工单物料id得到加工单号
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      processBean.regData(list,"jgdwlid");
                      ...
                      for(i=min; i<=max && i<detailRows.length; i++){
                        ...
                        String jgdwlid=detail.get("jgdwlid");
                        boolean isimport = !jgdwlid.equals("");//引入加工单，从表产品编码当前行不能修改
                    %>
                    ...
                    <iframe id="prod_<%=i%>" src="" width="0" height=0 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                       ...
                       <td class="td" nowrap><%=processBean.getLookupName(detail.get("jgdwlid"))%></td>
                       ...
                       <input type='hidden' id='truebl_<%=i%>' name='truebl_<%=i%>' value=''>
                        ...
                        <input type="hidden" class=ednone onKeyDown="return getNextElement();" id="drawsum_<%=i%>" name="drawsum_<%=i%>" value='<%=detail.get("drawsum")%>' readonly>
