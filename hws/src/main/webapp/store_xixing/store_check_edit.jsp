...
%><%
  engine.erp.store.xixing.B_StoreCheck storeCheckBean = engine.erp.store.xixing.B_StoreCheck.getInstance(request);
...
{
  ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsdw_'+i+'&srcVar=hsbl_'+i+'&srcVar=singleIdInput_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=hsbl&fieldVar=cpid', obj.value,'sumitForm(<%=storeCheckBean.SINGLE_PRODUCT_ADD%>,'+i+')');//
  document.all['dmsxid_'+i].value="0";
}
function prodchange(i){
  document.all['zcsl_'+i].value="0";
  document.all['zchssl_'+i].value="0";

}
function productNameSelect(obj,i)
{
  ProdNameChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsdw_'+i+'&srcVar=hsbl_'+i+'&srcVar=singleIdInput_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=hsbl&fieldVar=cpid', obj.value,'sumitForm(<%=storeCheckBean.SINGLE_PRODUCT_ADD%>,'+i+')');//
}
//02.20 18:21 下面两个函数为 增加上 查看或生成损溢单的图标 而加.同时相应的js和DIV也做了增加 yjg
function showInterFrame(pdid, deptid,storeid,row)
{
     var url = "../store_xixing/check_make_destroy.jsp?operate=0&pdid="+pdid+"&deptid="+deptid+"&storeid="+storeid+"&rownum="+row;
      ...
      <td><table border=0 CELLSPACING=0 CELLPADDING=0>
          <tr>
            <td class="activeVTab">库存盘点单(<%=title%>)
              ...
              <%if (!isAtFirst)
              {%>
              <a href="#" title="到上一笔(ALT+Z)" onClick="sumitForm(<%=storeCheckBean.PRIOR%>)">&lt
              <pc:shortcut key='z' script='<%="sumitForm("+storeCheckBean.PRIOR+")"%>'/></a>
             <%}%>
               <%if (!isAtLast)
              {%>
              <a href="#" title="到下一笔(ALT+X)" onClick="sumitForm(<%=storeCheckBean.NEXT%>)">&gt
              <pc:shortcut key='x' script='<%="sumitForm("+storeCheckBean.NEXT+")"%>'/></a>
             <%}//
              }%>
            </td>
          </tr>
                    ...
                    %>
                        <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                          ...
                          <td class="td" nowrap align="center">
                           <iframe id="prod_<%=i%>" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                            ...
                            <input name="image" class="img" type="image" title="单选物资" src="../images/select_prod.gif" border="0"
                             onClick="ProdSingleSelect('form1','srcVar=singleIdInput_<%=i%>','fieldVar=cpid','&storeid='+form1.storeid.value,'sumitForm(<%=storeCheckBean.SINGLE_PRODUCT_ADD%>,<%=i%>)')">
                         ...
                         %>
                          ...
                          <td class="td" nowrap align="right"><input type="text" class="ednone"   onKeyDown="return getNextElement();" id="jldw_<%=i%>" name="jldw_<%=i%>" value='<%=prodRow.get("jldw")%>' readonly></td>
                          <td class="td" nowrap align="right"><input type="text" class="ednone"   onKeyDown="return getNextElement();" id="zchssl_<%=i%>" name="zchssl_<%=i%>" value='<%=detail.get("zchssl")%>' readonly></td>
                         <td class="td" nowrap align="right"> <input type="text" <%=detailClass_r%>   onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=detail.get("hssl")%>' onFocus="saveCurrentVariable(<%=i%>)" onchange="sl_onchange(<%=i%>, true)" maxlength='<%=list.getColumn("hssl").getPrecision()%>' <%=readonly%>>
                          </td>
                          <%--
...
<%=storeCheckBean.adjustInputSize(new String[]{"cpbm","product", "jldw", "ph", "sxz", "zcsl", "scsl", "isLoss", "ce", "kwid"},  "form1", (storeCheckBean.max-min+1), min)%>
...
<SCRIPT language="javascript">
 ...
 {
    location.href='../pub/pdfprint.jsp?code=store_check_edit_bill&operate=<%=storeCheckBean.PRINT_BILL%>&a$pdid=<%=masterRow.get("pdid")%>&src=../store_xixing/store_check_edit.jsp';
 }
 //查看或生成损益单
 <%
   if ( !masterRow.get("pdhm").equals("") )
