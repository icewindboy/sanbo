...
%>
<%
  engine.erp.jit.Select_Gzz_Person Select_Gzz_PersonBean = engine.erp.jit.Select_Gzz_Person.getInstance(request);
  ...
  String retu = Select_Gzz_PersonBean.doService(request, response);
  ...
  EngineDataSet list = Select_Gzz_PersonBean.getOneTable();
...
<%
  if(inputName != null && fieldName != null)
  {
    ...
    for(int i=0; i< length; i++)
    {
      ...
      out.print(".value=");     out.println("obj.value");//+ (fieldName[i].equals("personid")?"value;":"innerText;"));
    }
  }
  if(methodName != null)
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">员工列表</TD>
  </TR>
</TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tbcontrol" width="90%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      <td height="21" nowrap class="td"><%String key = "invoiceprod"; pageContext.setAttribute(key, list);   int iPage = loginBean.getPageSize(); String pageSize = ""+iPage;%>
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkTr(<%=i%>);checkRadio(<%=i%>);selectCorp(<%=i%>);"<%}%>>
      <td class="td">
        <%if(isMultiSelect){%>
        <input type="checkbox" name="sel" value="<%=list.getValue("personid")%>" onKeyDown="return getNextElement();">
        <%}else{%>
        <input type="radio" name="sel" onKeyDown="return getNextElement();" value="<%=i%>"<%=i==0?" checked":""%>>
        <%}%><input type="hidden" id="personid_<%=i%>" value='<%=list.getValue("personid")%>'>
      </td>
      <td nowrap align="left" id="deptid_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=deptBean.getLookupName(list.getValue("deptid"))%></td>
      <td nowrap align="left" id="personid_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=personBean.getLookupName(list.getValue("personid"))%></td>
    </tr>
    <%  list.next();
      }
      for(; i < iPage; i++){
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD class="td" nowrap>
              <input type='txt' class="edbox" name="xm" value='<%=Select_Gzz_PersonBean.getFixedQueryValue("xm")%>' style='width:110'  >
              </TD>
             </TR>
            <TR>
            <TR>
