...
%><%
  engine.erp.jit.PlanSelectSaleDetail planSelectSaleDetail = engine.erp.jit.PlanSelectSaleDetail.getInstance(request);
  String pageCode = "produce_plan";
  if(!loginBean.hasLimits(pageCode, request, response))
    return;
  String retu = planSelectSaleDetail.doService(request, response);
  ...
  EngineDataSet list = planSelectSaleDetail.getOneTable();
%>
<html>
<head>
<title></title>
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      ...
      <td nowrap>未计划数量</td>
      <td nowrap>已计划数量</td>
      <td nowrap title="可用于生产计划的帐面库存供应量">计划可供量</td>
      <td nowrap>最低库存量</td>
      <td nowrap>计量单位</td>
      <!--td nowrap>生产数量</td>
      <td nowrap>生产单位</td-->
    ...
    <tr onClick="checkTr(<%=i%>)">
      <td nowrap align="center" class="td"><input type="checkbox" name="sel" value='<%=list.getValue("hthwid")%>' onKeyDown="return getNextElement();"><input type="hidden" id="hthwid_<%=i%>" value='<%=list.getValue("hthwid")%>'></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="htbh_<%=i%>" class="td"><%=list.getValue("htbh")%></td>
      <td nowrap align="center" id="dwmc_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("dwmc")%></td>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="dmsxid_<%=i%>" class="td"><%=list.getValue("sxz")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sl_<%=i%>" class="td" align="right"><%=list.getValue("sl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="wjhsl_<%=i%>" class="td" align="right"><%=list.getValue("wjhsl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="yjhsl_<%=i%>" class="td" align="right"><%=list.getValue("yjhsl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="jhkgl_<%=i%>" class="td" align="right"><%=list.getValue("jhkgl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="minsl_<%=i%>" class="td" align="right"><%=list.getValue("minsl")%></td>
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="htbh$a" name="htbh$a" value='<%=planSelectSaleDetail.getFixedQueryValue("htbh$a")%>' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>--</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="htbh$b" name="htbh$b" value='<%=planSelectSaleDetail.getFixedQueryValue("htbh$b")%>' onKeyDown="return getNextElement();"></TD>
              </TR>
              <TR>
              <TD class="td" nowrap>单位编码</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="dwdm$a" name="dwdm$a" value='<%=planSelectSaleDetail.getFixedQueryValue("dwdm$a")%>' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>--</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="dwdm$b" name="dwdm$b" value='<%=planSelectSaleDetail.getFixedQueryValue("dwdm$b")%>' onKeyDown="return getNextElement();"></TD>
              </TR>
              <%String cpid = planSelectSaleDetail.getFixedQueryValue("cpid");
                String dwtxid = planSelectSaleDetail.getFixedQueryValue("dwtxid");
             ...
             <td nowrap class='td'colspan='3'>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="dwmc" name="dwmc" value='<%=planSelectSaleDetail.getFixedQueryValue("dwmc")%>' onKeyDown="return getNextElement();"></TD>
              <%--TD align="center" nowrap class="td">产品名称</TD>
              <td nowrap class="td"><input class="EDLine" style="WIDTH:130px" name="cp_product" value='<%=prodBean.getLookupName(planSelectSaleDetail.getFixedQueryValue("cpid"))%>' onKeyDown="return getNextElement();"readonly>
                <INPUT TYPE="HIDDEN" NAME="cpid" value="<%=planSelectSaleDetail.getFixedQueryValue("cpid")%>">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="jhrq$a" value='<%=planSelectSaleDetail.getFixedQueryValue("jhrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="jhrq$b" style="WIDTH: 130px" name="jhrq$b" value='<%=planSelectSaleDetail.getFixedQueryValue("jhrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:130" id="cpbm" name="cpbm" value='<%=planSelectSaleDetail.getFixedQueryValue("cpbm")%>' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>品名规格</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:130" id="product" name="product" value='<%=planSelectSaleDetail.getFixedQueryValue("product")%>' onKeyDown="return getNextElement();"></TD>
            </TR>
            <%RowMap searchRow = planSelectSaleDetail.getSearchRow();
              EngineDataSet dsSpec = planSelectSaleDetail.getPropertyTable();
           %>
                       <%dsSpec.first();
      ...
      <TR>
        <TD colspan="4" nowrap class="td" align="center"><INPUT class="button" onClick="sumitFixedQuery(<%=planSelectSaleDetail.FIXED_SEARCH%>)" type="button" value=" 查询(F)" name="button" onKeyDown="return getNextElement();">
           <pc:shortcut key="f" script='<%="sumitFixedQuery("+ planSelectSaleDetail.FIXED_SEARCH +",-1)"%>'/>
