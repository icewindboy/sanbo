<%--生产计划中生产任务编辑页面从表--%><%@ page contentType="text/html; charset=UTF-8" %><%@ include file="../pub/init.jsp"%>
...
%><%
  engine.erp.jit.PlanMakeProcess planMakeProcessBean = engine.erp.jit.PlanMakeProcess.getInstance(request);
...
<script language="javascript">
   ...
   function backList()
   {
     location.href='plan_make_task.jsp';
   }
...
<%String retu = planMakeProcessBean.doService(request, response);
  if(retu.indexOf("backList();")>-1)
  {
    out.print(retu);
    return;
  }
  String SC_PRODUCE_UNIT_STYLE = planMakeProcessBean.SC_PRODUCE_UNIT_STYLE;
  ...
  EngineDataSet ds = planMakeProcessBean.getMaterTable();
  EngineDataSet list = planMakeProcessBean.getDetailTable();
  HtmlTableProducer masterProducer = planMakeProcessBean.masterProducer;
  HtmlTableProducer detailProducer = planMakeProcessBean.detailProducer;
  RowMap masterRow = planMakeProcessBean.getMasterRowinfo();
  RowMap[] detailRows= planMakeProcessBean.getDetailRowinfos();
  ...
  boolean isEnd = planMakeProcessBean.isApprove || (!planMakeProcessBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !planMakeProcessBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(planMakeProcessBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  String SYS_PRODUCT_SPEC_PROP =planMakeProcessBean.SYS_PRODUCT_SPEC_PROP;//生产用位换算的相关规格属性名称 得到的值为“宽度”……
%>
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" width="100%" bgcolor="#f0f0f0">
                 <%workShopBean.regData(ds,"deptid");%>
                  ...
                  <%String sumit = "if(form1.deptid.value!='"+masterRow.get("deptid")+"'){sumitForm("+planMakeProcessBean.ONCHANGE+");}";%>
                  <td noWrap class="td">
                    <%if(isEnd || !isCanAmend) out.print("<input type='text' value='"+workShopBean.getLookupName(masterRow.get("deptid"))+"' style='width:110' class='edline' readonly>");
                    else {%>
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      for(; i<detailRows.length; i++)   {
                        ...
                        if(planMakeProcessBean.isDouble(sl))
                          t_sl = t_sl.add(new BigDecimal(sl));
                        String scsl = detail.get("scsl");
                        if(planMakeProcessBean.isDouble(scsl))
                          t_scsl = t_scsl.add(new BigDecimal(scsl));
                        String yjgl = detail.get("yjgl");
                        if(planMakeProcessBean.isDouble(yjgl))
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap><%if(isEnd || !isCanRework)out.print("<input type='text' style='width:100' value='"+technicsRouteBean.getLookupName(detail.get("gylxid"))+"' class='ednone' readonly>");
                        else {%>
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
              ...
              <%if(!planMakeProcessBean.isApprove){%><input name="btnback" type="button" class="button" onClick="backList();" value=" 返回(C) ">
...
<%=planMakeProcessBean.adjustInputSize(new String[]{"cpbm", "product", "sxz", "csl","sl","scsl", "jgyq", "yjgl"}, "form1", detailRows.length)%>
  ...
  <%if(planMakeProcessBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</body>
</html>
