...
%>
<%
  engine.erp.jit.PlanSelectSale singleSelectOrderBean = engine.erp.jit.PlanSelectSale.getInstance(request);
  ...
  String retu = singleSelectOrderBean.doService(request, response);
  ...
  EngineDataSet list = singleSelectOrderBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center"><%=jhlx.equals("0")?"销售合同列表":"外贸合同列表"%></TD>
  </TR></TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectProduce(<%=i%>);"<%}%>>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="htrq_<%=i%>" class="td"><%=list.getValue("jhfhrq")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="dwtxid_<%=i%>" class="td"><%=corpBean.getLookupName(list.getValue("dwtxid"))%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="shprt_<%=i%>" class="td"><%=list.getValue("shprt")%></td>

      <td nowrap onClick="checkRadio(<%=i%>)" id="khlx_<%=i%>" class="td"><%=list.getValue("khlx")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="xm_<%=i%>" class="td"><%=personBean.getLookupName(list.getValue("personid"))%></td>
    </tr>
    <%  list.next();
      }
      for(; i < iPage; i++){
