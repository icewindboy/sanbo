...
<SCRIPT LANGUAGE="javascript">
  ...
  {
    var url = "sc_process_factory_edit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>执行工厂设置</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = B_Process_FactoryBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;"onKeyDown="return onInputKeyboard();">
...
</td> <TD align="right"><%if(B_Process_FactoryBean.retuUrl!=null){%><input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=B_Process_FactoryBean.retuUrl%>'" value=" 返回(C)"border="0">
  <% String back ="location.href='"+B_Process_FactoryBean.retuUrl+"'" ;%>
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      ...
      <%}%></td>
      <td class="td"><%=list.getValue("fctrybm")%></td>
      <td class="td"><%=list.getValue("fctryname")%></td>
      <td class="td"><%=list.getValue("fctrybz")%></td>
    </tr>
    <%  list.next();
      }
...
<div class="queryPop" id="detailDiv" name="detailDiv">
  ...
  <TABLE cellspacing=0 cellpadding=0 border=0>
    <TR>
      <TD><iframe id="interframe1" src="" width="450" height="300" marginWidth="0" marginHeight="0" frameBorder="0" noResize scrolling="no"></iframe>
      </TD>
    </TR>
  </TABLE>
</div>
