<%--工人工作量（按工人输入）列表--%><%@page contentType="text/html; charset=UTF-8" %><%@ include file="../pub/init.jsp"%>
...
function toDetail(){
  parent.location.href='workload_worker_edit.jsp';
}
function showDetail(masterRow){
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='workload_worker_bottom.jsp?operate=<%=workloadWorkerBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">工人工作量（按工人输入）列表</TD>
  </TR></TABLE>
<%String retu = workloadWorkerBean.doService(request, response);
  ...
  EngineDataSet list = workloadWorkerBean.getMaterTable();
  HtmlTableProducer table = workloadWorkerBean.masterProducer;
  ...
  <table id="tbcontrol" width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" nowrap align="right"><%if(hasSearchLimit){%><input name="search2" type="button" class="button" onClick="showFixedQuery()" value=" 查询(Q)" onKeyDown="return getNextElement();">
        ...
        <%if(workloadWorkerBean.retuUrl!=null){%><input name="button22" type="button" class="button" onClick="parent.location.href='<%=workloadWorkerBean.retuUrl%>'" value=" 返回(C)" onKeyDown="return getNextElement();">
        <% String back ="parent.location.href='"+workloadWorkerBean.retuUrl+"'" ;%>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%BigDecimal t_zje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        if(workloadWorkerBean.isDouble(zje))
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <%if(isComplete){%><input name="image5" class="img" type="image" title='完成' onClick="if(confirm('是否强制完成该纪录？'))sumitForm(<%=workloadWorkerBean.COMPLETE%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
      </td>
...
<%if(!workloadWorkerBean.masterIsAdd()){
    int row = workloadWorkerBean.getSelectedRow();
    if(row >= 0)
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
         ...
         <%=workShopBean.getList(workloadWorkerBean.getFixedQueryValue("deptid"))%></pc:select></TD>
              <TD class="td" nowrap>员工</TD>
              <TD nowrap class="td"><pc:select name="personid" addNull="1" style="width:130">
         <%=personBean.getList(workloadWorkerBean.getFixedQueryValue("personid"))%></pc:select></TD>
            </TR>
           <TR>
              <TD nowrap class="td">日期</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="rq$a" value='<%=workloadWorkerBean.getFixedQueryValue("rq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="rq$b" style="WIDTH: 130px" name="rq$b" value='<%=workloadWorkerBean.getFixedQueryValue("rq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              <TD align="center" nowrap class="td">产品名称</TD>
               <td nowrap class="td" colspan="3"><input class="EDLine" style="WIDTH:330px" name="product" value='<%=workloadWorkerBean.getFixedQueryValue("product")%>' onKeyDown="return getNextElement();"readonly>
                <INPUT TYPE="HIDDEN" NAME="gzlid" value="<%=workloadWorkerBean.getFixedQueryValue("gzlid")%>"><img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProdSingleSelect('fixedQueryform','srcVar=gzlid&srcVar=product','fieldVar=cpid&fieldVar=product',fixedQueryform.gzlid.value)">
               <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="gzlid.value='';product.value='';">
            </TD>
            </TR>
             <TR>
              <TD class="td" nowrap>产品编码</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:130" id="gzlid$cpbm" name="gzlid$cpbm" value='<%=workloadWorkerBean.getFixedQueryValue("gzlid$cpbm")%>' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>品名规格</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:130" id="gzlid$product" name="gzlid$product" value='<%=workloadWorkerBean.getFixedQueryValue("gzlid$product")%>' onKeyDown="return getNextElement();"></TD>
            </TR>
            <TR>
              <TD class="td" nowrap>状态</TD>
              <TD colspan="3" nowrap class="td">
                <%String zt = workloadWorkerBean.getFixedQueryValue("zt");%>
