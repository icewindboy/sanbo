...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tbcontrol" width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" nowrap align="right"><input name="search3" type="button" class="button" onClick="LookSaleOrder()" value=" 查看销售合同(T)" onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      ...
      <pc:shortcut key="a" script='<%="addExcute()"%>'/><%}%>
      </td>
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
     ...
     <%if(isShowMrp){%><input name="image" class="img" type="image" title=<%=zt.equals("1") ? "生成物料需求" : "查看物料需求"%> onClick="MakeMrp('form1','scjhid='+<%=list.getValue("scjhid")%>+'&deptid='+<%=list.getValue("deptid")%>+'&rownum='+<%=list.getRow()%>+'&jhlx='+<%=list.getValue("jhlx")%>)" src='../images/edit.old.gif' border="0"><%}%>
      <%if(isShowTask){%><input name="image" class="img" type="image" title="下达车间任务" onClick="MakeProduceTask('form1','scjhid='+<%=list.getValue("scjhid")%>+'&row='+<%=list.getRow()%>+'&jhlx='+<%=list.getValue("jhlx")%>)" src='../images/select_prod.gif' border="0"><%}%>
...
function MakeProduceTask(frmName, srcVar, methodName,notin)//下达生产任务
{
  ...
  paraStr = "../produce/plan_make_task.jsp?operate=0&srcFrm="+frmName+"&"+srcVar;
...
function MakeMrp(frmName, srcVar, methodName,notin)//下达物料需求
{
  var winopt = "location=no scrollbars=yes menubar=no status=no resizable=1 width=790 height=570 top=0 left=0";
  var winName= "MakeMrp";
  paraStr = "../produce/produce_make_mrp.jsp?operate=0&srcFrm="+frmName+"&"+srcVar;
  if(methodName+'' != 'undefined')
    paraStr += "&method="+methodName;
  if(notin+'' != 'undefined')
    paraStr += "&notin="+notin;
...
function LookSaleOrder()
{
  ...
  paraStr = "../produce/sale_order_list.jsp?operate=1016";
  newWin =window.open(paraStr,winName,winopt);
  newWin.focus();
}
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
         ...
         <%=workShopBean.getList(producePlanBean.getFixedQueryValue("deptid"))%></pc:select>
            </TR>
           <TR>
       ...
       <TABLE cellspacing=2 cellpadding=0 border=0>
        <TR>
         <TD>
          ...
          <%--<input type="radio" name="lx" value="1">B类计划--%>
          </TD>
          <TD>&nbsp;&nbsp;&nbsp;&nbsp;</TD>
         </TR>
        </TD>
