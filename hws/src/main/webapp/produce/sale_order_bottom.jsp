...
<%@ page import="com.borland.dx.dataset.DataSet"%>
  ...
  if(!loginBean.hasLimits(pageCode, request, response))
    return;
  engine.erp.produce.B_RelateOrder B_RelateOrderBean = engine.erp.sale.dafa.B_SaleOrder.getInstance(request);
  synchronized(B_RelateOrderBean){
  B_RelateOrderBean.doService(request, response);

  EngineDataSet list = B_RelateOrderBean.getDetailTable();
  HtmlTableProducer detailProducer = B_RelateOrderBean.detailProducer;
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      if(list.changesPending())
        B_RelateOrderBean.openDetailTable(B_RelateOrderBean.masterIsAdd());
