<%--生产工人工作量引入加工单界面--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%><%
  engine.erp.produce.ImportProcess importProcessBean = engine.erp.produce.ImportProcess.getInstance(request);
  String retu = importProcessBean.doService(request, response);
  ...
  EngineDataSet list = importProcessBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%RowMap prodRow = prodBean.getLookupRow(list.getValue("cpid"));%>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="jgdh_<%=i%>" class="td"><%=list.getValue("jgdh")%><input type="hidden" id="jgdmxid_<%=i%>" value='<%=list.getValue("jgdmxid")%>'></td>
