...
<%String retu = produceOutProcessBean.doService(request, response);
  ...
  EngineDataSet dsMaterail = produceOutProcessBean.getMaterailTable();//加工单领料清单
  ...
  HtmlTableProducer materailProducer = produceOutProcessBean.materailProducer;//领料清单
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <%if(isEdit){%>
                       <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=produceOutProcessBean.DETAIL_ADD%>)" src="../images/add.gif" border="0">
                              <pc:shortcut key="a" script='<%="sumitForm("+ produceOutProcessBean.DETAIL_ADD +")"%>'/><%}%>
                        </td>
                        ...
                        <tr>
                         <td colspan="8" noWrap class="td">
                           ...
                           <td height='20' align="center" nowrap width=30>
                             ...
                             <input name="image" class="img" type="image" title="新增(Q)" onClick="sumitForm(<%=produceOutProcessBean.MATERAIL_ADD%>)" src="../images/add.gif" border="0">
                              <pc:shortcut key="q" script='<%="sumitForm("+ produceOutProcessBean.MATERAIL_ADD +")"%>'/><%}%>
                            </td>
                            <td nowrap>物料编码</td>
                            <td nowrap>物料品名规格</td>
                        ...
                        %>
                          ...
                          <input name="image" class="img" type="image" title="单选物资" src="../images/select_prod.gif" border="0"
                             onClick="ProdSingleSelect('form1','srcVar=wlcpid_<%=i%>&srcVar=wlproduct_<%=i%>&srcVar=wlcpbm_<%=i%>&srcVar=wljldw_<%=i%>&srcVar=wlscydw_<%=i%>&srcVar=wlscdwgs_<%=i%>&srcVar=wlisprops_<%=i%>','fieldVar=cpid&fieldVar=product&fieldVar=cpbm&fielfVar=jldw&fieldVar=scydw&filedVar=scdwgs&fieldVar=isprops','')">
                              <input name="image" class="img" type="image" title="删除" onClick="if(confirm('是否删除该记录？')) sumitForm(<%=produceOutProcessBean.MATERAIL_DEL%>,<%=m%>)" src="../images/delete.gif" border="0">
                              <%}%>
                            </td>
...
<%=produceOutProcessBean.adjustInputSize(new String[]{"wlcpbm", "wlproduct", "wlsxz", "wlsl","wlscsl"}, "form1", materailRows.length)%>
