<%--物料需求引入生产计划主表界面--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%>
<%
  engine.erp.produce.Select_ProducePlan select_ProducePlanBean = engine.erp.produce.Select_ProducePlan.getInstance(request);
  String pageCode = "mrp";
  ...
  String retu = select_ProducePlanBean.doService(request, response);
  ...
  EngineDataSet list = select_ProducePlanBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">生产计划选择</TD>
  </TR></TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectProduce(<%=i%>);"<%}%>>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="jhh_<%=i%>" class="td"><%=list.getValue("jhh")%><input type="hidden" name="scjhid_<%=i%>" value="<%=list.getValue("scjhid")%>"></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="jhrq_<%=i%>" class="td"><%=list.getValue("jhrq")%></td>
      <td nowrap align="right" id="zsl_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("zsl")%></td>
      <td nowrap align="center" id="deptid_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=workShopBean.getLookupName(list.getValue("deptid"))%></td>
      <td nowrap id="jhsm_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("jhsm")%></td>
    </tr>
    <%  list.next();
      }
      for(; i < iPage; i++){
