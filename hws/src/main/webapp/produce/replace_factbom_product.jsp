<%--替换实际BOM表的可选件页面--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%>
<%
  engine.erp.produce.ClientBomReplace clientBomReplaceBean = engine.erp.produce.ClientBomReplace.getInstance(request);
  ...
  String retu = clientBomReplaceBean.doService(request, response);
  ...
  EngineDataSet list = clientBomReplaceBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">物料可替换件列表</TD>
  </TR></TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tbcontrol" width="90%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" align="right">
        ...
        <input name="button1" type="button" class="button" onClick="selectBomProduct();" value=" 选用 " onKeyDown="return getNextElement();">
        <%}%>
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectBomProduct(<%=i%>);"<%}%>>
      <td class="td">
      <input type="radio" name="sel" onKeyDown="return getNextElement();" value="<%=i%>"<%=i==0?" checked":""%>></td>
      <td nowrap onClick="checkRadio(<%=i%>)" align="left" id="cpbm_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=prodRow.get("cpbm")%><input type="hidden" name="cpid_<%=i%>" value="<%=list.getValue("cpid")%>"></td>
      <td nowrap onClick="checkRadio(<%=i%>)" align="left" id="product_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=prodRow.get("product")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" align="center" id="jldw_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=prodRow.get("jldw")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sl_<%=i%>" class="td" align="right"><%=list.getValue("sl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="shl_<%=i%>" align="right" class="td"><%=list.getValue("shl")%></td>
    </tr>
    <%  list.next();
      }
      for(; i < iPage; i++){
