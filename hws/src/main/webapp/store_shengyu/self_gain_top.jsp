...
function toDetail(){
  parent.location.href='self_gain_edit.jsp';
}
...
function showDetail(masterRow){
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='self_gain_bottom.jsp?operate=<%=selfGainBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
function deptChange(){
   associateSelect(document.all['prod'], '<%=engine.project.SysConstant.BEAN_WORK_GROUP%>', 'gzzid', 'deptid', eval('form1.SC_RECEIVEPROD$DEPTID.value'), '',true);
}
</script>
<BODY oncontextmenu="window.event.returnValue=true">
<iframe id="prod" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">自制收货列表</TD>
  </TR></TABLE>
<%String retu = selfGainBean.doService(request, response);
  ...
  HtmlTableProducer tableList = selfGainBean.masterListProducer;
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      ...
      <%tableList.printTitle(pageContext, "height='20'");%>
    </tr>
    ...
    <tr  id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <%tableList.printCells(pageContext, rowClass);%>
    </tr>
    <%  list.next();
      }
      i=count+1;
    ...
    <td class="td" nowrap></td>
    <td class="td" nowrap>&nbsp;</td>
     <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=selfGainBean.getZsl()%>' readonly></td>
     <td class="td" nowrap></td>
     <td class="td" nowrap></td>
     <td class="td" nowrap></td>
     <td class="td" nowrap></td>
     ...
     <td class="td" nowrap></td>
      ...
      for(; i < iPage; i++){
        out.print("<tr>");
        tableList.printBlankCells(pageContext, "class=td");
        out.print("<td>&nbsp;</td></tr>");
      }
    %>
  </table>
...
<form name="fixedQueryform" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <div class="queryPop" id="fixedQuery" name="fixedQuery">
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0><%selfGainBean.table.printWhereInfo(pageContext);%>
            <%--TR>
