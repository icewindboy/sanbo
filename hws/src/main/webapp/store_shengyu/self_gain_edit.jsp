...
<%String retu = selfGainBean.doService(request, response);
  ...
  engine.project.LookUp beanSalePriceBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_PROD_PRICE);//取得零售价
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="center">
                          ...
                          <img style='cursor:hand' title='单选物资' src='../images/select_prod.gif' border=0 onClick="ProdSingleSelect('form1','srcVar=cpid_<%=i%>&srcVar=cpbm_<%=i%>&srcVar=product_<%=i%>&srcVar=jldw_<%=i%>&srcVar=hsdw_<%=i%>&srcVar=hsbl_<%=i%>&srcVar=scydw_<%=i%>&srcVar=scdwgs_<%=i%>&srcVar=isprops_<%=i%>&srcVar=isbatchno_<%=i%>',
                          ...
                          <%}%>
                          <%if(!zt.equals("0")){%><input name="image" class="img" type="image" title="合格证打印" onClick='certifyCardPrint(form1.cpid_<%=i%>.value,form1.lsj_<%=i%>.value)' src="../images/print.gif" border="0"><%}%>
                        </td>
