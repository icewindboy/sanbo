...
function productCodeSelect(obj, i)
{
  ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsdw_'+i+'&srcVar=hsbl_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=hsbl', obj.value, 'product_change('+i+')');
}
...
{
  ProdNameChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsdw_'+'&srcVar=hsbl_'+i+'&storeid='+form1.storeid.value,
               'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=hsbl', obj.value,'product_change('+i+')');
}
...
<%String retu = storeBillBean.doService(request, response);
  ...
  boolean is_KC_IN_SHOW_PRICE = storeBillBean.KC_IN_SHOW_PRICE.equals("1")?true:false;//是否允许显示单价金额
  ...
  boolean isCanDelete = !isEnd && !storeBillBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete)
                        && loginId.equals(zdrid);//没有结束,在修改状态,并有删除权限,2004-08-04 并且登陆人等于制单人
  isEnd = isEnd
          || !(storeBillBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit))
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                        ...
                        <%if(!isEnd){%>
                        <img style='cursor:hand' src='../images/view.gif' border=0 onClick="if(form1.cpid_<%=i%>.value==''){alert('请先输入产品');return;}if('<%=isprops%>'=='0'){alert('该物资没有规格属性');return;}PropertySelect('form1','dmsxid_<%=i%>','sxz_<%=i%>',form1.cpid_<%=i%>.value)">
                        ...
                        <td class="td" nowrap><input type="text" class="ednone" id='jldw_<%=i%>' name='jldw_<%=i%>' value='<%=prodRow.get("jldw")%>' readonly></td>
                         ...
                         <td class="td" nowrap><input type='text' class="ednone" id='hsdw_<%=i%>' name='hsdw_<%=i%>' value='<%=prodRow.get("hsdw")%>' readonly></td>
                         <%if(is_KC_IN_SHOW_PRICE){%>
                         ...
                         <td class="td" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="je_<%=i%>" name="je_<%=i%>" value='<%=detail.get("je")%>' maxlength='<%=list.getColumn("je").getPrecision()%>' readonly></td>
                        <%}%>
                  ...
                  %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td">&nbsp;</td><%if(is_KC_IN_SHOW_PRICE){%><td class="td">&nbsp;</td><td class="td">&nbsp;</td><%}%>
...
<%=storeBillBean.adjustInputSize(
                     is_KC_IN_SHOW_PRICE?
                                         new String[]{"cpbm", "product", "ph", "hssl", "sl", "sxz", "bz", "kwid", "dj", "je", "jldw", "hsdw"}
                                         :new String[]{"cpbm", "product", "ph", "hssl", "sl", "sxz", "bz", "kwid", "jldw", "hsdw"},  "form1", detailRows.length)%>
  ...
  function nsl_onchange(i, isBigUnit)
  {
    ...
    var bjfs = <%=bjfs%>;
    var KC_IN_SHOW_PRICE = <%=loginBean.getSystemParam("KC_IN_SHOW_PRICE")%>;
  ...
  {
     location.href='../pub/pdfprint.jsp?code=constract_instore_edit_bill&operate=<%=storeBillBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("sfdjid")%>&src=../store/contract_instore_edit.jsp';
  }
       //新增空白行
