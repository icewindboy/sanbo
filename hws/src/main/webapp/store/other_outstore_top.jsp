...
/**
 * 02.17 新增 销售出库单主表
 *       实际这张页面是从原来的outputlist.jsp直接copy改一个名字过来的.目地是须要实现框架网页 yjg
...
%><%
  ...
  engine.project.LookUp saleOutBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_STORE_OTHER_OUT);
  ...
  String loginId = otherOutStoreBean.loginId;
%>
<html>
<head>
<title></title>
...
function showDetail(masterRow){
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='other_outstore_bottom.jsp?operate=<%=otherOutStoreBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">其他出库单列表</TD>
  </TR></TABLE>
<%String retu = otherOutStoreBean.doService(request, response);
  ...
  EngineDataSet list = otherOutStoreBean.getMaterTable();
  HtmlTableProducer table = otherOutStoreBean.masterProducer;
      ...
      <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
       ...
       <td class="td" nowrap align="right">
        <%if(loginBean.hasLimits(pageCode, op_add)){%>
         <input name="add" class="button" type="button" title="新增(ALT+A)" onClick="sumitForm(<%=Operate.ADD%>,-1)" value="新增其他出库单(A)" onKeyDown="return getNextElement();">
        <pc:shortcut key="a" script='<%="sumitForm("+Operate.ADD+", -1)"%>'/>
       <%}%>
       <input name="search2" type="button" class="button" title="记账(ALT+Z)" value="记账(Z)" onClick="if(confirm('是否确定记帐'))sumitForm(<%=otherOutStoreBean.RECODE_ACCOUNT%>)"  onKeyDown="return getNextElement();">
       <pc:shortcut key="z" script='<%="sumitForm("+ otherOutStoreBean.RECODE_ACCOUNT +",-1)"%>'/>
       ...
       <%}%>
         <%if(otherOutStoreBean.retuUrl!=null){%><input name="button22" type="button" class="button" title="返回(ALT+C)" value="返回(C)" onClick="buttonEventC()"  onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      ...
      for(; i<count; i++)   {
        ...
        if(otherOutStoreBean.isDouble(zsl))
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <%if(isCancel){%><input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=otherOutStoreBean.CANCEL_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%></td>
   ...
   <tr>
     <td class="tdTitle" nowrap>合计</td>
      ...
      <td class=td>&nbsp;</td><td class=td>&nbsp;</td><td class=td>&nbsp;</td><td class=td>&nbsp;</td><td class=td>&nbsp;</td><td class=td>&nbsp;</td>
      <td class=td>&nbsp;</td><td class=td>&nbsp;</td>
     <td class=td align=right><%=otherOutStoreBean.totalzsl%></td>
...
<%if(!otherOutStoreBean.masterIsAdd()){
    int row = otherOutStoreBean.getSelectedRow();
    if(row >= 0)
...
//返回
  ...
  {
    parent.location.href='<%=otherOutStoreBean.retuUrl%>';
  }
  //关闭查询小视窗
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="sfdjdh" name="sfdjdh" value='<%=otherOutStoreBean.getFixedQueryValue("sfdjdh")%>' onKeyDown="return getNextElement();"></TD>
              <TD nowrap class="td">仓库</TD>
              <TD nowrap class="td"><pc:select name="storeid" addNull="1" style="width:160">
            <%=storeBean.getList(otherOutStoreBean.getFixedQueryValue("storeid"))%></pc:select>
           </TD>
           </TR>
            <TR>
              <TD nowrap class="td">单据类别</TD>
              <TD nowrap class="td"><pc:select name="sfdjlbid" addNull="1" style="width:160">
            <%=saleOutBean.getList(otherOutStoreBean.getFixedQueryValue("sfdjlbid"))%></pc:select>
           </TD>
              <TD nowrap class="td">结算方式</TD>
              <TD nowrap class="td"><pc:select name="jsfsid" addNull="1" style="width:160">
            <%=balanceBean.getList(otherOutStoreBean.getFixedQueryValue("jsfsid"))%></pc:select>
           </TD>
           </TR>
            <TR>
              <TD nowrap class="td">收发日期</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="sfrq$a" value='<%=otherOutStoreBean.getFixedQueryValue("sfrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="sfrq$b" style="WIDTH: 130px" name="sfrq$b" value='<%=otherOutStoreBean.getFixedQueryValue("sfrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <tr>
                ...
                <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
                 <%=deptBean.getList(otherOutStoreBean.getFixedQueryValue("deptid"))%></pc:select>
                </td>
              <TD align="center" nowrap class="td">经手人</TD>
               <td nowrap class="td" colspan="3"><input class="edbox" name="jsr" value='<%=otherOutStoreBean.getFixedQueryValue("jsr")%>'>
            </td>
          </tr>
            <TR>
              <TD class="td" nowrap>购货单位</TD>
              <TD nowrap class="td" colspan="3">
                <input type="hidden" name="dwtxid" value='<%=otherOutStoreBean.getFixedQueryValue("dwtxid")%>'>
                <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=otherOutStoreBean.getFixedQueryValue("dwdm")%>' onchange="corpCodeSelect(this)" >
                <input type="text" name="buyerName" value='<%=otherOutStoreBean.getFixedQueryValue("buyerName")%>' style="width:130" class="edbox" onchange="corpNameSelect(this)">
            ...
            <TR>
              ...
              <TD nowrap class="td">
                <input type="text" class="edbox" style="width:70"  name="sfdjid$pm" onKeyDown="return getNextElement();" value='<%=otherOutStoreBean.getFixedQueryValue("sfdjid$pm")%>'>
                规格
                <input type="text" class="edbox" style="width:70"  name="sfdjid$gg" onKeyDown="return getNextElement();" value='<%=otherOutStoreBean.getFixedQueryValue("sfdjid$gg")%>'>
              </TD>
            </TR>
            <TR>
              <TD class="td" nowrap>产品编码</TD>
              <TD class="td" nowrap colspan="3"><INPUT class="edbox" style="WIDTH:70" id="sfdjid$cpbm" name="sfdjid$cpbm" value='<%=otherOutStoreBean.getFixedQueryValue("sfdjid$cpbm")%>' onKeyDown="return getNextElement();" onchange="productCodeSelect(this)">
              <INPUT class="edbox" style="WIDTH:130" id="cpmc" name="cpmc" value='<%=otherOutStoreBean.getFixedQueryValue("cpmc")%>' onKeyDown="return getNextElement();" onchange="productNameSelect(this)">
              <INPUT TYPE="HIDDEN" NAME="sfdjid" value="<%=otherOutStoreBean.getFixedQueryValue("sfdjid")%>"><img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProdSingleSelect('fixedQueryform','srcVar=sfdjid$cpbm&srcVar=cpmc','fieldVar=cpbm&fieldVar=product',fixedQueryform.sfdjid.value)">
            ...
            <TR>
              ...
              <TD colspan="3" nowrap class="td">
                <%String zt = otherOutStoreBean.getFixedQueryValue("zt");%>
