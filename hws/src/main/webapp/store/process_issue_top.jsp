...
%><%
  engine.erp.store.B_ReceiveMaterial receiveMaterailBean = engine.erp.store.B_ReceiveMaterial.getInstance(request);
  receiveMaterailBean.isout ="1";
  ...
  engine.project.LookUp processOutBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_STORE_PROCESS_OUT);
  String pageCode = "process_issue_list";
  if(!loginBean.hasLimits(pageCode, request, response))
    return;
  boolean hasSearchLimit = loginBean.hasLimits(pageCode, op_search);
  String loginId = receiveMaterailBean.loginId;
  String SYS_APPROVE_ONLY_SELF =receiveMaterailBean.SYS_APPROVE_ONLY_SELF;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
  String SC_OUTSTORE_SHOW_ADD_FIELD =receiveMaterailBean.SC_OUTSTORE_SHOW_ADD_FIELD;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
  ...
  function toDetail(){
    parent.location.href='process_issue_edit.jsp';
  }
  ...
  function showDetail(masterRow){
    selectRow();
    lockScreenToWait("处理中, 请稍候！");
    parent.bottom.location.href='process_issue_bottom.jsp?operate=<%=receiveMaterailBean.SHOW_DETAIL%>&rownum='+masterRow;
    unlockScreenWait();
  }
  ...
  <TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
      <TD NOWRAP align="center">外加工发料单列表</TD>
    </TR></TABLE>
  <%String retu = receiveMaterailBean.doService(request, response);
    if(retu.indexOf("toDetail();")>-1 || retu.indexOf("location.href=")>-1)
    {
      out.print(retu);
      return;
    }
    EngineDataSet list = receiveMaterailBean.getMaterTable();
    HtmlTableProducer table =null;
    if(SC_OUTSTORE_SHOW_ADD_FIELD.equals("1"))
      table = receiveMaterailBean.masterProducer;
    else
      table = receiveMaterailBean.newmasterProducer;
  ...
  <form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
    ...
    <table id="tbcontrol" width="100%" border="0" cellspacing="1" cellpadding="1" align="center">
      <tr>
        ...
        <td class="td" nowrap align="right">
         <%if(loginBean.hasLimits(pageCode, op_add)){%><input name="search3" type="button" class="button" onClick="sumitForm(<%=Operate.ADD%>,-1)" value="新增外加工发料单(A)" onKeyDown="return getNextElement();">
            <pc:shortcut key="a" script='<%="sumitForm("+ Operate.ADD +",-1)"%>'/>
           <input name="search4" type="button" class="button" onClick="sumitForm(<%=receiveMaterailBean.ADD_BACK_MATERAIL%>,-1)" value="新增外加工退料单(T)" onKeyDown="return getNextElement();">
            <pc:shortcut key="t" script='<%="sumitForm("+ receiveMaterailBean.ADD_BACK_MATERAIL +",-1)"%>'/>
           <%}%>
            <input name="search2" type="button" class="button" title="记账(ALT+Z)" value="记账(Z)" onClick="if(confirm('是否确定记帐'))sumitForm(<%=receiveMaterailBean.RECODE_ACCOUNT%>)"  onKeyDown="return getNextElement();">
       <pc:shortcut key="z" script='<%="sumitForm("+ receiveMaterailBean.RECODE_ACCOUNT +",-1)"%>'/>
        ...
        <%}%>
          <%if(receiveMaterailBean.retuUrl!=null){%><input name="button22" type="button" class="button" title="返回(ALT+C)" value="返回(C)" onClick="buttonEventC();" onKeyDown="return getNextElement();">
    ...
    <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
      ...
      <%corpBean.regData(list,"dwtxid");
        ...
        for(; i<count; i++)   {
          ...
          if(receiveMaterailBean.isDouble(zsl))
      ...
      <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
        ...
        <%if(isCancel){%><input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=receiveMaterailBean.RECEIVE_CANCEL_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%></td>
    ...
    <%}%>
      ...
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
       <td align="right" class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=receiveMaterailBean.getZsl()%>' readonly></td>
       <td class="td" nowrap></td>
       <td class="td" nowrap></td>
       <td class="td" nowrap></td>
       <td class="td" nowrap></td>
  ...
  <%if(!receiveMaterailBean.masterIsAdd()){
    int row = receiveMaterailBean.getSelectedRow();
    if(row >= 0)
...
//返回
  ...
  {
    parent.location.href='<%=receiveMaterailBean.retuUrl%>';
  }
//关闭查询小视窗
  ...
  <form name="fixedQueryform" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
    ...
    <div class="queryPop" id="fixedQuery" name="fixedQuery">
      ...
      <TABLE cellspacing=3 cellpadding=0 border=0>
        <TR>
          <TD> <TABLE cellspacing=3 cellpadding=0 border=0><%receiveMaterailBean.table.printWhereInfo(pageContext);%>
              <%--TR>
                <TD class="td" nowrap>单据号</TD>
                <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="sfdjdh" name="sfdjdh" value='<%=receiveMaterailBean.getFixedQueryValue("sfdjdh")%>' maxlength='10' onKeyDown="return getNextElement();"></TD>
                <TD class="td" nowrap>部门</TD>
                <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
           <%=deptBean.getList(receiveMaterailBean.getFixedQueryValue("deptid"))%></pc:select>
             </TD>
              <TR>
                <TD nowrap class="td">收发日期</TD>
                <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="sfrq$a" value='<%=receiveMaterailBean.getFixedQueryValue("sfrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
                ...
                <TD class="td" nowrap><INPUT class="edbox" id="sfrq$b" style="WIDTH: 130px" name="sfrq$b" value='<%=receiveMaterailBean.getFixedQueryValue("sfrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <%=produceUseBean.getList(receiveMaterailBean.getFixedQueryValue("ytid"))%></pc:select>
                </TD>
                <TD nowrap class="td">仓库</TD>
                <TD nowrap class="td"><pc:select name="storeid" addNull="1" style="width:160">
              <%=storeBean.getList(receiveMaterailBean.getFixedQueryValue("storeid"))%></pc:select>
             </TD>
              </TR>
             </TR>
              <TR>
                <TD align="center" nowrap class="td">产品名称</TD>
                 <td nowrap class="td" colspan="3"><input class="EDLine" style="WIDTH:330px" name="product" value='<%=receiveMaterailBean.getFixedQueryValue("product")%>' onKeyDown="return getNextElement();"readonly>
                  <INPUT TYPE="HIDDEN" NAME="sfdjid" value="<%=receiveMaterailBean.getFixedQueryValue("sfdjid")%>"><img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProdSingleSelect('fixedQueryform','srcVar=sfdjid&srcVar=product','fieldVar=cpid&fieldVar=product',fixedQueryform.sfdjid.value)">
                 <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="sfdjid.value='';product.value='';">
              </td>
              </tr>
               <TR>
                <TD class="td" nowrap>产品编码</TD>
                <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="sfdjid$cpbm" name="sfdjid$cpbm" value='<%=receiveMaterailBean.getFixedQueryValue("sfdjid$cpbm")%>' onKeyDown="return getNextElement();"></TD>
                <TD class="td" nowrap>品名规格</TD>
                <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" id="sfdjid$product" name="sfdjid$product" value='<%=receiveMaterailBean.getFixedQueryValue("sfdjid$product")%>' onKeyDown="return getNextElement();"></TD>
              </TR>
              <TR>
                <TD class="td" nowrap>状态</TD>
                <TD colspan="3" nowrap class="td">
                  <%String zt = receiveMaterailBean.getFixedQueryValue("zt");%>
