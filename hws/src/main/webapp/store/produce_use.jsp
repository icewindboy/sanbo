...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>单据用途设置</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = produceUseBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
...
</td> <TD align="right"><%if(produceUseBean.retuUrl!=null){%><input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=produceUseBean.retuUrl%>'" value=" 返回 "border="0"><%}%></TD>
    </TR>
  </TABLE>
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      ...
      <%}%></td>
      <td class="td"><%=list.getValue("ytbh")%></td>
      <td class="td"><%=list.getValue("ytmc")%></td>
    </tr>
    <%  list.next();
      }
