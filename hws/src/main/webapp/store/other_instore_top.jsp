...
%><%
  ...
  otherInStoreBean.djxz = 9;
  engine.project.LookUp prodBean = engine.project.LookupBeanFacade.getInstance(request,engine.project.SysConstant.BEAN_PRODUCT);
  engine.project.LookUp corpBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_CORP);
  engine.project.LookUp deptBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_DEPT);
  engine.project.LookUp storeBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_STORE);
  engine.project.LookUp buyInBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_STORE_OTHER_IN);//LookUp入库单单据类别
  ...
  String loginId = otherInStoreBean.loginId;
%>
<html>
<head>
<title></title>
...
function toDetail(){
  parent.location.href='other_instore_edit.jsp';
}
function showDetail(masterRow){
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
  parent.bottom.location.href='other_instore_bottom.jsp?operate=<%=otherInStoreBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">其它入库单</TD>
  </TR></TABLE>
<%String retu = otherInStoreBean.doService(request, response);
  if(retu.indexOf("toDetail();")>-1 || retu.indexOf("location.href=")>-1)
  {
    out.print(retu);
    return;
  }
  EngineDataSet list = otherInStoreBean.getMaterTable();
  HtmlTableProducer table = otherInStoreBean.masterProducer;
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
    ...
    <tr>
      ...
      <td class="td" nowrap align="right">
       <%if(loginBean.hasLimits(pageCode, op_add)){%>
        <input name="add" class="button" type="button" title="新增(ALT+A)" value="新增其它入库单(A)" onClick="sumitForm(<%=Operate.ADD%>,-1)">
       <pc:shortcut key="a" script='<%="sumitForm("+Operate.ADD+", -1)"%>'/>
      <%}%>
      <input name="search2" type="button" class="button" title="记账(ALT+Z)" value="记账(Z)" onClick="if(confirm('是否确定记帐'))sumitForm(<%=otherInStoreBean.RECODE_ACCOUNT%>)"  onKeyDown="return getNextElement();">
       <pc:shortcut key="z" script='<%="sumitForm("+ otherInStoreBean.RECODE_ACCOUNT +",-1)"%>'/>
      <%if(hasSearchLimit){%>
      ...
      <%}%>
        <%if(otherInStoreBean.retuUrl!=null){%><input name="button22" type="button" class="button" title="返回(ALT+C)" value="返回(C)" onClick="buttonEventC()"  onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      ...
      for(; i<count; i++)   {
       ...
       if(otherInStoreBean.isDouble(zsl))
    ...
    <tr   id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <%if(isCancel){%><input name="image2" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=otherInStoreBean.CANCEL_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%></td>
    ...
    <td class=td>&nbsp;</td>
    <td class=td>&nbsp;</td>
    <td class=td align=right><%=tt_zsl%>
 ...
 <tr>
    ...
    <td class=td>&nbsp;</td>
    <td class=td>&nbsp;</td>
    <td class=td align=right><%=otherInStoreBean.totalzsl%>
    <td class=td align=right>
    </td><td class=td>&nbsp;</td>
    <td class=td>&nbsp;</td>
    <td class=td>&nbsp;</td>
...
<SCRIPT LANGUAGE="javascript">initDefaultTableRow('tableview1',1);
    <%if(!otherInStoreBean.masterIsAdd()){
    int row = otherInStoreBean.getSelectedRow();
    if(row >= 0)
...
{
  parent.location.href='<%=otherInStoreBean.retuUrl%>';
}
    //关闭查询小视窗
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="sfdjdh" name="sfdjdh" value='<%=otherInStoreBean.getFixedQueryValue("sfdjdh")%>'  onKeyDown="return getNextElement();"></TD>
              <TD nowrap class="td">仓库</TD>
              <TD nowrap class="td"><pc:select name="storeid" addNull="1" style="width:160">
            <%=storeBean.getList(otherInStoreBean.getFixedQueryValue("storeid"))%></pc:select>
           </TD>
           </TR>
            <TR>
              <TD nowrap class="td">收发日期</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="sfrq$a" value='<%=otherInStoreBean.getFixedQueryValue("sfrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="sfrq$b" style="WIDTH: 130px" name="sfrq$b" value='<%=otherInStoreBean.getFixedQueryValue("sfrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <%=buyInBean.getList(otherInStoreBean.getFixedQueryValue("sfdjlbid"))%></pc:select>
           </TD>
              <TD nowrap class="td">结算方式</TD>
              <TD nowrap class="td"><pc:select name="jsfsid" addNull="1" style="width:160">
            <%=balanceBean.getList(otherInStoreBean.getFixedQueryValue("jsfsid"))%></pc:select>
           </TD>
           </TR>
            <tr>
             <TD class="td" nowrap>部门</TD>
              <TD nowrap class="td"><pc:select name="deptid" addNull="1" style="width:160">
                <%=deptBean.getList(otherInStoreBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD align="center" nowrap class="td">经手人</TD>
               <td nowrap class="td" colspan="3"><input class="edbox" id="jsr" name="jsr" value='<%=otherInStoreBean.getFixedQueryValue("jsr")%>' onKeyDown="return getNextElement();">
            </td>
          </tr>
            <TR>
              <TD class="td" nowrap>供货单位</TD>
              <TD nowrap class="td" colspan="3">
                <input type="hidden" name="dwtxid" value='<%=otherInStoreBean.getFixedQueryValue("dwtxid")%>'>
                <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=otherInStoreBean.getFixedQueryValue("dwdm")%>' onchange="corpCodeSelect(this)" >
                <input type="text" name="dwmc" value='<%=otherInStoreBean.getFixedQueryValue("dwmc")%>' style="width:130" class="edbox" onchange="corpNameSelect(this)">
            ...
            <TR>
              ...
              <TD nowrap class="td">
                <input type="text" class="edbox" style="width:70"  name="sfdjid$pm" onKeyDown="return getNextElement();" value='<%=otherInStoreBean.getFixedQueryValue("sfdjid$pm")%>'>
                规格
                <input type="text" class="edbox" style="width:70"  name="sfdjid$gg" onKeyDown="return getNextElement();" value='<%=otherInStoreBean.getFixedQueryValue("sfdjid$gg")%>'>
              </TD>
            </TR>
            <TR>
              <TD align="center" nowrap class="td">产品编码</TD>
              <TD class="td" nowrap colspan="3"><INPUT class="edbox" style="WIDTH:70" id="sfdjid$cpbm" name="sfdjid$cpbm" value='<%=otherInStoreBean.getFixedQueryValue("sfdjid$cpbm")%>' onKeyDown="return getNextElement();" onchange="productCodeSelect(this)">
              <INPUT class="edbox" style="WIDTH:130" id="cpmc" name="cpmc" value='<%=otherInStoreBean.getFixedQueryValue("cpmc")%>' onKeyDown="return getNextElement();" onchange="productNameSelect(this)">
              <INPUT TYPE="HIDDEN" NAME="sfdjid" value="<%=otherInStoreBean.getFixedQueryValue("sfdjid")%>"><img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProdSingleSelect('fixedQueryform','srcVar=sfdjid$cpbm&srcVar=cpmc','fieldVar=cpbm&fieldVar=product',fixedQueryform.sfdjid.value)">
            ...
            <TR>
              <TD class="td" nowrap>规格属性</TD>
                <TD nowrap class="td"> <input type="text" class="edbox" style="width:90"  name="sfdjid$sxz" onKeyDown="return getNextElement();" value='<%=otherInStoreBean.getFixedQueryValue("sfdjid$sxz")%>'>
                </TD>
              </TD>
            </TR>
            <TR>
              ...
              <TD colspan="3" nowrap class="td">
                <%String zt = otherInStoreBean.getFixedQueryValue("zt");%>
            ...
            <%--TR>
              <TD class="td" nowrap>品名</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" name="cgsqdid$pm" value='<%=otherInStoreBean.getFixedQueryValue("cgsqdid$pm")%>' maxlength='10' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>规格</TD>
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH:160" name="cgsqdid$gg" value='<%=otherInStoreBean.getFixedQueryValue("cgsqdid$gg")%>' maxlength='10' onKeyDown="return getNextElement();">
              </TD>
            </TR--%>
            <TR>
