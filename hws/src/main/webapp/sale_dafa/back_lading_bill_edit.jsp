...
<%
  ...
  String CustClass = isEnd ? "class=edline" : "class=edbox";
                ...
                <td  noWrap class="td">
                  ...
                  <td  noWrap class="td">
                    <%
                      ...
                      if(isEnd) out.print("<input type='text' name='sendmodeid' value='"+sendRow.get("ordertype")+"' style='width:85' class='edline' readonly>");
                    else {%>
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:800;overflow-y:auto;overflow-x:auto;">
                    <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <td height='20' align="center" nowrap>
                          ...
                          <input name="image" class="img" type="image" title="新增" onClick="detailAdd()" src="../images/add_big.gif" border="0">
                    ...
                    <%
                      ...
                      int i=0;
                      String[] widthName = new String[]{"sl", "xsj", "xsje","zk","jje","bz"};
                      int[] widthMin = new int[]{60, 60, 70,60,60,60,60,150};
                      int[] widths = b_ladingbillBean.getMaxStyleWidth(detailRows, widthName, widthMin);
                      RowMap detail = null;
                      for(; i<detailRows.length; i++)   {
                        ...
                        if(!detail.get("hthwid").equals(""))
                          ...
                          if(dhssl==0)
                            ...
                            hssl=String.valueOf(dhssl);
                          if(dsl==0)
                            sl="";
                          else
                            sl=String.valueOf(dsl);
                     ...
                     %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="center">
                          <%if(!isEnd){%>
                          <%if(detail.get("hthwid").equals("")){%><img style='cursor:hand' src='../images/select_prod.gif' border=0 onClick="if(form1.khlx.value==''){alert('请选择客户类型');return}SaleProdSingleSelect('form1','srcVar=wzdjid_<%=i%>&srcVar=xsj_<%=i%>&srcVar=cpid_<%=i%>&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=xsj&fieldVar=cpid','','sumitForm(<%=b_ladingbillBean.DETAIL_CHANGE%>,-1)')"><%}%>
                        ...
                        <td class="td" nowrap><input type="text" <%=prodClass%> style="width:70" onKeyDown="return getNextElement();" name="cpbm_<%=i%>" value='<%=productRow.get("cpbm")%>' onchange="productCodeSelect(this,<%=i%>)" <%=prodreanonly%>></td><!--产品编码(存货代码)--->
                        <td class="td" nowrap>
                          <input type="text" <%=prodClass%> style="width:150" onKeyDown="return getNextElement();" id="product_<%=i%>" name="product_<%=i%>" value='<%=productRow.get("product")%>'  onchange="productNameSelect(this,<%=i%>)"   <%=prodreanonly%>></td>
                        </td><!--品名 规格(存货名称与规格)--->
                        <td class="td" nowrap>
                        ...
                        <td class="td" nowrap><input type="text" <%=djandjjeClass%> style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=hssl%>' maxlength='<%=list.getColumn("hssl").getPrecision()%>' onchange="sl_onchange(<%=i%>, true)" <%=canedt?"":" readonly"%>></td><!--换算数量--->
                        ...
                        <td class="td" nowrap><input type="text" <%=djandjjeClass%> style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=sl%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onchange="sl_onchange(<%=i%>, false)" <%=canedt?"":" readonly"%>></td><!--数量-->
                         <td class="td" nowrap><%=productRow.get("jldw")%></td><!--计量单位--->
                        <td class="td" nowrap><input type="text" class='ednone_r'  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="xsj_<%=i%>" name="xsj_<%=i%>" value='<%=detail.get("xsj")%>' readonly></td><!--销售价-->
                        <td class="td" nowrap><input type="text" class='ednone_r'  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="xsje_<%=i%>" name="xsje_<%=i%>" value='<%=String.valueOf(dxsje)%>' readonly></td><!--销售金额-->
                        <td class="td" nowrap><input type="text" <%=detailClass_r%>  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="zk_<%=i%>" name="zk_<%=i%>" value='<%=detail.get("zk")%>'  onchange="dj_onchange(<%=i%>, true)" <%=readonly%> ></td><!--折扣-->
                        <td class="td" nowrap>
                            <input type="text" <%=djandjjeClass%>  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="dj_<%=i%>" name="dj_<%=i%>" value='<%=detail.get("dj")%>' onchange="dj_onchange(<%=i%>, false)" <%=canedt?"":" readonly"%> >
                            <input type="HIDDEN" id="wzdjid_<%=i%>" name="wzdjid_<%=i%>" value='<%=detail.get("wzdjid")%>' >
                        </td><!--单价-->
                        <td class="td" nowrap><input type="text" class='ednone_r' style="width:<%=widths[2]%>" onKeyDown="return getNextElement();" id="jje_<%=i%>" name="jje_<%=i%>" value='<%=String.valueOf(djje)%>'  readonly></td><!--净金额-->
                        <td class="td" nowrap align="left"><input type="text" <%=detailClass%> style="width:<%=widths[5]%>" onKeyDown="return getNextElement();" name="bz_<%=i%>" id="bz_<%=i%>" value='<%=detail.get("bz")%>' maxlength='100'  <%=readonly%> ></td><!--备注-->
                        <td class="td" nowrap align="right"><%=detail.get("sthssl") %></td><!--实提换算数量-->
                        <td class="td" nowrap align="right"><%=detail.get("stsl") %></td><!--实提数量-->
                  ...
                  %>
                      ...
                      <tr id="rowinfo_end">
                        ...
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
                        <td class="td" nowrap><input id="t_hssl" name="t_hssl" type="text" class='ednone_r' style="width:<%=widths[0]%>" value='<%=engine.util.Format.formatNumber(t_hssl.toString(),loginBean.getSumFormat())%>' readonly></td>
                        <td class="td">&nbsp;</td>
                        <td class="td" nowrap><input id="t_sl" name="t_sl" type="text" class='ednone_r' style="width:<%=widths[0]%>" value='<%=engine.util.Format.formatNumber(t_sl.toString(),loginBean.getSumFormat())%>' readonly></td>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
                        <td class="td" nowrap><input id="t_xsje" name="t_xsje" type="text" class='ednone_r' style="width:70" value='<%=engine.util.Format.formatNumber(t_xsje.toString(),loginBean.getPriceFormat())%>' style="width:<%=widths[2]%>" readonly></td>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
                        <td class="td" nowrap><input id="t_jje" name="t_jje" type="text" class='ednone_r' style="width:70" value='<%=engine.util.Format.formatNumber(t_jje.toString(),loginBean.getPriceFormat())%>' style="width:<%=widths[2]%>" readonly></td>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
      ...
      <td>
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            ...
            <%
             }if(zt.equals("0")||zt.equals("1")||zt.equals("2")){
            ...
            <%
            }}if(isCanDelete&&!b_ladingbillBean.isReport&&czyid.equals(loginid)){
              String del = "if(confirm('是否删除该记录？'))sumitForm("+Operate.DEL+");";
              String detNull = "sumitForm("+b_ladingbillBean.DEL_NULL+");";
            %>
            <input name="button3"  style="width:120" type="button" class="button" title="删除数量为空行(R)" value="删除数量为空行(R)" onClick="<%=detNull%>">
           ...
           <%}%>
            ...
            if(!b_ladingbillBean.isReport){
            %>
            <%if(!masterRow.get("zt").equals("0")){%>
             <input name="btnback"  style="width:50"  type="button" class="button" onClick="ladingprint(<%=masterRow.get("tdid")%>);" value="套打(P)">
             <pc:shortcut key="p" script='<%="ladingprint("+masterRow.get("tdid")+");"%>' />
             <%}%>
...
<script language="javascript">
 <%=b_ladingbillBean.adjustInputSize(new String[]{"cpbm","product","sxz","hssl","sl","xsj","xsje", "zk","dj","jje","bz"}, "form1", detailRows.length)%>
  ...
  function nsl_onchange(i, isBigUnit)
  {
    ...
    yfObj.value =parseFloat(formatQty(yfdjObj.value))*parseFloat(t_slObj.value);
  }
  ...
  function MultiSelectOrder(frmName, srcVar, methodName,notin)
     {
       ...
       paraStr = "../sale_dafa/select_order_product.jsp?operate=0&srcFrm="+frmName+"&"+srcVar;
...
function OrderSingleSelect(frmName,srcVar,fieldVar,curID,methodName,notin)
  {
   ...
   paraStr = "../sale_dafa/import_order.jsp?operate=0&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+curID;
   if(methodName+'' != 'undefined')
     paraStr += "&method="+methodName;
   if(notin+'' != 'undefined')
     paraStr += "&notin="+notin;
...
function checkdwst()
{
  ...
  OrderMultiSelect('form1','srcVar=importOrder&dwtxid='+form1.dwtxid.value+'&storeid='+form1.storeid.value+'&djlx='+form1.djlx.value+'&personid='+form1.personid.value+'&jsfsid='+form1.jsfsid.value+'&khlx='+form1.khlx.value+'&sendmodeid='+form1.sendmodeid.value+'&yfdj='+form1.yfdj.value+'&isnet=<%=masterRow.get("isnet")%>')
}
function OrderMultiSelect(frmName, srcVar, methodName,notin)
{
  var winopt = "location=no scrollbars=yes menubar=no status=no resizable=1 width=700 height=600 top=0 left=0";
  var winName= "OrdersSelector";
  paraStr = "../sale_dafa/back_import_order.jsp?operate=0&srcFrm="+frmName+"&"+srcVar;
  if(methodName+'' != 'undefined')
    paraStr += "&method="+methodName;
  if(notin+'' != 'undefined')
    paraStr += "&notin="+notin;
...
function checkdwtxid()
{
  ...
  MultiSelectOrder('form1','srcVar=importOrderproduct&dwtxid='+form1.dwtxid.value+'&storeid='+form1.storeid.value+'&djlx='+form1.djlx.value+'&personid='+form1.personid.value+'&khlx='+form1.khlx.value+'&jsfsid='+form1.jsfsid.value+'&isnet=<%=masterRow.get("isnet")%>')
}
