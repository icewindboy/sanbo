...
<%@ page import="engine.action.Operate,engine.common.LoginBean,engine.erp.baseinfo.*,engine.project.*,engine.erp.sale.*"%>
...
<%
  if(!loginBean.hasLimits("saler_prix", request, response))
    return;
  B_SalerPrixDetail b_SalerPrixDetailBean = B_SalerPrixDetail.getInstance(request);
...
<script language="javascript">
  ...
  {
    paraStr = "saler_prix_modify.jsp?operate="+oper+"&rownum="+rownum;
...
<%
  ...
  EngineDataSet dsCjjTable = b_SalerPrixDetailBean.getCjjTable();
...
<table cellspacing="2" cellpadding="0" border="0" width="100%" bgcolor="#f0f0f0">
    ...
    <table cellspacing=0 width="100%" cellpadding=0>
     ...
     <%
      ...
      RowMap[] cjjRows = b_SalerPrixDetailBean.getCjjRows();
     ...
     %>
      <tr onclick="selectRow();" onDblClick="selectRow();showInterFrame(<%=b_SalerPrixDetailBean.SHOW_DETAIL%>,<%=tdhwid%>)">
...
<div id="cntDivINFO_EX_1" class="tabContent" style="display:none;width:1000;hight:300;overflow-y:auto;overflow-x:auto;">
    <center>
     ...
     <%
      RowMap[] detailRows = b_SalerPrixDetailBean.getTcjRows();
      int k=0;
      double ztcje=0;
      double ztcj=0;
      double zhllx=0;
...
<div id="cntDivINFO_EX_2" class="tabContent" style="display:none;width:900;hight:300;overflow-y:auto;overflow-x:auto;">
     ...
     <%
      RowMap zggzjldetail = b_SalerPrixDetailBean.getLxRow();
     %>
                <tr>
                <td class="td"  align="left">利息奖=(A+B-C-D)*日利率:&nbsp;<INPUT TYPE="TEXT"  VALUE="<%=engine.util.Format.formatNumber(zggzjldetail.get("lxj"),"#0.00") %>"   class="edline"  readonly></td>
                </tr>
                <tr>
                <td class="td"  align="left">A:销售允许天数利息=回款天数*销售额:&nbsp;<INPUT TYPE="TEXT"  class="edline"  VALUE="<%=engine.util.Format.formatNumber(zggzjldetail.get("XSA"),"#0.00") %>"  <%=readonly%>></td>
                </tr>
                <tr>
                <td class="td"  align="left">B:销售回款利息=(月结算天数-回款日)*回款金额:&nbsp;<INPUT TYPE="TEXT"  class="edline" VALUE="<%=engine.util.Format.formatNumber(zggzjldetail.get("XSB"),"#0.00") %>"   readonly></td>
                </tr>
                <tr>
                <td class="td"  align="left">C:应收款月初余额利息=月初余额* 月结算天数:&nbsp;<INPUT TYPE="TEXT"  class="edline"  VALUE="<%=engine.util.Format.formatNumber(zggzjldetail.get("XSC"),"#0.00") %>"    readonly></td>
                </tr>
                <tr>
                <td class="td"  align="left">D:销售利息=(月结算天数-销售提货日期)*销售额:&nbsp;<INPUT TYPE="TEXT"   class="edline" VALUE="<%=engine.util.Format.formatNumber(zggzjldetail.get("XSD"),"#0.00") %>"    readonly></td>
                </tr>
                <tr>
                <td class="td"  align="left">其中日利率:&nbsp;<INPUT TYPE="TEXT"  VALUE="<%=zggzjldetail.get("rll")%>"  class="edline"  readonly></td>
                </tr>
                <tr>
                <td class="td"  align="left">月结算天数:&nbsp;<INPUT TYPE="TEXT"  VALUE="<%=zggzjldetail.get("days")%>"    class="edline"  readonly></td>
                </tr>
            </table>
      </center>
</div>
