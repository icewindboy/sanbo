...
function backList()
{
  location.href='lading_init.jsp';
}
//打印
function ladingprint()
{
  location.href='lading_bill_print.jsp?operate=<%=b_LadingInitBean.WRAPPER_PRINT%>';
}
//输入购货单位代码
function customerCodeSelect(obj)
{
  ...
  CustCodeChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dz&srcVar=lxr&srcVar=dh','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr&fieldVar=lxr&fieldVar=tel',obj.value,'sumitForm(<%=b_LadingInitBean.DWTXID_CHANGE%>,-1)');
}
//选择购货单位
...
CustSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dz&srcVar=lxr&srcVar=dh','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr&fieldVar=lxr&fieldVar=tel',form1.dwtxid.value,'sumitForm(<%=b_LadingInitBean.DWTXID_CHANGE%>,-1)');
}
//输入购货单位名称
function customerNameSelect(obj)
{
  ...
  CustNameChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dz&srcVar=lxr&srcVar=dh','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr&fieldVar=lxr&fieldVar=tel',obj.value,'sumitForm(<%=b_LadingInitBean.DWTXID_CHANGE%>,-1)');
}
//提单日期变化--引起回款日期变化
...
<%
  ...
  //engine.project.LookUp salePriceBean = b_LadingInitBean.getSalePriceBean(request);//销售价
  String curUrl = request.getRequestURL().toString();

  EngineDataSet ds = b_LadingInitBean.getMaterTable();
  EngineDataSet list = b_LadingInitBean.getDetailTable();

  HtmlTableProducer masterProducer = b_LadingInitBean.masterProducer;
  HtmlTableProducer detailProducer = b_LadingInitBean.detailProducer;

  creditBean.regData(ds,"dwtxid");
  RowMap masterRow = b_LadingInitBean.getMasterRowinfo();//主表一行
  RowMap[] detailRows= b_LadingInitBean.getDetailRowinfos();//从表多行
  String zt=masterRow.get("zt");//0.初始化,1.已开提单(已经审核),4.作废,9.出库
  if(b_LadingInitBean.isApprove)
  {
  ...
  //b_LadingInitBean.isReport||
  boolean isEnd =  b_LadingInitBean.isReport||b_LadingInitBean.isApprove || (!b_LadingInitBean.masterIsAdd() && !zt.equals("0"));
  //没有结束,在修改状态,并有删除权限
  boolean isCanDelete = !isEnd && !b_LadingInitBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);
  isEnd = isEnd || !(b_LadingInitBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  boolean mustConversion = b_LadingInitBean.conversion;//是否需要强制转换
          ...
          <tr>
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                  <td  noWrap class="tdTitle">提单日期</td>
                  <td  noWrap class="td">
                    ...
                    <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <td height='20' align="center" nowrap>
                          <%if(!isEnd){%>
                          <input name="image" class="img" type="image" title="新增(A)" onClick="if(form1.dwtxid.value==''){alert('请选择购货单位!');return;}if(form1.storeid.value==''){alert('请选择仓库!');return;}if(form1.khlx.value==''){alert('请选择客户类型!');return;}sumitForm(<%=Operate.DETAIL_ADD%>)" src="../images/add_big.gif" border="0">
                    ...
                    <%
                      ...
                      for(; i<detailRows.length; i++)   {
                        ...
                        if(!detail.get("hthwid").equals(""))
                          ...
                          wtsl = engine.util.Format.formatNumber(dsl-dstsl,"#0.00");
                      ...
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="center">
                         <iframe id="prod_<%=i%>" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                          <%if(!isEnd){%>
                          <%if(detail.get("hthwid").equals("")){%><img style='cursor:hand' src='../images/select_prod.gif' border=0 onClick="if(form1.khlx.value==''){alert('请选择客户类型');return}SaleProdSingleSelect('form1','srcVar=wzdjid_<%=i%>&srcVar=xsj_<%=i%>&srcVar=cpid_<%=i%>&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=xsj&fieldVar=cpid','','sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,-1)')"><%}%>
                          ...
                          <input name="image" class="img" type="image" title="复制当前行" onClick="sumitForm(<%=b_LadingInitBean.DETAIL_COPY%>,<%=i%>)" src="../images/copyadd.gif" border="0">
                          <%}%></td>
                        <td class="td" nowrap>
                        <%
                        ...
                        <td class="td" nowrap><input type="text" <%=detailClass_r%>   onKeyDown="return getNextElement();" id="xsj_<%=i%>" name="xsj_<%=i%>" value='<%=detail.get("xsj")%>'  onchange="dj_onchange(<%=i%>, true)" <%=canedt?"":" readonly"%> ></td><!--销售价-->
                        ...
                        <td class="td" nowrap align="right"><input type="text" <%=djandjjeClass%>  onKeyDown="return getNextElement();" name="hltcl_<%=i%>" id="hltcl_<%=i%>" value='<%=detail.get("hltcl")%>' maxlength='100'  <%=readonly%> ></td>
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             ...
             %>
              ...
              <%
              }}if(isCanDelete&&!b_LadingInitBean.isReport){
               String del = "if(confirm('是否删除该记录？'))sumitForm("+Operate.DEL+");";
               String detNull = "sumitForm("+b_LadingInitBean.DEL_NULL+");";
              %>
              ...
              %>
             <%--<input name="btnback" type="button" class="button" onClick="sumitForm(<%=b_LadingInitBean.WRAPPER_PRINT%>);" value=" 打印 ">
             <%if(!masterRow.get("zt").equals("0")){%>
             <input name="btnback"  style="width:50"  type="button" class="button" onClick="ladingprint();" value="打印(P)">
...
<script language="javascript">
  ...
  <%=b_LadingInitBean.adjustInputSize(new String[]{"cpbm","product","sxz","hssl","sl","xsj","xsje", "zk","dj","jje","bz"}, "form1", detailRows.length)%>initDefaultTableRow('tableview1',1);
...
function propertiesInput(obj,cpid,i)
  {
    PropertyNameChange(document.all['prod'], obj.form.name, 'srcVar=dmsxid_'+i+'&srcVar=sxz_'+i, 'fieldVar=dmsxid&fieldVar=sxz', cpid, obj.value, 'sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,'+i+')');
  }
function selctorder()
  {
  if(form1.dwtxid.value=='')
  ...
  form1.selectedhtid.value='';
    OrderSingleSelect('form1','srcVar=selectedhtid','fieldVar=htid','&dwtxid='+form1.dwtxid.value+'&djlx='+form1.djlx.value+'&storeid='+form1.storeid.value,"sumitForm(<%=b_LadingInitBean.IMPORT_ORDER%>,-1)");
  }
function checkdwst()
  {
  if(form1.storeid.value=='')
...
{
    SaleProdCodeChange(document.all['prod'], obj.form.name,'srcVar=wzdjid_'+i+'&srcVar=xsj_'+i+'&srcVar=cpid_'+i+'&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=xsj&fieldVar=cpid',obj.value,'sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,'+i+')');
}
//产品名称
function productNameSelect(obj, i)
{
    SaleProdNameChange(document.all['prod'], obj.form.name,'srcVar=wzdjid_'+i+'&srcVar=xsj_'+i+'&srcVar=cpid_'+i+'&storeid='+form1.storeid.value,'fieldVar=wzdjid&fieldVar=xsj&fieldVar=cpid',obj.value,'sumitForm(<%=b_LadingInitBean.DETAIL_CHANGE%>,'+i+')');
}
</script>
<%//&#$
if(b_LadingInitBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>