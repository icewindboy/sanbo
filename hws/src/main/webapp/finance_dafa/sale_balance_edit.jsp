...
<%@ page import="engine.dataset.*,engine.project.Operate,java.math.BigDecimal,engine.html.*,java.util.ArrayList,engine.erp.finance.dafa.*"%><%!
...
<script language="javascript">
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    function hxje_onchange(i)
    {
      ...
      var tcjObj = document.all['tcj_'+i];
        ...
        */
        ...
        tcjObj.value=formatSum(parseFloat(jsjeObj.value) * parseFloat(tclObj.value))*0.01;
        cal_tot('jsje');
        //cal_tot('tcj');
    }
     function cal_tot(type)
    {
      ...
      if(type == 'jsje')
      {
        document.all['t_jsje'].value = formatSum(tot);
        document.all['t_tcj'].value = formatSum(ttcj);
        document.all['t_jje'].value = formatSum(tjje);
