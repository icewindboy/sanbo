...
<%
  ...
  String tdlx=fpzl.equals("2")?"企业普通发票":"增值税发票";
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                    <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <td nowrap><%=detailProducer.getFieldInfo("gsl").getFieldname()%>(%)</td>
                        <td nowrap><%=detailProducer.getFieldInfo("gs").getFieldname()%></td>
                        <%if(fpzl.equals("1")){%>
                        <td nowrap>增值税率(%)</td>
                     ...
                     %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap align="right">
                             <input type="text" <%=hsslClass%>  onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=detail.get("hssl")%>' maxlength='<%=list.getColumn("hssl").getPrecision()%>' onchange="sl_onchange(<%=i%>, true)"<%=hsslReadonly%>></td>
                        <td class="td" nowrap>
                            <input type="text" style="width:20" class='ednone_r'  onKeyDown="return getNextElement();" id="hsdw_<%=i%>" name="hsdw_<%=i%>" value='<%=prodRow.get("hsdw")%>'  readonly></td>

                        <td class="td" nowrap align="right"><input type="text" <%=slClass%>  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>"     value='<%=detail.get("sl")%>'   maxlength='<%=list.getColumn("sl").getPrecision()%>'   onchange="sl_onchange(<%=i%>, false)" <%=slReadonly%>></td><!--数量-->
                       <td class="td" nowrap><input type="text" class=ednone style="width:100%" onKeyDown="return getNextElement();" id="jldw_<%=i%>" name="jldw_<%=i%>" value='<%=prodRow.get("jldw")%>' readonly></td><!--计量单位--->
                        ...
                        <td class="td" nowrap align="right">
                       <input type="text" class='ednone_r'  align="right"     style="width:<%=widths[2]%>" onKeyDown="return getNextElement();" id="je_<%=i%>" name="je_<%=i%>"     value='<%=detail.get("je")%>'   maxlength='<%=list.getColumn("je").getPrecision()%>'   readonly></td>
                        <td class="td" nowrap align="right"><input type="text" <%=detailClass_r%>  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="gsl_<%=i%>" name="gsl_<%=i%>" value='<%=detail.get("gsl")%>' maxlength='<%=list.getColumn("gsl").getPrecision()%>' onchange="gsl_onchange(<%=i%>)"></td>
                        <td class="td" nowrap align="right"><input type="text" class='ednone_r'  style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="gs_<%=i%>" name="gs_<%=i%>" value='<%=detail.get("gs")%>' maxlength='<%=list.getColumn("gs").getPrecision()%>' readonly>
                       <%if(!fpzl.equals("1")){%>
                        <input type="hidden" <%=detailClass_r%>  style="width:<%=widths[2]%>" onKeyDown="return getNextElement();" id="zzsl_<%=i%>" name="zzsl_<%=i%>" value='<%=detail.get("zzsl")%>' maxlength='<%=list.getColumn("zzsl").getPrecision()%>' onchange="zzsl_onchange(<%=i%>, false)" <%=readonly%>><!--税率-->
                        <input type="hidden" <%=detailClass_r%> align="right"    style="width:<%=widths[0]%>" onKeyDown="return getNextElement();" id="hsdj_<%=i%>" name="hsdj_<%=i%>" value='<%=detail.get("hsdj")%>' maxlength='<%=list.getColumn("hsdj").getPrecision()%>'  onchange="hsdj_onchange(<%=i%>, true)" <%=readonly%>><!--含税单价-->
                        <input type="hidden" class='ednone_r'   align="right"     style="width:<%=widths[5]%>" onKeyDown="return getNextElement();" name="se_<%=i%>" id="se_<%=i%>"     value='<%=detail.get("se")%>'   maxlength='<%=list.getColumn("se").getPrecision()%>'   readonly><!--含税金额-->
                        <input type="hidden" class='ednone_r'  align="right"     style="width:<%=widths[5]%>" onKeyDown="return getNextElement();" name="jshj_<%=i%>" id="jshj_<%=i%>" value='<%=detail.get("jshj")%>' maxlength='<%=list.getColumn("jshj").getPrecision()%>' readonly><!--价税合计-->
                        <%}%>
                        </td><!--无税金额-->
                        <%if(fpzl.equals("1")){%>
                  ...
                  %>
                      ...
                      <tr id="rowinfo_end">
                        ...
                        <td class="td">&nbsp;</td>
                        <td class="td" align="right"><input id="t_gs" name="t_gs" type="text" class='ednone_r' style="width:70" value='<%=t_gs%>' style="width:<%=widths[2]%>" readonly></td>
                        <%if(fpzl.equals("1")){%>
                        <td class="td">&nbsp;</td>
                        <td class="td">&nbsp;</td>
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             ...
             <%}%>
              ...
              <%}%>
              <input type="button"  style="width:60" class="button" value="打印(P)" onclick="location.href='../pub/pdfprint.jsp?code=buy_invoice_edit_bill&operate=<%=Operate.PRINT_BILL%>&a$cgfpid=<%=masterRow.get("cgfpid")%>&src=../finance/buy_invoice_edit.jsp'">
...
<script language="javascript">
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    {
      var gslObj = document.all['gsl_'+i];
      var gsObj = document.all['gs_'+i];
      ...
      jshjObj.value=formatQty(parseFloat(jeObj.value) *(1+parseFloat(gslObj.value)*0.01)*(1+parseFloat(zzslObj.value)*0.01));
      <%}else{%>
      jeObj.value=formatQty(parseFloat(hsslObj.value) * parseFloat(wsdjObj.value));
      seObj.value=formatQty(parseFloat(hsslObj.value) * parseFloat(wsdjObj.value)*parseFloat(zzslObj.value)*0.01);
      jshjObj.value=formatQty(parseFloat(jeObj.value) *(1+parseFloat(gslObj.value)*0.01)*(1+parseFloat(zzslObj.value)*0.01));
      <%}%>
      cal_tot('sl');
      cal_tot('je');
    ...
    function gsl_onchange(i)
    {
      ...
      if(isNaN(gslObj.value)){
        ...
        gsObj.value=formatQty(parseFloat(jeObj.value) * parseFloat(gslObj.value)*0.01);
        jshjObj.value=formatQty(parseFloat(jeObj.value) *(1+parseFloat(gslObj.value)*0.01)*(1+parseFloat(zzslObj.value)*0.01));
    ...
    function cal_tot(type)
    {
      ...
      <%}%>
        ...
        document.all['t_gs'].value = formatSum(tot);
}
function impot()
{
...
function print()
{
   location.href='../pub/pdfprint.jsp?code=buy_invoice_edit_bill&operate=<%=Operate.PRINT_BILL%>&a$cgfpid=<%=masterRow.get("cgfpid")%>&src=../finance/buy_invoice_edit.jsp'
}
//产品编码
