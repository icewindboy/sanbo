...
/**
 * <p>Title: 基础信息维护--产品选择列表</p>
 * <p>Description: 基础信息维护--产品选择列表<br>
 * </p>
...
public final class B_ProductSelect extends BaseAction implements Operate
{
  public  static final String PRODUCT_KIND_CHANGE = "10001";

  private static final String PRODUCT_NORMAL
      = "SELECT * FROM VW_KC_DM_EXIST a WHERE @ ORDER BY cpbm";//WHERE isdelete=0 ";

  private static final String PRODUCT_RUIJIAO
      = "SELECT a.* FROM VW_KC_DM_EXIST a, kc_dmlb b "
      + "WHERE a.wzlbid=b.wzlbid AND @ ORDER BY b.bm, a.pm, a.gg";
   ...
   * @return 返回往来单位信息的实例
   */
  public static B_ProductSelect getInstance(HttpServletRequest request)
  {
    B_ProductSelect productSelectBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      productSelectBean = (B_ProductSelect)session.getAttribute("productSelectBean_a");
      if(productSelectBean == null)
      {
        productSelectBean = new B_ProductSelect(request);
        session.setAttribute("productSelectBean_a", productSelectBean);
      }
    }
   ...
   * 构造函数
   */
  private B_ProductSelect(HttpServletRequest request)
  {
    custName = LoginBean.getInstance(request).getSystemParam("SYS_CUST_NAME");
  ...
  protected final void jbInit() throws java.lang.Exception
  {
    ...
    //dsOneTable.setSort(new SortDescriptor("", new String[]{"cpbm"}, new boolean[]{false}, null, 0));
  ...
  {
    personid = LoginBean.getInstance(data.getRequest()).getUserID();
    ...
    prodSQL = "ruijiao".equals(custName) ? PRODUCT_RUIJIAO : PRODUCT_NORMAL;
    prodSQL = BaseAction.combineSQL(prodSQL, "@",
      new String[]{" (a.storeid IN (SELECT storeid FROM jc_ckqx WHERE personid='"+ personid +"') OR storeid IS NULL) AND @ "});
    if(method.length() > 0)
    {
    ...
    if(storeid.length() > 0&&storeid.indexOf("storeid")<0)
      prodSQL = BaseAction.combineSQL(prodSQL, "@", new String[]{" @ AND (a.storeid is null OR a.storeid="+storeid+")"});
    else if(storeid.length() > 0)
    	prodSQL = BaseAction.combineSQL(prodSQL, "@", new String[]{" @ AND (a.storeid is null OR a."+storeid+")"});
    if(chxz.length() > 0)
  ...
  final class Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //SQL = PRODUCT_SQL +(SQL.equals("")? "" : "WHERE (" +SQL +")") +" ORDER BY cpbm";
      getOneTable().setQueryString(SQL);
      getOneTable().setRowMax(null);
    }
  }
