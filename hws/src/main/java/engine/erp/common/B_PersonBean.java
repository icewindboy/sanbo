...
public final class B_PersonBean extends BaseAction implements Operate
{
  ...
  {
    B_PersonBean B_PersonBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_PersonBean";
      B_PersonBean = (B_PersonBean)session.getAttribute(beanName);
      if(B_PersonBean == null)
      {
        B_PersonBean = new B_PersonBean(LoginBean.getInstance(request).getUser());
        session.setAttribute(beanName, B_PersonBean);
      }
    }
    return B_PersonBean;
  }
    ...
    */
    ...
    if(method != null && !method.equals(""))
    {
      ...
      if(notin != null)
      {
        ...
        corpSQL = BaseAction.combineSQL(corpSQL, "@", new String[]{"t.empid NOT IN ("+ notin +") AND @ "});
      }
    }
  ...
  final class CodeSearch implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String deptids = currUser.getHandleDeptValue("t.deptid");
      String SQL = BaseAction.combineSQL(corpSQL, "@", new String[]{"t.dwdm LIKE '"+dwdm+"%' AND", custType, deptids, fgsid});
  ...
  final class NameSearch implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String deptids = currUser.getHandleDeptValue("t.deptid");
      String SQL = BaseAction.combineSQL(corpSQL, "@", new String[]{"t.dwmc LIKE '%"+name+"%' AND", custType, deptids, fgsid});
      EngineDataSet ds = getOneTable();
      ds.setQueryString(SQL);
      if(ds.isOpen())
      {
  ...
  final class Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String deptids = currUser.getHandleDeptValue("t.deptid");
      SQL = BaseAction.combineSQL(corpSQL, "@", new String[]{SQL.length()==0 ? "" :SQL+" AND", custType, deptids, fgsid});
      getOneTable().setQueryString(SQL);
      getOneTable().setRowMax(null);
    }
  }
