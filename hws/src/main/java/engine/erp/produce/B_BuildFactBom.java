...
public final class B_BuildFactBom extends BaseAction implements Operate
{
  ...
  private ClientBomReplace clientBomReplaceBean = null;//制定实际BOM表替换可选件的BEAN
   ...
   * @return 返回实际BOM列表的实例
   */
  public static B_BuildFactBom getInstance(HttpServletRequest request)
  {
    B_BuildFactBom buildFactBomBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "buildFactBomBean";
      buildFactBomBean = (B_BuildFactBom)session.getAttribute(beanName);
      if(buildFactBomBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        buildFactBomBean = new B_BuildFactBom();
        buildFactBomBean.qtyFormat = loginBean.getQtyFormat();

        buildFactBomBean.loginId = loginBean.getUserID();
        buildFactBomBean.loginName = loginBean.getUserName();
        buildFactBomBean.SYS_PRODUCT_SPEC_PROP = loginBean.getSystemParam("SYS_PRODUCT_SPEC_PROP");//生产用位换算的相关规格属性名称 得到的值为“宽度”……
        //设置格式化的字段
        buildFactBomBean.dsMasterTable.setColumnFormat("xql", buildFactBomBean.qtyFormat);
        buildFactBomBean.dsMasterTable.setColumnFormat("scxql", buildFactBomBean.qtyFormat);
        session.setAttribute(beanName, buildFactBomBean);
      }
    }
    return buildFactBomBean;
  }

  /**
   * 构造函数
   */
  private B_BuildFactBom()
  {
    try {
      jbInit();
    }
   ...
   */
  public ClientBomReplace getReplaceProductBean(HttpServletRequest req)
  {
    if(clientBomReplaceBean == null)
      clientBomReplaceBean = ClientBomReplace.getInstance(req);
