...
import engine.erp.subsection.*;
/**
 * <p>Title: 销售管理--同价调回单--</p>
 ...
 * @version 1.0
 */
public final class B_SaleInstepBack extends BaseAction implements Operate
{
 public  static final String PROMOTION_PRODUCT_ADD  = "1010";//
  ...
  public  static final String LADING_OUT_TWO         = "111015";//出库确认
  ...
  public  static final String THLX_CHANGE         = "1028754";
  public  static final String ONCHANGE      = "1028";//仓库选择触发事件
  ...
  private static final String MASTER_SQL           = "SELECT * FROM xs_td WHERE djlx=12 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_EDIT_SQL       = "SELECT * FROM xs_td WHERE tdid='?' ";
  private static final String MASTER_SUM_SQL       = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=12 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL        = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=12 AND ? AND fgsid=? ?  ";
  ...
  private static final String DETAIL_SQL            = " SELECT a.*,b.zxh from xs_tdhw a, (SELECT e.wjid,e.zxh from xs_tdhw d, kc_sfdjmx e WHERE d.tdhwid=e.wjid AND e.djxz=2 and d.tdid='?' ) b,kc_dm c WHERE a.tdhwid=b.wjid(+) AND a.cpid=c.cpid and a.tdid='?' ";//
  private static final String DETAIL_EDIT_SQL      = "SELECT * FROM xs_tdhw WHERE tdid='?' ";
  ...
  private static final String SEARCH_SQL            = "SELECT * FROM VW_SALE_FB_XS WHERE 1=1 ? ";//提货单主从表查询
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td.12");
  public  HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "xs_td.12");
  ...
  private B_DxImportApply b_DxImportApplyBean = null; //提货单引用销售合同
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_SaleInstepBack getInstance(HttpServletRequest request)
  {
    B_SaleInstepBack B_SaleInstepBackBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_SaleInstepBackBean";
      B_SaleInstepBackBean = (B_SaleInstepBack)session.getAttribute(beanName);
      if(B_SaleInstepBackBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        B_SaleInstepBackBean = new B_SaleInstepBack();
        B_SaleInstepBackBean.qtyFormat = loginBean.getQtyFormat();
        B_SaleInstepBackBean.priceFormat = loginBean.getPriceFormat();
        B_SaleInstepBackBean.sumFormat = loginBean.getSumFormat();
        B_SaleInstepBackBean.fgsid = loginBean.getFirstDeptID();
        B_SaleInstepBackBean.loginId = loginBean.getUserID();
        B_SaleInstepBackBean.loginName = loginBean.getUserName();
        B_SaleInstepBackBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          B_SaleInstepBackBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          B_SaleInstepBackBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        B_SaleInstepBackBean.dsMasterTable.setColumnFormat("zje", B_SaleInstepBackBean.sumFormat);
        B_SaleInstepBackBean.dsMasterTable.setColumnFormat("zsl", B_SaleInstepBackBean.qtyFormat);

        B_SaleInstepBackBean.dsDetailTable.setColumnFormat("sl", B_SaleInstepBackBean.qtyFormat);
        B_SaleInstepBackBean.dsDetailTable.setColumnFormat("hssl", B_SaleInstepBackBean.qtyFormat);
        B_SaleInstepBackBean.dsDetailTable.setColumnFormat("xsj", B_SaleInstepBackBean.priceFormat);
        B_SaleInstepBackBean.dsDetailTable.setColumnFormat("dj", B_SaleInstepBackBean.priceFormat);
        B_SaleInstepBackBean.dsDetailTable.setColumnFormat("jje", B_SaleInstepBackBean.priceFormat);
        B_SaleInstepBackBean.dsDetailTable.setColumnFormat("xsje", B_SaleInstepBackBean.priceFormat);

        session.setAttribute(beanName, B_SaleInstepBackBean);
      }
    }
    return B_SaleInstepBackBean;
  }
  /**
   * 构造函数
   */
  private B_SaleInstepBack()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.12','tdbh') from dual"}));
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("khlx", "C");
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.12','tdbh') from dual");
  ...
  public class Add_Approve  implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterList, dsMasterList.getRow(),"instepback", content,deptid, this);
    }
  ...
  //&#$
   ...
   class Add_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.putAproveList(dsMasterList, dsMasterList.getRow(),"instepback", content,deptid);
     }
  }*/
  /**
   * 取消审批
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterList,dsMasterList.getRow(),"instepback");
      dsMasterList.readyRefresh();
    }
  }
  /**
  ...
  class Lading_Out implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //openDetailTable(false);
      String SQL =  combineSQL(DETAIL_EDIT_SQL,"?",new String[]{tdid});
      dsDetailTable.setQueryString(SQL);
      if(!dsDetailTable.isOpen())
      {
        dsDetailTable.open();
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        detail.setValue("zk", detailrow.get("zk"));//折扣formatNumber(String.valueOf(sl * dj),"#0.00"));
      ...
      ds.setValue("kc__storeid", rowInfo.get("kc__storeid"));//仓库id
    ...
    private String checkMasterInfo()
    {
     ...
     //   return showJavaScript("alert('请选择结算方式！');");
      ...
      if(temp.equals(""))
        return showJavaScript("alert('请选择拨出仓库！');");
      temp = rowInfo.get("kc__storeid");
      if(temp.equals(""))
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("tdrq"), null, null, null, "a", ">="),//出库日期
        new QueryColumn(master.getColumn("tdrq"), null, null, null, "b", "<="),//出库日期
 ...
 class Detail_Multi_Add implements Obactioner
 {
  ...
  class Detail_SALE_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //保存输入的明细信息
      putDetailInfo(data.getRequest());
      String importOrder = m_RowInfo.get("directimportOrder");
      String dwtxid =  m_RowInfo.get("dwtxid");
      ...
      if(importOrder.length() == 0)
        return;
      //实例化查找数据集的类
      EngineRow locateGoodsRow = new EngineRow(dsDetailTable, "sqdhwid");
      String[] SQDhwIDs = parseString(importOrder,",");//解析出合同货物ID数组
      b_DxImportApplyBean =getb_ImportOrderBean(req);     //提货
      BigDecimal bd = new BigDecimal(0);
      double max = 0;
      for(int i=0; i < SQDhwIDs.length; i++)
      {
        if(SQDhwIDs[i].equals("-1"))
          continue;
        if(!isMasterAdd)
          dsMasterTable.goToInternalRow(masterRow);
        String tdid = dsMasterTable.getValue("tdid");
        RowMap detailrow = null;
        locateGoodsRow.setValue(0, SQDhwIDs[i]);
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          RowMap orderRow =null;
          orderRow = b_DxImportApplyBean.getLookupRow(SQDhwIDs[i]);//提货
          if(dwtxid.equals(""))
          {
           ...
           RowMap corRow = corpBean.getLookupRow(dwtxid);
             String storeid = dataSetProvider.getSequence("SELECT a.storeid FROM kc_ck a,dwtx b WHERE a.deptid=b.deptid_equal and dwtxid='"+dwtxid+"'");
             m_RowInfo.put("dwtxid",dwtxid);
             if(m_RowInfo.get("personid").equals("")){
               m_RowInfo.put("personid", corRow.get("personid"));
             }
             if(m_RowInfo.get("deptid").equals("")){
               m_RowInfo.put("deptid", corRow.get("deptid"));
             }
             m_RowInfo.put("dz", corRow.get("addr"));
...
//          dsDetailTable.setValue("tdhwid", "-1");
          ...
          dsDetailTable.setValue("sqdhwid", SQDhwIDs[i]);
...
//          dsDetailTable.setValue("hltcl", orderRow.get("hltcl"));
          ...
          double wkhtl=Double.parseDouble(orderRow.get("wkhtl").equals("")?"0":orderRow.get("wkhtl"));//未开单数量
          double wkhthssl = Double.parseDouble(orderRow.get("wkhthssl").equals("")?"0":orderRow.get("wkhthssl"));
//          double xhssl = hssl*(sl-skdsl)/sl;
          double xsj = Double.parseDouble(orderRow.get("xsj").equals("")?"0":orderRow.get("xsj"));//销售价为出厂价，在视图里已定义
          ...
          {*/
            ...
            dsDetailTable.setValue("hssl", String.valueOf(wkhthssl));
            dsDetailTable.setValue("sl", String.valueOf(wkhtl));
            dsDetailTable.setValue("xsje",formatNumber(String.valueOf(xsj*wkhtl), sumFormat) );
            dsDetailTable.setValue("jje", formatNumber(String.valueOf(dj*wkhtl), sumFormat) );
//          }
          dsDetailTable.setValue("xsj", orderRow.get("xsj"));
          dsDetailTable.setValue("zk", "100");
          dsDetailTable.setValue("dj", orderRow.get("dj"));//单价为出厂价，在视图里已定义
...
/*      HttpServletRequest req = data.getRequest();
  ...
  class Dwtxid_Change implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      {
        String sstoreid = dataSetProvider.getSequence("SELECT a.storeid FROM kc_ck a,dwtx b WHERE a.deptid=b.deptid_equal and dwtxid='"+dwtxid+"'");
        m_RowInfo.put("dwtxid",dwtxid);
        ...
        m_RowInfo.put("storeid",sstoreid);
   ...
   */
  public B_DxImportApply getb_ImportOrderBean(HttpServletRequest req)
  {
    if(b_DxImportApplyBean == null)
      b_DxImportApplyBean = B_DxImportApply.getInstance(req);
