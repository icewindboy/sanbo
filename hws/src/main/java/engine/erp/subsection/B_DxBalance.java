 ...
 *  * @version 1.0
 */
public final class B_DxBalance extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM zx_xsjs WHERE 1<>1 ";
  private static final String MASTER_SEARCH_SQL    = "SELECT * FROM zx_xsjs WHERE 1=1 AND ? AND fgsid =? ?  ?  order by djh desc";
  private static final String MASTER_SQL    = "SELECT zx_xsjs.* FROM zx_xsjs,dwtx WHERE zx_xsjs.dwtxid=dwtx.dwtxid  ? AND zx_xsjs.fgsid =? ?  ? ";

  private static final String JE_SQL    = "select SUM(nvl(je,0)*nvl(djxz,0))zje FROM zx_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String HXJE_SQL    = "select SUM(nvl(hxje,0)*nvl(djxz,0))zhxje FROM zx_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String WHXJE_SQL    = "select SUM(nvl(whxje,0)*nvl(djxz,0))zwhxje FROM zx_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";

  private static final String COLLECT_SQL = "SELECT sum(nvl(a.je,0)*nvl(djxz,0)-nvl(a.hxje,0)*nvl(djxz,0))totalWhxje FROM zx_xsjs a WHERE   a.fgsid=? and a.dwtxid='?' and a.personid='?'  and a.khlx='?' GROUP BY a.dwtxid";//汇总

  private static final String DETAIL_STRUT_SQL = "SELECT * FROM cw_xsjshx WHERE 1<>1  order by tdhwid ";
  private static final String DETAIL_SQL    = "SELECT * FROM cw_xsjshx WHERE xsjsID='?' order by tdhwid ";//

  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM zx_xsjs WHERE xsjsID='?'";
  private static final String AUTO_CANCER_SQL = "SELECT * FROM vw_sale_autobalance where 1=1 and fgsid=? and dwtxid=? and personid=? and khlx='?' ORDER BY px, tdrq";
  private EngineDataSet dsMasterTable  = new EngineDataSet();//主表
  private EngineDataSet dsDetailTable  = new EngineDataSet();//从表
  private EngineDataSet tdMxTable      = new EngineDataSet();//提单明细

  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "zx_xsjs","zx_xsjs");
  public  HtmlTableProducer detailProducer = new HtmlTableProducer(dsDetailTable, "cg_xsjshx");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_DxBalance getInstance(HttpServletRequest request)
  {
    B_DxBalance B_DxBalanceBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_DxBalanceBean";
      B_DxBalanceBean = (B_DxBalance)session.getAttribute(beanName);
      if(B_DxBalanceBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        B_DxBalanceBean = new B_DxBalance();
        B_DxBalanceBean.qtyFormat = loginBean.getQtyFormat();
        B_DxBalanceBean.priceFormat = loginBean.getPriceFormat();
        B_DxBalanceBean.sumFormat = loginBean.getSumFormat();
        B_DxBalanceBean.fgsid = loginBean.getFirstDeptID();
        B_DxBalanceBean.loginid = loginBean.getUserID();
        B_DxBalanceBean.loginName = loginBean.getUserName();
        //设置格式化的字段
        B_DxBalanceBean.user = loginBean.getUser();
        B_DxBalanceBean.dsMasterTable.setColumnFormat("je", B_DxBalanceBean.sumFormat);
        B_DxBalanceBean.dsMasterTable.setColumnFormat("hxje", B_DxBalanceBean.sumFormat);
        B_DxBalanceBean.dsMasterTable.setColumnFormat("whxje", B_DxBalanceBean.sumFormat);


        B_DxBalanceBean.dsDetailTable.setColumnFormat("jsje", B_DxBalanceBean.sumFormat);
        session.setAttribute(beanName, B_DxBalanceBean);
      }
    }
    return B_DxBalanceBean;
  }
  /**
   * 构造函数
   */
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"djh"}, new String[]{"SELECT pck_base.billNextCode('zx_xsjs','djh') from dual"}));
    //dsMasterTable.setSort(new SortDescriptor("", new String[]{"djh"}, new boolean[]{true}, null, 0));
    dsMasterTable.setTableName("zx_xsjs");
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        String djh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('zx_xsjs','djh') from dual");
 ...
 public final String getZwhxje()
 {
  ...
  class Balance_Over implements Obactioner
  {
   ...
   class Cancer_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(),"dx_balance");
     }
  }
   //&#$
   /**
   ...
   public class Add_Approve  implements Obactioner,ApproveListener
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.putAproveList(dsMasterTable, dsMasterTable.getRow(),"dx_balance", content,deptid, this);
     }
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      zt = new String[]{""};
      isMasterAdd = true;
      String SQL = " AND zx_xsjs.zt<>8 AND zx_xsjs.zt<>4 ";
      String deptvalue = user.getHandleDeptValue("deptid");
      if(deptvalue.indexOf(">")>0)
        deptvalue = " 1<>1 ";
      else
        deptvalue = " and zx_xsjs."+user.getHandleDeptValue("deptid");
      String MSQL =  combineSQL(MASTER_SQL, "?", new String[]{deptvalue, fgsid, SQL," order by zx_xsjs.djh desc"});
 ...
 class Master_Add_Edit implements Obactioner
 {
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ds.insertRow(false);
        xsjsid = dataSetProvider.getSequence("s_zx_xsjs");
        String djh = rowInfo.get("djh");
        String count = dataSetProvider.getSequence("select count(*) from zx_xsjs t where t.djh='"+djh+"'");
        if(!count.equals("0"))
          djh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('zx_xsjs','djh') from dual");
        ds.setValue("djh", djh);//
  ...
  class Master_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String zt = dataSetProvider.getSequence("SELECT a.zt FROM zx_xsjs a WHERE a.xsjsid='"+xsjsid+"'");
  ...
  final class Orderby implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      SQL = combineSQL(MASTER_SQL, "?", new String[]{
                       ...
                       fgsid,
                       deptvalue,
                       orderBy.length() > 0 ? "ORDER BY "+orderBy+" desc " : " ORDER BY zx_xsjs.djh desc"
                       });
      dsMasterTable.setQueryString(SQL);
      dsMasterTable.readyRefresh();
    }
