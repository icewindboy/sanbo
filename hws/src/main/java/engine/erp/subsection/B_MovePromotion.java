...
/**
 * <p>Title: 销售管理--同价调拨单--</p>
 * <p>Description: 销售管理--同价调拨单--<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author engine
 * @version 1.0
 */
public final class B_MovePromotion extends BaseAction implements Operate
{
  ...
  public  static final String MUTI_ADD  = "1111004";//添加多行产品
  ...
  private static final String MASTER_SQL           = "SELECT * FROM xs_td WHERE djlx=13 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_EDIT_SQL       = "SELECT * FROM xs_td WHERE tdid='?' ";
  private static final String MASTER_SUM_SQL       = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=13 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL        = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=13 AND ? AND fgsid=? ?  ";
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td.13");
  public  HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "xs_td.13");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_MovePromotion getInstance(HttpServletRequest request)
  {
    B_MovePromotion B_MovePromotionBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_MovePromotionBean";
      B_MovePromotionBean = (B_MovePromotion)session.getAttribute(beanName);
      if(B_MovePromotionBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        B_MovePromotionBean = new B_MovePromotion();
        B_MovePromotionBean.qtyFormat = loginBean.getQtyFormat();
        B_MovePromotionBean.priceFormat = loginBean.getPriceFormat();
        B_MovePromotionBean.sumFormat = loginBean.getSumFormat();
        B_MovePromotionBean.fgsid = loginBean.getFirstDeptID();
        B_MovePromotionBean.loginId = loginBean.getUserID();
        B_MovePromotionBean.loginName = loginBean.getUserName();
        B_MovePromotionBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          B_MovePromotionBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          B_MovePromotionBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        B_MovePromotionBean.dsMasterTable.setColumnFormat("zje", B_MovePromotionBean.sumFormat);
        B_MovePromotionBean.dsMasterTable.setColumnFormat("zsl", B_MovePromotionBean.qtyFormat);

        B_MovePromotionBean.dsDetailTable.setColumnFormat("sl", B_MovePromotionBean.qtyFormat);
        B_MovePromotionBean.dsDetailTable.setColumnFormat("hssl", B_MovePromotionBean.qtyFormat);
        B_MovePromotionBean.dsDetailTable.setColumnFormat("xsj", B_MovePromotionBean.priceFormat);
        B_MovePromotionBean.dsDetailTable.setColumnFormat("dj", B_MovePromotionBean.priceFormat);
        B_MovePromotionBean.dsDetailTable.setColumnFormat("jje", B_MovePromotionBean.priceFormat);
        B_MovePromotionBean.dsDetailTable.setColumnFormat("xsje", B_MovePromotionBean.priceFormat);

        session.setAttribute(beanName, B_MovePromotionBean);
      }
    }
    return B_MovePromotionBean;
  }
  /**
   * 构造函数
   */
  private B_MovePromotion()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.13','tdbh') from dual"}));
    ...
    addObactioner(String.valueOf(MUTI_ADD), new Multi_Add());
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.13','tdbh') from dual");
...
/*
  ...
  //&#$
   ...
   class Add_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.putAproveList(dsMasterList, dsMasterList.getRow(),"inpromotion", content,deptid);
     }
  }
  /**
   * 取消审批
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterList,dsMasterList.getRow(),"inpromotion");
      dsMasterList.readyRefresh();
    }
  }
  /**
...
class Onchange implements Obactioner
{
 ...
 class Detail_Add implements Obactioner
 {
   ...
   *
   * */
  class Multi_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      HttpServletRequest req = data.getRequest();
