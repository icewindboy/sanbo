...
public final class B_DirectSell extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL           = "SELECT * FROM xs_td_fb WHERE djlx=4 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_EDIT_SQL       = "SELECT * FROM xs_td_fb WHERE tdid='?' ";
  private static final String MASTER_SUM_SQL       = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td_fb WHERE djlx=4 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL        = "SELECT SUM(nvl(zje,0))zje FROM xs_td_fb WHERE djlx=4 AND ? AND fgsid=? ?  ";
  ...
  private static final String DETAIL_SQL            = " SELECT a.*,b.zxh,c.hsdw ,f.fgsid,f.storeid from xs_tdhw_fb a, (SELECT e.wjid,e.zxh from xs_tdhw_fb d, kc_sfdjmx e WHERE d.tdhwid=e.wjid AND e.djxz=2 and d.tdid='?' ) b,kc_dm c,xs_td_fb f WHERE a.tdid=f.tdid  and  a.tdhwid=b.wjid(+) AND a.cpid=c.cpid and a.tdid='?' ";//
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td_fb.4");
  public  HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "xs_td_fb.4");
...
//    PorductStockTableInit(); //库存量
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td_fb.4','tdbh') from dual"}));
...
//    p_RowInfos = null;
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td_fb.4','tdbh') from dual");
  ...
  public final RowMap[] getTdqtfyRowinfos() {
    ...
    /* 得到与从表一样的表，主要是用于top.jsp的状态描述 2004.12.08 wlc*/
 ...
 */
  ...
  public String getChlbmc(String tdid) throws Exception
  {
    ...
    setDataSetProperty(dsChlbmc,"SELECT c.chmc FROM xs_tdhw_fb a,kc_dm b,kc_chlb c WHERE a.cpid=b.cpid AND b.chlbid=c.chlbid AND a.tdid='"+tdid+"'");
    dsChlbmc.open();
    dsChlbmc.first();
 ...
 */
  public class Add_Approve  implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterList, dsMasterList.getRow(),"directsell", content,deptid, this);
    }
    public void processApprove(ApproveResponse[] reponses) throws Exception
    {
      ...
      for(int i=0;i<reponses.length;i++)
      {
        ...
        if(reponses[i].getProjectFlowCode().equals("special_directsell_mc")&&chmc.indexOf(tmp)>=0)
        {
          reponses[i].add();
        }
        else
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterList,dsMasterList.getRow(),"directsell");
      dsMasterList.readyRefresh();
    }
  }
  /**
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ...
        if(!count.equals("0"))
        {
          tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td_fb','tdbh') from dual");
        }
        ds.insertRow(false);
