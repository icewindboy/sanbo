 ...
 * 2004-4-26 10:18 新增 where cs<=1 ORDER BY bm 对应解决mantis上0000421 bug:.不取物资类别明细,并且要对结果排序. yjg
 * 2004-4-23 11:06 新增 为新编码的盘点机而加 yjg
 * 2004-4-14 18:45 为按sxz排序而加的刷新语句
 * 2004-4-14 18:21 为了按规格属性排序而新加入的kc_dmsx c 表及条件. yjg
 * 2004-4-9 11:06 新加入了判断库位如果有设的话那么就必须指定库位. 其程序中其它地方的相关的新增 yjg
 * 2004-4-2 23:12 修改 暂时修改取产品编码前七位 yjg
 * 2004-3-30 14:53 新增 如果是保存按钮行为完成后定位数据集.目地是:实现保存不返回 yjg
 * 2004-3-30 10:05 修改 Single_Product_Add操作中修改zcsl为空 yjg
 * 2004-3-29 23:43 新增 新增空白行dmsxid yjg
 * 2004-3-29 23:43 新增 从表新增库存里有的物资的时候新增dmsxid.同时view中做了相应修改 yjg
...
public final class B_DirectStoreCheck extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT a.*, b.sfdjid FROM kc_pd a, kc_sfdj b ,bm c WHERE a.pdid=b.pdid(+) AND c.ismember='1' AND a.deptid=c.deptid AND a.fgsid=? ? AND a.? AND a.? ORDER BY a.pdhm DESC";
  ...
  private static final String STOCK_COLLECT_ADD = "SELECT * FROM VW_DX_STORE_COLLECT ";
  //批量增加视图
  private static final String STOCK_BIG_ADD
      = "SELECT cpbm, cpid, wzlbid, chlbid, storeid FROM kc_dm a "
      +" WHERE cpid NOT IN(SELECT cpid FROM VW_DX_STORE_COLLECT WHERE ? ) AND a.isdelete = 0 AND ?  order by cpbm";
  //只取库存物资明细表中的数据
  private static final String STOCK_MATERIAL_LIST
      = " SELECT * FROM "
      + " ( "
      + "   SELECT b.storeid, b.cpid, b.dmsxid, b.ph, SUM(nvl(b.zl,0)) kcsl "
      + "   FROM dx_wzmx b, vw_kc_dm_exist a "
      + "    WHERE  b.cpid = a.cpid "
   ...
   * @return 返回库存盘点单列表的实例
   */
  public static B_DirectStoreCheck getInstance(HttpServletRequest request)
  {
    B_DirectStoreCheck DirectStoreCheckBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "DirectStoreCheckBean";
      DirectStoreCheckBean = (B_DirectStoreCheck)session.getAttribute(beanName);
      if(DirectStoreCheckBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        DirectStoreCheckBean = new B_DirectStoreCheck();
        DirectStoreCheckBean.qtyFormat = loginBean.getQtyFormat();
        DirectStoreCheckBean.priceFormat = loginBean.getPriceFormat();
        DirectStoreCheckBean.sumFormat = loginBean.getSumFormat();
        SC_STORE_UNIT_STYLE = loginBean.getSystemParam("SC_STORE_UNIT_STYLE");//计量单位和辅单位换算方式1=强制换算,0=仅空值时换算
        DirectStoreCheckBean.widthSyskey = loginBean.getSystemParam("SYS_PRODUCT_SPEC_PROP");
        DirectStoreCheckBean.lengthSyskey = loginBean.getSystemParam("SYS_PRODUCT_SPEC_LENGTH");

        DirectStoreCheckBean.fgsid = loginBean.getFirstDeptID();
        DirectStoreCheckBean.loginId = loginBean.getUserID();
        DirectStoreCheckBean.loginName = loginBean.getUserName();
        DirectStoreCheckBean.loginDept = loginBean.getDeptID();
        DirectStoreCheckBean.user = loginBean.getUser();
        //设置格式化的字段
        DirectStoreCheckBean.dsDetailTable.setColumnFormat("zcsl", DirectStoreCheckBean.qtyFormat);
        DirectStoreCheckBean.dsDetailTable.setColumnFormat("scsl", DirectStoreCheckBean.qtyFormat);
        session.setAttribute(beanName, DirectStoreCheckBean);
      }
    }
    return DirectStoreCheckBean;
  }

  /**
   * 构造函数
   */
  private B_DirectStoreCheck()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"pdhm"}, new String[]{"SELECT pck_base.billNextCode('kc_pd.direct','pdhm') from dual"}));
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=min; i<=max; i++)
    {
      ...
      detailRow.put("hssl", formatNumber(hssl, qtyFormat));//换算数量
      detailRow.put("zchssl", formatNumber(zchssl, qtyFormat));//帐存换算数量
  ...
  class Single_Product_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
     ...
     detailrow.put("cpid", cpid);
     detailrow.put("zcsl", "0");//2004-3-30 10:05 修改 Single_Product_Add操作中修改zcsl为空 yjg
     detailrow.put("bz", "0");
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      */
      ...
      SQL = combineSQL(MASTER_SQL, "?", new String[]{fgsid, SQL,user.getHandleDeptValue(),user.getHandleStoreValue() });
 ...
 class Add_Approve implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     //03.16 15:04 修改 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
     //以实现:根据下达的部门，进行提交审批；
     approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "direct_check", content, dsMasterTable.getValue("deptid"));
   }
  }
  /**
   * 取消审批触发操作
  ...
  class Cancle_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(), "direct_check");
    }
  }
  ...
  class Store_Onchange implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      putDetailInfo(req);
      String storeid = m_RowInfo.get("storeid");
      String sql="SELECT a.deptid FROM kc_ck a where a.storeid="+storeid;
      String deptid = dataSetProvider.getSequence(sql);
  ...
  class Detail_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      RowMap tmprowinfo = null;
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        tmprowinfo = (RowMap)d_RowInfos.get(i);
        long swapdetailRow = Long.parseLong(tmprowinfo.get("InternalRow"));
        detail.goToInternalRow(swapdetailRow);

        String cpid = tmprowinfo.get("cpid");
        String cpbm = tmprowinfo.get("cpbm");
        String dmsxid = tmprowinfo.get("dmsxid");
        String ph = tmprowinfo.get("ph");
        String scsl = tmprowinfo.get("scsl");
        String hssl = tmprowinfo.get("hssl");
        String zchssl = tmprowinfo.get("zchssl");
        String kwid = tmprowinfo.get("kwid");

        detail.setValue("cpid", cpid);
        detail.setValue("cpbm", cpbm);
        detail.setValue("ph", ph);//批号
        detail.setValue("dmsxid", dmsxid);//规格属性 02.14新增为配合页面上新增此字段而新增长的.
        detail.setValue("scsl", formatNumber(scsl, qtyFormat));//实存数量
        detail.setValue("hssl", formatNumber(hssl, qtyFormat));//实存数量
        detail.setValue("zchssl", formatNumber(zchssl, qtyFormat));//实存数量
      ...
      if(scope.equals("4"))
      {
        ...
        //2004-08-31  修改 加入了parentid in (sql) yjg
        SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" +storeid+"') AND ( wzlbid IN("+sql+") or parentid in (" + sql +") )";
      }
      if(stockCollectData == null)
      {
        stockCollectData = new EngineDataSet();
      ...
      if (row>=0)
      {
        RowMap detailrow = (RowMap)d_RowInfos.get(row);
        long l = Long.parseLong(detailrow.get("InternalRow"));
        detail.goToInternalRow(l);
        detail.deleteRow();
        //d_RowInfos.add(detailrow);
