...
public final class Direct_Check_Make_Destroy extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL       = "SELECT * FROM kc_sfdj WHERE djxz='17' AND fgsid=? and pdid=? ";
  ...
  private static final String STORE_CHECK_SQL  = "SELECT a.*, b.hsbl, c.sxz FROM kc_pdmx a, kc_dm b, kc_dmsx c WHERE a.cpid=b.cpid AND (nvl(a.scsl,0)<>nvl(a.zcsl,0) or nvl(a.hssl, 0)<>nvl(a.zchssl, 0)) AND a.dmsxid = c.dmsxid(+) AND a.cpid = c.cpid(+) AND pdid='?' ";
   ...
   */
  public static Direct_Check_Make_Destroy getInstance(HttpServletRequest request)
  {
    Direct_Check_Make_Destroy checkMakeDestroyBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "checkMakeDestroyBean";
      checkMakeDestroyBean = (Direct_Check_Make_Destroy)session.getAttribute(beanName);
      if(checkMakeDestroyBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        checkMakeDestroyBean = new Direct_Check_Make_Destroy();
   ...
   * 构造函数
   */
  private Direct_Check_Make_Destroy()
  {
    try {
      jbInit();
    }
