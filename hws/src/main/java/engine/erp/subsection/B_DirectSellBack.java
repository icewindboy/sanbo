 ...
 * @version 1.0
 */
public final class B_DirectSellBack extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM xs_td_fb WHERE 1<>1  ";
  private static final String MASTER_SQL    = "SELECT * FROM xs_td_fb WHERE djlx=-1 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_EDIT_SQL       = "SELECT * FROM xs_td_fb WHERE tdid='?' ";
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td_fb WHERE djlx=-1 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL    = "SELECT SUM(nvl(zje,0))zje FROM xs_td_fb WHERE djlx=-1 AND ? AND fgsid=? ?  ";
  ...
  private static final String DETAIL_STRUT_SQL      = "SELECT * FROM xs_tdhw_fb WHERE 1<>1 ";
  private static final String DETAIL_SQL            = "SELECT a.* FROM xs_tdhw_fb a,kc_dm b WHERE tdid='?' and a.cpid=b.cpid order by b.cpbm";//
  ...
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM xs_td_fb WHERE tdid='?'";
  private static final String CAN_OVER_SQL = "select count(*) from xs_tdhw_fb a where nvl(a.sl,0)>nvl(a.stsl,0) and a.tdid=";//
  private static final String CAN_CANCER_SQL = "select count(*) from xs_tdhw_fb a where nvl(a.stsl,0)>0 and a.tdid=";//
  private static final String REFERENCED_SQL = "select count(*) from xs_tdhw_fb a where nvl(a.stsl,0)>0 and a.tdid=";//
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td_fb.t");
  public  HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "xs_td_fb.t");
...
//  private B_ImportOrderToBackLading b_ImportOrderToBackLadingBean = null; //提货单引用销售合同
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_DirectSellBack getInstance(HttpServletRequest request)
  {
    B_DirectSellBack b_DirectSellBackBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_DirectSellBackBean";
      b_DirectSellBackBean = (B_DirectSellBack)session.getAttribute(beanName);
      if(b_DirectSellBackBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_DirectSellBackBean = new B_DirectSellBack();
        b_DirectSellBackBean.qtyFormat = loginBean.getQtyFormat();
        b_DirectSellBackBean.priceFormat = loginBean.getPriceFormat();
        b_DirectSellBackBean.sumFormat = loginBean.getSumFormat();
        b_DirectSellBackBean.fgsid = loginBean.getFirstDeptID();
        b_DirectSellBackBean.loginId = loginBean.getUserID();
        b_DirectSellBackBean.loginName = loginBean.getUserName();
        b_DirectSellBackBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          b_DirectSellBackBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          b_DirectSellBackBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        b_DirectSellBackBean.dsMasterTable.setColumnFormat("zje", b_DirectSellBackBean.sumFormat);
        b_DirectSellBackBean.dsMasterTable.setColumnFormat("zsl", b_DirectSellBackBean.qtyFormat);

        b_DirectSellBackBean.dsDetailTable.setColumnFormat("sl", b_DirectSellBackBean.qtyFormat);
        b_DirectSellBackBean.dsDetailTable.setColumnFormat("hssl", b_DirectSellBackBean.qtyFormat);
        b_DirectSellBackBean.dsDetailTable.setColumnFormat("xsj", b_DirectSellBackBean.priceFormat);
        b_DirectSellBackBean.dsDetailTable.setColumnFormat("dj", b_DirectSellBackBean.priceFormat);
        b_DirectSellBackBean.dsDetailTable.setColumnFormat("jje", b_DirectSellBackBean.priceFormat);
        b_DirectSellBackBean.dsDetailTable.setColumnFormat("xsje", b_DirectSellBackBean.priceFormat);

        session.setAttribute(beanName, b_DirectSellBackBean);
      }
    }
    return b_DirectSellBackBean;
  }
  /**
   * 构造函数
   */
  private B_DirectSellBack()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsDetailTable.setTableName("xs_tdhw_fb");
    setDataSetProperty(dsSearchTable, combineSQL(SEARCH_SQL,"?",new String[]{""}));

    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td_fb.t','tdbh') from dual"}));

    dsDetailTable.setSequence(new SequenceDescriptor(new String[]{"tdhwid"}, new String[]{"s_xs_tdhw_fb"}));
...
//    addObactioner(String.valueOf(PRODUCT_ADD), new Multi_Product_Add());
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td_fb.t','tdbh') from dual");
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String zt = dataSetProvider.getSequence("SELECT a.zt FROM xs_td_fb a WHERE a.tdid='"+tdid+"'");
      if(zt!=null&&!zt.equals("0"))
      {
        dsMasterList.readyRefresh();
        return;
      }
      approve.putAproveList(dsMasterList, dsMasterList.getRow(),"directsellback", content,deptid);
    }
  }
  /**
   * 取消审批
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterList,dsMasterList.getRow(),"directsellback");
    }
  }
  /**
   * 作废
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ...
        String count = dataSetProvider.getSequence("select count(*) from xs_td_fb t where t.tdbh='"+tdbh+"'");
        if(!count.equals("0"))
        {
          tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td_fb.t','tdbh') from dual");
        }
        ds.insertRow(false);
        tdid = dataSetProvider.getSequence("s_xs_td_fb");
  ...
  class Master_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String count = dataSetProvider.getSequence("SELECT SUM(nvl(a.stsl,0)) FROM xs_tdhw_fb a WHERE a.tdid='"+id+"'");
      if(!count.equals("0"))
      {
        data.setMessage(showJavaScript("alert('该提单已被引用,不能删除!')"));
        return;
      }
      String zt = dataSetProvider.getSequence("SELECT a.zt FROM xs_td_fb a WHERE a.tdid='"+id+"'");
      if(zt!=null&&!zt.equals("0"))
      {
