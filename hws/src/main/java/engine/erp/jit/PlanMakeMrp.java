...
public final class PlanMakeMrp extends BaseAction implements Operate
{
   ...
   */
  public static PlanMakeMrp getInstance(HttpServletRequest request)
  {
    PlanMakeMrp produceMakeMrpBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "produceMakeMrpBean";
      produceMakeMrpBean = (PlanMakeMrp)session.getAttribute(beanName);
      if(produceMakeMrpBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        produceMakeMrpBean = new PlanMakeMrp();
