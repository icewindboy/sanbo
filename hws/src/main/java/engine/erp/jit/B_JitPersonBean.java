 ...
 * @version 1.0
 */
public final class B_JitPersonBean extends BaseAction implements Operate
{
  private static final String personSql = " SELECT a.deptid, a.gzzid, a.gzzmc, b.personid, "
                                               + "   c.bm, c.xm, c.zw, c.sex, c.isdelete "
                                               + " FROM sc_gzz a, sc_gzzry b, emp c "
                                               + " WHERE b.personid = c.personid(+) "
                                               + "       AND a.gzzid = b.gzzid ";

  private static final String personSqlStructure = " SELECT a.deptid, a.gzzid, a.gzzmc, b.personid, "
                                               + "   c.bm, c.xm, c.zw, c.sex, c.isdelete "
                                               + " FROM sc_gzz a, sc_gzzry b, emp c "
                                               + " WHERE 1<>1 ";
   ...
   * @return 返回收发单据信息的实例
   */
  public static B_JitPersonBean getInstance(HttpServletRequest request)
  {
    B_JitPersonBean B_JitPersonBeanBean = null;
    HttpSession session = request.getSession(true);
    synchronized(session)
    {
      String beanName = "B_JitPersonBeanBean";
      B_JitPersonBeanBean = (B_JitPersonBean)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(B_JitPersonBeanBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        B_JitPersonBeanBean = new B_JitPersonBean();
        B_JitPersonBeanBean.user = loginBean.getUser();
        B_JitPersonBeanBean.isWorkNo = loginBean.getSystemParam("SYS_CUST_NAME").equals("ruijiao");
        session.setAttribute(beanName, B_JitPersonBeanBean);
      }
    }
    return B_JitPersonBeanBean;
  }
  /**
   * 构造函数
   */
  public B_JitPersonBean()
  {
    try {
      jbInit();
    }
  ...
  {
    setDataSetProperty(dsmaster, personSqlStructure);
    //添加操作的触发对象
    setDataSetProperty(dsSelect, personSqlStructure);
  ...
  public String code_name_Change(String code,String name,String operate,String isdelete){
    ...
    SelectSQL=personSql +" and  "+SelectColumnName+" like '%"+SelectColumn+"%' and a."+dp+"";
    ...
    String personid = dsSelect.getValue("personid");
    return personid;
  }
  ...
  class B_WorkProcedure_Init implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //try{
        String SQL=personSql;
        String deptid=data.getParameter("deptid");
        String gzzid=data.getParameter("gzzid");
        String xm=data.getParameter("xm");
        String isdelete=data.getParameter("isdelete");
        if(deptid!=null&&!deptid.equals(""))
          SQL+=" and a.deptid = " + deptid;
        if(gzzid!=null&&!gzzid.equals(""))
          SQL+=" and a.gzzid = " + gzzid;
        if(xm!=null&&!xm.equals(""))
          SQL+=" and xm like '%"+xm+"%' and a."+dp+"";
  ...
  class Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      SQL = personSql+" where 1=1 "+SQL;//combineSQL(PERSON_SQL, "?", new String[]{stat, deptids, SQL});
