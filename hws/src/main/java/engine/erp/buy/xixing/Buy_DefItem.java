 ...
 * @version 1.0
 */
public final class Buy_DefItem extends BaseAction implements Operate
{
  private static final String SENDMODE_SQL = "SELECT * FROM jc_orderdefitem WHERE 1=1 ? ";
   ...
   * @return 返回银行信用卡信息的实例
   */
  public static Buy_DefItem getInstance(HttpServletRequest request)
  {
    Buy_DefItem httkBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "httkBean";
      httkBean = (Buy_DefItem)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(httkBean == null)
      {
        httkBean = new Buy_DefItem();
        session.setAttribute(beanName, httkBean);//加入到session中
      }
    }
    return httkBean;
  }
  ...
  //  dsB_OrderType.setSequence(new SequenceDescriptor(new String[]{"ITEMCODE"}, new String[]{"s_jc_orderdefitem"}));//设置主健的sequence
    //添加操作的触发对象
 ...
 //     {
 //     //调用存贮过程
 //     String code = dataSetProvider.getSequence("SELECT pck_base.fieldNextCode('jc_orderdefitem','funditemcode','','',3) from dual");
 ...
 //     }
  ...
  class B_BillType_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
     ...
     // String itemcode = rowInfo.get("itemcode");
      String itemvalue = rowInfo.get("itemvalue");
     // if(funditemcode.equals("")){
       // data.setMessage(showJavaScript("alert('编号不能为空！');"));
        //return;
      //}
      if(itemvalue.equals("")){
        data.setMessage(showJavaScript("alert('条款内容不能为空！');"));
        return;
      }
    ...
    //  {
    //    ds.insertRow(false);
    //    ds.setValue("itemcode", "-1");
    //  }
      ds.setValue("itemcode", "cg_ht");
      ds.setValue("itemvalue", itemvalue);
      ds.post();
      ds.saveChanges();
