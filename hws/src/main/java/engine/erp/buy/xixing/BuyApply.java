package engine.erp.buy.xixing;//得到包
...
public final class BuyApply extends BaseAction implements Operate
{
  ...
  private static final String DETAIL_SQL    = "SELECT a.* FROM cg_sqdhw a ,kc_dm b where a.cpid=b.cpid and a.cgsqdid='?' order by b.cpbm";//
  private static final String DETAIL_STRUCYT_SQL    = "SELECT a.* FROM cg_sqdhw a ,kc_dm b where 1<>1 order by b.cpbm";//
  //用于审批时候提取一条记录
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable,"cg_sqd","cg_sqd");
  ...
  public static BuyApply getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(buyApplyBean == null)
      {
        ...
        buyApplyBean.dsDetailTable.setColumnFormat("hssl", buyApplyBean.qtyFormat);
        buyApplyBean.dsDetailTable.setColumnFormat("dj", buyApplyBean.priceFormat);
        buyApplyBean.dsDetailTable.setColumnFormat("je", buyApplyBean.priceFormat);
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsDetailTable.setTableName("cg_sqdhw");
  ...
  //得到一行信息
  public final RowMap getLookupRow(String cgsqdhwid)
  {
    RowMap row = new RowMap();
    if(cgsqdhwid == null || cgsqdhwid.equals(""))
      return row;//返回
    EngineRow locateRow = new EngineRow(dsDetailTable, "cgsqdhwid");//构建指定DataSet组件的1列的EngineRow（但是没有数据）
    if(locateRow == null)
      locateRow = new EngineRow(getDetailTable(), "cgsqdhwid");
    locateRow.setValue(0, cgsqdhwid);
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<dsDetailTable.getRowCount(); i++)
      {
        ...
        //新添的记录
        if(isMasterAdd)
          dsDetailTable.setValue("cgsqdid", cgsqdid);
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("cgsqdid"), "VW_CG_SQDHW", "cgsqdid", "sxz", "sxz", "like")
      });
      isInitQuery = true;
    }
  }
