...
public final class Buy_Out extends BaseAction implements Operate
{
 ...
 // private static final String DETAIL_SQL    = "SELECT * FROM cg_htjhdhw WHERE jhdid='?' ORDER BY hthwid,cpid";//
  private static final String DETAIL_SQL    = "SELECT a.* FROM cg_htjhdhw a ,kc_dm b where a.cpid=b.cpid and a.jhdid='?' order by b.cpbm  ";
  //判断进货单是否被入库单引用
  ...
  private static final String ORDER_DETAIL_SQL
      = "SELECT a.* FROM cg_hthw a, kc_dm b WHERE a.cpid=b.cpid AND nvl(a.sl,0)>0 AND (b.storeid IS NULL OR b.storeid=? ) AND htid=? ";//

  private EngineDataSet dsMasterTable  = new EngineDataSet();//主表
  private EngineDataSet dsDetailTable  = new EngineDataSet();//从表
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cg_htthd");
   ...
   * @return 返回销售合同列表的实例
   */
  public static Buy_Out getInstance(HttpServletRequest request)
  {
    Buy_Out buyOutBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "buyOutBean";
      buyOutBean = (Buy_Out)session.getAttribute(beanName);
      if(buyOutBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        buyOutBean = new Buy_Out();
        buyOutBean.qtyFormat = loginBean.getQtyFormat();
        buyOutBean.priceFormat = loginBean.getPriceFormat();
        buyOutBean.sumFormat = loginBean.getSumFormat();
        buyOutBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");

        buyOutBean.fgsid = loginBean.getFirstDeptID();
        buyOutBean.loginId = loginBean.getUserID();
        buyOutBean.loginName = loginBean.getUserName();
        buyOutBean.loginDept = loginBean.getDeptID();
        buyOutBean.user = loginBean.getUser();
        buyOutBean.SYS_APPROVE_ONLY_SELF = loginBean.getSystemParam("SYS_APPROVE_ONLY_SELF");//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
        SYS_CUST_NAME = loginBean.getSystemParam("SYS_CUST_NAME");//客户名称,用于客制化程序
        //buyOutBean.deptLimit = loginBean.getUser().getHandleDeptValue();
        //设置格式化的字段
        buyOutBean.dsDetailTable.setColumnFormat("sl", buyOutBean.qtyFormat);
        buyOutBean.dsDetailTable.setColumnFormat("hssl", buyOutBean.qtyFormat);
        buyOutBean.dsDetailTable.setColumnFormat("dj", buyOutBean.priceFormat);
        buyOutBean.dsDetailTable.setColumnFormat("je", buyOutBean.sumFormat);
        buyOutBean.dsDetailTable.setColumnFormat("sjrkl", buyOutBean.sumFormat);
        buyOutBean.dsMasterTable.setColumnFormat("zsl", buyOutBean.sumFormat);
        buyOutBean.dsMasterTable.setColumnFormat("zje", buyOutBean.sumFormat);
        session.setAttribute(beanName, buyOutBean);
      }
    }
    return buyOutBean;
  }
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("bz", rowInfo.get("bz_"+i));//备注
        detailRow.put("dhqk", rowInfo.get("dhqk_"+i));//到货情况
  ...
  class Instore_Confirm implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0;i<dsDetailTable.getRowCount(); i++)
      {
        ...
        {
        data.setMessage(showJavaScript("alert('和出库数量不一致，不能出库确认')"));
          return;
       }
       ...
       /*改变入库确认条件--2004.11.13*/
        ...
        /*
        if(!action.equals(OPERATE_A))
        {
          if(sjrkhsl.compareTo(hssl)!=0)
          {
            data.setMessage(showJavaScript("if(confirm('没有完全出库，是否还要出库!')) sumitForm("+OPERATE_A+","+data.getParameter("rownum")+")"));
            return;
          }
        }
       */
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(billType.equals("-1"))
        approve.putAproveList(dsMasterTable, row, "buy_out", content,deptid);
      //else
       //approve.putAproveList(dsMasterTable, row, "unbuy_out", content,deptid);
    }
  }
  /**
   * 取消审批触发操作
  ...
  class Cancle_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      jhdid = dsMasterTable.getValue("jhdid");
     String count = dataSetProvider.getSequence("select count(*) from cg_sqdhw a,cw_cgjshx b where b.jhdhwid=a.jhdhwid and a.jhdid="+jhdid);
    if(!count.equals("0"))
    {
      data.setMessage(showJavaScript("alert('该单据已被采购结算单引用，不能取消审批，您可以删除销售结算单中的相应货物后再取消审批!')"));
      return;
    }
    String count2 = dataSetProvider.getSequence("select count(*) from cg_sqdhw a,cw_cgfpmx b where b.jhdhwid=a.jhdhwid and a.jhdid="+jhdid);
   if(!count2.equals("0"))
   {
     ...
     return;
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(), "buy_out");
    }
  }
  /**
  * 输入产品编码把产品iD存入RorMap
   ...
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
        ...
        for(int i=0; i<d_RowInfos.size(); i++)
      {
         ...
         if(!row.get("hthwid").equals(""))
         {
           ...
           if(count.equals("0"))
           {
             data.setMessage(showJavaScript("alert('退货单明细中第"+(i+1)+"行所引合同被取消审批或作废，请核对审批后再保存!');"));
             return;
           }
         }
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if(djlx.equals("1"))//单据为进货单时保存正数即页面信息
          detail.setValue("je", bjfs.equals("1") ? String.valueOf(hssl*(je/hssl)) : String.valueOf(sl * dj));//金额
        else//单据为退货单时保存负数即页面数据乘以-1
          detail.setValue("je", bjfs.equals("1") ? String.valueOf(hssl*(je/hssl)) : String.valueOf(1*sl * dj));//金额
        ...
        detail.setValue("jhrq", detailrow.get("jhrq"));//到货情况
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("jhdid"), "VW_CG_HTJHDHW", "jhdid", "sxz", "sxz", "like"),//属性值
...
//dsDetailTable.setValue("sl", String.valueOf(-1*sl));
             ...
             * */
//}
             dsDetailTable.setValue("hssl", priceRow.get("hssl"));
