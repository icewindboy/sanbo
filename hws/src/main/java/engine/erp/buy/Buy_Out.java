...
public final class Buy_Out extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM cg_htjhd WHERE djlx=-1 AND 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM cg_htjhd WHERE djlx=-1 AND ? AND fgsid=? ? ORDER BY jhdbm DESC";
  ...
  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM cg_htjhd WHERE djlx=-1 AND jhdid='?'";
  //通过合同ID得到合同货物信息
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cg_htjhd.1");
   ...
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     for(int i=0; i < processData.getRowCount(); i++)
     {
       ...
       if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
       {
         ...
         //dsDetailTable.setValue("dhqk", processData.getValue("dhqk"));
   ...
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        detail.setValue("gyszyh", detailrow.get("gyszyh"));//供应商料号
        detail.setValue("dhqk", detailrow.get("dhqk"));//供应商料号
  ...
  class Detail_Copy implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        detail.setValue("dhqk", rowinfo.get("dhqk"));
