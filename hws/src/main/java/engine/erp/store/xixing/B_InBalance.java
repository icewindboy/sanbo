...
public final class B_InBalance extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM kc_nbjs WHERE 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM kc_nbjs WHERE ? and fgsid=? ? ORDER BY jsrq DESC";
  private static final String TOTALZSL_SQL  = "SELECT SUM(nvl(zsl,0)) tzsl ,SUM(nvl(zje,0)) tzje FROM kc_nbjs a WHERE ? AND fgsid=? ? ORDER BY nbjsdh DESC";
  private static final String DETAIL_STRUT_SQL = "SELECT * FROM kc_nbjsmx WHERE 1<>1";
  //02.28 23:15  修改 将下面此句的sql,用了'?'来代替原来的nbjsid=.
  //解决了当sdfdjid是空的时候会页面上(主要是contract_instore_bottom.jsp上)会出现sql错误的问题.yjg
  private static final String DETAIL_SQL = "SELECT kc_nbjsmx.* FROM kc_nbjsmx kc_nbjsmx, kc_dm b WHERE kc_nbjsmx.cpid = b.cpid and kc_nbjsmx.nbjsid='?' order by b.cpbm ";//
  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM kc_nbjs WHERE  nbjsid='?'";
  //查询数据库是否有记账的单据
  private static final String RECODE_DATASQL     = " SELECT COUNT(*) FROM kc_nbjs a WHERE a.zdrid='?'  AND a.zt=1";
  //把符合记帐功能的数据全部记帐
  private static final String RECODE             = "UPDATE kc_nbjs a SET a.zt=2 WHERE a.zdrid='?' AND a.zt=1";
  //只取库存物资明细表中的数据
  ...
  public  HtmlTableProducer masterProducer    = new HtmlTableProducer(dsMasterTable, "kc_nbjs");
  public  HtmlTableProducer detailProducer    = new HtmlTableProducer(dsDetailTable, "kc_nbjsmx");
   ...
   * @return 返回入库单列表的实例
   */
  public static B_InBalance getInstance(HttpServletRequest request)
  {
    B_InBalance inBalanceBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "inBalanceBean";
      inBalanceBean = (B_InBalance)session.getAttribute(beanName);
      if(inBalanceBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        inBalanceBean = new B_InBalance();
        inBalanceBean.qtyFormat = loginBean.getQtyFormat();

        inBalanceBean.fgsid = loginBean.getFirstDeptID();
        inBalanceBean.loginId = loginBean.getUserID();
        inBalanceBean.loginName = loginBean.getUserName();
        inBalanceBean.loginDept = loginBean.getDeptID();
        inBalanceBean.user = loginBean.getUser();
        //设置格式化的字段
        inBalanceBean.dsDetailTable.setColumnFormat("sl", inBalanceBean.qtyFormat);
        inBalanceBean.dsDetailTable.setColumnFormat("hssl", inBalanceBean.qtyFormat);
        inBalanceBean.dsMasterTable.setColumnFormat("zsl", inBalanceBean.sumFormat);
        inBalanceBean.dsMasterTable.setColumnFormat("zje", inBalanceBean.sumFormat);
        inBalanceBean.dsMasterTable.setColumnFormat("dj", inBalanceBean.priceFormat);
        inBalanceBean.dsMasterTable.setColumnFormat("je", inBalanceBean.sumFormat);
//        inBalanceBean.dsMasterTable.setColumnFormat("drsl", inBalanceBean.qtyFormat);
        session.setAttribute(beanName, inBalanceBean);
      }
    }
    return inBalanceBean;
  }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"nbjsdh"}, new String[]{"SELECT pck_base.billNextCode('kc_nbjs','nbjsdh') from dual"}));
    //dsMasterTable.setSort(new SortDescriptor("", new String[]{"nbjsdh"}, new boolean[]{true}, null, 0));
    dsDetailTable.setTableName("kc_nbjsmx");
    dsDetailTable.setSequence(new SequenceDescriptor(new String[]{"nbjsmxid"}, new String[]{"s_kc_nbjsmx"}));
 ...
 /*打开从表*/
  public final void openDetailTable(boolean isMasterAdd)
  {
    ...
    //解决了当sdfdjid是空的时候会页面上(主要是contract_instore_bottom.jsp上)会出现sql错误的问题.yjg
    dsDetailTable.setQueryString(combineSQL(DETAIL_SQL, "?", new String[]{isMasterAdd ? "-1" : nbjsid}));
  ...
  */
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //03.16 15:04 修改 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
      //以实现:根据下达的部门，进行提交审批；
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "in_balance_list", content, dsMasterTable.getValue("deptid"));
    }
  }
  /**
   * 取消审批的操作类
  ...
  class Cancel_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable, dsMasterTable.getRow(), "in_balance_list");
    }
  }
...
class Cancer implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    String nbjsid = data.getParameter("rownum");
    String zt = dataSetProvider.getSequence("SELECT a.zt FROM kc_nbjs a WHERE a.nbjsid='"+nbjsid+"'");
    dsMasterTable.setQueryString(combineSQL(MASTER_APPROVE_SQL,"?",new String[]{nbjsid}));
    if(!dsMasterTable.isOpen())
      dsMasterTable.openDataSet();
    else
      dsMasterTable.refresh();
    //String today = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
    //dsMasterTable.setValue("jsrq",today);
 ...
 class Onchange implements Obactioner
 {
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ds.insertRow(false);
        nbjsid = dataSetProvider.getSequence("s_kc_nbjs");
        ds.setValue("nbjsid", nbjsid);
      ...
      ds.setValue("kc_storeid", rowInfo.get("kc_storeid"));//仓库id
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        new QueryColumn(master.getColumn("nbjsdh"), null, null, null),
        ...
        new QueryColumn(master.getColumn("nbjsid"), "kc_nbjsmx", "nbjsid", "cpid", null, "="),
        new QueryColumn(master.getColumn("nbjsid"), "VW_KC_NBJSHW", "nbjsid", "cpbm", "cpbm", "like"),
        new QueryColumn(master.getColumn("nbjsid"), "VW_KC_NBJSHW", "nbjsid", "product", "product", "like"),
        new QueryColumn(master.getColumn("nbjsid"), "VW_KC_NBJSHW", "nbjsid", "pm", "pm", "like"),
        new QueryColumn(master.getColumn("nbjsid"), "VW_KC_NBJSHW", "nbjsid", "gg", "gg", "like"),
        new QueryColumn(master.getColumn("nbjsid"), "VW_KC_NBJSHW", "nbjsid", "sxz", "sxz", "like"),
 ...
 class Detail_Add implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     detail.setValue("nbjsid", isMasterAdd ? "-1" : nbjsid);
     detail.post();
 ...
 class Detail_Add_Blank implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     if(!isMasterAdd)
       ds.goToInternalRow(masterRow);
     String nbjsid = ds.getValue("nbjsid");
     detail.insertRow(false);
     detail.setValue("nbjsid", isMasterAdd ? "-1" : nbjsid);
     detail.post();
...
//            String djlx = saleLadingRow.get("djlx");
            ...
            double wcksl = sl;
            dsDetailTable.insertRow(false);
            dsDetailTable.setValue("nbjsmxid", "-1");
            dsDetailTable.setValue("nbjsid", isMasterAdd ? "-1" : nbjsid);
