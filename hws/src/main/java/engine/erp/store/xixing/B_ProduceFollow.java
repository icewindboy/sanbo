...
/**
 * <p>Title: 生产转序单</p>
 * <p>Description: 生产转序单<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author zjb
 * @version 1.0
 */
public final class B_ProduceFollow extends BaseAction implements Operate
{
  ...
  public  static final String RECODE_ACCOUNT_BY_ROW = "9997";//2005-9-12 17:48 新增 逐笔记帐功能的sql zjb
  ...
  public  static final String DETAIL_LADINGGOODS_ADD = "11586"; //办事处调拨单的bean的引用事件, 用于提取办事处调拨单货物信息
  public  static final String LADDING_CANCER      = "11587";     //内部结算单作废

  private static final String MASTER_STRUT_SQL = "SELECT * FROM kc_cjzx WHERE 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM kc_cjzx WHERE ? and fgsid=? ? ORDER BY jsrq DESC";
  private static final String TOTALZSL_SQL  = "SELECT SUM(nvl(zsl,0)) tzsl ,SUM(nvl(zje,0)) tzje FROM kc_cjzx a WHERE ? AND fgsid=? ? ORDER BY cjzxdh DESC";
  private static final String DETAIL_STRUT_SQL = "SELECT * FROM kc_cjzxmx WHERE 1<>1";
  //02.28 23:15  修改 将下面此句的sql,用了'?'来代替原来的cjzxid=.
  //解决了当sdfdjid是空的时候会页面上(主要是contract_instore_bottom.jsp上)会出现sql错误的问题.yjg
  private static final String DETAIL_SQL = "SELECT kc_cjzxmx.* FROM kc_cjzxmx kc_cjzxmx, kc_dm b WHERE kc_cjzxmx.cpid = b.cpid and kc_cjzxmx.cjzxid='?' order by b.cpbm ";//
  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM kc_cjzx WHERE  cjzxid='?'";
  //查询数据库是否有记账的单据
  private static final String RECODE_DATASQL     = " SELECT COUNT(*) FROM kc_cjzx a WHERE a.zdrid='?'  AND a.zt=1";
  //把符合记帐功能的数据全部记帐
  private static final String RECODE             = "UPDATE kc_cjzx a SET a.zt=2 WHERE a.zdrid='?' AND a.zt=1";
  //只取库存物资明细表中的数据
  ...
  public  HtmlTableProducer masterProducer    = new HtmlTableProducer(dsMasterTable, "kc_cjzx");
  public  HtmlTableProducer detailProducer    = new HtmlTableProducer(dsDetailTable, "kc_cjzxmx");

  private ImportMaterail importMaterailBean = null; //物资明细的bean的引用, 用于提取物资明细
  private engine.erp.subsection.ImportInstepGoods importInstepGoods = null; //办事处调拨单的bean的引用, 用于提取办事处调拨单货物信息
   ...
   * @return 返回入库单列表的实例
   */
  public static B_ProduceFollow getInstance(HttpServletRequest request)
  {
    B_ProduceFollow producFollowBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "producFollowBean";
      producFollowBean = (B_ProduceFollow)session.getAttribute(beanName);
      if(producFollowBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        producFollowBean = new B_ProduceFollow();
        producFollowBean.qtyFormat = loginBean.getQtyFormat();

        producFollowBean.fgsid = loginBean.getFirstDeptID();
        producFollowBean.loginId = loginBean.getUserID();
        producFollowBean.loginName = loginBean.getUserName();
        producFollowBean.loginDept = loginBean.getDeptID();
        producFollowBean.user = loginBean.getUser();
        //设置格式化的字段
        producFollowBean.dsDetailTable.setColumnFormat("sl", producFollowBean.qtyFormat);
        producFollowBean.dsMasterTable.setColumnFormat("zsl", producFollowBean.sumFormat);
        producFollowBean.dsMasterTable.setColumnFormat("zje", producFollowBean.sumFormat);
        producFollowBean.dsMasterTable.setColumnFormat("dj", producFollowBean.priceFormat);
        producFollowBean.dsMasterTable.setColumnFormat("je", producFollowBean.sumFormat);
//        producFollowBean.dsMasterTable.setColumnFormat("drsl", producFollowBean.qtyFormat);
        session.setAttribute(beanName, producFollowBean);
      }
    }
    return producFollowBean;
  }

  /**
   * 构造函数
   */
  private B_ProduceFollow()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"cjzxdh"}, new String[]{"SELECT pck_base.billNextCode('kc_cjzx','cjzxdh') from dual"}));
    dsDetailTable.setTableName("kc_cjzxmx");
    dsDetailTable.setSequence(new SequenceDescriptor(new String[]{"cjzxmxid"}, new String[]{"s_kc_cjzxmx"}));
    ...
    addObactioner(String.valueOf(RECODE_ACCOUNT_BY_ROW), new Recode_Account_ByRow());//逐行记账
    ...
    addObactioner(String.valueOf(DETAIL_LADINGGOODS_ADD), new Detail_LadingGoods_Add());//办事处调拨单的bean的引用事件, 用于提取办事处调拨单货物信息
 ...
 /*打开从表*/
  ...
  {
    String cjzxid = dsMasterTable.getValue("cjzxid");
    //02.28 23:15  修改 将下面此句setQueryString中的sql由原来的手动用+号组成sql`改成现在用combineSQL来组成.
    //解决了当sdfdjid是空的时候会页面上(主要是contract_instore_bottom.jsp上)会出现sql错误的问题.yjg
    dsDetailTable.setQueryString(combineSQL(DETAIL_SQL, "?", new String[]{isMasterAdd ? "-1" : cjzxid}));
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      row.put("zt", "0");
      row.put("jsrq$a", startDay);
      row.put("jsrq$b", today);
      isMasterAdd = true;
      isDetailAdd = false;
      //
      String SQL = " AND zt<>2  AND ZT<>4";
      ...
      String tempTzslSql = " AND zt<>2 AND ZT<>4";
      tempTzslSql = combineSQL(TOTALZSL_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "zdrid"),fgsid,tempTzslSql});
      ...
      else
      {
        ...
        totalzje = dsTotalZsl.getValue("tzje");
      }
    }
  }
  ...
  /**
    ...
    * @return
  */
   class Recode_Account_ByRow implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       String aab= data.getParameter("rownum");
       dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
       cjzxid = dsMasterTable.getValue("cjzxid");
       dsMasterTable.setValue("zt", "2");
       dsMasterTable.post();
       dsMasterTable.saveDataSets(new EngineDataSet[]{dsMasterTable}, null);
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      String aa= data.getParameter("rownum");
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      String content = dsMasterTable.getValue("cjzxdh");
      //03.16 15:04 修改 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
      //以实现:根据下达的部门，进行提交审批；
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "produce_follow_list", content, dsMasterTable.getValue("deptid"));
    }
  }
  /**
   * 取消审批的操作类
  ...
  class Cancel_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable, dsMasterTable.getRow(), "produce_follow_list");
    }
  }
...
class Cancer implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    String cjzxid = data.getParameter("rownum");
    String zt = dataSetProvider.getSequence("SELECT a.zt FROM kc_cjzx a WHERE a.cjzxid='"+cjzxid+"'");
    dsMasterTable.setQueryString(combineSQL(MASTER_APPROVE_SQL,"?",new String[]{cjzxid}));
    if(!dsMasterTable.isOpen())
 ...
 class Onchange implements Obactioner
 {
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ds.insertRow(false);
        cjzxid = dataSetProvider.getSequence("s_kc_cjzx");
        ds.setValue("cjzxid", cjzxid);
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        detail.setValue("je", String.valueOf(je));
      ...
      ds.setValue("kc__deptid", rowInfo.get("kc__deptid"));//部门id
      ds.setValue("jsrq", rowInfo.get("jsrq"));//收发日期
    ...
    private String checkMasterInfo()
    {
     ...
     if(temp.equals(""))
        return showJavaScript("alert('请选择收车间！');");
     temp = rowInfo.get("kc__deptid");
     if(temp.equals(""))
       return showJavaScript("alert('请选择发车间！');");
     return null;
    }
  }
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        new QueryColumn(master.getColumn("cjzxdh"), null, null, null),
        ...
        new QueryColumn(master.getColumn("kc__deptid"), null, null, null, null, "="),//部门
        new QueryColumn(master.getColumn("cjzxid"), "kc_cjzxmx", "cjzxid", "cpid", null, "="),
        new QueryColumn(master.getColumn("cjzxid"), "VW_KC_CJLZHW", "cjzxid", "cpbm", "cpbm", "like"),
        new QueryColumn(master.getColumn("cjzxid"), "VW_KC_CJLZHW", "cjzxid", "product", "product", "like"),
        new QueryColumn(master.getColumn("cjzxid"), "VW_KC_CJLZHW", "cjzxid", "pm", "pm", "like"),
        new QueryColumn(master.getColumn("cjzxid"), "VW_KC_CJLZHW", "cjzxid", "gg", "gg", "like"),
        new QueryColumn(master.getColumn("cjzxid"), "VW_KC_CJLZHW", "cjzxid", "sxz", "sxz", "like"),
 ...
 class Detail_Add implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     detail.setValue("cjzxid", isMasterAdd ? "-1" : cjzxid);
     detail.post();
 ...
 class Detail_Add_Blank implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     if(!isMasterAdd)
       ds.goToInternalRow(masterRow);
     String cjzxid = ds.getValue("cjzxid");
     detail.insertRow(false);
     detail.setValue("cjzxid", isMasterAdd ? "-1" : cjzxid);
     detail.post();
 ...
 class Show_Detail implements Obactioner
 {
   ...
   */
   class transferScan implements Obactioner
   {
     ...
     String sql = combineSQL(STOCK_MATERIAL_LIST, "@", new String[]{});
    ...
    class Detail_LadingGoods_Add implements Obactioner
    {
      public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
      {
        ...
        for(int i=0; i < tddhwIDs.length; i++)
        {
          ...
          if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
          {
            RowMap saleLadingRow = getInstepGoodsBean(req).getLookupRow(tddhwIDs[i]);
...
//            String djlx = saleLadingRow.get("djlx");
            ...
            double wcksl = sl;
            dsDetailTable.insertRow(false);
            dsDetailTable.setValue("cjzxid", "-1");
            dsDetailTable.setValue("cjzxid", isMasterAdd ? "-1" : cjzxid);
     ...
     */
    private engine.erp.subsection.ImportInstepGoods getInstepGoodsBean(HttpServletRequest req)
    {
      if(importInstepGoods == null)
        importInstepGoods = engine.erp.subsection.ImportInstepGoods.getInstance(req);
      return importInstepGoods;
    }
