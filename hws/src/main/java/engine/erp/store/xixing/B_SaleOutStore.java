 ...
 * <p>right: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author 李建华
 * @version 1.0
 */

public final class B_SaleOutStore extends BaseAction implements Operate
{
  ...
  public  static final String MATERIAL_ON_CHANGE = "21564";//改变原料
  ...
  public  static final String CERTIFYCARD_PRINT      = "1013";// 新增 为明细表格番页而加的事件
  public  static final String LADING_OUT             = "1015";//出库确认
  public  static final String DETAIL_CHANGE          = "9007";//从表明细改变，引入物资价格
  ...
  private static String MASTER_EDIT_SQL              = "select * from kc_sfdj where sfdjid = '?' ";
  ...
  //未审也递减 2004-12-6 wlc
  private static final String LADDING_DETAIL_SQL
      = " SELECT a.djlx,a.khlx,a.storeid,a.dwtxid,a.jsfsid,a.deptid,a.personid, "
      + "b.tdhwid,b.tdid,b.cpid,b.hthwid,b.wzdjid, "
      + "(nvl(b.hssl,0)-nvl((SELECT SUM(nvl(k.hssl,0)) FROM kc_sfdjmx k WHERE b.tdhwid = k.wjid AND k.djxz=2), 0)) hssl, "
      + "(nvl(b.sl,0)-nvl((SELECT SUM(nvl(k.sl,0)) FROM kc_sfdjmx k WHERE b.tdhwid = k.wjid AND k.djxz=2), 0)) sl, "
      + "(nvl(b.jje,0)-nvl((SELECT SUM(nvl(k.je,0)) FROM kc_sfdjmx k WHERE b.tdhwid = k.wjid AND k.djxz=2), 0)) jje, "
      + "b.xsj,b.xsje,b.zk,b.bz,b.sthssl,b.stsl,b.ssje,b.dj,b.skpsl,b.dmsxid,b.ckrq,b.hltcl,b.cjtcl,b.jzj,b.jxts,b.hlts,b.sfth,b.kdsl,b.sfcz,b.yf,b.js,b.jhrq "
      + " FROM xs_td a, xs_tdhw b WHERE a.tdid=b.tdid AND abs(nvl(b.sl,0))>abs(nvl((SELECT SUM(nvl(k.sl,0)) FROM kc_sfdjmx k WHERE b.tdhwid = k.wjid AND k.djxz=2), 0)) AND b.tdid= '?' ";
  ...
  private static final String CERTIFCARED_PRING_SQL = " SELECT a.rkdmxid, a.cpid,c.xsj "
      + " FROM kc_sfdjmx a, xs_tdhw c "
      + " WHERE a.wjid = c.tdhwid AND a.djxz = 2 "
      + "       AND a.rkdmxid = ?  ";
  ...
  private EngineDataSet dsDetailTable            = new EngineDataSet();//从表
    private EngineDataSet productStockData = new EngineDataSet();//库存量数据集
  ...
  public  HtmlTableProducer masterProducer       = new HtmlTableProducer(dsMasterTable, "kc_sfdj.2");
  public  HtmlTableProducer masterListProducer   = new HtmlTableProducer(dsMasterList, "kc_sfdj.2");
  public  HtmlTableProducer detailProducer       = new HtmlTableProducer(dsDetailTable, "kc_sfdjmx.2");
  ...
  private EngineDataSet dsCertifyCardPrint       = new EngineDataSet();//2004-09-27 10:48 新增 专门为合格证打印
   ...
   * @return 返回入库单列表的实例
   */
  public static B_SaleOutStore getInstance(HttpServletRequest request)
  {
    B_SaleOutStore saleOutStoreBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "saleOutStoreBean";
      saleOutStoreBean = (B_SaleOutStore)session.getAttribute(beanName);
      if(saleOutStoreBean == null)
      {
        ...
        saleOutStoreBean = new B_SaleOutStore();
        ...
        //saleOutStoreBean.dsMasterTable.setColumnFormat("zje", saleOutStoreBean.qtyFormat);
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    setDataSetProperty(dsCertifyCardPrint, null);
    ...
    addObactioner(String.valueOf(MATERIAL_ON_CHANGE), new MaterialOnChange()); //改变原料的到库存量
    ...
    addObactioner(CERTIFYCARD_PRINT, new CertifyCard_Print());//翻页事件
  ...
  public final void valueUnbound(HttpSessionBindingEvent event)
  {
    ...
    if(dsCertifyCardPrint != null){
     dsCertifyCardPrint.close();
     dsCertifyCardPrint = null;
    }
    log = null;
    m_RowInfo = null;
    d_RowInfos = null;
  ...
  public final RowMap[] getPorductStockInfos(){
     RowMap[] p_RowInfos = null; //与从表象对应的库存量表
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=min; i<=max; i++)
    {
      ...
      detailRow.put("sssl", formatNumber(rowInfo.get("sssl_"+i), qtyFormat));//
      detailRow.put("sshssl", formatNumber(rowInfo.get("sshssl_"+i), qtyFormat));//
      ...
      detailRow.put("zxh", rowInfo.get("zxh_"+i));//
  ...
  /*得到合格证表对象*/
  public final EngineDataSet getCertifyCardTable(){
    if(!dsCertifyCardPrint.isOpen())
      dsCertifyCardPrint.openDataSet();
    return dsCertifyCardPrint;
  }
  /*打开从表*/
  ...
  public String getRcount(String sfdjid)
  {
    String sql = "select COUNT(*) from kc_sfdjmx t WHERE t.sssl IS NULL AND t.sfdjid='"+sfdjid+"'";
  ...
  class Onchange implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String oldsfdjlbid = rowinfo.get("sfdjlbid");
      putDetailInfo(data.getRequest());
      String dwtxid = rowinfo.get("dwtxid");
      String storeid = rowinfo.get("storeid");
      String sfdjlbid = rowinfo.get("sfdjlbid");
      if(!oldDwtxid.equals(dwtxid) || !oldstoreid.equals(storeid) || !oldsfdjlbid.equals(sfdjlbid)){
  ...
  */
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterList.goToRow(Integer.parseInt(data.getParameter("rownum")));
      String sfdjid = dsMasterList.getValue("sfdjid");
      String zt = dataSetProvider.getSequence("SELECT a.zt FROM kc_sfdj a WHERE a.sfdjid='"+sfdjid+"'");
...
class Lading_Out implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    //传入的rownum是tdid
    //dsMasterList.goToRow(Integer.parseInt(data.getParameter("rownum")));
    sfdjid = data.getParameter("rownum");

    dsMasterTable.setQueryString(combineSQL(MASTER_EDIT_SQL,"?",new String[]{sfdjid}));
    if(!dsMasterTable.isOpen())
      dsMasterTable.openDataSet();
    else
      dsMasterTable.refresh();
    ...
    {
      String sssl = dsDetailTable.getValue("sssl");
      String sl = dsDetailTable.getValue("sl");
      if(sssl.equals(""))
      {
        data.setMessage(showJavaScript("alert('实收数量没有回填,还不能出库确认!')"));
        return;
      }
        if((Math.abs(Double.parseDouble(sl.equals("")?"0":sl)))<(Math.abs(Double.parseDouble(sssl.equals("")?"0":sssl))))
      {
        data.setMessage(showJavaScript("alert('实收数量不能大于出库数量!')"));
        return;
      }
      dsDetailTable.next();
    }
    ...
    */
    ...
    dsMasterTable.setValue("sfrq",today);
    dsMasterTable.post();
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
        ...
        for(int i=0; i<d_RowInfos.size(); i++)
     {
        ...
        String  aa=row.get("wjid");
        if(!row.get("wjid").equals(""))
        {
          String wjid = row.get("wjid");
          String count = dataSetProvider.getSequence("select count(*) from xs_tdhw b,xs_td c where  c.tdid=b.tdid and c.zt=1 and b.tdhwid="+wjid);
          if(count.equals("0"))
          {
            data.setMessage(showJavaScript("alert('该单据明细中第"+(i+1)+"行所引发货单被取消审批或作废，请核对审批后再保存!');"));
            return;
          }
        }
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        double sssl = detailrow.get("sssl").length() > 0 ? Double.parseDouble(detailrow.get("sssl")) : 0;
        ...
        detail.setValue("je", String.valueOf(sssl * dj));//金额
        //}
        detail.setValue("sssl", detailrow.get("sssl"));//实收数量
        detail.setValue("sshssl", detailrow.get("sshssl"));//实收换算数量
        detail.setValue("cpid", detailrow.get("cpid"));
        ...
        detail.setValue("zxh", detailrow.get("zxh"));
      ...
      ds.setValue("sfdjlb", rowInfo.get("sfdjlb"));//收发单据类别
   ...
   //   System.out.println(sfdjid);
    //  System.out.println(ds.getValue("sfdjid"));
      ds.post();
    ...
    private String checkDetailInfo()
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        boolean isAllowPlus = srlx.equals("-1");//是否允许为负数数.-1时允许
        BigDecimal tmpsl = new BigDecimal(sl);
        BigDecimal sm = new BigDecimal("0");
        boolean isPlus = tmpsl.compareTo(sm)>=0;
        //根据是否srlx允许为负数.如不允许则弹出警示
        boolean isAlert = isAllowPlus?!isAllowPlus:tmpsl.compareTo(sm)<=0;
        if (isAlert)
        ...
        BigDecimal tmphssl = new BigDecimal(hssl.length()>0?hssl:"0");
        sm = new BigDecimal("0");
        isAlert = isAllowPlus?!isAllowPlus:tmphssl.compareTo(sm)<=0;
        if(hssl.length()>0 && isAlert)
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        new QueryColumn(master.getColumn("sfdjdh"), null, null, null, null, "like"),
        ...
        new QueryColumn(master.getColumn("sfdjid"), "VW_KC_SFDJQUERY", "sfdjid", "sxz", "sxz", "like"),//从表产品名称
 ...
 */
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < tddhwIDs.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          double hssl  = saleLadingRow.get("hssl").length() > 0 ? Double.parseDouble(saleLadingRow.get("hssl")) : 0;//实际换算数量
          double sl  = saleLadingRow.get("sl").length() > 0 ? Double.parseDouble(saleLadingRow.get("sl")) : 0;//实际数量
          ...
          //djlx=-1表示退库
          double wckhssl = djlx.equals("-1")?-1*hssl:hssl;
          double wcksl = djlx.equals("-1")?-1*sl:sl;
          dsDetailTable.insertRow(false);
          dsDetailTable.setValue("rkdmxid", "-1");
          dsDetailTable.setValue("sfdjid", isMasterAdd ? "-1" : sfdjid);
  ...
  class SingleLadding_Detail_LadingGoods_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < dsSingleLaddingDetail.getRowCount(); i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          double hssl  = dsSingleLaddingDetail.getValue("hssl").length() > 0 ? Double.parseDouble(dsSingleLaddingDetail.getValue("hssl")) : 0;//实际换算数量
          double sl  = dsSingleLaddingDetail.getValue("sl").length() > 0 ? Double.parseDouble(dsSingleLaddingDetail.getValue("sl")) : 0;//实际数量
          ...
          //dsMasterTable.setValue("dwtxid", dsSingleLaddingDetail.getValue("dwtxid"));
          //2004-3-31 10:47 新增 根据系统参数将提单单价也引过来 yjg
          //if (KC_OUT_SHOW_PRICE.equals("1")) //1=显示,0=不显示
          //{
  ...
  class Single_Ladding implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      rowinfo.put("deptid", laddingGoodsData.getValue("deptid"));//仓库
      ...
      for(int i=0; i < laddingGoodsData.getRowCount(); i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          double wthsl = djlx.equals("-1")?(hssl):hssl;//未提换算数量.-1表:销售退货单
          double wtsl = djlx.equals("-1")?(sl):sl;//未提数量
          jje = djlx.equals("-1")?jje:jje;
          if(sl==0)
            continue;
          dsDetailTable.insertRow(false);
   ...
   *  合格证打印
   */
  class CertifyCard_Print implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      RowMap detailrow = (RowMap)d_RowInfos.get(rownum);
      long l_row = Long.parseLong(detailrow.get("InternalRow"));
      dsDetail.goToInternalRow(l_row);
      String rkdmxid = dsDetail.getValue("rkdmxid");
      String cpid = dsDetail.getValue("cpid");
      cpid = cpid.equals("")?cpid:" and a.cpid = "+cpid;
      String sql = combineSQL(CERTIFCARED_PRING_SQL, "?", new String[]{rkdmxid});
      dsCertifyCardPrint.setQueryString(sql);
      if (dsCertifyCardPrint.isOpen())
        dsCertifyCardPrint.refresh();
      else
        dsCertifyCardPrint.openDataSet();
    }
  }
  ...
  class Copy_CurrentRow implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        detail.setValue("fgsid", fgsid);
        detail.post();
//        productStockData.goToInternalRow(swapdetailRow);
//        productStockData.setValue("fgsid",fgsid);
//        productStockData.setValue("storeid",dsMasterTable.getValue("storeid")==null?"-1":dsMasterTable.getValue("storeid"));
//        productStockData.setValue("cpid",rowinfo.get("cpid"));
...
//       productStockData.setValue("fgsid",fgsid);
//       productStockData.setValue("storeid",dsMasterTable.getValue("storeid")==null?"-1":dsMasterTable.getValue("storeid"));
//       productStockData.setValue("cpid",detailRow.get("cpid"));
  ...
  class Detail_Change implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(rownum==i)
      {
        ...
        String lsj = dataSetProvider.getSequence("select t.lsj from xs_wzdj t WHERE t.cpid='"+cpid+"'");
  ...
  */
  ...
  private String checkNumRigtt(int tempmin, int tempmax) throws Exception
  {
    ...
    for(int i=0; i<d_RowInfos.size(); i++)
    {
      ...
      boolean isAllowPlus = srlx.equals("1");//是否允许为正数.1时允许
      BigDecimal tmpsl = new BigDecimal(sl);
      BigDecimal sm = new BigDecimal("0");
      boolean isPlus = tmpsl.compareTo(sm)>=0;
      //根据是否srlx允许为负数.如不允许则弹出警示
      boolean isAlert = isAllowPlus?!isAllowPlus:tmpsl.compareTo(sm)>=0;
      if (isAlert)
        return showJavaScript("alert('第"+row+"行数量不能为正数！');");
      ...
      isAlert = isAllowPlus?!isAllowPlus:tmphssl.compareTo(sm)>0;
      if(hssl.length()>0 && isAlert)
        return showJavaScript("alert('第"+row+"行换算数量不能为正数！');");
