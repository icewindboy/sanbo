...
/**
 * <p>Title: 财务管理-仓库辅助材料收发存汇总表</p>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: ENGINE</p>
 * @version 1.0
 */
public final class B_StoreAssistantMaterialCollect extends BaseAction implements Operate
{
  ...
  private static final String KC_DM_SQL =" select cpid from vw_kc_sy_fc where 1=1 ? ";
   ...
   */
  public static B_StoreAssistantMaterialCollect getInstance(HttpServletRequest request)
  {
    B_StoreAssistantMaterialCollect B_StoreAssistantMaterialCollectBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_StoreAssistantMaterialCollectBean";//名称-值对应
      B_StoreAssistantMaterialCollectBean = (B_StoreAssistantMaterialCollect)session.getAttribute(beanName);
      if(B_StoreAssistantMaterialCollectBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        String fgsID = loginBean.getFirstDeptID();
        B_StoreAssistantMaterialCollectBean = new B_StoreAssistantMaterialCollect(fgsID);//调用构造函数
        B_StoreAssistantMaterialCollectBean.loginName = loginBean.getUserName();
        B_StoreAssistantMaterialCollectBean.personid=loginBean.getUserID();
        session.setAttribute(beanName,B_StoreAssistantMaterialCollectBean);
      }
    }
    return B_StoreAssistantMaterialCollectBean;
  }
  /**
   * 构造函数(实例变量:分公司ID为初始化参数)
   */
  private B_StoreAssistantMaterialCollect(String fgsid)
  {
    this.fgsid = fgsid;
    try {
      jbInit();
  ...
  class FIXED_SEARCH implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String cksql = combineSQL(KC_DM_SQL, "?", new String[]{" and  sfrq>=to_date('"+ksrq+"','yyyy-mm-dd') and  sfrq<=to_date('"+jsrq+"','yyyy-mm-dd') and storeid="+storeid+" group by cpid"}) ;
    ...
    public void getData(String cpid,String ksrq,String jsrq,String storeid)
    {
      ...
      //{
        ...
        String tcksql = combineSQL(KC_CQ_SQL, "?", new String[]{"  and cpid="+cpid+"  and djlb=1 and sfrq<to_date('"+jsrq+"','yyyy-mm-dd') and storeid="+storeid+" group by cpid"}) ;
      ...
      //}
        ...
        EngineDataSet tdscqck = new EngineDataSet();
        String cksql = combineSQL(KC_CQ_SQL, "?", new String[]{" and cpid="+cpid+" and sfrq<to_date('"+ksrq+"','yyyy-mm-dd') and storeid="+storeid+" group by cpid"}) ;
        setDataSetProperty(tdscqck,cksql);
        tdscqck.openDataSet();
      ...
      String srcsql = combineSQL(KC_SRMX_SQL, "?", new String[]{"   cpid="+cpid+" and sfrq>=to_date('"+ksrq+"','yyyy-mm-dd') and  sfrq<=to_date('"+jsrq+"','yyyy-mm-dd') and storeid="+storeid+""}) ;
      ...
      for(int i=0;i<dsczrc.getRowCount();i++)
      {
        ...
        if(djlb.equals("1")||djlb.equals("2"))
        {
          //购入
