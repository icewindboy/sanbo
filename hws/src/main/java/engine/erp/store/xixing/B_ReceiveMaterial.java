...
public final class B_ReceiveMaterial extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM sc_drawmaterial WHERE 1<>1";
  ...
  //查询过后总数量
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(totalnum,0)) zsl FROM sc_drawmaterial WHERE ? AND ? AND filialeid='?' AND isout='?' ?  ";
  //用于审批时候提取一条记录
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        double dj = detailrow.get("drawprice").length() > 0 ? Double.parseDouble(detailrow.get("drawprice")) : 0;//单价
        double sl = detailrow.get("drawNum").length() > 0 ? Double.parseDouble(detailrow.get("drawNum")) : 0;//数量
        double je = detailrow.get("drawSum").length() > 0 ? Double.parseDouble(detailrow.get("drawSum")) : 0;//金额
        ...
        detail.setValue("drawSum", String.valueOf(dj*sl));//保存金额
...
//      p_ds.deleteRow();
      ds.post();
//      p_ds.post();
      data.setMessage(showJavaScript("sumitForm("+MATERIAL_ON_CHANGE+",-1)"));
    }
  }
  /**
   *自制收货单复制一行操作
