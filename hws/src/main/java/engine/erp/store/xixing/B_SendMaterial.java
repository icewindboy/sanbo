 ...
 */
/**
 * <p>Title:  发料单表结构改过，不在kc_sfdj,kc_sfdjmx中了.现有表为sc_drawmaterail,sc_drawmaterialdetail</p>
 */

public final class B_SendMaterial extends BaseAction implements Operate
{
  ...
  public  static final String MATERIAL_ON_DELETE = "21565";//删除从表一行2005.6.9 zjb
  ...
  public  static final String ONCHANGE_CORP = "9997";//选择内部往来单位后的触发事件



  private static final String MASTER_STRUT_SQL = "SELECT * FROM sc_drawmaterail WHERE 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM sc_drawmaterial WHERE ? AND ? AND filialeID='?' AND isout='?' ? ORDER BY  drawcode DESC";
  ...
  private static final String SINGLE_PROCESS_SQL = "SELECT * FROM VW_SELECT_PROCESSMATERAIL WHERE jgdid='?' "
      + " and  fgsid='?' and  (storeid IS NULL OR storeid='?') "
      + " and drawType = '?' ";//生产领料单引入加工单明细并根据BOM表从表增加
  //查询过后总数量
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(totalnum,0)) zsl,SUM(nvl(totalsum,0)) zje  FROM sc_drawmaterial WHERE ? AND ? AND filialeid='?' AND isout='?' ?  ";
  //用于审批时候提取一条记录
  ...
  private static final String KHCPZK_SQL           = "SELECT * FROM xs_khcpzk where sflsbj=0 and cpid='?' and  dwtxid='?'";
  ...
  public  HtmlTableProducer newmasterProducer = new HtmlTableProducer(dsMasterTable, "sc_newdrawmaterial.2");
  ...
  private LookUp corpBean = null;//内部往来单位Bean
  /**
   * 入库单列表的实例
   * @param request jsp请求
   ...
   * @return 返回入库单列表的实例
   */
  public static B_SendMaterial getInstance(HttpServletRequest request)
  {
    B_SendMaterial sendMaterialBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "sendMaterialBean";
      sendMaterialBean = (B_SendMaterial)session.getAttribute(beanName);
      if(sendMaterialBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        sendMaterialBean = new B_SendMaterial();
        sendMaterialBean.qtyFormat = loginBean.getQtyFormat();

        sendMaterialBean.fgsid = loginBean.getFirstDeptID();
        sendMaterialBean.loginId = loginBean.getUserID();
        sendMaterialBean.loginName = loginBean.getUserName();
        sendMaterialBean.loginDept = loginBean.getDeptID();
        sendMaterialBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");
        sendMaterialBean.isHandwork = loginBean.getSystemParam("KC_HANDIN_STOCK_BILL");//是否可以手工添加系统参数1=允许手工输入,0=不允许手工输入
        sendMaterialBean.SYS_APPROVE_ONLY_SELF = loginBean.getSystemParam("SYS_APPROVE_ONLY_SELF");//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
        sendMaterialBean.SC_DRAW_MATERIAL = loginBean.getSystemParam("SC_DRAW_MATERIAL");//生产领料配批是否以库存数量为准,1=以库存数量为准,0=没有变化
        sendMaterialBean.SC_STORE_UNIT_STYLE = loginBean.getSystemParam("SC_STORE_UNIT_STYLE");//计量单位和辅单位换算方式1=强制换算,0=仅空值时换算
        sendMaterialBean.KC_PRODUCE_UNIT_STYLE = loginBean.getSystemParam("KC_PRODUCE_UNIT_STYLE");//计量单位和生产单位换算方式1=强制换算,0=仅空值时换算
        sendMaterialBean.SYS_PRODUCT_SPEC_PROP = loginBean.getSystemParam("SYS_PRODUCT_SPEC_PROP");//生产用位换算的相关规格属性名称 得到的值为“宽度”……
        sendMaterialBean.SC_OUTSTORE_SHOW_ADD_FIELD = loginBean.getSystemParam("SC_OUTSTORE_SHOW_ADD_FIELD");//是否显示生产领料单的附加字段
        sendMaterialBean.user = loginBean.getUser();
        //设置格式化的字段
        sendMaterialBean.dsMasterTable.setColumnFormat("totalnum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsMasterTable.setColumnFormat("totalsum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsDetailTable.setColumnFormat("drawnum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsDetailTable.setColumnFormat("producenum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsDetailTable.setColumnFormat("drawprice", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsDetailTable.setColumnFormat("drawsum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsDetailTable.setColumnFormat("drawBigNum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsMasterTable.setColumnFormat("netNum", sendMaterialBean.qtyFormat);
        sendMaterialBean.dsMasterTable.setColumnFormat("zgf", sendMaterialBean.priceFormat);
        sendMaterialBean.dsMasterWage.setColumnFormat("zdf2", sendMaterialBean.priceFormat);
        sendMaterialBean.dsDetailWage.setColumnFormat("jjgz", sendMaterialBean.priceFormat);
        sendMaterialBean.dsDetailWage.setColumnFormat("jjgz2", sendMaterialBean.priceFormat);
        session.setAttribute(beanName, sendMaterialBean);
      }
    }
    return sendMaterialBean;
  }

  /**
   * 构造函数
   */
  private B_SendMaterial()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    //dsMasterTable.setSort(new SortDescriptor("", new String[]{"drawcode"}, new boolean[]{true,true,true}, null, 0));
    ...
    addObactioner(String.valueOf(MATERIAL_ON_CHANGE), new MaterialOnChange()); //改变原料的到库存量2004.11.12 wlc
    addObactioner(String.valueOf(MATERIAL_ON_DELETE), new MaterialDelete()); //从表删除一行2005.6.9 zjb
    ...
    addObactioner(ONCHANGE_CORP, new Onchange_Corp());//选择内部往来单位后的触发事件
  }
 ...
 //       p_RowInfos = new RowMap[dsDetail.getRowCount()];
      }
      else if(isInit){
        d_RowInfos.clear();
//        productStockData.deleteAllRows(); //不delete的话，从表库存就不会改变
      }
...
//        productStockData.setValue("cpid",dsDetailTable.getValue("cpid")==null?"-1":dsDetailTable.getValue("cpid"));
        dsDetail.next();
//        productStockData.next();
      }
...
//      productStockData.setValue("fgsid",fgsid);
//      productStockData.setValue("storeid",rowInfo.get("storeid")==null?"-1":rowInfo.get("storeid"));
//      productStockData.setValue("cpid",detailRow.get("cpid")==null?"-1":detailRow.get("cpid"));
//      productStockData.next();
    }
  }
  /**
 ...
 /*打开从表*/
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String SQL = combineSQL(MASTER_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid","creatorID"),user.getHandleStoreValue(),fgsid,isout,temp});
      ...
      String SUM_SLSQL =  combineSQL(MASTER_SUM_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "creatorID"),user.getHandleStoreValue(), fgsid, isout, temp});
...
//      productStockData.setValue("fgsid",dsMasterTable.getValue("fgsid"));
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < jgdwlID.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          String ylsl = importMaterailRow.get("ylsl");
          String ylscsl = importMaterailRow.get("ylscsl");
          String wlsl = importMaterailRow.get("wlsl");
          String wlscsl = importMaterailRow.get("wlscsl");
          String drawNum = srlx.equals("1")?wlsl:Double.toString(-1*Double.parseDouble(ylsl));
          String produceNum = srlx.equals("1")?wlscsl:Double.toString(-1*Double.parseDouble(ylscsl));
          ...
          dsDetailTable.setValue("drawNum", drawNum);
          dsDetailTable.setValue("produceNum", produceNum);
 ...
 *改变内部往来单位触发的事件
 */
class Onchange_Corp implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    HttpServletRequest req = data.getRequest();
    ...
    if(!oldDwtxid.equals(dwtxid)){
      ...
      {
        String hthwid = detail.getValue("jgdmxID");
        if(!hthwid.equals(""))
        {
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        detail.setValue("produceNum", detailrow.get("produceNum"));//保存换算数量
        //detail.setValue("produceNum", detailrow.get("produceNum"));//保存生产数量
        detail.setValue("drawprice", detailrow.get("drawprice"));//保存单价
        detail.setValue("drawSum", detailrow.get("drawSum"));//保存单价
        ...
        */
        ...
        totalSum = totalSum.add(detail.getBigDecimal("drawsum"));
      ...
      ds.setValue("totalSum", totalSum.toString());//总金额
      ds.setValue("memo", rowInfo.get("memo"));//备注
      ds.setValue("isout", "1");//1为外加工，0为自加工
...
/*        if(!isout.equals("1")){
          ...
          BigDecimal tmpscsl = new BigDecimal(scsl);
          sm = new BigDecimal("0");
          isPlus = tmpscsl.compareTo(sm)>=0;
          //根据是否srlx允许为负数.如不允许则弹出警示
          isAlert = isAllowPlus?!isAllowPlus:tmpscsl.compareTo(sm)>=0;
          if (isAlert)
            return showJavaScript("alert('第"+row+"行生产数量不能为正数！');");
        }
        else{
          ...
          String drawPrice = detailrow.get("drawPrice");
          if((temp = checkNumber(drawPrice, "第"+row+"行单价")) != null&&!drawPrice.equals(""))
            return temp;
          //if(drawPrice.length()>0 && drawPrice.equals("0"))
          //  return showJavaScript("alert('第"+row+"行单价不能为零！');");

          String drawSum = detailrow.get("drawSum");
          if((temp = checkNumber(drawSum, "第"+row+"行金额")) != null&&!drawSum.equals(""))
            return temp;
          //if(drawSum.length()>0 && drawSum.equals("0"))
          //   return showJavaScript("alert('第"+row+"行金额不能为零！');");
  ...
  class Master_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ds.deleteRow();
      ds.saveDataSets(new EngineDataSet[]{ds, dsDetailTable,dsMasterWage,dsDetailWage}, null);
      //LookupBeanFacade.refreshLookup(SysConstant.BEAN_STORE_PRODUCE_IN);
      LookupBeanFacade.refreshLookup(SysConstant.BEAN_STORE_PRODUCE_OUT);
      //
      d_RowInfos.clear();
  ...
  class Master_Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(SQL.length() > 0)
        SQL = " AND "+SQL;
      String NEWSQL = combineSQL(MASTER_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid","creatorID"),user.getHandleStoreValue(), fgsid, isout, SQL});
      dsMasterTable.setQueryString(NEWSQL);
      dsMasterTable.setRowMax(null);

      String SUM_SLSQL =  combineSQL(MASTER_SUM_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "creatorID"),user.getHandleStoreValue(), fgsid,isout, SQL});
...
//      productStockData.insertRow(false);
//      productStockData.setValue("fgsid",fgsid);
//      productStockData.setValue("storeid",detailrow.get("storeid")==null?"-1":detailrow.get("storeid"));
//      productStockData.setValue("cpid",detailrow.get("cpid")==null?"-1":detailrow.get("cpid"));
//      productStockData.post();
      d_RowInfos.add(detailrow);
    }
  ...
  */
  class Receive_Sel_Process implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String SQL = combineSQL(SINGLE_PROCESS_SQL,"?", new String[]{selectProcess,fgsid,storeid, srlx});
      ...
      for(int i=0; i < processBomData.getRowCount(); i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          String ylsl = processBomData.getValue("ylsl");
          String ylscsl = processBomData.getValue("ylscsl");
          String wlsl = processBomData.getValue("wlsl");
          String wlscsl = processBomData.getValue("wlscsl");
          String drawNum = srlx.equals("1")?wlsl:Double.toString(-1*Double.parseDouble(ylsl));
          String produceNum = srlx.equals("1")?wlscsl:Double.toString(-1*Double.parseDouble(ylscsl));
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < drawdetailID.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          //String ss = receiveProdRow.get("drawDetailID");
          dsDetailTable.insertRow(false);
          dsDetailTable.setValue("drawdetailid", "-1");
...
//      EngineDataSet p_ds = getProductStockData();
 ...
 //     p_ds.goToInternalRow(l_row);
//      p_ds.goToRow(rownum);
      ds.deleteRow();
//      p_ds.deleteRow();
      ds.post();
//      p_ds.post();
      data.setMessage(showJavaScript("sumitForm("+MATERIAL_ON_DELETE+",-1)"));
    }
  }
  /**
   *自制收货单复制一行操作
...
//      productStockData.goToInternalRow(Long.parseLong(temprow.get("InternalRow")));
...
//        productStockData.setValue("fgsid",fgsid);
//        productStockData.setValue("storeid",dsMasterTable.getValue("storeid")==null?"-1":dsMasterTable.getValue("storeid"));
//        productStockData.setValue("cpid",cpid);

//        productStockData.post();
        RowMap detailrow = new RowMap(detail);
        detailrow.put("InternalRow", String.valueOf(detail.getInternalRow()));
        d_RowInfos.add(detailrow);
        getPorductStockInfos();
      }
  ...
  class Onchange implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(!oldsfdjlbid.equals(sfdjlbid)){
        EngineDataSet detail = getDetailTable();
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if(rownum==i)
        {
          ...
          if(dj==null||dj.equals(""))
          {
            data.setMessage(showJavaScript("alert('该物质没有定价')"));
            return;
          }
          detailRow.put("drawprice", dj);
        }
      }
      data.setMessage(showJavaScript("document.form1.drawnum_"+rownum+".focus();"));
    }
  }
  /**
   *引入单位的客户管理信息
  ...
  public RowMap getkhcpzk(String dwtxid,String cpid)
  {
    ...
    try{
      ...
      setDataSetProperty(dskhcpzk,combineSQL(KHCPZK_SQL,"?",new String[]{cpid,dwtxid}));
      dskhcpzk.open();
      dskhcpzk.first();
...
* 从表删除一行的触发类  zjb
*/
  class MaterialDelete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      HttpServletRequest req = data.getRequest();
