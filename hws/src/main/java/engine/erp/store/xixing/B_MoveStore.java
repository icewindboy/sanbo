...
public final class B_MoveStore extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM kc_sfdj WHERE ? AND ? AND djxz=? and fgsid=? ? ORDER BY sfrq DESC";
  private static final String TOTALZSL_SQL  = "SELECT SUM(nvl(zsl,0)) tzsl FROM kc_sfdj a WHERE ? AND ? AND djxz=? AND fgsid=? ? ORDER BY sfdjdh DESC";
  ...
  private static final String DETAIL_SQL = "SELECT kc_sfdjmx.* FROM kc_sfdjmx kc_sfdjmx, kc_dm b WHERE kc_sfdjmx.cpid = b.cpid and kc_sfdjmx.sfdjid='?' order by b.cpbm ";//
  //用于审批时候提取一条记录
  ...
  public static B_MoveStore getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(moveStoreBean == null)
      {
        ...
        moveStoreBean.dsMasterTable.setColumnFormat("zje", moveStoreBean.qtyFormat);
//        moveStoreBean.dsMasterTable.setColumnFormat("drsl", moveStoreBean.qtyFormat);
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=min; i<=max; i++)
    {
      ...
      detailRow.put("dj", formatNumber(rowInfo.get("dj_"+i), qtyFormat));//单价
      detailRow.put("je", formatNumber(rowInfo.get("je_"+i), qtyFormat));//金额
 ...
 /*打开从表*/
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //
      String SQL = " AND zt<>2";
      SQL = combineSQL(MASTER_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "zdrid"), user.getHandleStoreValue(),String.valueOf(djxz),fgsid,SQL});
      ...
      tempTzslSql = combineSQL(TOTALZSL_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "zdrid"), user.getHandleStoreValue(), String.valueOf(djxz),fgsid,tempTzslSql});
...
//        double drsl = detailrow.get("drsl").length() > 0 ? Double.parseDouble(detailrow.get("drsl")) : 0;//换算数量
        detail.setValue("wjid", detailrow.get("wjid"));//保存单据引用外键
        detail.setValue("sl", detailrow.get("sl"));//保存数量
        detail.setValue("dj", detailrow.get("dj"));//保存单价
        detail.setValue("je", detailrow.get("je"));//保存金额
    ...
    private String checkMasterInfo()
    {
      ...
      if(storeid.equals(kc__storeid))
        return showJavaScript("alert('调入仓库不能与调出仓库相等！');");
      //03.16 16:03 加入部门不能为空的判断. yjg
      ...
      public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
      {
        ...
        for(int i=0; i < tddhwIDs.length; i++)
        {
          ...
          if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
          {
            ...
            m_RowInfo.put("storeid", saleLadingRow.get("storeid"));
            m_RowInfo.put("kc__storeid", saleLadingRow.get("kc__storeid"));
