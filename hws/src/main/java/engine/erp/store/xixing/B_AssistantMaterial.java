 ...
 * @version 1.0
 */
public final class B_AssistantMaterial extends BaseAction implements Operate
{
   ...
   */
  public static B_AssistantMaterial getInstance(HttpServletRequest request)
  {
    B_AssistantMaterial B_AssistantMaterialBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_AssistantMaterialBean";//名称-值对应
      B_AssistantMaterialBean = (B_AssistantMaterial)session.getAttribute(beanName);
      if(B_AssistantMaterialBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        String fgsID = loginBean.getFirstDeptID();
        B_AssistantMaterialBean = new B_AssistantMaterial(fgsID);//调用构造函数
        B_AssistantMaterialBean.loginName = loginBean.getUserName();
        B_AssistantMaterialBean.personid=loginBean.getUserID();
        session.setAttribute(beanName,B_AssistantMaterialBean);
      }
    }
    return B_AssistantMaterialBean;
  }
  /**
   * 构造函数(实例变量:分公司ID为初始化参数)
   */
  private B_AssistantMaterial(String fgsid)
  {
    this.fgsid = fgsid;
    try {
      jbInit();
  ...
  {
    addObactioner(String.valueOf(INIT), new B_AssistantMaterial_Init());//初始化
   ...
   * 初始化操作的触发类
   */
  class B_AssistantMaterial_Init implements Obactioner
  {
  ...
  class FIXED_SEARCH implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ////////*************计算期初*********************/////////
        ...
        String tcksql = combineSQL(KC_CQ_SQL, "?", new String[]{"  and cpid="+cpid+"  and djlb=1 and sfrq<to_date('"+jsrq+"','yyyy-mm-dd') group by cpid"}) ;
      ...
      String srcsql = combineSQL(KC_SRMX_SQL, "?", new String[]{" cpid='"+cpid+"' and  sfrq>=to_date('"+ksrq+"','yyyy-mm-dd') and  sfrq<=to_date('"+jsrq+"','yyyy-mm-dd') order by cpid"}) ;
      ...
      for(int i=0;i<dsczrc.getRowCount();i++)
      {
        ...
        String cpbm2 = dsczrc.getValue("cpbm");
        String product2 = dsczrc.getValue("product");
        String jldw2 = dsczrc.getValue("jldw");
