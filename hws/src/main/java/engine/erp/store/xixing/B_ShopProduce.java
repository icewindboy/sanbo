 ...
 */
/**
 * <p>Title:  自制收货单列表表结构改过，不在kc_sfdj,kc_sfdjmx中了，表为kc_cjcn,kc_cjcnmx</p>
 */
/**
 * //2004-06-14 09:21 修改原来的batchRow.get("wzmxid")为只赋值为1
 */

public final class B_ShopProduce extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM kc_cjcn WHERE 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM kc_cjcn WHERE ? AND ? AND filialeID='?' AND isout='?' ? ORDER BY receivedate DESC,receiveCode DESC";
  private static final String DETAIL_STRUT_SQL = "SELECT * FROM kc_cjcnmx WHERE 1<>1";
  private static final String DETAIL_SQL = "SELECT * FROM kc_cjcnmx WHERE cjcnid='?' ORDER BY jgdwlid,cpid";//

  //查询过后总数量
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(totalnum,0)) zsl FROM kc_cjcn WHERE ? AND ? AND filialeid='?' AND isout='?' ?  ";

  private static final String NEWDETAIL_SQL = "SELECT * FROM kc_cjcnmx WHERE cjcnid='?' ORDER BY cjcnmxid";//复制收货单用，不按产品排序
  ...
  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM kc_cjcn WHERE cjcnid='?'";

  //查询数据库是否有记账的单据
  private static final String RECODE_DATASQL = " SELECT COUNT(*) FROM kc_cjcn a WHERE a.creatorid='?' AND  a.isout ='?' AND a.state=1";
  //把符合记帐功能的数据全部记帐
  private static final String RECODE = "UPDATE kc_cjcn a SET a.state=2 WHERE a.creatorid='?' AND  a.isout ='?' AND a.state=1";
  //
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "kc_cjcn");
  public  HtmlTableProducer masterProducerProcess = new HtmlTableProducer(dsMasterTable, "sc_processinstore");
  public  HtmlTableProducer detailProducer = new HtmlTableProducer(dsDetailTable, "kc_cjcnmx");
  public  HtmlTableProducer table = new HtmlTableProducer(dsMasterTable, "kc_cjcn", "kc_cjcn");//查询得到数据库中配置的字段
  ...
  private String SUM_SLSQL="";//统计数量的SQL
   ...
   * @return 返回入库单列表的实例
   */
  public static B_ShopProduce getInstance(HttpServletRequest request)
  {
    B_ShopProduce shopProduceBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "shopProduceBean";
      shopProduceBean = (B_ShopProduce)session.getAttribute(beanName);
      if(shopProduceBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        shopProduceBean = new B_ShopProduce();
        shopProduceBean.qtyFormat = loginBean.getQtyFormat();

        shopProduceBean.fgsid = loginBean.getFirstDeptID();
        shopProduceBean.loginId = loginBean.getUserID();
        shopProduceBean.loginName = loginBean.getUserName();
        shopProduceBean.loginDept = loginBean.getDeptID();
        shopProduceBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");
        shopProduceBean.isHandwork = loginBean.getSystemParam("KC_HANDIN_STOCK_BILL");//是否可以手工添加系统参数1=允许手工输入,0=不允许手工输入
        shopProduceBean.SYS_APPROVE_ONLY_SELF = loginBean.getSystemParam("SYS_APPROVE_ONLY_SELF");//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
        shopProduceBean.SC_DRAW_MATERIAL = loginBean.getSystemParam("SC_DRAW_MATERIAL");//生产领料配批是否以库存数量为准,1=以库存数量为准,0=没有变化
        shopProduceBean.SC_STORE_UNIT_STYLE = loginBean.getSystemParam("SC_STORE_UNIT_STYLE");//计量单位和辅单位换算方式1=强制换算,0=仅空值时换算
        shopProduceBean.KC_PRODUCE_UNIT_STYLE = loginBean.getSystemParam("KC_PRODUCE_UNIT_STYLE");//计量单位和生产单位换算方式1=强制换算,0=仅空值时换算
        shopProduceBean.SYS_PRODUCT_SPEC_PROP = loginBean.getSystemParam("SYS_PRODUCT_SPEC_PROP");//生产用位换算的相关规格属性名称 得到的值为“宽度”……
        shopProduceBean.SC_INSTORE_SORT_FIELD = loginBean.getSystemParam("SC_INSTORE_SORT_FIELD");//自制收货单明细的排序字段
        shopProduceBean.user = loginBean.getUser();
        //设置格式化的字段
        shopProduceBean.dsMasterTable.setColumnFormat("totalnum", shopProduceBean.qtyFormat);
        shopProduceBean.dsMasterTable.setColumnFormat("totalsum", shopProduceBean.qtyFormat);
        shopProduceBean.dsDetailTable.setColumnFormat("drawnum", shopProduceBean.qtyFormat);
        shopProduceBean.dsDetailTable.setColumnFormat("producenum", shopProduceBean.qtyFormat);
        shopProduceBean.dsDetailTable.setColumnFormat("drawprice", shopProduceBean.qtyFormat);
        shopProduceBean.dsDetailTable.setColumnFormat("drawsum", shopProduceBean.qtyFormat);
        shopProduceBean.dsDetailTable.setColumnFormat("drawBigNum", shopProduceBean.qtyFormat);
        session.setAttribute(beanName, shopProduceBean);
      }
    }
    return shopProduceBean;
  }

  /**
   * 构造函数
   */
  private B_ShopProduce()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    //dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"receiveCode"}, new String[]{"SELECT pck_base.billNextCode('kc_cjcn','receiveCode') from dual"}));
    dsMasterTable.setSort(new SortDescriptor("", new String[]{"receiveCode"}, new boolean[]{true}, null, 0));

    dsDetailTable.setSequence(new SequenceDescriptor(new String[]{"cjcnmxid"}, new String[]{"s_kc_cjcnmx"}));
    if(SC_INSTORE_SORT_FIELD==null)
      ...
      dsDetailTable.setSort(new SortDescriptor("", new String[]{"cjcnmxid"}, new boolean[]{false}, null, 0));
  ...
  public final void valueUnbound(HttpSessionBindingEvent event)
  {
    ...
    */
    ...
    if(masterProducerProcess != null)
    {
      masterProducerProcess.release();
      masterProducerProcess = null;
    }
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=min; i<=max; i++)
    {
      ...
      String ccc=rowInfo.get("dj_"+i);
      detailRow.put("kwid", rowInfo.get("kwid_"+i));//库位
      //detailRow.put("dj", rowInfo.get("dj_"+i));//等级
      ...
      detailRow.put("zc", formatNumber(rowInfo.get("zc_"+i), qtyFormat));//正长
      detailRow.put("ep", formatNumber(rowInfo.get("ep_"+i), qtyFormat));//二品
      detailRow.put("sp", formatNumber(rowInfo.get("sp_"+i), qtyFormat));//三品
      detailRow.put("fg", formatNumber(rowInfo.get("fg_"+i), qtyFormat));//废管
      detailRow.put("fl", formatNumber(rowInfo.get("fl_"+i), qtyFormat));//废料
      detailRow.put("qt", formatNumber(rowInfo.get("qt_"+i), qtyFormat));//其他
  ...
  class Matching_Batch implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String cjcnid = dsDetailTable.getValue("cjcnid");
      ...
      for(int i=0; i < wzmxIDs.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          if(i==0){
            ...
            dsDetailTable.setValue("cjcnid", cjcnid);
 ...
 /*打开从表*/
  ...
  {
    String cjcnid = dsMasterTable.getValue("cjcnid");
    ...
    dsDetailTable.setQueryString(combineSQL(SQL, "?", new String[]{isMasterAdd ? "-1" : cjcnid}));
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      masterProducerProcess.init(request, loginId);
      //初始化查询项目和内容
      ...
      SUM_SLSQL =  combineSQL(MASTER_SUM_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "creatorID"),user.getHandleStoreValue(), fgsid,isout, temp});
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //以实现:mantis上库存管理中0000158bug描述的:根据下达的部门，进行提交审批；
        approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "shop_produce_list", content, dsMasterTable.getValue("deptid"));
    }
  }
  /**
   * 取消审批触发操作
  ...
  class Cancel_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(), "shop_produce_list");
    }
  }
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ds.insertRow(false);
        cjcnid = dataSetProvider.getSequence("s_kc_cjcnmx");
        ds.setValue("cjcnid", cjcnid);
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        //新添的记录
        if(isMasterAdd)
          detail.setValue("cjcnid", cjcnid);

        String cjcnmxid = detail.getValue("cjcnmxid");
        String detailid = null;
        if(cjcnmxid.equals("-1") || cjcnmxid==null)
            detailid = dataSetProvider.getSequence("s_kc_cjcnmx");
        detail.setValue("cjcnmxid", detailid);
        detail.setValue("drawNum", detailrow.get("drawNum"));//保存数量
        detail.setValue("zc", detailrow.get("zc"));//正长
        detail.setValue("ep", detailrow.get("ep"));//二品
        detail.setValue("sp", detailrow.get("sp"));//三品
        detail.setValue("fg", detailrow.get("fg"));//废管
        detail.setValue("fl", detailrow.get("fl"));//废料
        detail.setValue("qt", detailrow.get("qt"));//其他
        ...
        //detail.setValue("dj", detailrow.get("dj"));//等级
      ...
      ds.post();
      if(isout=="1")
        dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"receiveCode"}, new String[]{"SELECT pck_base.billNextCode('kc_cjcn','receiveCode','a') from dual"}));
      ...
      setDataSetProperty(dssl,SUM_SLSQL);
      dssl.open();
      dssl.first();
      if(dssl.getRowCount()<1)
        zzsl="0";
    ...
    private String checkDetailInfo() throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if((temp = checkNumber(sl, "第"+row+"行数量")) != null&&!sl.equals(""))
          return temp;
      ...
      for(int t=0; t<d_RowInfos.size(); t++)
      {
        ...
        if(!batchNo.equals("") && isbatchno.equals("1")){
          ...
          if(isMasterAdd)
            ...
            String cjcnid = dsMasterTable.getValue("cjcnid");
            count  = dataSetProvider.getSequence("SELECT COUNT(*) FROM vw_kc_storebill t WHERE t.djxz IN(1,3,5,7,9) AND t.ph='"+batchNo+"' AND t.cpid='"+cpid+"' AND t.sfdjid<>'"+cjcnid+"'");
          }
          if(!count.equals("0"))
  ...
  class Detail_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      detail.setValue("cjcnid", isMasterAdd ? "-1" : cjcnid);
      detail.setValue("cjcnmxid", "-1");
      detail.post();
  ...
  class Self_Sel_Process implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < processData.getRowCount(); i++)
      {
        ...
        locateGoodsRow.setValue(0, processData.getValue("jgdwlid"));
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          dsDetailTable.insertRow(false);
          dsDetailTable.setValue("cjcnmxid","-1");
          dsDetailTable.setValue("jgdwlid",processData.getValue("wjid"));
          String cpid = processData.getValue("cpid");
          String wrksl = processData.getValue("wrksl");
          String wrkhssl = processData.getValue("wrkhssl");
          String jgdj = processData.getValue("jgdj");
          String jgje = processData.getValue("jgje");
          dsDetailTable.setValue("cpid",processData.getValue("cpid"));
          dsDetailTable.setValue("drawNum",wrksl);
          dsDetailTable.setValue("drawbigNum",wrkhssl);
          dsDetailTable.setValue("produceNum",processData.getValue("wrkscsl"));
          dsDetailTable.setValue("drawPrice",jgdj);
          dsDetailTable.setValue("drawSum",jgje);
          //03.10 09:53 新增 新增当 引入加工单 事件在这里做处理是也要把dmsxid(规格属性取得) yjg
          dsDetailTable.setValue("dmsxid",processData.getValue("dmsxid"));
          dsDetailTable.setValue("cjcnid", isMasterAdd ? "" : cjcnid);
          dsDetailTable.post();
          //创建一个与用户相对应的行
       ...
       for(int m=0;m<tmpcpids.size();m++)
       {
         ...
         if(dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
         {
           ...
           else if(!dsDetailTable.getValue("batchNo").equals(ph) && !dsDetailTable.getValue("batchNo").equals(""))
           {
             ...
             dsDetailTable.insertRow(false);
             dsDetailTable.setValue("cjcnmxid","-1");
             dsDetailTable.setValue("cjcnid", isMasterAdd ? "-1" : cjcnid);
             dsDetailTable.setValue("cpid", cpid);
  ...
  class Copy_Self implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<tempDetailData.getRowCount();i++){
        dsDetailTable.insertRow(false);
        dsDetailTable.setValue("cjcnmxid","-1");
        dsDetailTable.setValue("cjcnid",  "-1" );
