...
public final class B_ReceiveMaterial extends BaseAction implements Operate
{
  ...
  //查询过后总数量
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(totalnum,0)) zsl FROM sc_drawmaterial WHERE  ? AND filialeid='?' AND isout='?' ?  ";
  //用于审批时候提取一条记录
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < processBomData.getRowCount(); i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          dsDetailTable.setValue("drawNum", processBomData.getValue("wlsl"));
