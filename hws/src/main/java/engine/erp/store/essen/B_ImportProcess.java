...
{
  private static final String IMPORT_PROCESS_SQL = "SELECT DISTINCT d.jgdid,d.deptid, "
                                                 + " d.jgdh,d.describe,d.jglx,d.sfwjg,"
                                                 + " d.dwtxid,d.rq,d.zdr,d.zdrq,d.zt,d.zsl,d.fgsid,d.scjhid, c.scph "
                                                 + " FROM ( "
                                                 + "        SELECT a.jgdid, a.cpid, a.sl,a.yrksl, "
                                                 + "               a.scsl,a.scph, b.storeid "
                                                 + "        FROM sc_jgdmx a, kc_dm b "
                                                 + "        WHERE  a.cpid=b.cpid "
                                                 + "               AND (b.storeid IS NULL OR b.storeid='?') "
                                                 + "      ) c, sc_jgd d "
                                                 + " WHERE nvl(c.yrksl,0)<nvl(c.sl,0) AND c.jgdid=d.jgdid "
                                                 + "       AND d.zt<>8 AND d.fgsid='?' AND (d.sfwjg IS NULL OR d.sfwjg='?') ";//自制收货单引用
