...
/**
 * <p>Title: 库存--库存盘点单列表</p>
 * <p>Description: 库存--库存盘点单列表<br>
 ...
 * size<2默认为编码不合格.同时cpids也要删除刚刚做此操作cpids.add(cpid);add进来的这笔cpid. 2004-4-26 10:18
 * 新增 where cs<=1 ORDER BY bm 对应解决mantis上0000421 bug:.不取物资类别明细,并且要对结果排序. yjg
 * 2004-4-23 11:06 新增 为新编码的盘点机而加 yjg 2004-4-14 18:45 为按sxz排序而加的刷新语句 2004-4-14
 * 18:21 为了按规格属性排序而新加入的kc_dmsx c 表及条件. yjg 2004-4-9 11:06
 * 新加入了判断库位如果有设的话那么就必须指定库位. 其程序中其它地方的相关的新增 yjg 2004-4-2 23:12 修改 暂时修改取产品编码前七位
 * yjg 2004-3-30 14:53 新增 如果是保存按钮行为完成后定位数据集.目地是:实现保存不返回 yjg 2004-3-30 10:05 修改
 * Single_Product_Add操作中修改zcsl为空 yjg 2004-3-29 23:43 新增 新增空白行dmsxid yjg
 * 2004-3-29 23:43 新增 从表新增库存里有的物资的时候新增dmsxid.同时view中做了相应修改 yjg 03.23 11:36 修改
 * 将上方的02.25 11:14 bug中所做的修改的基础上再修改:现在又改回了: 删除一行库存量为空的一笔记录的条件改为:实际库存数量是空的. yjg
 * 03.16 15:04 修改
 * 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
 * 以实现:根据下达的部门，进行提交审批；
 */

public final class OtherStoreCheck extends BaseAction implements Operate {
	...
	public static final String TRANSFERSCAN = "10061";// 调用盘点单 02.18 11:39
														// 新增因为此盘点功能要新增一个盘点机功能
														// yjg
	...
	private static final String MASTER_SQL = "SELECT a.*, b.sfdjid FROM kc_pd a, kc_sfdj b WHERE a.pdid=b.pdid(+) AND a.fgsid=? ? ORDER BY a.rq DESC";
	...
	// 打开物资类别的SQL
	// 2004-4-26 10:18 新增 where a.cs=1 ORDER BY bm 对应解决mantis上0000421
	// bug:.不取物资类别明细,并且要对结果排序. yjg
	private static final String PRODUCT_SORT_SQL = "SELECT a.wzlbid, a.bm, a.mc FROM kc_dmlb a where a.cs=1 ORDER BY a.bm ";
	// 用于添加从表信息的SQL，根据条件和汇总表得到信息
	private static final String STOCK_COLLECT_ADD = "SELECT * FROM VW_STORE_COLLECT ";
	// 批量增加视图
	private static final String STOCK_BIG_ADD = "SELECT cpbm, cpid, wzlbid, chlbid, storeid FROM kc_dm a "
			+ " WHERE cpid NOT IN(SELECT cpid FROM vw_store_collect WHERE ? ) AND a.isdelete = 0 AND ?  order by cpbm";
	// 只取库存物资明细表中的数据
	private static final String STOCK_MATERIAL_LIST = " SELECT * FROM " + " ( " + "   SELECT b.storeid, b.cpid, b.dmsxid, b.ph, SUM(nvl(b.zl,0)) kcsl "
			+ "   FROM kc_wzmx b, vw_kc_dm_exist a " + "    WHERE  b.cpid = a.cpid " + "    GROUP BY b.storeid, b.cpid, b.dmsxid, b.ph " + "  ) b "
	 ...
	 * @return 返回库存盘点单列表的实例
	 */
	public static OtherStoreCheck getInstance(HttpServletRequest request) {
		OtherStoreCheck storeCheckBean = null;
		HttpSession session = request.getSession(true);
		synchronized (session) {
			String beanName = "otherStoreCheck";
			storeCheckBean = (OtherStoreCheck) session.getAttribute(beanName);
			if (storeCheckBean == null) {
				// 引用LoginBean
				OtherLoginBean loginBean = OtherLoginBean.getInstance(request);

				storeCheckBean = new OtherStoreCheck();
				...
				storeCheckBean.loginDept = loginBean.getUser().getDeptId();
				// 设置格式化的字段
	...
	private final void jbInit() throws java.lang.Exception {
		...
		// dsMasterTable.setSort(new SortDescriptor("", new String[]{"pdhm"},
		...
		addObactioner(SHOW_DETAIL, new Show_Detail());// 02.17 21:54 新增
														...
														// yjg
		// 02.18 11:41 新增 注册一个处理盘点机按钮事件的操作.此操作通过 transferScan() 类来完成.这个类也是要新增的.
		// yjg
	...
	private final void putDetailInfo(HttpServletRequest request) {
		...
		for (int i = min; i <= max; i++) {
			...
			// 保存用户自定义的字段
			/*
			 * FieldInfo[] fields = detailProducer.getBakFieldCodes(); for(int
			 * j=0; j<fields.length; j++) { String fieldCode =
			 * fields[j].getFieldcode(); detailRow.put(fieldCode,
			 * rowInfo.get(fieldCode + "_" + i)); }
			 */
		}
	}
	...
	class Single_Product_Add implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			String SQL = STOCK_COLLECT_ADD + " WHERE cpid='" + cpid + "' AND storeid='" + storeid + "'";
			...
			detailrow.put("cpid", cpid);
			detailrow.put("zcsl", "0");// 2004-3-30 10:05 修改
										// Single_Product_Add操作中修改zcsl为空 yjg
			detailrow.put("bz", "0");
	...
	public final void openDetailTable(boolean isMasterAdd) {
		// 02.17 10:58 修改 修改原来的三元运算符最后的pdid值改为现在的从
		// dsMasterTable.getValue("pdid")
		// 中取出来.即原来是这样的. ( isMasterAdd ? "-1" : pdid ),而现在改成下面这样. yjg
		// String SQL = DETAIL_SQL + (isMasterAdd ? "-1" :
		// dsMasterTable.getValue("pdid"));
	...
	class Init implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			/**
			 * RowMap row = fixedQuery.getSearchRow(); row.clear(); String today
			 * = new SimpleDateFormat("yyyy-MM-dd").format(new Date()); String
			 * startDay = new SimpleDateFormat("yyyy-MM-01").format(new Date());
			 * row.put("zt", "0"); row.put("rq$a", startDay); row.put("rq$b",
			 * today);
			 */
			isMasterAdd = true;
			isDetailAdd = false;
	...
	class Master_Post implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int i = 0; i < d_RowInfos.size(); i++) {
				...
				detail.setValue("zcsl", detailrow.get("zcsl"));
				detail.setValue("scsl", detailrow.get("scsl"));
				detail.setValue("zchssl", detailrow.get("zchssl"));
				 ...
				 * for(int j=0; j<fields.length; j++) { String fieldCode =
				 * fields[j].getFieldcode(); detail.setValue(fieldCode,
				 * detailrow.get(fieldCode)); }
				 */
				detail.post();
				// detail.next();
			}
			...
			// 保存用户自定义的字段
			/*
			 * FieldInfo[] fields = masterProducer.getBakFieldCodes(); for(int
			 * j=0; j<fields.length; j++) { String fieldCode =
			 * fields[j].getFieldcode(); detail.setValue(fieldCode,
			 * rowInfo.get(fieldCode)); }
			 */
			ds.post();
		...
		private String checkDetailInfo() {
			...
			for (int i = 0; i < d_RowInfos.size(); i++) {
				...
				// if((temp = checkNumber(zchssl, "第"+row+"行实存数量")) != null)
				// return temp;
				...
				// 新增 :如果cpid, ph, dmsxid的组合有相同的则是不允许的.盘点点中不允许有这样的情况. 02.14
				// 21:19 yjg
	...
	class Detail_Add implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			if (scope.equals("1"))// 选择查询条件为所有存货
				SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" + storeid + "')";
			// 查询条件为按存货类别筛选
			if (scope.equals("2")) {
				if (chlbid.equals("")) {
				...
				SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" + storeid + "') AND chlbid='" + chlbid + "'";
			}
			// 按产品编码范围
			if (scope.equals("3")) {
				 ...
				 */
				if (!cpbm_a.equals("") && !cpbm_b.equals(""))
					SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" + storeid + "') AND cpbm>='" + cpbm_a + "' AND cpbm<='" + cpbm_b + "'";
				if (cpbm_a.equals("") && !cpbm_b.equals(""))
					SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" + storeid + "') AND cpbm<='" + cpbm_b + "'";
				if (!cpbm_a.equals("") && cpbm_a.equals(""))
					SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" + storeid + "') AND cpbm>='" + cpbm_a + "'";
			}
			// 选择物资大类
			if (scope.equals("4")) {
				if (sel.length < 1) {
				...
				SQL = STOCK_COLLECT_ADD + " WHERE (storeid='" + storeid + "') AND wzlbid IN(" + sql + ")";
			}
			if (stockCollectData == null) {
				stockCollectData = new EngineDataSet();
				setDataSetProperty(stockCollectData, null);
			...
			for (int i = 0; i < stockCollectData.getRowCount(); i++) {
				...
				// 如果宽度上下限都不为空的时候才去与查出来的资料进行比较
				if (!(sxz_a.equals("") && sxz_b.equals(""))) {
					if (tmpSQLSxz.equals(""))
						SQLSxzCondition = false;
					else
						SQLSxzCondition = checkNumber(tmpSQLSxz, "sql查询结果中的属性值") == null;
					// 如果sql查询出来的和输入的属性值宽度都是数值型的那么就比较宽度了,否则就忽略不管它.
					if (SQLSxzCondition) {
						if (sxz_a.equals(""))
							lower_limit = true;
						else {
							lower_limit = checkNumber(sxz_a, "宽度下限") == null;
							if (lower_limit)
								lower_limit = Float.parseFloat(sxz_a) <= Float.parseFloat(tmpSQLSxz);
						}
						if (sxz_b.equals(""))
							upper_limit = true;
						else {
							upper_limit = checkNumber(sxz_b, "宽度上限") == null;
							if (upper_limit)
								upper_limit = Float.parseFloat(sxz_b) >= Float.parseFloat(tmpSQLSxz);
						}
						if (!(lower_limit && upper_limit))
							continue;
					} else// 如果查出来的数据中的属性值不是数值型的那么就不插入这条记录,而直接取下一笔
					{
						continue;
					}
				}
	...
	/**
	 * 调用盘点单触发的事件 盘点机做的操作为:
	 * 1.盘点机操作首先把符合指定条件的库存物资明细表中的数据全部取过来作为盘点数据记录放到dsDetailTable中用为盘点数据 2.接着比较:
	 * 2.1
	 * 如果盘点机机器中读入的一条数据能在1.操作中取到的库存物资明细表中的数据定位到.那么,就把这笔设置为盘点数据的scsl设为物资明细的kcsl
	 * 2.2 如果盘点机机器中读入的一条数据在库存物资明细表中的数据中定位不到那么还是把这笔盘点机器中的数据插入dsDetailTable中.
	 * 但scsl设置为0 3.保存数据.
	 */
