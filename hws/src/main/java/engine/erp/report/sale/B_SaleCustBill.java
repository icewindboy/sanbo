     ...
     * @version 1.0
     */
    public class B_SaleCustBill implements ReportDataLoadedListener
    {

      public B_SaleCustBill()
      {
      }
      public void dataLoaded(ServletRequest parm1, TempletData parm2, ContextData parm3, EngineDataSet parm4)
      {
        ...
        String ksrq = where.getWhereValue("a$tdrq$a");//查询条件开始日期
        String jsrq = where.getWhereValue("a$tdrq$b");//查询条件结束日期
        ...
        for(int i=0; i<objects.length; i++)
        {
          if(objects[i] instanceof String)
          {
            ...
            {
              s = "<SCRIPT LANGUAGE='javascript' id='where'>var ksrq='"+ksrq+"'; var jsrq='"+jsrq+"'; var djlx='"+djlx+"'; </SCRIPT>";
              parm2.setOther(i, s);
            }
          }
        }
