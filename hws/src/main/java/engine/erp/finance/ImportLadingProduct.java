...
public final class ImportLadingProduct extends BaseAction implements Operate
 {
   private static final String IMPORT_SALE_SQL = "SELECT * FROM VW_SALE_BALANCE_IMPORT_TDDETAL WHERE fgsid=? ? order by  tdrq,je ";//分公司; 往来单位
   private static final String MASTER_SQL    = "SELECT * FROM VW_SALE_BALANCE_IMPORT_TDDETAL WHERE  fgsid=? ?  order by   tdrq ,je";

   private static final String zsje_SQL = "SELECT SUM(zsje)zsje FROM VW_SALE_BALANCE_IMPORT_TDDETAL WHERE fgsid=? ? order by je, tdrq ";//分公司; 往来单位
   private static final String zje_SQL = "SELECT SUM(je)zje FROM VW_SALE_BALANCE_IMPORT_TDDETAL WHERE fgsid=? ? order by je, tdrq ";//分公司; 往来单位
...
* */
   ...
   class Init implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       {
         dwtxid = dwxid;
         SQL=" AND dwtxid='"+dwtxid+"' AND personid='"+personid+"' AND khlx='"+khlx+"' AND isnet='"+isnet+"'";
       }
       String sql= combineSQL(IMPORT_SALE_SQL, "?", new String[]{fgsid,SQL});
       dsSaleOrderProduct.setQueryString(sql);
       dsSaleOrderProduct.setRowMax(null);

       EngineDataSet dssl = new EngineDataSet();
       setDataSetProperty(dssl,combineSQL(zsje_SQL, "?", new String[]{fgsid,SQL}));
       dssl.open();
       dssl.first();
       if(dssl.getRowCount()<1)
         zsje="0";
       else
         zsje=dssl.getValue("zsje");

       EngineDataSet dsje = new EngineDataSet();
       setDataSetProperty(dsje,combineSQL(zje_SQL, "?", new String[]{fgsid,SQL}));
       dsje.open();
       dsje.first();
       if(dsje.getRowCount()<1)
         zje="0";
       else
        zje=dsje.getValue("zje");
