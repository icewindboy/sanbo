...
{
  private static final String IMPORT_SALE_SQL = "SELECT * FROM VW_BUY_INVOICE_SEL_JHD WHERE ? and fgsid=?  ";//分公司; 往来单位
  private static final String MASTER_SQL    = "SELECT * FROM VW_BUY_INVOICE_SEL_JHD WHERE ? and  fgsid=? ?  ";
  ...
  public static Select_Bill_Of_Stocking getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(Select_Bill_Of_StockingBean == null)
      {
        ...
        Select_Bill_Of_StockingBean.user = loginBean.getUser();
...
* */
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String SQL = combineSQL(IMPORT_SALE_SQL, "?", new String[]{user.getHandleDeptWhereValue("deptid", "czyid"),fgsid});
      if(!dwxid.equals(""))
      {
        dwtxid = dwxid;
