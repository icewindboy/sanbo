...
public final class B_SaleBalance extends BaseAction implements Operate
{
  ...
  private static final String JE_SQL    = "select SUM(nvl(je,0)*nvl(djxz,0))zje FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String HXJE_SQL    = "select SUM(nvl(hxje,0)*nvl(djxz,0))zhxje FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String WHXJE_SQL    = "select SUM(nvl(whxje,0)*nvl(djxz,0))zwhxje FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";

  private static final String COLLECT_SQL = "SELECT sum(nvl(a.je,0)*nvl(djxz,0)-nvl(a.hxje,0)*nvl(djxz,0))totalWhxje FROM cw_xsjs a WHERE   a.fgsid=? and a.dwtxid='?'   and a.khlx='?' GROUP BY a.dwtxid";//汇总

  private static final String DETAIL_STRUT_SQL = "SELECT * FROM cw_xsjshx WHERE 1<>1  order by rkdmxid ";
  private static final String DETAIL_SQL    = "SELECT * FROM cw_xsjshx WHERE xsjsID='?' order by rkdmxid ";//

  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM cw_xsjs WHERE xsjsID='?'";
  private static final String AUTO_CANCER_SQL = "SELECT * FROM vw_sale_autobalance_ckd where 1=1 and fgsid=? and dwtxid=? and  khlx='?' ORDER BY px, sfrq";
  ...
  private engine.erp.finance.xixing.ImportLadingProduct ImportLadingProductBean = null; //提单货物引用
  ...
  public static B_SaleBalance getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(b_SaleBalanceBean == null)
      {
        ...
        b_SaleBalanceBean.dsMasterTable.setColumnFormat("je", b_SaleBalanceBean.priceFormat);
        b_SaleBalanceBean.dsMasterTable.setColumnFormat("hxje", b_SaleBalanceBean.priceFormat);
        b_SaleBalanceBean.dsMasterTable.setColumnFormat("whxje", b_SaleBalanceBean.priceFormat);
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      //detailRow.put("rkdmxid", rowInfo.get("rkdmxid_"+i));//
      ...
      detailRow.put("rkdmxid", rowInfo.get("rkdmxid_"+i));//应回日期
      //保存用户自定义的字段
 ...
 public final String getZwhxje()
 {
  ...
  {
    String SQL =  combineSQL(DETAIL_SQL,"?",new String[]{(isMasterAdd ? "-1" : xsjsid)});
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String MSQL =  combineSQL(MASTER_SQL, "?", new String[]{" and cw_xsjs."+user.getHandleDeptValue("deptid"), fgsid, SQL," order by cw_xsjs.djh desc"});
 ...
 class Master_Add_Edit implements Obactioner
 {
  ...
  final class Orderby implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      SQL = combineSQL(MASTER_SQL, "?", new String[]{
                       ...
                       fgsid+" and ",
                       "cw_xsjs."+user.getHandleDeptValue("deptid"),
  ...
  class Detail_Auto_Cancer implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      setDataSetProperty(dscollect,combineSQL(COLLECT_SQL,"?",new String[]{fgsid,dwtxid,personid,khlx}));
      dscollect.open();
      dscollect.first();
      ...
      String SQL = combineSQL(AUTO_CANCER_SQL,"?",new String[]{fgsid,dwtxid,personid,khlx});
      ...
      for(int i=0;i<dstmp.getRowCount();i++)
      {
        ...
        dsDetailTable.setValue("rkdmxid", dstmp.getValue("rkdmxid"));
  ...
  */
  ...
  class Detail_Auto_Cancer implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(dwtxid.equals("")||khlx.equals(""))
        return;

      EngineDataSet dscollect = new EngineDataSet();
      setDataSetProperty(dscollect,combineSQL(COLLECT_SQL,"?",new String[]{fgsid,dwtxid,khlx}));
      dscollect.open();
      dscollect.first();
      if(dscollect.getRowCount()==1)
        totalWhxje = dscollect.getValue("totalWhxje");
      ...
      String SQL = combineSQL(AUTO_CANCER_SQL,"?",new String[]{fgsid,dwtxid,khlx});
      EngineDataSet dstmp = new EngineDataSet();
      setDataSetProperty(dstmp,SQL);
  ...
  class Detail_SALE_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      engine.erp.finance.xixing.ImportLadingProduct ImportLadingProductBean =getsaleOrderBean(req);
      for(int i=0; i < rkdmxid.length; i++)
      {
        ...
        {
          RowMap saleRow = ImportLadingProductBean.getLookupRow(rkdmxid[i]);

          dsDetailTable.insertRow(false);
          dsDetailTable.setValue("xsjshxID", "-1");
          dsDetailTable.setValue("xsjsid", xsjsid);
          dsDetailTable.setValue("rkdmxid", rkdmxid[i]);
   ...
   */
  public engine.erp.finance.xixing.ImportLadingProduct getsaleOrderBean(HttpServletRequest req)
  {
    if(ImportLadingProductBean == null)
      ImportLadingProductBean = engine.erp.finance.xixing.ImportLadingProduct.getInstance(req);
