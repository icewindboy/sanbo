...
public final class ImportLadingProduct extends BaseAction implements Operate
 {
   private static final String IMPORT_SALE_SQL = "SELECT * FROM vw_sale_autobalance_ckd WHERE fgsid=? ? order by sfrq ";//分公司; 往来单位
   private static final String MASTER_SQL    = "SELECT * FROM vw_sale_autobalance_ckd WHERE  fgsid=? ?  order by sfrq ";
...
* */
   ...
   //得到查询的行
   public final RowMap getLookupRow(String rkdmxid)
   {
     RowMap row = new RowMap();
     if(rkdmxid == null || rkdmxid.equals(""))
       return row;//返回
     EngineRow locateRow = new EngineRow(dsSaleOrderProduct, "rkdmxid");//构建指定DataSet组件的1列的EngineRow（但是没有数据）
     if(locateRow == null)
       locateRow = new EngineRow(getOneTable(), "rkdmxid");
     locateRow.setValue(0, rkdmxid);
   ...
   class Init implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       {
         dwtxid = dwxid;
         SQL=" AND dwtxid='"+dwtxid+"' AND khlx='"+khlx+"'";
       }
