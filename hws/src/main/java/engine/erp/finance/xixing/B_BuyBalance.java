...
import engine.erp.finance.SelectBuyGoods;
 ...
 * @version 1.0
 */
public final class B_BuyBalance extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM cw_cgjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  ...
  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM cw_cgjs WHERE cgjsid='?'";
  private static final String AUTO_CANCER_SQL = "SELECT * FROM vw_buy_balance where 1=1 and dwtxid='?' and  personid='?' and khlx='?'  ORDER BY jhrq";
  private static final String COLLECT_SQL = "SELECT sum(nvl(a.je,0))zje,sum(nvl(a.hxje,0))zhxje,sum(nvl(a.zfkje,0)-nvl(a.hxje,0))totalWhxje FROM cw_cgjs a WHERE a.dwtxid='?' and a.personid='?' and a.khlx='?' GROUP BY a.dwtxid";//汇总
  private EngineDataSet dsMasterTable  = new EngineDataSet();//主表
  private EngineDataSet dsDetailTable  = new EngineDataSet();//从表
  private EngineDataSet tdMxTable      = new EngineDataSet();//提单明细
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cw_cgjs");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_BuyBalance getInstance(HttpServletRequest request)
  {
    B_BuyBalance b_BuyBalanceBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_BuyBalanceBean";
      b_BuyBalanceBean = (B_BuyBalance)session.getAttribute(beanName);
      if(b_BuyBalanceBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_BuyBalanceBean = new B_BuyBalance();
        b_BuyBalanceBean.qtyFormat = loginBean.getQtyFormat();
        b_BuyBalanceBean.priceFormat = loginBean.getPriceFormat();
        b_BuyBalanceBean.sumFormat = loginBean.getSumFormat();
        b_BuyBalanceBean.fgsid = loginBean.getFirstDeptID();
        b_BuyBalanceBean.loginId = loginBean.getUserID();
        b_BuyBalanceBean.loginName = loginBean.getUserName();
        //设置格式化的字段
        b_BuyBalanceBean.user = loginBean.getUser();
        b_BuyBalanceBean.dsMasterTable.setColumnFormat("je", b_BuyBalanceBean.priceFormat);
        b_BuyBalanceBean.dsMasterTable.setColumnFormat("hxje", b_BuyBalanceBean.priceFormat);
        b_BuyBalanceBean.dsMasterTable.setColumnFormat("whxje", b_BuyBalanceBean.priceFormat);
        b_BuyBalanceBean.dsDetailTable.setColumnFormat("jsje", b_BuyBalanceBean.sumFormat);
        session.setAttribute(beanName, b_BuyBalanceBean);
      }
    }
    return b_BuyBalanceBean;
  }
  /**
   * 构造函数
   */
  private B_BuyBalance()
  {
    try {
      jbInit();
    }
  ...
  class Balance_Over implements Obactioner
  {
   ...
   class Cancer_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(),"buy_balance");
     }
  }
   //&#$
   /**
   ...
   class Add_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "buy_balance", content,deptid);
     }
  }
  /**
   * 初始化操作的触发类
 ...
 class Master_Add_Edit implements Obactioner
 {
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ds.setValue("dfdh", rowInfo.get("dfdh"));
      ds.setValue("dfcz", rowInfo.get("dfcz"));
  ...
  class Master_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      EngineDataSet ds = getMaterTable();
      ds.goToInternalRow(masterRow);
      cgjsid = ds.getValue("cgjsid");
      String zt = dataSetProvider.getSequence("SELECT a.zt FROM cw_cgjs a WHERE a.cgjsid='"+cgjsid+"'");
  ...
  class Detail_Auto_Cancer implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      dje = Double.parseDouble(je.equals("")?"0":je);//本单据实付金额
      if(dje<=sjje)
        sjje = dje;//最多只能核消本单据付款金额
      if(sjje<=zjsje)
        return;
  ...
  class Detail_SALE_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < jhdhwid.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          RowMap saleRow = ImportLadingProductBean.getLookupRow(jhdhwid[i]);
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String zh = corRow.get("zh");
        ...
        else
        {
          ...
          m_RowInfo.put("dfdh",corRow.get("tel"));
          m_RowInfo.put("dfcz",corRow.get("cz"));
