...
public final class InSelectBuyGoods extends BaseAction implements Operate
{
  private static final String IMPORT_BUY_JHD_SQL = "SELECT * FROM VW_BUY_INVOICE_SEL_STOCK WHERE djlx IN(2,-2) AND fgsid=? ? ";//分公司; 往来单位(根据购货单位提取数据)
   ...
   *
   * */
  public static InSelectBuyGoods getInstance(HttpServletRequest request)
  {
    InSelectBuyGoods InSelectBuyGoodsBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "InSelectBuyGoodsBean";
      InSelectBuyGoodsBean = (InSelectBuyGoods)session.getAttribute(beanName);
      if(InSelectBuyGoodsBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        InSelectBuyGoodsBean = new InSelectBuyGoods();
        InSelectBuyGoodsBean.fgsid = loginBean.getFirstDeptID();
        session.setAttribute(beanName, InSelectBuyGoodsBean);
      }
    }
