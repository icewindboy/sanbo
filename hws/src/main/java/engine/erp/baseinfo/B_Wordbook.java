...
public class B_Wordbook extends CommonClass
{
  //与地区操作相关变量
   ...
   * @return 角色列表的实例
   */
  public static B_Wordbook getInstance(HttpServletRequest request)
  {
    B_Wordbook wordbookBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      wordbookBean = (B_Wordbook)session.getAttribute("wordbookBean_aa");
      if(wordbookBean == null)
      {
        wordbookBean = new B_Wordbook();
        session.setAttribute("wordbookBean_aa", wordbookBean);
      }
    }
    return wordbookBean;
  }
  ...
  private String doService(HttpServletRequest request, HttpServletResponse response, int operate) throws Exception
  {
    ...
    switch(operate)
    {
      ...
      case TYPE_CHANGE:
        ...
        if(lx < PERSON_DUTY || lx > PERSON_POLITY)
          lx = PERSON_DUTY;
        break;
      //
      case OPERATE_ADD:
      ...
      case OPERATE_POST:
        ...
        if(mc.equals(""))
          return engine.action.BaseAction.showJavaScript("alert('名称不能为空！');");
