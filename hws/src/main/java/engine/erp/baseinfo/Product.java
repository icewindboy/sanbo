...
/**
 * <p>Title: 基础维护－物资代码维护</p>
 * <p>Description: 基础维护－物资代码维护<br>
...
public class Product extends BaseAction implements Operate
{
  ...
  private static final String PRODUCT_STURCT_SQL
      = "SELECT * FROM kc_dm a WHERE 1<>1";

  private static final String PRODUCT_NORMAL_SQL
      = "SELECT a.* FROM kc_dm a WHERE (a.storeid IS NULL OR a.storeid IN (SELECT c.storeid FROM jc_ckqx c WHERE c.personid='{personid}')) "
      + "AND a.isdelete=0 ? ORDER BY a.cpbm";

  private static final String PRODUCT_RUIJIAO_SQL
      = "SELECT a.* FROM kc_dm a, kc_dmlb b WHERE a.wzlbid=b.wzlbid AND a.isdelete=0 ? ORDER BY b.bm, a.pm, a.gg";

  private static final String PRODUCT_LEN_SQL
      = "SELECT autolen FROM jc_coderule_cont WHERE coderule='product'";
  private static final String UPDATE_SQL
      = "{CALL PCK_PRODUCE.updateBomProduceTime(@)}";
  private static final String PRODUCT_LIKE_1
      = "(a.pm LIKE '%{prod}%' AND a.gg LIKE'%{prod2}%' ) OR (a.pm LIKE '%{prod}%' AND a.gg is null)";
  private static final String PRODUCT_LIKE_2
      = "a.hh LIKE '%{prod}%' OR a.ks LIKE'%{prod}%'";
  ...
  class Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      {
        product = CUST_SHENGYU.equals(systemCustName) ?
                  PRODUCT_LIKE_1+" OR "+PRODUCT_LIKE_2 : PRODUCT_LIKE_1;
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("a", new QueryColumn[]{
        ...
        //new QueryColumn(master.getColumn("pm"), null, null, null,"b","="),//品名等于
  ...
  class Add_Edit implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isAdd && isWzAddProduct)
      {
        ...
        String code = dataSetProvider.getSequence("SELECT pck_base.fieldNextCode('kc_dm','cpbm','"+prefix+"','isdelete=0',"+product_len+",0) from dual");
  ...
  class Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(chlbid.equals(""))
     {
      ...
      if(hsblVal<=0)
      {
        data.setMessage(showJavaScript("alert('换算比例必须大于零！');"));
        return;
      }

      if(scdwgs.length()>0 && (temp = checkNumber(scdwgs, "生产公式"))!=null)
      {
        data.setMessage(temp);
        return;
      }
      */
      ...
      {
        String count = !isCheckCode ? "0" :
            dataSetProvider.getSequence("SELECT COUNT(*) FROM kc_dm WHERE cpbm='"+cpbm+"' AND isdelete=0");
        if(!count.equals("0"))
        {
        ...
        //
        count = !isCheckBarCode ? "0" :
                dataSetProvider.getSequence("SELECT COUNT(*) FROM kc_dm WHERE txm='"+txm+"' AND isdelete=0");
        if(!count.equals("0"))
        {
          data.setMessage(showJavaScript("alert('条形码("+ txm +")已经存在!');"));
          return;
        }
        count = dataSetProvider.getSequence("SELECT COUNT(*) FROM kc_dm where pm='"+pm+"' and gg='"+gg+"' and storeid='"+storeid+"' and isdelete=0");
        if (!count.equals("0"))
        {
      ...
      {
        String counter = dataSetProvider.getSequence("SELECT COUNT(*) FROM kc_dm where pm='"+pm+"' and gg='"+gg+"' and storeid='"+storeid+"' and cpid<>'"+cpids+"' and isdelete=0");
        if (!counter.equals("0"))
        {
          data.setMessage(showJavaScript("alert('已有该品名,规格的物资存放在该仓库')"));
          return;
      ...
      //ds.setValue("scydw", scydw.equals("") ? rowInfo.get("jldw") : scydw);//生产用单位
...
//      dsProduct.setValue("CZRQ", CZRQ);
  ...
  private void initRowInfo(boolean isAdd, boolean isInit) throws Exception
  {
    ...
    /*else
    {
      String code = dataSetProvider.getSequence("SELECT pck_base.fieldNextCode('kc_dm','cpbm','','isdelete=0',"+product_len+") from dual");
      rowInfo.put("cpbm", code);
    }*/
  }
