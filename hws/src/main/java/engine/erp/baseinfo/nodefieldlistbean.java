 ...
 * @version 1.0
 */
public final class nodefieldlistbean extends BaseAction implements Operate
{
   ...
   */
  public static nodefieldlistbean getInstance(HttpServletRequest request)
  {
    nodefieldlistbean nodelistBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "nodelist";
      nodelistBean = (nodefieldlistbean)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(nodelistBean == null)
      {
        nodelistBean = new nodefieldlistbean();
        session.setAttribute(beanName, nodelistBean);
      }
    }
    return nodelistBean;
  }
  ...
  protected void jbInit() throws Exception
  {
    ...
    //添加操作的触发对象
    nodefieldlistbean_Add_Edit add_edit = new nodefieldlistbean_Add_Edit();
    nodefieldlistbean_Post post = new nodefieldlistbean_Post();
    addObactioner(String.valueOf(INIT), new nodefieldlistbean_Init());
    addObactioner(String.valueOf(ADD), add_edit);
    addObactioner(String.valueOf(EDIT), add_edit);
    addObactioner(String.valueOf(POST), new nodefieldlistbean_Post());
   ...
   * 初始化操作的触发类
   */
  class nodefieldlistbean_Init implements Obactioner
  {
   ...
   * 添加或修改操作的触发类
   */
  class nodefieldlistbean_Add_Edit implements Obactioner
  {
    /**
     * 添加或修改的触发操作
     * @parma  action 触发执行的参数（键值）
   ...
   * 保存操作的触发类
   */
  class nodefieldlistbean_Post implements Obactioner
  {
    /**
     * 触发保存操作
     * @parma  action 触发执行的参数（键值）
   ...
   * 删除操作的触发类
   */
  class nodefieldlistbean_Delete implements Obactioner
  {
    /**
     * 触发删除操作
     * @parma  action 触发执行的参数（键值）
