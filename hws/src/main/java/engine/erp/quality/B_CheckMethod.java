...
public final class B_CheckMethod extends BaseAction implements Operate
{
  /**
   * 提取收发单据列表所有信息的SQL语句
   */
  private static final String Quality_SQL = "SELECT * FROM ZL_CHECKMETHOD";//
   ...
   * @return 返回收发单据信息的实例
   */
  public static B_CheckMethod getInstance(HttpServletRequest request)
  {
    B_CheckMethod b_WorkCheckBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_WorkCheckBean";
      b_WorkCheckBean = (B_CheckMethod)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(b_WorkCheckBean == null)
      {
        b_WorkCheckBean = new B_CheckMethod();
  ...
  protected void jbInit() throws Exception
  {
    ...
    dsWorkCheck.setSort(new SortDescriptor("", new String[]{"ID"}, new boolean[]{false}, null, 0));
        dsWorkCheck.setSequence(new SequenceDescriptor(new String[]{"ID"}, new String[]{"S_ZL_CHECKMETHOD"}));
    //添加操作的触发对象
  ...
  class B_WorkProcedure_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String checkmethod = rowInfo.get("checkmethod");
      if(code.equals("")){
      ...
      ds.setValue("checkmethod", checkmethod);
      ds.post();
      ds.saveChanges();
