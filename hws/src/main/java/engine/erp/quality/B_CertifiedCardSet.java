...
public final class B_CertifiedCardSet extends BaseAction implements Operate
{
  /**
   * 提取收发单据列表所有信息的SQL语句
   */
  private static final String Quality_SQL = "SELECT * FROM kc_chlb";//
   ...
   * @return 返回收发单据信息的实例
   */
  public static B_CertifiedCardSet getInstance(HttpServletRequest request)
  {
    B_CertifiedCardSet b_CardSetBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_CardSetBean";
      b_CardSetBean = (B_CertifiedCardSet)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(b_CardSetBean == null)
      {
        b_CardSetBean = new B_CertifiedCardSet();
        session.setAttribute(beanName, b_CardSetBean);
      }
    }
    return b_CardSetBean;
  }
  /**
   * 构造函数
   */
  private B_CertifiedCardSet()
  {
    try {
      jbInit();
    }
  ...
  protected void jbInit() throws Exception
  {
    ...
    dsWorkStandard.setSort(new SortDescriptor("", new String[]{"pxh"}, new boolean[]{false}, null, 0));
       // dsWorkStandard.setSequence(new SequenceDescriptor(new String[]{"chlbID"}, new String[]{"S_kc_chlb"}));
    //添加操作的触发对象
  ...
  class B_WorkProcedure_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0;i<=ds.getRowMax();i++){
      String dygs = rowInfo.get("dygs_"+String.valueOf(i));
      ds.goToRow(i);
      ds.setValue("dygs", dygs);
      }
      ds.post();
      ds.saveChanges();
