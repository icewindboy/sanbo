...
public final class B_CheckStandard extends BaseAction implements Operate
{
  /**
   * 提取收发单据列表所有信息的SQL语句
   */
  private static final String Quality_SQL = "SELECT * FROM zl_checkStandard";//
   ...
   * @return 返回收发单据信息的实例
   */
  public static B_CheckStandard getInstance(HttpServletRequest request)
  {
    B_CheckStandard b_StandardBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_StandardBean";
      b_StandardBean = (B_CheckStandard)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(b_StandardBean == null)
      {
        b_StandardBean = new B_CheckStandard();
        session.setAttribute(beanName, b_StandardBean);
      }
    }
    return b_StandardBean;
  }
  /**
   * 构造函数
   */
  private B_CheckStandard()
  {
    try {
      jbInit();
    }
  ...
  protected void jbInit() throws Exception
  {
    ...
    dsWorkStandard.setSort(new SortDescriptor("", new String[]{"standardID"}, new boolean[]{false}, null, 0));
        dsWorkStandard.setSequence(new SequenceDescriptor(new String[]{"standardID"}, new String[]{"S_zl_checkStandard"}));
    //添加操作的触发对象
  ...
  class B_WorkProcedure_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String standardCode = rowInfo.get("standardCode");
      String standardName = rowInfo.get("standardName");
      //if(standardCode.equals("")){
      ...
      {
        ds.insertRow(false);
        ds.setValue("standardID", "-1");
      }
      ds.setValue("standardCode", standardCode);
      ds.setValue("standardName", standardName);
      ds.post();
      ds.saveChanges();
