...
public final class B_EmployeeCardNo extends BaseAction implements Operate
{
  ...
  private final void initRowInfo(boolean isAdd, boolean isInit)
  {
    ...
    if(d_RowInfos == null)
      d_RowInfos = new ArrayList(ds_rl_ygxykh_tmp.getRowCount());
    else if(isInit)
      d_RowInfos.clear();

    ds_rl_ygxykh_tmp.first();
    for(int i=0; i<ds_rl_ygxykh_tmp.getRowCount(); i++)
    {
      RowMap row = new RowMap(ds_rl_ygxykh_tmp);
      d_RowInfos.add(row);
      ds_rl_ygxykh_tmp.next();
    }
  }
  ...
  class B_EmployeeCardNo_Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ds_rl_ygxykh.setQueryString(combineSQL(B_EMPLOYEECARDNO_SQL, "?", new String[]{" and "+user.getHandleDeptWhereValue("deptid", "czyid"),""}));
  ...
  class FIXED_SEARCH implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(SQL.length() > 0)
        SQL = " AND "+SQL;
      SQL = combineSQL(B_EMPLOYEECARDNO_SQL,"?",new String[]{" and "+user.getHandleDeptWhereValue("deptid", "czyid"),SQL});
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("ygxykh"), null, null, null, null, "="),
      });
      isInitQuery = true;
    }
  }
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        String xykID=detailrow.get("xykID");
          if(!ygxykh.trim().equals("")&&!xykID.trim().equals(""))
          {
