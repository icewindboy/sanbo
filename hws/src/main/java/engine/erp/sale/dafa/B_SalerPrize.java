...
public final class B_SalerPrize extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM xs_jjzb where lx=2 and 1<>1";//业务员奖金主表结构
  private static final String MASTER_SQL    = "SELECT * FROM xs_jjzb ";//
  private static final String SEARCH_SQL = "SELECT * FROM (SELECT a.*,b.nf,b.yf,b.sfjz FROM xs_jj a,xs_jjzb b WHERE a.jjzbid=b.jjzbid and b.lx=2) WHERE 1=1 ?  ";
  ...
  private static final String SALER_TCJ_SQL = "select sum(nvl(a.tcj,0))tcj  from VW_SALER_TCJ_TWO a   WHERE  1=1  ";
  private static final String SALER_INIT_TCJ_SQL = "SELECT SUM(nvl(b.jje,0)*nvl(b.hltcl,0)*0.01*c.fundxs*0.01)tcj FROM xs_td a,xs_tdhw b,xs_khdjxs c,xs_khxyed d  WHERE a.tdid=b.tdid AND a.Isinit=1  AND c.xydj=d.xydj AND a.dwtxid=d.dwtxid ";
