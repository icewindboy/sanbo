    ...
    /**
     * <p>Title: 销售子系统_销售结算_引入销售提单</p>
     * <p>Description: 销售子系统_销售结算_引入销售提单</p>
     * <p>Copyright: Copyright (c) 2003</p>
     * <p>Company: </p>
     * @author engine
     * @version 1.0
     */
    public final class ImportLading extends BaseAction implements Operate
    {
      private static final String IMPORT_SALE_SQL = "SELECT * FROM vw_sale_balance_import_td WHERE  fgsid=?  ";//分公司; 往来单位
      private static final String MASTER_SQL    = "SELECT * FROM vw_sale_balance_import_td WHERE   fgsid=? ?  ";
       ...
       * 析构函数
       * */
      public static ImportLading getInstance(HttpServletRequest request)
      {
        ImportLading ImportLadingBean = null;
        HttpSession session = request.getSession(true);
        synchronized (session)
        {
          String beanName = "ImportLadingBean";
          ImportLadingBean = (ImportLading)session.getAttribute(beanName);
          if(ImportLadingBean == null)
          {
            LoginBean loginBean = LoginBean.getInstance(request);
            ImportLadingBean = new ImportLading();
            ImportLadingBean.fgsid = loginBean.getFirstDeptID();
            session.setAttribute(beanName, ImportLadingBean);
          }
        }
        return ImportLadingBean;
      }
      /**
       * 构造函数
       */
      private ImportLading()
      {
        try {
          jbInit();
        }
