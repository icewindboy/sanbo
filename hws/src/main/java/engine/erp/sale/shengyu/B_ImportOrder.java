...
public final class B_ImportOrder extends BaseAction implements Operate
{
  ...
  private static final String DETAIL_SQL    = "select * From VW_SALE_HTHW  where 1=1 and abs(nvl(skdsl,0))<abs(nvl(sl,0))  AND zt=1 and htid= ";
  private static final String HTHW_SQL    = "select * From VW_SALE_HTHW where 1=1 and abs(nvl(skdsl,0))<abs(nvl(sl,0)) AND zt =1 AND ? AND fgsid=? ? ";
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("jzj", formatNumber(rowInfo.get("jzj_"+i), priceFormat));//基准价
      detailRow.put("cjtcl", formatNumber(rowInfo.get("cjtcl_"+i), qtyFormat));//差价提成率
      ...
      detailRow.put("hltcl", formatNumber(rowInfo.get("hltcl_"+i), qtyFormat));//回笼提成率
