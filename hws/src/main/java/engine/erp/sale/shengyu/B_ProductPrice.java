  ...
  class B_ProductPrice_Post implements Obactioner
  {
    ...
    private String checkDetailInfo()
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if(ccj.length()!=0)
        {
        if((temp = checkNumber(ccj, "出厂价")) != null)
          return temp;
        }
        String msj = detailrow.get("msj");
        if(msj.length()!=0)
        {
        if((temp = checkNumber(msj, "门市价")) != null)
          return temp;
        }
        String lsj = detailrow.get("lsj");
        if(lsj.length()!=0)
        ...
        if(qtjg1.length()!=0)
        {
        if((temp = checkNumber(qtjg1, "其他价格1")) != null)
          return temp;
        }
        String qtjg2 = detailrow.get("qtjg2");
        if(qtjg2.length()!=0)
        {
        if((temp = checkNumber(qtjg2, "其他价格2")) != null)
          return temp;
        }
        String qtjg3 = detailrow.get("qtjg3");
        if(qtjg3.length()!=0)
        {
        if((temp = checkNumber(qtjg3, "其他价格3")) != null)
          return temp;
        }
        String mrjg = detailrow.get("mrjg");
        if(mrjg.equals(""))
          return showJavaScript("alert('默认价格必选!')");
        String mrzk = detailrow.get("mrzk");
        if(mrzk.equals(""))
          return showJavaScript("alert('默认折扣必填!')");
        if((temp = checkNumber(mrzk, "默认折扣")) != null)
          return temp;
      }
      return null;
    }
