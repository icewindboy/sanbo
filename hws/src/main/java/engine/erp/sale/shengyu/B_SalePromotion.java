...
/**
 * <p>Title: 销售管理--销售货物管理--广告促销单--</p>
 * <p>Description: 销售管理--销售货物管理--广告促销单<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author engine
 * @version 1.0
 */
public final class B_SalePromotion extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM xs_td WHERE djlx=2 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_EDIT_SQL       = "SELECT * FROM xs_td WHERE tdid='?' ";


  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=2 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL    = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=2 AND ? AND fgsid=? ?  ";
  ...
  private static final String SALE_PROMOTION_SQL = "select t.dwtxid,t.cpid,t.startdate,t.enddate,t.memo,t.prom_price from xs_promotion t WHERE t.dwtxid='?' AND t.startdate<=to_date('?','yyyy-mm-dd') AND t.enddate>=to_date('?','yyyy-mm-dd')";
  ...
  public  HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "xs_td.2");
  ...
  public String zkl = "";
  public RowMap[] promotionRows =null;
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_SalePromotion getInstance(HttpServletRequest request)
  {
    B_SalePromotion b_SalePromotionBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_SalePromotionBean";
      b_SalePromotionBean = (B_SalePromotion)session.getAttribute(beanName);
      if(b_SalePromotionBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_SalePromotionBean = new B_SalePromotion();
        b_SalePromotionBean.qtyFormat = loginBean.getQtyFormat();
        b_SalePromotionBean.priceFormat = loginBean.getPriceFormat();
        b_SalePromotionBean.sumFormat = loginBean.getSumFormat();
        b_SalePromotionBean.fgsid = loginBean.getFirstDeptID();
        b_SalePromotionBean.loginId = loginBean.getUserID();
        b_SalePromotionBean.loginName = loginBean.getUserName();
        b_SalePromotionBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          b_SalePromotionBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          b_SalePromotionBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        b_SalePromotionBean.dsMasterTable.setColumnFormat("zje", b_SalePromotionBean.sumFormat);
        b_SalePromotionBean.dsMasterTable.setColumnFormat("zsl", b_SalePromotionBean.qtyFormat);

        b_SalePromotionBean.dsDetailTable.setColumnFormat("sl", b_SalePromotionBean.qtyFormat);
        b_SalePromotionBean.dsDetailTable.setColumnFormat("hssl", b_SalePromotionBean.qtyFormat);
        b_SalePromotionBean.dsDetailTable.setColumnFormat("xsj", b_SalePromotionBean.priceFormat);
        b_SalePromotionBean.dsDetailTable.setColumnFormat("dj", b_SalePromotionBean.priceFormat);
        b_SalePromotionBean.dsDetailTable.setColumnFormat("jje", b_SalePromotionBean.sumFormat);
        b_SalePromotionBean.dsDetailTable.setColumnFormat("xsje", b_SalePromotionBean.sumFormat);

        session.setAttribute(beanName, b_SalePromotionBean);
      }
    }
    return b_SalePromotionBean;
  }
  /**
   * 构造函数
   */
  private B_SalePromotion()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.2','tdbh') from dual"}));
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.2','tdbh') from dual");
   ...
   * 促销产品管理
   * */
  public RowMap[] getSalePromotionRow(String dwtxid)
  {
    if(dwtxid==null||dwtxid.equals(""))
      return null;
    String today = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
    RowMap[] promotionrows ;
    try{
      EngineDataSet dspromotion = new EngineDataSet();
      setDataSetProperty(dspromotion,combineSQL(SALE_PROMOTION_SQL,"?",new String[]{dwtxid,today}));
      dspromotion.open();
      promotionrows = new RowMap[dspromotion.getRowCount()];
      dspromotion.first();
      for(int i=0;i<dspromotion.getRowCount();i++)
      {
        promotionrows[i]= new RowMap(dspromotion);
        dspromotion.next();
      }
      }catch(Exception e)
      {
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterList, dsMasterList.getRow(),"sale_promotion", content,deptid);
    }
  }
  /**
   * 取消审批
  ...
  class Lading_Out implements Obactioner
  {
    ...
    // for(int i=0;i<dsDetailTable.getRowCount();i++)
      ...
      dsMasterList.setValue("zsl",String.valueOf(dzsl));
      dsMasterList.setValue("zje",formatNumber(String.valueOf(dzje),"#0.00"));
      //dsMasterTable.setValue("zsl",bzsl.toString());
      //dsMasterTable.setValue("zje",bzje.toString());
      dsMasterList.setValue("tdrq",today);
      dsMasterList.post();
      dsMasterList.saveDataSets(new EngineDataSet[]{dsMasterList, dsDetailTable}, null);
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ...
        //ds.setValue("tdrq",new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
    ...
    private String checkMasterInfo()
    {
      ...
      if(!temp.equals("")&&!isDate(temp))
        return showJavaScript("alert('非法计划发货日期！');");
