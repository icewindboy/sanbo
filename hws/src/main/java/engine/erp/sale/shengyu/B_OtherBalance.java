...
public final class B_OtherBalance extends BaseAction implements Operate
{
  ...
  private EngineDataSet dsBalanceTable = new EngineDataSet();//数据集
  ...
  //public  HtmlTableProducer table = new HtmlTableProducer(dsBalanceTable, "xs_other_fund", "a");//查询得到数据库中配置的字段
  ...
  public static B_OtherBalance getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(otherBalanceBean == null)
      {
        ...
        session.setAttribute(beanName, otherBalanceBean);//加入到session中
      }
    }
  ...
  protected void jbInit() throws Exception
  {
    ...
    dsBalanceTable.setSequence(new SequenceDescriptor(new String[]{"otherfundid"}, new String[]{"S_XS_OTHER_FUND"})); //设置主健的sequence
  ...
  class Search implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      {
        dsBalanceTable.setQueryString(MSQL);
        dsBalanceTable.setRowMax(null);//以便dbNavigator刷新数据集
      }
    }
    /**
     * 初始化查询的各个列
