...
public final class OtherLookupHelperPoolImpl extends OtherLookupHelperPool implements SysConstant, java.io.Serializable {
	private Map hashSql = null;

	public OtherLookupHelperPoolImpl() {
		hashSql = new Hashtable(100, 1);
		// -----------------------------------------------------------------------
		// --基础
		// -----------------------------------------------------------------------
		// 仓库
		hashSql.put(BEAN_STORE, new LookupHelper("SELECT * FROM VW_BEAN_STORE", null, "storeid", "ORDER BY ckbm", new String[] { "ckmc" }, true));
		// 仓库库位JAVABEAN
