...
public class OtherLookupBeanFacade implements LookUp, HttpSessionBindingListener {
	private static Hashtable lookupPool = new Hashtable();// OtherLookupBeanFacade的缓存池
	...
	public synchronized static void refreshLookup(String beanName) {
		OtherDataSetPool.refreshDataSet(beanName);
	}
	 ...
	 * @return 返回是否还有子结点
	 */
	private static boolean createOptionTree(OtherLookupBeanFacade lookupBean, String fatherCode, int level, String selectNodeId,
			StringBuffer buf) {
	 ...
	 * @return 包含option的字符串
	 */
	public synchronized static String dataSetToOption(DataSet ds, String idColumn, String capColumns[], String selectIdValue,
		...
		return dataSetToOption(ds, new String[] { idColumn }, new String[] { selectIdValue }, capColumns, paramids);
	}
	 ...
	 * @return 包含option的字符串
	 */
	private static String dataSetToOption(DataSet ds, String[] selectIds, String[] selectIdValues, String capColumns[],
			LookupParam[] paramIds) {
	...
	public synchronized static EngineDataSet getLookupDataSet(String beanName) {
		...
		synchronized (lookupPool) {
			OtherLookupBeanFacade lookupBean = (OtherLookupBeanFacade) lookupPool.get(beanName);
			if (lookupBean == null) {
				eds = OtherDataSetPool.getDataSet(beanName);
				lookupBean = new OtherLookupBeanFacade(help, eds);
	...
	public synchronized static LookUp getInstance(HttpServletRequest request, String beanName) {
		OtherLookupBeanFacade lookupBean = null;
		// 设置SQL语句的缓存
		LookupHelper help = OtherLookupHelperPool.getLookupHelper(beanName);
		if (help == null)
			return null;
		// 得到登陆Bean
		engine.common.OtherLoginBean login = engine.common.OtherLoginBean.getInstance(request);
		// 如果是一次打开数据的
		if (help.isOneOpen()) {
			lookupBean = (OtherLookupBeanFacade) lookupPool.get(beanName);
			if (lookupBean == null) {
				EngineDataSet eds = OtherDataSetPool.getDataSet(beanName);
				lookupBean = new OtherLookupBeanFacade(help, eds);
				lookupPool.put(beanName, lookupBean);
			}
		} else {
			...
			// beanname$sessionid
			String sessBeanName = new StringBuffer(beanName).append("$OTHER").append(session.getId()).toString();
			// 在session中查找OtherLookupBeanFacade的实例，不存在则创建一个
			lookupBean = (OtherLookupBeanFacade) session.getAttribute(sessBeanName);
			if (lookupBean == null) {
				// 在session中查找EngineDataSetProvider的实例，不存在则创建一个
				String sessProdviderName = new StringBuffer("prodvider$Other").append(session.getId()).toString();
		...
		LookupParam[] filerValues = new LookupParam[] { new LookupParam("isshow", new String[] { "1" }),
	...
	private Log log = new Log("OtherLookupBeanFacade");

	/**
	 * 构造函数
	 */
	private OtherLookupBeanFacade(LookupHelper help, EngineDataSet dsData) {
		this.help = help;
		this.dsData = dsData;
	 ...
	 * @return 返回是否有此记录
	 */
	private boolean locateDataSet(DataSet ds, String[] columnNames, String[] columnValues)// throws
																							// Exception
	{
	...
	private static String arrryToSQL(String select, String initSelect, LookupParam[] wheresValues) {
		synchronized (wheresValues) {
			...
			for (int i = 0; i < wheresValues.length; i++) {
				...
				// add AND
				if (i < wheresValues.length - 1 && columnValues.length > 0 && wheresValues[i + 1] != null
						&& wheresValues[i + 1].getValues() != null && wheresValues[i + 1].getValues().length > 0)
					buf.append(" AND ");
			}
			// 若没有变化，则取结构
	 ...
	 *             异常
	 */
	public synchronized void regConditionData(String regColumnName, String[] regColumnValues) // throws
																								// Exception
	{
		...
		// 组装主键的WHERE IN 子句
		LookupParam param = new LookupParam(regColumnName == null ? help.keyColumns[0] : regColumnName, regColumnValues);
	...
	{
		return idValue == null || idValue.length() == 0 ? "" : getLookupName(new String[] { idValue });
	}
	...
	public synchronized RowMap getLookupRow(String idValue) {
		return idValue == null || idValue.length() == 0 || !locateDataSet(idValue) ? new RowMap() : new RowMap(viewData);
	}
	...
	public synchronized RowMap getLookupRow(String[] idValues) {
		return idValues == null || idValues.length == 0 || !locateDataSet(idValues) ? new RowMap() : new RowMap(viewData);
	}
	 ...
	 * @return 返回下拉框信息
	 */
	public synchronized String getList(String selectid, String paramid, String paramvalue)// throws
																							// Exception
	{
		LookupParam[] paramids = null;
																	...
																	// Exception
	{
		if (help.isTree) {
			...
			return dataSetToOption(viewData, help.keyColumns, selectid == null ? null : new String[] { selectid }, help.capsColumn,
					this.filterValues);
		// 有条件的
		else if (this.filterValues == null)
			return dataSetToOption(dsOtherData, help.keyColumns, selectid == null ? null : new String[] { selectid }, help.capsColumn,
					paramids);
		//
