...
public class LookupBeanFacade implements LookUp, HttpSessionBindingListener {
	 ...
	 * @return 返回是否还有子结点
	 */
	private static boolean createOptionTree(LookupBeanFacade lookupBean,
			String fatherCode, int level, String selectNodeId, StringBuffer buf) {
		...
		for (int i = 0; i < curNodeIds.length; i++)
			curNodeIds[i] = EngineDataSet.getValue(ds,
					ds.getColumn(help.keyColumns[i]).getOrdinal());

		String curNodeCode = EngineDataSet.getValue(ds,
				ds.getColumn(codeFieldName).getOrdinal());
		// 得到显示的名称的字符串
		StringBuffer curNodeName = new StringBuffer();
		for (int i = 0; i < help.capsColumn.length; i++) {
			String caps = EngineDataSet.getValue(ds,
					ds.getColumn(help.capsColumn[i]).getOrdinal());
		...
		if (ds.next()) {
			String nextCode = EngineDataSet.getValue(ds,
					ds.getColumn(codeFieldName).getOrdinal());
		...
		buf.append("AddSelectItem('")
				.append(StringUtils.getArrayValue(curNodeIds)).append("','");
		...
		if (!isLeaf) {
			// 打印子结点
			while (true) {
				boolean isHasChild = createOptionTree(lookupBean, curNodeCode,
						level + 1, selectNodeId, buf);
				if (!isHasChild)
					break;
			}

			String name = EngineDataSet.getValue(ds, ds
					.getColumn(codeFieldName).getOrdinal());
	...
	public synchronized static String dataSetToOption(DataSet ds,
			String idColumn, String capColumns[], String selectIdValue,
			String existColumn, String existValue) {
		LookupParam[] paramids = null;
		if (existColumn != null && existValue != null)
			paramids = new LookupParam[] { new LookupParam(existColumn,
					new String[] { existValue }) };

		return dataSetToOption(ds, new String[] { idColumn },
				new String[] { selectIdValue }, capColumns, paramids);
	}
	 ...
	 * @param selectIds
	 *            id列名和初始化option
	 * @param selectIdValues
	 *            id列名和初始化option时，默认选中的id列的值
	 * @param capColumns
	 *            显示的名称
	 * @param paramids
	 *            需要打印的列名和相应的值
	 * @return 包含option的字符串
	 */
	private static String dataSetToOption(DataSet ds, String[] selectIds,
			String[] selectIdValues, String capColumns[], LookupParam[] paramIds) {
		...
		synchronized (ds) {
			...
			for (int i = 0; i < ds.getRowCount(); i++) {
				if (paramIds == null)
					...
					for (int j = 0; j < paramIds.length; j++) {
						...
						String data = EngineDataSet.getValue(ds,
								ds.getColumn(key).getOrdinal());
			...
			//
			String selectValuesStr = selectIdValues == null ? null
					: StringUtils.getArrayValue(selectIdValues);
			if (selectValuesStr != null && selectValuesStr.length() > 0)
				buf.append("SetSelectedIndex('").append(selectValuesStr)
						.append("');");
		}
		return buf.toString();
	}
	 ...
	 *            显示的各个字段的Ordianal
	 */
	private static void addSelectItem(StringBuffer buf, DataSet ds,
			int idOrdianal, int[] capOrdinals) {
	 ...
	 */
	private static void addSelectItem(StringBuffer buf, DataSet ds,
			int[] idOrdianals, int[] capOrdinals) {
		...
		buf.append("AddSelectItem('")
				.append(StringUtils.getArrayValue(idValues)).append("','");
		int j = 0;
		for (; j < capOrdinals.length - 1; j++) {
	 ...
	 * @return 包含option的字符串
	 */
	public synchronized static String listToOption(List[] lists,
			int selectedIndex) {
		StringBuffer buf = new StringBuffer();
		synchronized (lists) {
			for (int i = 0; i < lists[0].size(); i++) {
				buf.append("AddSelectItem('").append(lists[0].get(i))
						.append("','");
				buf.append(lists[1].get(i)).append("');");
			}
			if (selectedIndex > -1 && selectedIndex < lists[0].size())
				buf.append("SetSelectedIndex('")
						.append(lists[0].get(selectedIndex)).append("');");
			else
				buf.append("SetSelectedIndex('');");
		}
		return buf.toString();
	...
	public synchronized static EngineDataSet getLookupDataSet(String beanName) {
		...
		synchronized (lookupPool) {
			LookupBeanFacade lookupBean = (LookupBeanFacade) lookupPool
					.get(beanName);
			if (lookupBean == null) {
	 ...
	 *            web请求
	 * @param beanName
	 *            baen的名称，各个bean的名称见类engine.project.SysConstant(如仓库,
	 *            入参为SysConstant.BEAN_STORE)
	 * @return 返回对象的引用
	 */
	public synchronized static LookUp getInstance(HttpServletRequest request,
			String beanName) {
		...
		if (help == null)
			return null;
		// 得到登陆Bean
		engine.common.LoginBean login = engine.common.LoginBean
				.getInstance(request);
		// 如果是一次打开数据的
		if (help.isOneOpen()) {
			...
			// beanname$sessionid
			String sessBeanName = new StringBuffer(beanName).append("$")
			...
			if (lookupBean == null) {
				...
				String sessProdviderName = new StringBuffer("prodvider$")
						.append(session.getId()).toString();
				EngineDataSetProvider provider = (EngineDataSetProvider) session
						.getAttribute(sessProdviderName);
				if (provider == null) {
		...
		LookupParam[] filerValues = new LookupParam[] {
				...
				deptids != null ? new LookupParam("deptid", login.getUser()
						.getHandleDepts()) : null,
				storeids != null ? new LookupParam("storeid", login.getUser()
						.getHandleStores()) : null };
	...
	private void setFilterValues(LookupParam[] values) {
		...
		if (keys == null)
			...
			for (int i = 0; i < keys.length; i++) {
				this.filterValues[i] = LookupParam.getLookupParam(values,
						keys[i]);
				if (this.filterValues[i] == null)
					this.filterValues[i] = new LookupParam(keys[i],
							new String[] { "null" });
			}
		}
	}
	...
	private void setWhereClause(Map mapWhereClause) {
		...
		for (int i = 0; i < help.whereClause.length; i++) {
			...
			if (upperValue.indexOf("IN") > -1 || upperValue.indexOf("<>") > -1)
				buf.append(help.whereClause[i]).append(" ").append(value)
						.append(" ");
			else
				buf.append(help.whereClause[i]).append("='").append(value)
						.append("'");
	...
	private void openOneDataSet() {
		synchronized (dsData) {
			if (!dsData.isOpen()) {
				dsData.setQueryString(help.sortStr != null ? (help.select + " " + help.sortStr)
						: help.select);
				dsData.openDataSet();
				if (viewData == null)
	...
	private boolean locateDataSet(String columnValue)// throws Exception
	{
		...
		if (help.keyColumns.length > 1)
			throw new InvalidLookupArgumentException(
					"lookup length of key not equals length of value");

		return locateDataSet(viewData, help.keyColumns,
	 ...
	 * @return 返回是否有此记录
	 */
	private boolean locateDataSet(DataSet ds, String[] columnNames,
			String[] columnValues)// throws Exception
	{
		...
		if (!ds.locate(locateRow, Locate.FIRST)) {
			EngineDataSetProvider provider = (EngineDataSetProvider) dsData
					.getProvider();
			StringBuffer buf = new StringBuffer(SQL);
			// 组装SQL
			buf.append(SQL.toUpperCase().indexOf("WHERE") > -1 ? " AND "
					: " WHERE ");
			...
			provide = null;
			// StoreDataSetData storeDsd = provider.getDataSetData_info(new
			// String[]{buf.toString()}, null)[0];
			/*
	 ...
	 *            传入的数据集（需要有）
	 * @param idName
	 *            与默认主键值相对应的数据集ds的字段名称, 主要用于提取数据集中的各行的值
	 * @throws Exception
	 *             异常信息
	 */
	public synchronized void regData(DataSet ds, String idColumnName) // throws
																		// Exception
	{
		if (help.isOneOpen()) {
			openOneDataSet();
	 ...
	 *            传入的数据集（需要有）
	 * @param idColumnNames
	 *            与默认主键值相对应的数据集ds的字段名称数组, 主要用于提取数据集中的各行的值
	 * @throws Exception
	 *             异常信息
	 */
	 ...
	 *            传入的数据集（需要有）
	 * @param regColumnName
	 *            需要用于注册的字段名称, 数据集ds中的名称可能与之不同, 等于null将用默认的主键字段
	 * @param regValuesColumn
	 *            于regColumnName相对应的数据集ds的字段名称, 主要用于提取数据集中的各行的值
	 * @throws Exception
	 *             异常信息
	 * 
	 *             private void regData(DataSet ds, String regColumnName, String
	 *             regValueColumnName) //throws Exception { if(help.isOneOpen())
	 *             { openOneDataSet(); return; } if(help.keyColumns.length > 1)
	 *             throw new InvalidLookupArgumentException(
	 *             "lookup length of key not equals length of value");
	 * 
	 *             String[] ids = columnToArray(ds, regValueColumnName); //
	 *             regData(new LookupParam[]{new LookupParam(regColumnName,
	 *             ids)}); }
	 * 
	 *             /** 注册数据,有些需要表由于记录的太多，需要注册要得到数据的记录，以用于抽取数据
	 * @param idValues
	...
	public synchronized void regData(String[] idValues) // throws Exception
	{
		...
		regData(new LookupParam[] { new LookupParam(help.keyColumns[0],
				idValues) });
	}
	 ...
	 * 
	 * @param regInfos
	 *            注册数据的字段名称以及相应的数据值数组, 用该字段提取数据(非主键字段)
	 */
	public synchronized void regData(LookupParam[] regInfos)// throws Exception
	{
		...
		// regColumnName == null ? help.keyColumns[0] : regColumnName,
		// regColumnValues);
	 ...
	 *            select..from 部分SQL语句
	 * @param initSelect
	 *            初始化的select.部分SQL语句，不带多余的where条件的
	 * @param wheresValues
	 *            WHERE条件字段名称和值的数组
	 * @return 返回装后的SQL语句
	 */
	private static String arrryToSQL(String select, String initSelect,
			LookupParam[] wheresValues) {
		synchronized (wheresValues) {
			StringBuffer buf = new StringBuffer(select);
			buf.append(select.toUpperCase().indexOf("WHERE") > -1 ? " AND "
					: " WHERE ");
			...
			for (int i = 0; i < wheresValues.length; i++) {
				...
				for (int j = 0; j < columnValues.length; j++) {
					String value = columnValues[j] == null
							|| columnValues[j].length() == 0 ? "-1"
							: columnValues[j];
					buf.append(value).append(
							j == columnValues.length - 1 ? "')" : "','");
				}
				// add AND
				if (i < wheresValues.length - 1 && columnValues.length > 0
						&& wheresValues[i + 1] != null
						&& wheresValues[i + 1].getValues() != null
			...
			if (buf.length() == startLength) {
				...
				buf.append(initSelect.toUpperCase().indexOf("WHERE") > -1 ? " AND "
						: " WHERE ");
				buf.append("1<>1");
			}
	...
	/**
	 * 注册其他条件数据, 在调用方法getList(String selectid, String paramid, String
	 * paramvalue)之前
	 * 
	 * @param ds
	 *            数据集
	 ...
	 *             异常
	 */
	public synchronized void regConditionData(DataSet ds, String regColumnName) // throws
																				// Exception
	{
	 ...
	 * @param regColumnName
	 *            注册的字段名称
	 * @param regColumnValues
	 *            注册的字段值
	 * @throws Exception
	 *             异常
	 */
	public synchronized void regConditionData(String regColumnName,
			String[] regColumnValues) // throws Exception
	{
		...
		// String tempSQL = regColumnValues.length >0 ? SQL : help.select;
		String tempSQL = arrryToSQL(SQL, help.select,
				new LookupParam[] { param });
		if (help.sortStr != null)
			tempSQL = new StringBuffer(tempSQL).append(" ")
					.append(help.sortStr).toString();
		// 打开数据或刷新数据集
...
//		System.out.println(idValue);
	...
	public synchronized String getLookupName(String[] idValues) {
		...
		if (idValues == null || idValues.length == 0
				|| !locateDataSet(idValues))
			return "";

		StringBuffer name = new StringBuffer();
		int i = 0;
		for (; i < help.capsColumn.length; i++) {
			String cap = EngineDataSet.getValue(viewData,
					viewData.getColumn(help.capsColumn[i]).getOrdinal());
			name.append(cap);
			if (i < help.capsColumn.length - 1)
				name.append(" ");
		}
	...
	public synchronized RowMap getLookupRow(String idValue) {
		if(idValue!=null&&idValue.length()>0)
			idValue = idValue.replace(",", "");
		return idValue == null || idValue.length() == 0
				|| !locateDataSet(idValue) ? new RowMap()
				: new RowMap(viewData);
	}
	...
	public synchronized RowMap getLookupRow(String[] idValues) {
		return idValues == null || idValues.length == 0
				|| !locateDataSet(idValues) ? new RowMap() : new RowMap(
				viewData);
	}
	 ...
	 * @return 返回查找到的记录的所有信息
	 * @throws Exception
	 *             异常信息 public String[] getLookupValues(String idValue, String[]
	 *             lookUpColumns) throws Exception { String[] temps = new
	 *             String[lookUpColumns.length]; for(int i=0;
	 *             i<lookUpColumns.length; i++) {
	 * 
	...
	/**
	 * 得到下拉框信息列表,需要先调用regConditionData方法(如&lt;option
	 * value=1&gt;显示的值&lt;/option&gt;)
	 * 
	 * @param selectid
	 *            需要选中的id
	 * @param paramid
	 *            制定打印含有显示特定字段和值的下拉框列表
	 * @param paramvalue
	 *            制定打印含有显示特定字段和值的下拉框列表
	 * @return 返回下拉框信息
	 */
	public synchronized String getList(String selectid, String paramid,
			String paramvalue)// throws Exception
	{
		LookupParam[] paramids = null;
		if (paramid != null && paramvalue != null) {
			if (dsOtherData == null) {
				log.warn(help.select
						+ ": you should call method of regConditionData() at first!");
				throw new LookupNoRegException(
						"you should call method of regConditionData() at first!");
			}
			// 定位数据集（到数据库）
			locateDataSet(dsOtherData, new String[] { paramid },
					new String[] { paramvalue });
			LookupParam param = new LookupParam(paramid,
					new String[] { paramvalue });
	 ...
	 *             异常
	 */
	private String getList(String selectid, LookupParam[] paramids)// throws
																	// Exception
	{
		if (help.isTree) {
			synchronized (this) {
				...
				while (viewData.inBounds()) {
					String fatherCode = EngineDataSet.getValue(viewData,
							viewData.getColumn(help.treeCode).getOrdinal());
					createOptionTree(this, fatherCode, 0, selectid, buf);
				}
				if (selectid != null)
					buf.append("SetSelectedIndex('").append(selectid)
							.append("');");
				return buf.toString();
			}
		} else if (paramids == null)
			return dataSetToOption(viewData, help.keyColumns,
					selectid == null ? null : new String[] { selectid },
					help.capsColumn, this.filterValues);
		// 有条件的
		else if (this.filterValues == null)
			return dataSetToOption(dsOtherData, help.keyColumns,
					selectid == null ? null : new String[] { selectid },
					help.capsColumn, paramids);
		//
		/* @todo:考虑去掉dsOther,因为现在是有filterValues。 */
		else {
			LookupParam[] allParams = LookupParam.union(this.filterValues,
					paramids);
			return dataSetToOption(dsOtherData, help.keyColumns,
					selectid == null ? null : new String[] { selectid },
	...
	private void checkDataSetReg() throws LookupNoRegException {
		...
		if (viewData == null) {
			log.warn(help.select
					+ ": you should call method of regData() at first!");
			throw new LookupNoRegException(
					"you should call method of regData() at first!");
		}
	}

	// 如果help实例中的whereClause!=null 或 isOneOpen==false，
	// 就将实例推入特定的session中。而不放在bean池中
