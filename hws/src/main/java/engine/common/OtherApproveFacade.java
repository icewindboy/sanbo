...
/*
 * 审批记录明细ID spjlmxID NUMBER(12) TRUE FALSE TRUE 人员ID personid 审批人 NUMBER(6)
 * FALSE TRUE FALSE 审批项目特殊明细ID spxmtsmxID NUMBER(9) FALSE TRUE FALSE 审批项目明细ID
 * spxmmxID NUMBER(9) FALSE TRUE FALSE 审批记录ID spjlID NUMBER(11) FALSE TRUE FALSE
 * 审批等级 spdj NUMBER(2) FALSE FALSE FALSE 审批名称 spmc VARCHAR2(32) FALSE FALSE
 * FALSE 通过标志 sftg 0=未审批,1=通过,2=驳回 NUMBER(1) FALSE FALSE FALSE 审批日期 sprq DATE
 * FALSE FALSE FALSE 审批意见 spyj 审批意见 VARCHAR2(256) FALSE FALSE FALSE URL URL
 * VARCHAR2(128) FALSE FALSE FALSE
 */
public final class OtherApproveFacade extends BaseAction implements Operate {
	// 提取审批项目的主键字段名信息
	// private static final String APPROVE_KEY_SQL =
	// "SELECT zjzdm FROM sp_xm WHERE spxmbm='@'";
	...
	// 审批项目中特殊审批列表
	private static final String SPECIAL_APPROVE_SQL = "SELECT m.spxmmxid, m.spxmtsmxid, t.tsmxbm, m.tsz FROM sp_xm x, sp_xmmx m, sp_xmtsmx t "
	 ...
	 * + "FROM sp_jl j, sp_spr s, (" + "   SELECT a.spxmmxid, a.spjlid FROM (" +
	 ...
	 */
	// 一条当前人员审批记录
	private static final String APPROVE_FOR_ID_SQL = "SELECT j.spjlid, j.spxmbm, j.spxmmc, j.spnr, j.zjid, j.deptid, j.personid, j.tjsprq " + "FROM  sp_jl j WHERE j.spjlid=@ ";
	// 一条审批记录对应的明细
	...
	// 审批项目流程数量
	private static final String APPROVE_FLOW_COUNT_SQL = "SELECT COUNT(d.spxmmxid) FROM sp_xm m, sp_xmmx d WHERE m.spxmid=d.spxmmxid AND m.spxmbm='@'";
	 ...
	 * @return 返回销售合同列表的实例
	 */
	public static OtherApproveFacade getInstance(HttpServletRequest request) {
		OtherApproveFacade approveFacadeBean = null;
		HttpSession session = request.getSession(true);
		synchronized (session) {
			String beanName = "otherApproveFacade";
			approveFacadeBean = (OtherApproveFacade) session.getAttribute(beanName);
			if (approveFacadeBean == null) {
				// 引用LoginBean
				OtherLoginBean loginBean = OtherLoginBean.getInstance(request);

				approveFacadeBean = new OtherApproveFacade();
				...
				approveFacadeBean.deptid = loginBean.getUser().getDeptId();
	...
	/*
	 * final class MasterLoadListener implements LoadListener,
	 * java.io.Serializable { public void dataLoaded(LoadEvent e) {
	 * openDetailTable(); } }
	 */
	/**
	 * 初始化操作的触发类
	...
	final class Post implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			// dsDetailTable.setValue("sftg", info.get("sftg").equals("1") ? "1"
			// : "2");
		 ...
		 *            字段值
		 * @param isDetailId
		 *            false:key的值是spjlid字段的值, true:key的值是spjlmxid字段的值
		 * @return
		 */
	 ...
	 *             { //将特定分隔符的字符串转化为数组 String result =
	 *             combineSQL(APPROVE_PERSON_SQL, "@", new String[]{spjlid});
	 *             result = dataSetProvider.getSequence(result); ArrayList
	 *             personids = null; if(result == null) personids = new
	 *             ArrayList(); else{ String[] temp = parseString(result, ",");
	 *             personids = new ArrayList(temp.length); for(int i=0;
	 *             i<temp.length; i++) personids.add(temp[i]); }
	 *             //如果该审批项目没有结束,则得到主表数据和从表数据 DataSetData detailData = null;
	 *             if(personids.size() > 0) { //得到主表数据 String sql =
	 *             combineSQL(APPROVE_FOR_ID_SQL, "@", new String[]{spjlid});
	 *             if(dsApproveOne == null) { dsApproveOne = new
	 *             EngineDataSet(); setDataSetProperty(dsApproveOne, null); }
	 *             dsApproveOne.setQueryString(sql); if(dsApproveOne.isOpen())
	 *             dsApproveOne.refresh(); else dsApproveOne.open();
	 * 
	 *             //得到从表数据 sql = combineSQL(APPROVE_DETAIL_ONE_SQL, "@", new
	 *             String[]{spjlid}); ProvideInfo provideInfo = new
	 *             ProvideInfo(sql); provideInfo.setLoadDataUseSelf(false);
	 *             detailData = dataSetProvider.getDataSetData(new
	 *             ProvideInfo[]{provideInfo})[0]; } //同步数据
	 *             synchronized(approvePool) { //得到字段和值 Object[] objs =
	 *             approvePool.values().toArray(); for(int i=0; i < objs.length;
	 *             i++) { ApproveFacade approve = (ApproveFacade)objs[i];
	 *             synchronized(approve) { //是否对该用户有效
	 *             if(personids.indexOf(approve.loginId) > -1) {
	 *             if(approve.locateApproveList(spjlid))
	 *             approve.dsMasterTable.deleteRow(); //添加审批主表记录
	 *             if(dsApproveOne.getRowCount() > 0) { EngineRow masterData =
	 *             new EngineRow(approve.dsMasterTable);
	 *             dsApproveOne.copyTo(masterData);
	 *             approve.dsMasterTable.addRow(masterData); }
	 *             approve.dsMasterTable.resetPendingStatus(true); //添加审批从表信息记录
	 *             if(detailData != null) { if(!approve.dsDetailTable.isOpen())
	 *             approve.dsDetailTable.open();
	 *             detailData.loadDataSet(approve.dsDetailTable); } } else
	 *             if(approve.locateApproveList(spjlid)){ //已经设置级联删除，从表记录会跟者删除的
	 ...
	 *             异常
	 * 
	 *             public void putAproveList(EngineDataSet ds, int row, String
	 *             projectCode, String content) throws Exception {
	 ...
	 *             异常
	 */
	public void putAproveList(EngineDataSet ds, int row, String projectCode, String content, String deptid) throws Exception {
	 ...
	 * 
	 *             public void putAproveList(EngineDataSet ds, int row, String
	 *             projectCode, String content, ApproveListener listener) throws
	 *             Exception { putAproveList(ds, row, projectCode, content,
	 *             this.deptid, listener); }
	 */
	 ...
	 *             异常
	 */
	public void putAproveList(EngineDataSet ds, int row, String projectCode, String content, String deptid, ApproveListener listener) throws Exception {
		if (deptid == null)
			deptid = this.deptid;
		content = content == null ? "" : engine.util.StringUtils.getUnicodeSubString(content, 128);
		// String s = content == null ? "" : ? content.substring(0, 64):
		// content;
		// String content =
		// "HT000006合同编号2合同编号3合同编号4合同编号5合同编号6合同编号7合同编号8合同编号9合同编号10合同编号11合同编号12合同编号13合同编号14合同编号15合同编号";
		...
		synchronized (ds) {
			...
			checkSQL.append(" WHERE ").append(dsTemp.getValue("ztzdm")).append(" ='").append(dsTemp.getValue("ztbtgz")).append("'");
			...
			if (listener != null) {
				...
				if (data.getRowCount() > 0) {
					...
					data.first();
					for (int i = 0; i < reponses.length; i++)// m.spxmtsmxid,
																// t.tsmxbm,
																// m.tsz
					{
					...
					for (int i = 0; i < reponses.length; i++) {
						if (reponses[i].response)
							continue;
						buf = buf == null ? new StringBuffer(reponses[i].projectFlowId) : buf.append(",").append(reponses[i].projectFlowId);
					}
					if (buf != null)
			...
			ds.goToRow(row);
			// 添加到审核列表
			sql = combineSQL(ADD_APPROVE_SQL, "@", new String[] { projectCode, content, "", deptid, keyValue, noExecute });
	...
	public void cancelAprove(EngineDataSet ds, int row, String projectCode) throws Exception {
		synchronized (ds) {
			...
			StringBuffer checkSQL = new StringBuffer();
			checkSQL.append("SELECT COUNT(*) FROM ").append(dsTemp.getValue("dybm"));
			checkSQL.append(" WHERE ").append(dsTemp.getValue("ztzdm")).append(" ='").append(dsTemp.getValue("zttgz")).append("'");
			checkSQL.append(" AND ").append(keyColumn).append("='").append(keyValue).append("'");
			if (!dataSetProvider.getSequence(checkSQL.toString()).equals("1")) {
				dsTemp.closeDataSet();
