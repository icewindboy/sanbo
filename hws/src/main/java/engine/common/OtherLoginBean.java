 ...
 * @version 1.0
 */
public final class OtherLoginBean extends CommonClass {
	...
	// NODES信息
	private static Log log = new Log("OtherLoginBean");

	private boolean isNeedChange = false; // 是否需要更改密码
	...
	private OtherUser user = new OtherUser();// 当前用户信息
	 ...
	 * @return 返回往来单位信息的实例
	 */
	public static OtherLoginBean getInstance(HttpServletRequest request) {
		OtherLoginBean otherLoginBean = null;
		HttpSession session = request.getSession(true);
		synchronized (session) {
			otherLoginBean = (OtherLoginBean) session.getAttribute("otherLoginBean");
			if (otherLoginBean == null) {
				otherLoginBean = new OtherLoginBean();
				session.setAttribute("otherLoginBean", otherLoginBean);

			}
//			if (otherLoginBean.isLogin()) {
//				// OtherLoginBean.user.setLastAccessDate();
//			}
		}
		return otherLoginBean;
	}

	/**
	 * 构造函数
	 */
	private OtherLoginBean() {
		try {
			jbInit();
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 */
	private String logIn(String dwdm, String loginName, String password, boolean isEncode) throws Exception {
		try {
...
//				// 加密
				newPass = password.length() == 0 ? "" : password;
			}
			EngineDataSet tdsUserInfo = new EngineDataSet();
			tdsUserInfo.setProvider(dataSetProvider);
			tdsUserInfo.setQueryString("{CALL pck_login.getOtherUserInfo(?,'" + dwdm + "','" + loginName + "','" + newPass + "')}");
			...
			if (isLogin) {
				...
				user.setDwdm(tdsUserInfo.getValue("dwdm"));
				user.setName(tdsUserInfo.getValue("xm"));
				user.setLoginPassword(tdsUserInfo.getValue("loginPassword"));
				user.setLoginName(tdsUserInfo.getValue("loginName"));
				user.setDwmc(tdsUserInfo.getValue("dwmc"));
				user.setDwtxId(tdsUserInfo.getValue("DWTXID"));
				user.setFilialeId("0");
				user.setUserId(tdsUserInfo.getValue("LXRID"));
				user.setDeptId(tdsUserInfo.getValue("deptid"));
				user.setTypes(tdsUserInfo.getValue("types"));
				user.login();
	...
	public static String doPost(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		otherLoginBean = OtherLoginBean.getInstance(request);
		synchronized (otherLoginBean) {
			// OtherLoginBean.user.setIp(request.getRemoteAddr());
			// OtherLoginBean.user.setUserAgent(request.getHeader("User-Agent"));
			//
			switch (operate) {
			case LOGIN:
				...
				if (dwdm == null || user == null || password == null || "".equals(user) || "".equals(dwdm))
					return sRe;
				...
				if (!code.equals(innerCode)) {
					...
					try {
						message = otherLoginBean.logIn(dwdm, user, password, isEncode);
					} catch (Exception ex) {
						log.error("login system fail! user:" + user, ex);
					}
				}

				if (otherLoginBean.user.isLogin()) {
//					otherLoginBean.isNeedChange = password.getBytes().length < 6;
					try {
						int screen = Integer.parseInt(request.getParameter("screen"));
						// OtherLoginBean.user.setScreen(screen);
					} catch (Exception e) {
					}
				} else
			...
			case CHANGE_PASS:
				...
				sRe = otherLoginBean.changepass(oldpass, newpass, cofirmpass, isEncode_change);
				break;
			//
			case CHANGE_FILIALE:
				otherLoginBean.cdsAllFiliale.goToRow(Integer.parseInt(request.getParameter("filialeRow")));
	...
	private String changepass(String oldpass, String newpass, String confirm, boolean isEncode) {
		...
		if (!oldpass.equals(user.getLoginPassword()))
		...
		String sRetu = null;
		// try {
		// dataSetResolver.updateQuery(new String[] {
		// "CALL pck_login.changepass('" + getUserID().toString() + "','" +
		// newpass + "')" });
		// this.user.setLoginPassword(newpass);
		// this.isNeedChange = false;
		// sRetu = "ok";
		// } catch (Exception ex) {
		// log.error("changepass fail!", ex);
		// sRetu = showMessage(ex.getMessage(), true) +
		// showJavaScript("alert('更改密码失败！')");// "fail";
		// }
		return sRetu;
	}
	...
	public boolean isLogin(HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
		...
		if (!user.isLogin()) {
			response.sendRedirect(context + "/associate/login.jsp?errorMessage=nologin");
			return false;
		}
		// 如果需要更改密码
