<%--生产领料单单选生产加工单--%><%@ page contentType="text/html;charset=gb2312"%><%@ include file="../pub/init.jsp"%>
...
%>
<%
  engine.erp.store.shengyu.B_DrawSingleProcess drawSingleProcessBean = engine.erp.store.shengyu.B_DrawSingleProcess.getInstance(request);
  ...
  String retu = drawSingleProcessBean.doService(request, response);
  ...
  String title = isout.equals("1") ? "外加工单列表" : "生产加工单列表";
  String curUrl = request.getRequestURL().toString();
  EngineDataSet list = drawSingleProcessBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%//prodBean.regData(list,"cpid");
      ...
      int i=0;
      for(; i<count; i++)   {
        String jglx = list.getValue("jglx");
    ...
    <td nowrap onClick="checkRadio(<%=i%>)" id="dwmc_<%=i%>" class="td"><%=corpBean.getLookupName(list.getValue("dwtxid"))%></td>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="jglx_<%=i%>" class="td"><%=jglx.equals("1") ? "分切" : "通用"%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="wlsl_<%=i%>" class="td"><%=list.getValue("wlsl")%></td>
