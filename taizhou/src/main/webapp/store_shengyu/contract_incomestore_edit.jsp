...
<%String retu = storeOutBillBean.doService(request, response);
  ...
  engine.project.LookUp buyInBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_STORE_IN_SALE);//LookUp外贸入库单单据类别
  ...
  EngineDataSet ds = storeOutBillBean.getMaterTable();//得到主表
  EngineDataSet list = storeOutBillBean.getDetailTable();//得到从表
  ...
  RowMap masterRow = storeOutBillBean.getMasterRowinfo();//主表行信息
  RowMap[] detailRows= storeOutBillBean.getDetailRowinfos();//从表行信息
  ...
  boolean isShowPrice = loginBean.getSystemParam("KC_IN_SHOW_PRICE").equals("1");//得到是否可以手工增加的系统参数
  ...
  boolean isEnd = storeOutBillBean.isReport || storeOutBillBean.isApprove || (!storeOutBillBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !storeOutBillBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  ...
  <table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="center" width="760">
    <tr valign="top">
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            <td class="activeVTab">外贸入库单(<%=title%>)
            <%//当是新增的时候不显示出上一笔下一笔
              ...
              <%if (!isAtFirst)
              {%>
              <a href="#" title="到上一笔(ALT+Z)" onClick="sumitForm(<%=storeOutBillBean.PRIOR%>)">&lt</a>
              <pc:shortcut key='z' script='<%="sumitForm("+storeOutBillBean.PRIOR+")"%>'/>
             <%}%>
               <%if (!isAtLast)
              {%>
              <a href="#" title="到下一笔(ALT+X)" onClick="sumitForm(<%=storeOutBillBean.NEXT%>)">&gt</a>
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                        ...
                        <iframe id="prod_<%=i%>" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                          ...
                          <%}if(!isEnd && isHasDeptLimit){%>
                          <input name="image" class="img" type="image" title="复制当前行" onClick="sumitForm(<%=storeOutBillBean.COPYROW%>,<%=i%>)" src="../images/copyadd.gif" border="0">
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
              ...
              <input name="btnback" type="button" class="button" title="引入外贸进(退)货单(ALT+Q)"  value="引入外贸进(退)货单(Q)" style="width:140" onClick="buttonEventQ()">
              ...
              <input name="btnback" type="button" class="button" title="引入外贸进(退)货单(ALT+W)" value="引进外贸(退)货单货物(W)" style="width:155" onClick="buttonEventW()">
