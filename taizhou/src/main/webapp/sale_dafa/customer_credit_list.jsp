...
<%if(!loginBean.hasLimits("customer_credit_list", request, response))
  return;
engine.erp.sale.dafa.CustomerCreditBean xs_khxyedbean = engine.erp.sale.dafa.CustomerCreditBean.getInstance(request);
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onclick="selectRow()">
      ...
      <%if(canedit){%>
      <pc:select name='<%="xydj_"+i%>' addNull="0"    style="width:130"  >
      <%=xs_khxyedbean.listToOption(lists, opkey.indexOf(list.getValue("xydj")))%>
      </pc:select>
      <%}else{
       int index =  opkey.indexOf(list.getValue("xydj"));
       out.print(index==-1?"":opval.get(index));
      }%>
      </td>
      <%--td class="td" nowrap>
      <INPUT TYPE="TEXT" NAME="xydj_<%=i%>" VALUE="<%=list.getValue("xydj")%>" style="width:80" MAXLENGTH="<%=list.getColumn("xydj").getPrecision()%>" <%=detailClass_r%>  <%=readonly%>  onKeyDown="return getNextElement();" >
      </td--%>
      ...
      <TR>
        <TD>
            <TR>
              ...
              <td class="td" nowrap>
              <pc:select name="xydj" addNull="1"    style="width:130"  >
              <%=xs_khxyedbean.listToOption(lists, opkey.indexOf(xs_khxyedbean.getFixedQueryValue("xydj")))%>
              </pc:select>
              </td>
            </TR>
             <TR>
