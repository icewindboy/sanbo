...
<%
  ...
  if(!loginBean.hasLimits(pageCode, request, response))
    return;
  engine.erp.sale.dafa.B_SalerPrix b_SalerPrixBean = engine.erp.sale.dafa.B_SalerPrix.getInstance(request);
  ...
  String rll = b_SalerPrixBean.getTrueRll();//得到日利率

%>
<%
  String retu = b_SalerPrixBean.doService(request, response);
...
<%
   ...
   EngineDataSet tmp=b_SalerPrixBean.getDetailTable();//工资数据
   EngineDataSet list = b_SalerPrixBean.getMaterTable();//主表
   ...
   */
   ...
   String t="sumitForm("+b_SalerPrixBean.MONTH_CHANGE+",-1)";//saleOrderBean.DETAIL_CHANGE;
   String djxz = "";//row.get("djxz");
   String jjzbid = b_SalerPrixBean.getJjzbid();
...
<table id="tbcontrol" width="100%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      ...
      <td class="td" >
      <pc:select name="month" style="width:50" value="<%=b_SalerPrixBean.getMonth()%>" onSelect="<%=t%>">
      ...
      <%if(sfjz.equals("0")&&editandadd){
       String f = "if(confirm('完成后不能修改,确认要完成吗?')) sumitForm("+b_SalerPrixBean.OVER+",-1)";
       String cal = "sumitForm("+b_SalerPrixBean.CALCULATE+",-1)";
       String sav = "sumitForm("+Operate.POST+",-1)";
       String r = "sumitForm("+b_SalerPrixBean.GET+",-1)";
       %>
      <input name="search" type="button" style="width:80" class="button" onClick="sumitForm(<%=b_SalerPrixBean.GET%>,-1)" value="取业务员(R)">
      <pc:shortcut key="r" script='<%=r%>'/>
      <input name="search" type="button" class="button" style="width:50" onClick="if(confirm('完成后不能修改,确认要完成吗?')) sumitForm(<%=b_SalerPrixBean.OVER%>,-1)" value="完成(W)">
      <pc:shortcut key="w" script='<%=f%>'/>
      <input name="search" type="button" class="button" style="width:50" onClick="sumitForm(<%=b_SalerPrixBean.CALCULATE%>,-1)" value="计算(E)">
      ...
      <%}%>
       <%String ret = "location.href='"+b_SalerPrixBean.retuUrl+"'";%>
      <input name="button2" type="button" class="button"  style="width:50" onClick="location.href='<%=b_SalerPrixBean.retuUrl%>'" value="返回(C)">
        ...
        <TR>
          <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <td noWrap class="td">
              <input type="text" name="nf" value='<%=b_SalerPrixBean.getFixedQueryValue("nf")%>' maxlength='6' style="width:80" class="edbox">
              </td>
            <td noWrap class="td" align="center">月份</td>
           <td class="td" ><%String yf =b_SalerPrixBean.getFixedQueryValue("yf"); %>
            <pc:select name="yf" style="width:80" value="<%=yf%>" >
             ...
             <td noWrap class="td" align="center">部门</td>
              <td noWrap class="td">
              <%
                String deptid = b_SalerPrixBean.getFixedQueryValue("deptid");
                String personid = b_SalerPrixBean.getFixedQueryValue("personid");
              ...
              <pc:select name="deptid" addNull="1" style="width:110"  onSelect="<%=onChange%>" >
                <%=deptBean.getList(b_SalerPrixBean.getFixedQueryValue("deptid"))%>
              </pc:select>
              </td>
              <td noWrap class="td" align="center">姓名</td>
              <td noWrap class="td">
              <pc:select name="personid" style="width:110">
                    <%=personBean.getList(b_SalerPrixBean.getFixedQueryValue("personid"), "deptid",b_SalerPrixBean.getFixedQueryValue("deptid"))%>
              </pc:select>
              </td>
            </tr>
            <TR>
...
function cal(n)
{
 ...
 for(int k=0;k<dsField.getRowCount();k++)
 {
   ...
   out.println("jj.value=formatPrice("+b_SalerPrixBean.getGS()+");");
   %>
  }
