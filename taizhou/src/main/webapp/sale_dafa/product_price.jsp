...
if(!loginBean.hasLimits("product_price", request, response))
    return;
  engine.erp.sale.dafa.B_ProductPrice b_productpricebean = engine.erp.sale.dafa.B_ProductPrice.getInstance(request);//得到实例(初始化实例变量)
...
<SCRIPT language="javascript">
  ...
  function sumitFixedQuery(oper)
 {
  ...
  function showFixedQuery()
   ...
   {
     var url = "product_price_edit.jsp?operate="+oper+"&rownum="+rownum;
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    %>
     <tr <%=onlclick%>>
      <td class="td" nowrap align="left"><%=row.get("cpbm")%>
    <INPUT TYPE="hidden" NAME="wzdjid_<%=i%>"  VALUE="<%=row.get("wzdjid")%>">
   </td><!--品名 规格-->
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
                ...
                <INPUT class="edbox" style="WIDTH:120" id="cpid$pm" name="cpid$pm" value='<%=b_productpricebean.getFixedQueryValue("cpid$pm")%>' maxlength='50' onKeyDown="return getNextElement();"  onchange="productNameSelect(this)">
                <INPUT class="edline" style="WIDTH:120" id="cpid$gg" name="cpid$gg" value='<%=b_productpricebean.getFixedQueryValue("cpid$gg")%>' maxlength='50' onKeyDown="return getNextElement();" readonly>
...
<div class="queryPop" id="detailDiv" name="detailDiv">
  ...
  <TABLE cellspacing=0 cellpadding=0 border=0>
    <TR>
      <TD><iframe id="interframe1" src="" width="400" height="400" marginWidth="0" marginHeight="0" frameBorder="0" noResize scrolling="no"></iframe>
      </TD>
    </TR>
  </TABLE>
</div>
