...
<%String retu = receiveMaterialBean.doService(request, response);
  ...
  String show = Type.equals("1") ? "外加工发料单" : "外加工退料单";
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                        ...
                        <%--td class="td" align="center" nowrap><input type="text" <%=detailClass%>  onKeyDown="return getNextElement();" id="batchno_<%=i%>" name="batchno_<%=i%>" value='<%=detail.get("batchno")%>' maxlength='<%=list.getColumn("batchno").getPrecision()%>' onchange="if(form1.cpid_<%=i%>.value==''){alert('请输入产品');return;}if(form1.storeid.value==''){alert('请输入产品'); return;}BatchMultiSelect('form1','srcVar=mutibatch_<%=i%>&cpid='+form1.cpid_<%=i%>.value+'&storeid='+form1.storeid.value+'&dmsxid='+form1.dmsxid_<%=i%>.value+'&ph='+form1.batchno_<%=i%>.value, true, <%=i%>, false)" <%=readonly%>></td--%>
...
function onload(){
    <%=receiveMaterialBean.adjustInputSize(new String[]{"cpbm","product", "jldw", "batchno", "drawnum", "drawprice", "drawsum","sxz", "memo","drawbignum","producenum","hsdw", "scydw"},  "form1", (receiveMaterialBean.max-min+1), min)%>
}
  ...
  //打印
    ...
    {
      location.href='../pub/pdfprint.jsp?code=process_issue_edit_bill&operate=<%=receiveMaterialBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("drawid")%>&src=../store/process_issue_edit.jsp';
    }
    //新增
