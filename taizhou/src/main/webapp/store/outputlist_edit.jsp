      ...
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            ...
            <%//当是新增的时候不显示出上一笔下一笔
              if (isPriorNext)
              {
                ds.goToInternalRow(saleOutStoreBean.getMasterRow());
                boolean isAtFirst = ds.atFirst();boolean isAtLast = ds.atLast();
              %>
              <%if (!isAtFirst)
              {%>
              <a href="#" title="到上一笔(ALT+Z)" onClick="sumitForm(<%=saleOutStoreBean.PRIOR%>)">&lt</a>
              <pc:shortcut key='z' script='<%="sumitForm("+saleOutStoreBean.PRIOR+")"%>'/>
             <%}%>
               <%if (!isAtLast)
              {%>
              <a href="#" title="到下一笔(ALT+X)" onClick="sumitForm(<%=saleOutStoreBean.NEXT%>)">&gt</a>
              <pc:shortcut key='x' script='<%="sumitForm("+saleOutStoreBean.NEXT+")"%>'/>
             <%}
              }%>
   </td>
          </tr>
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
              <%if(!isEnd){%>
              <input type="hidden" name="singleSelectLadding" value="引入提单" ><%--02.16 修改 将 button显示的名称改为 引入提货单 yjg--%>
              <input name="btnback" type="button" class="button"  title="引入提货单(Q)" value="引入提货单(Q)" style="width:90" onClick="buttonEventQ()">
