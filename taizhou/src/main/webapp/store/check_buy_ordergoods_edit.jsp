...
function backList()
{
  location.href='check_buy_ordergoods.jsp';
}
function corpCodeSelect(obj)
{
  ProvideCodeChange(document.all['prod'], obj.form.name, 'srcVar=dwtxid&srcVar=dwdm&srcVar=dwmc',
                 'fieldVar=dwtxid&fieldVar=dwdm&fieldVar=dwmc', obj.value, 'sumitForm(<%=checkBuyOrderGoodsBean.ONCHANGE%>)');
}
...
<%String retu = checkBuyOrderGoodsBean.doService(request, response);
  ...
  EngineDataSet ds = checkBuyOrderGoodsBean.getMaterTable();
  ...
  HtmlTableProducer masterProducer = checkBuyOrderGoodsBean.masterProducer;
  ...
  RowMap[] detailRows= checkBuyOrderGoodsBean.getDetailRowinfos();
  ...
  if(checkBuyOrderGoodsBean.isApprove)
  {
  ...
  boolean isEnd = true;//checkBuyOrderGoodsBean.isRep || checkBuyOrderGoodsBean.isApprove || (!checkBuyOrderGoodsBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !checkBuyOrderGoodsBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(checkBuyOrderGoodsBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  boolean isAdd = checkBuyOrderGoodsBean.isDetailAdd;
  boolean  isRead = checkBuyOrderGoodsBean.isRep || checkBuyOrderGoodsBean.isApprove || (!checkBuyOrderGoodsBean.masterIsAdd() && (zt.equals("9") || zt.equals("2")));//表示在审批中或已入库不能修改
  isRead = isRead || !(checkBuyOrderGoodsBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0">
                ...
                <tr>
                  ...
                  <td noWrap class="td">
                    <%String onChange = "if(form1.deptid.value!='"+masterRow.get("deptid")+"')sumitForm("+checkBuyOrderGoodsBean.DEPT_CHANGE+")";%>
                ...
                <tr>
                  ...
                  <%if(!isEnd && isCanAmend && isHasDeptLimit){%>
                  <img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=checkBuyOrderGoodsBean.ONCHANGE%>)');"><img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';sumitForm(<%=checkBuyOrderGoodsBean.ONCHANGE%>)">
                  <%}%>
                  </td>
                      ...
                      <tr class="tableTitle">
                        ...
                        <%prodBean.regData(list,"cpid");
                          ...
                          for(; i<detailRows.length; i++)   {
                            ...
                            if(checkBuyOrderGoodsBean.isDouble(sjrkl))
                        ...
                        <tr id="rowinfo_<%=i%>">
                         ...
                         %>
                          ...
                          <td class="td" nowrap><input type="text" class=ednone onKeyDown="return getNextElement();" id="_<%=i%>" name="th_<%=i%>" value='<%=prodRow.get("th")%>'  readonly></td>
                          ...
                          <td class="td" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=sl%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onchange="sl_onchange(<%=i%>, false)" readonly></td>
                          
                          
                          
                          <td class="td" nowrap>
                          
                          <DIV ID='d2_check_result_<%=i%>' class=edFocused style='width:5;' onClick=ToggleSelect('check_result_<%=i%>')>
                          <table border=0 cellspacing=0 cellpadding=0><tr><td nowrap><INPUT CLASS='ednone' NAME='v_check_result_<%=i%>' onKeyDown="return SelectKeyDown('check_result_<%=i%>');" onKeyUp='SelectKeyUp(this.value);' onChange="SelectChange('check_result_<%=i%>');" value=''></td><td nowrap><IMG SRC='../images/down_arrow.gif' style='cursor:hand;' border=0></td></tr></table>
                          <INPUT TYPE=HIDDEN NAME='check_result_<%=i%>' link='v_check_result_<%=i%>' value=''>
                          </DIV>
                        <script language='javascript'>
                            o_check_result_<%=i%>=new TSelectObject('check_result_<%=i%>','check_result_<%=i%>','v_check_result_<%=i%>','d1_check_result_<%=i%>','d2_check_result_<%=i%>',false,null,true);
                            RegisterSelect(o_check_result_<%=i%>);
                           SetDestSelectObject(o_check_result_<%=i%>);AddSelectItem('','');
                           AddSelectItem('A','合格');
                           AddSelectItem('B','挑选');
                           AddSelectItem('C','让步');
                           AddSelectItem('D','退货');
                           SetSelectedIndex('<%=detail.get("check_result")%>');
                        </script>
                  </td>
                          <td class="td" nowrap><input type="text" class=edFocused_r onKeyDown="return getNextElement();" id="check_number_<%=i%>" name="check_number_<%=i%>" value='<%=detail.get("check_number")%>' maxlength='<%=list.getColumn("check_number").getPrecision()%>' ></td>
                         <td class="td" nowrap><input type="text" class=edbox onKeyDown="return getNextElement();" id="check_explain_<%=i%>" name="check_explain_<%=i%>" value='<%=detail.get("check_explain")%>' maxlength='<%=list.getColumn("check_explain")%>' ></td>
                          <td class="td" nowrap>
                           <DIV ID='d2_check_permit_<%=i%>' class=edFocused style='width:2;' onClick=ToggleSelect('check_permit_<%=i%>')>
                          <table border=0 cellspacing=0 cellpadding=0><tr><td nowrap><INPUT CLASS='ednone' NAME='v_check_permit_<%=i%>' onKeyDown="return SelectKeyDown('check_permit_<%=i%>');" onKeyUp='SelectKeyUp(this.value);' onChange="SelectChange('check_permit_<%=i%>');" value=''></td><td nowrap><IMG SRC='../images/down_arrow.gif' style='cursor:hand;' border=0></td></tr></table>
                          <INPUT TYPE=HIDDEN NAME='check_permit_<%=i%>' link='v_check_permit_<%=i%>' value=''>
                          </DIV>
                        <script language='javascript'>
                            o_check_permit_<%=i%>=new TSelectObject('check_permit_<%=i%>','check_permit_<%=i%>','v_check_permit_<%=i%>','d1_check_permit_<%=i%>','d2_check_permit_<%=i%>',false,null,true);
                            RegisterSelect(o_check_permit_<%=i%>);
                           SetDestSelectObject(o_check_permit_<%=i%>);AddSelectItem('','');
                           AddSelectItem('1','是');
                           AddSelectItem('-1','否');
                           SetSelectedIndex('<%=detail.get("check_permit")%>');
                        </script>
                        </td>
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             <%if(!isEnd){%>
              ...
              <input name="btnback" class="button" type="button" value="引入合同(W)" style="width:100" onClick="if(form1.storeid.value==''){alert('请选择仓库');return;}OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=checkBuyOrderGoodsBean.SINGLE_SELECT_ORDER%>)')">
                <pc:shortcut key="w" script="importOrder();"/>
              <input type="hidden" name="importOrder" value="引合同货物" onchange="sumitForm(<%=checkBuyOrderGoodsBean.DETAIL_ORDER_ADD%>)">
              ...
              <input name="button2" type="button" class="button" title="删除数量为零行(ALT+X)" value="删除数量为零行(X)" style='width:120' onClick="sumitForm(<%=checkBuyOrderGoodsBean.DELETE_BLANK%>);">
                <pc:shortcut key="x" script='<%="sumitForm("+checkBuyOrderGoodsBean.DELETE_BLANK+")"%>'/>
              ...
              <%if(isCanDelete && isCanAmend && !checkBuyOrderGoodsBean.isRep){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);" value=" 删除(D)">
             <pc:shortcut key="d" script='delMaster();'/><%}%>
              ...
              <%if(!checkBuyOrderGoodsBean.isApprove && !checkBuyOrderGoodsBean.isRep){%><input name="btnback" type="button" class="button" onClick="backList();" value=" 返回(C)">
...
<%=checkBuyOrderGoodsBean.adjustInputSize(new String[]{"cpbm","product", "sl", "dj", "je","sxz","sjrkl","gyszyh","bz","ybje"}, "form1", detailRows.length)%>
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    function importOrder(){
     ...
     OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=checkBuyOrderGoodsBean.SINGLE_SELECT_ORDER%>)')
     }
...
<%if(checkBuyOrderGoodsBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>