...
function backList()
{
  location.href='other_instore_list.jsp';
}
function productCodeSelect(obj, i)
{
  ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsdw_'+i+'&srcVar=isprops_'+i+'&storeid='+form1.storeid.value,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=isprops', obj.value,'product_change('+i+')');
}
function corpCodeSelect(obj)
{
  ...
  CustomerCodeChange('1',document.all['prod'], obj.form.name, 'srcVar=dwtxid&srcVar=dwdm&srcVar=dwmc',
                 'fieldVar=dwtxid&fieldVar=dwdm&fieldVar=dwmc', obj.value, 'sumitForm(<%=otherInStoreBean.ONCHANGE%>)');
  //ProvideCodeChange(document.all['prod'], obj.form.name, 'srcVar=dwtxid&srcVar=dwdm&srcVar=dwmc',
                 //'fieldVar=dwtxid&fieldVar=dwdm&fieldVar=dwmc', obj.value, 'sumitForm(<%=otherInStoreBean.ONCHANGE%>)');
}
function productNameSelect(obj,i)
{
   ProdNameChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i+'&srcVar=hsdw_'+i+'&srcVar=isprops_'+i+'&storeid='+form1.storeid.value,
            'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw&fieldVar=hsdw&fieldVar=isprops', obj.value,'product_change('+i+')');
}
...
<%String retu = otherInStoreBean.doService(request, response);
  ...
  EngineDataSet ds = otherInStoreBean.getMaterTable();//得到主表
  EngineDataSet list = otherInStoreBean.getDetailTable();//得到从表
  HtmlTableProducer masterProducer = otherInStoreBean.masterProducer;
  HtmlTableProducer detailProducer = otherInStoreBean.detailProducer;
  RowMap masterRow = otherInStoreBean.getMasterRowinfo();//主表行信息
  RowMap[] detailRows= otherInStoreBean.getDetailRowinfos();//从表行信息
  ...
  if(otherInStoreBean.isApprove)
  {
    corpBean.regData(ds, "dwtxid");
    personBean.regData(ds, "personid");
  }
  boolean isEnd = otherInStoreBean.isReport || otherInStoreBean.isApprove || (!otherInStoreBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !otherInStoreBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete)
                        && loginId.equals(zdrid);//没有结束,在修改状态,并有删除权限,2004-08-04 并且登陆人等于制单人
  isEnd = isEnd
          || !(otherInStoreBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit))
  ...
  boolean isAdd = otherInStoreBean.isDetailAdd;
  ...
  <table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="center" width="760">
    <tr valign="top">
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            <td class="activeVTab">其它入库单(<%=title%>)
            <%
              boolean isAtFirst = ds.atFirst();boolean isAtLast = ds.atLast();
              %>
              <%if (!isAtFirst)
              {%>
              <a href="#" title="到上一笔(ALT+Z)" onClick="sumitForm(<%=otherInStoreBean.PRIOR%>)">&lt</a>
              <pc:shortcut key='z' script='<%="sumitForm("+otherInStoreBean.PRIOR+")"%>'/>
             <%}%>
               <%if (!isAtLast)
              {%>
              <a href="#" title="到上一笔(ALT+X)" onClick="sumitForm(<%=otherInStoreBean.NEXT%>)">&gt</a>
              <pc:shortcut key='x' script='<%="sumitForm("+otherInStoreBean.NEXT+")"%>'/>
             <%}%>
            </td>
          </tr>
        </table>
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0">
                ...
                %>
                  ...
                  <td  noWrap class="td">
                    <%String sumit = "if(form1.storeid.value!='"+masterRow.get("storeid")+"'){sumitForm("+otherInStoreBean.STORE_ONCHANGE+");}";%>
                ...
                <%RowMap corpRow = corpBean.getLookupRow(masterRow.get("dwtxid"));%>
                  <td  noWrap class="td" colspan="3"><input type="text" name="dwdm" value='<%=corpRow.get("dwdm")%>' style="width:60" <%=edClass%> onKeyDown="return getNextElement();" onchange="corpCodeSelect(this);" <%=readonly%>>
                   ...
                   <input type="text" name="dwmc" value='<%=corpBean.getLookupName(masterRow.get("dwtxid"))%>' style="width:180" <%=edClass%> onKeyDown="return getNextElement();" onchange="corpNameSelect(this);" <%=readonly%>>
                    <%if(!isEnd && isHasDeptLimit){%>
                    <img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=otherInStoreBean.ONCHANGE%>)');">
                    <img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';sumitForm(<%=otherInStoreBean.ONCHANGE%>)">
                    <%}%>
                  </td>
                  ...
                  <td  noWrap class="td">
                    <%if(isEnd || !isHasDeptLimit) out.print("<input type='text' value='"+buyInBean.getLookupName(masterRow.get("sfdjlbid"))+"' style='width:110' class='edline' readonly>");
                    else {%>
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      for(; i<detailRows.length; i++)   {
                        ...
                        if(otherInStoreBean.isDouble(je))
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                        ...
                        <iframe id="prod_<%=i%>" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                          ...
                          <%}if(!isEnd && isHasDeptLimit){%>
                          <input name="image" class="img" type="image" title="复制当前行" onClick="sumitForm(<%=otherInStoreBean.COPYROW%>,<%=i%>)" src="../images/copyadd.gif" border="0">
                        ...
                        <%if(!isEnd){%>
                        <img style='cursor:hand' src='../images/view.gif' border=0 onClick="if(form1.cpid_<%=i%>.value==''){alert('请先输入产品');return;}if(form1.isprops_<%=i%>.value=='0'){alert('该物资没有规格属性');return;}PropertySelect('form1','dmsxid_<%=i%>','sxz_<%=i%>',form1.cpid_<%=i%>.value)">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             <%if(!isEnd && isHasDeptLimit){%>
              ...
              <%if(isCanDelete && isHasDeptLimit && !otherInStoreBean.isReport){%>
             ...
             <%}%>
              <%if(!otherInStoreBean.isApprove && !otherInStoreBean.isReport){%>
...
<%=otherInStoreBean.adjustInputSize(new String[]{"cpbm","product","jldw", "ph", "hssl", "sl", "dj", "je", "sxz", "bz", "kwid"},  "form1", detailRows.length)%>
  ...
  function OrderGoodsMultiSelect(frmName, srcVar, methodName,notin)
 {
  ...
  function transferScan()//调用盘点机
  {
    ...
    if(scanValueObj.value=='')
      return;
    sumitForm(<%=otherInStoreBean.TRANSFERSCAN%>);
  }
  //盘点机事件.
 ...
 {
    location.href='../pub/pdfprint.jsp?code=other_instore_edit_bill&operate=<%=otherInStoreBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("sfdjid")%>&src=../store/other_instore_edit.jsp';
 }
//新增
...
<%if(otherInStoreBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/>
<%}%>
<%out.print(retu);%>
</BODY>
</Html>