...
<input name="button2222232" type="button" align="Right" class="button" onClick="location.href='<%=paymentBean.retuUrl%>'" value="返回(C)" border="0"><%}%></TD>
  ...
  <td class="td" nowrap>
    ...
    <td  nowrap <%=rowClass%>>&nbsp;<%=currencyBean.getLookupName(list.getValue("wbid"))%></td><%--外币--%>
    <td  nowrap <%=rowClass%>>&nbsp;<%=list.getValue("hl")%></td><%--汇率--%>
    <td  nowrap <%=rowClass%>>&nbsp;<%=list.getValue("je")%></td><%--金额--%>
    <td  nowrap <%=rowClass%>>&nbsp;<%=list.getValue("ybje")%></td><%--外币金额--%>
    ...
    <%=state%></a></td><%--审批状态--%>
    <td nowrap <%=rowClass%>>&nbsp;<%=list.getValue("fgsiD")%></td><%-- 分公司--%>
