...
%>
<%
  engine.erp.finance.Select_LadingBill_Product select_LadingBill_ProductBean = engine.erp.finance.Select_LadingBill_Product.getInstance(request);
  ...
  String retu = select_LadingBill_ProductBean.doService(request, response);
  ...
  EngineDataSet list = select_LadingBill_ProductBean.getOneTable();
...
<%
  if(inputName != null && fieldName != null)
  {
    ...
    for(int i=0; i< length; i++)
    {
      ...
      out.print(".value=");     out.println("obj."+ (fieldName[i].equals("tdhwid")?"value;":"innerText;"));
    }
  }
  if(methodName != null)
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">提单货物选择</TD>
  </TR>
</TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkTr(<%=i%>);checkRadio(<%=i%>);selectCorp(<%=i%>);"<%}%>>
      <td class="td">
        <%if(isMultiSelect){%>
        <input type="checkbox" name="sel" value="<%=list.getValue("tdhwid")%>" onKeyDown="return getNextElement();">
        <%}else{%>
        <input type="radio" name="sel" onKeyDown="return getNextElement();" value="<%=i%>"<%=i==0?" checked":""%>>
        <%}%><input type="hidden" id="tdhwid_<%=i%>" value='<%=list.getValue("tdhwid")%>'>
      </td>
      ...
      <td nowrap align="center" id="zsl_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("zsl")%></td>
      <td nowrap align="center" id="sl_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=list.getValue("sl")%></td>
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="tdbh" name="tdbh" value='<%=select_LadingBill_ProductBean.getFixedQueryValue("tdbh")%>' maxlength='10' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>购货单位</TD>
              <TD nowrap class="td">
                <%if(twldw.equals("")){%>
                <input type="hidden" name="dwtxid" value='<%=select_LadingBill_ProductBean.getFixedQueryValue("dwtxid")%>'>
                <input type="text" name="buyerName" value='<%=select_LadingBill_ProductBean.getFixedQueryValue("buyerName")%>' style="width:130" class="edline" readonly>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="tdrq$a" value='<%=select_LadingBill_ProductBean.getFixedQueryValue("tdrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="tdrq$b" value='<%=select_LadingBill_ProductBean.getFixedQueryValue("tdrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <%=deptBean.getList(select_LadingBill_ProductBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>提单类型</TD>
              <TD nowrap class="td">
                <pc:select name="djlx" addNull="1" style="width:130" >
                <%=select_LadingBill_ProductBean.listToOption(lists, opkey.indexOf(select_LadingBill_ProductBean.getFixedQueryValue("djlx")))%>
               </pc:select>
              </TD>
             </TR>
            <TR>
