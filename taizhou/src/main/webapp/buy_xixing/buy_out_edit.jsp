...
sumitForm(<%=buyOutBean.MATERIAL_ON_CHANGE%>,i)
...
<%String retu = buyOutBean.doService(request, response);
  ...
  EngineDataSet ds = buyOutBean.getMaterTable();
  ...
  HtmlTableProducer masterProducer = buyOutBean.masterProducer;
  HtmlTableProducer detailProducer = buyOutBean.detailProducer;
  RowMap masterRow = buyOutBean.getMasterRowinfo();
  RowMap[] detailRows= buyOutBean.getDetailRowinfos();
  ...
  boolean isEnd = buyOutBean.isRep || buyOutBean.isApprove || (!buyOutBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !buyOutBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(buyOutBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  boolean isEndSl = buyOutBean.isRep || buyOutBean.isApprove || (!buyOutBean.masterIsAdd() && zt.equals("8"));//数量只有在完全入库后才不能修改
  isEndSl = isEndSl || !(buyOutBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  boolean isAdd = buyOutBean.isDetailAdd;
  boolean  isRead = buyOutBean.isRep || buyOutBean.isApprove || (!buyOutBean.masterIsAdd() && (zt.equals("9") || zt.equals("8")));//表示在审批中或已入库不能修改
  isRead = isRead || !(buyOutBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0">
                ...
                <tr>
                  ...
                  <td noWrap class="td">
                    <%String store_Change = "if(form1.storeid.value!='"+masterRow.get("storeid")+"')sumitForm("+buyOutBean.STORE_CHANGE+")";%>
                  ...
                  <td noWrap class="td">
                    <%String onChange = "if(form1.deptid.value!='"+masterRow.get("deptid")+"')sumitForm("+buyOutBean.DEPT_CHANGE+")";%>
                ...
                <tr>
                  ...
                  <%if(!isEnd && isCanAmend && isHasDeptLimit){%>
                  <img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=buyOutBean.ONCHANGE%>)');"><img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';sumitForm(<%=buyOutBean.ONCHANGE%>)">
                  <%}%>
                  </td>
                ...
                <tr>
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                      ...
                      <%--    <td nowrap><%=detailProducer.getFieldInfo("sjrkl").getFieldname()%></td>--%>
                          <td height='20' nowrap>实际出库量</td>
                          <td height='20' nowrap>实际出库换算量</td>
                        ...
                        <%
                          ...
                          for(; i<detailRows.length; i++)   {
                            ...
                            //BigDecimal je = s_je.length() > 0 ? (new BigDecimal(s_je)) : new BigDecimal(0),t_hssl = new BigDecimal(0);
                        ...
                        <tr id="rowinfo_<%=i%>">
                         ...
                         <iframe id="prod_<%=i%>" src="" width="95%" height=25 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no style="display:none"></iframe>
                          ...
                          <%}if(!isEnd && isCanRework && isHasDeptLimit){%>
                           <input name="image" class="img" type="image" title="复制当前行" onClick="if(form1.cpid_<%=i%>.value==''){alert('请输入产品');return;}sumitForm(<%=buyOutBean.DETAIL_COPY%>,<%=i%>)" src="../images/copyadd.gif" border="0">
                        ...
                        <%--  <td class="td" nowrap><input type="text" <%=isHsbj ? detailClass :"class=ednone_r"%> onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=sl%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onchange="sl_onchange(<%=i%>, false)" <%=isHsbj ? "readonly" : readonly%>></td>--%>
                         ...
                         <td class="td" nowrap><input type="text" <%=detailClass_r%> onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=hssl%>' maxlength='<%=list.getColumn("hssl").getPrecision()%>' onchange="hssl_onchange(<%=i%>, false)" <%=readonly%>></td>
                          ...
                          <td class="td" nowrap><input type="text"<%=detailClass_rsl%> onKeyDown="return getNextElement();" id="dj_<%=i%>" name="dj_<%=i%>" value='<%=detail.get("dj")%>'  maxlength='<%=list.getColumn("dj").getPrecision()%>'  onchange="dj_onchange(<%=i%>, false)" <%=slreadonly%>></td>
                          ...
                          <td class="td" align="right" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="sjrkl_<%=i%>" name="sjrkl_<%=i%>" value='<%=detail.get("sjrkl")%>' maxlength='<%=list.getColumn("sjrkl").getPrecision()%>' readonly></td>
                          <td  class="td" align="right" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="sjrkhsl_<%=i%>" name="sjrkhsl_<%=i%>" value='<%=detail.get("sjrkhsl")%>' maxlength='<%=list.getColumn("sjrkhsl").getPrecision()%>' readonly></td>
                        <%-- <td class="td" nowrap><input type="text" <%=detailClass%> style="width:65" onKeyDown="return getNextElement();" name="jhrq_<%=i%>" id="jhrq_<%=i%>"value='<%=detail.get("jhrq")%>' maxlength='10'<%=readonly%> onchange="checkDate(this)"></td>--%>
                          ...
                          <td class="td" nowrap align="right"><input type="text" <%=detailClass%> onKeyDown="return getNextElement();" name="dhqk_<%=i%>" id="dhqk__<%=i%>" value='<%=detail.get("dhqk")%>' maxlength='<%=list.getColumn("dhqk").getPrecision()%>'<%=readonly%>></td>
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             <%if(!isEnd){%>
              ...
              <input name="btnback" class="button" type="button" value="引入合同(W)" style="width:100" onClick="if(form1.storeid.value==''){alert('请选择仓库');return;}OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=buyOutBean.SINGLE_SELECT_ORDER%>)')">
                <pc:shortcut key="w" script="importOrder();"/>
              <input type="hidden" name="importOrder" value="引合同货物" onchange="sumitForm(<%=buyOutBean.DETAIL_ORDER_ADD%>)">
              ...
              <input name="button2" type="button" class="button" title="删除数量为零行(ALT+X)" value="删除数量为零行(X)" style='width:120' onClick="sumitForm(<%=buyOutBean.DELETE_BLANK%>);">
                <pc:shortcut key="x" script='<%="sumitForm("+buyOutBean.DELETE_BLANK+")"%>'/>
              ...
              <%if(isCanDelete && isCanAmend && !buyOutBean.isRep&&!isEnd){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);" value=" 删除(D)">
             <pc:shortcut key="d" script='delMaster();'/><%}%>
              ...
              <%if(!buyOutBean.isApprove && !buyOutBean.isRep){%><input name="btnback" type="button" class="button" onClick="backList();" value=" 返回(C)">
...
<%=buyOutBean.adjustInputSize(new String[]{"cpbm","product", "sl", "dj", "je","sxz","sjrkl","bz","hssl","sjrkhsl","dhqk"}, "form1", detailRows.length)%>
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    function importOrder(){
     ...
     OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=buyOutBean.SINGLE_SELECT_ORDER%>)')
     }
     ...
     if(!isBigUnit){
       if(bjfs==0){
         if(slObj.value!="" && !isNaN(slObj.value && slObj.value!="0")){
           ...
           if(hsslObj.value*jeObj.value<0)
           {
       ...
       if(bjfs==0){
         if(hlObj.value!="" && !isNaN(hlObj.value) && hlObj!="0")
           jeObj.value = formatPrice(parseFloat(ybjeObj.value) * parseFloat(hlObj.value));
...
<%if(buyOutBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>