...
<%!
  ...
  String pageCode = "other_payable";
%>
<%
  if(!loginBean.hasLimits("other_payable", request, response))
    return;
  engine.erp.buy.xixing.B_OtherPayable otherPayableBean  =  engine.erp.buy.xixing.B_OtherPayable.getInstance(request);
  String retu = otherPayableBean.doService(request, response);
...
<SCRIPT LANGUAGE="javascript">
  function toDetail(oper, row){
    ...
    location.href='other_payable_edit.jsp?operate='+oper+'&rownum='+row;
  }
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>其他应付款</TD>
  </TR>
</TABLE>
<%
  ...
  EngineDataSet list = otherPayableBean.getOneTable();
  HtmlTableProducer table = otherPayableBean.masterProducer;//主表数据的表格打印
  ...
  <TABLE width="95%" BORDER=0 CELLSPACING=0 CELLPADDING=0 align="center">
    <TR>
       <td class="td" nowrap>
        ...
        <%if(otherPayableBean.retuUrl!=null){
          String s = "location.href='"+otherPayableBean.retuUrl+"'";
           %>
         <input name="button22" type="button" class="button"  style="width:60"  onClick="location.href='<%=otherPayableBean.retuUrl%>'" value=" 返回(C) " onKeyDown="return getNextElement();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      ...
      for(; i<list.getRowCount(); i++)
      {
        ...
        String loginid=otherPayableBean.loginid;
        String sprid=list.getValue("sprid");
        boolean isCancer=rowzt.equals("1") && loginid.equals(sprid);
        boolean cansubmit=false;
        boolean submitType = otherPayableBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
    ...
    <tr <%if(isCanEdit){%>onDblClick="toDetail(<%=Operate.EDIT%>,<%=list.getRow()%>)"<%}%> onClick="selectRow()" >
      ...
      <%if(isCancer){%>
         <input name="image3" class="img" type="image"  title='取消审批' onClick="if(confirm('确认要取消审批吗？'))sumitForm(<%=otherPayableBean.CANCER_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
     <%
       if(rowzt.equals("1")&&(loginBean.hasLimits(pageCode, op_over))){//&&(loginBean.hasLimits(pageCode, op_over))
      %>
         <input name="image3" class="img" type="image"   title='完成' onClick="if(confirm('确认要完成吗？'))sumitForm(<%=otherPayableBean.SALE_OVER%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
      </td>
        ...
        <TD>
         <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="otherfundno" name="otherfundno" value='<%=otherPayableBean.getFixedQueryValue("otherfundno")%>' maxlength='16' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>制单人</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="czy" name="czy" value='<%=otherPayableBean.getFixedQueryValue("czy")%>' maxlength='16' onKeyDown="return getNextElement();">
              </TD>
            </tr>
                <TR>
                  <TD align="center" nowrap class="td">客户名称</TD>
                  <td nowrap class="td" colspan=3>
                  <input type="text" class="edbox" style="width:100" onKeyDown="return getNextElement();" name="dwdm" value='<%=otherPayableBean.getFixedQueryValue("dwdm")%>' onchange="customerCodeSelect(this)" >
                  <input type="text" name="dwmc"  style="width:260" value='<%=otherPayableBean.getFixedQueryValue("dwmc")%>' class="edbox"  onchange="customerNameSelect(this)" >
                  <INPUT TYPE="HIDDEN" NAME="dwtxid" value='<%=otherPayableBean.getFixedQueryValue("dwtxid")%>'>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="otherfunddate$a" value='<%=otherPayableBean.getFixedQueryValue("otherfunddate$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="otherfunddate$b" value='<%=otherPayableBean.getFixedQueryValue("otherfunddate$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <%=deptBean.getList(otherPayableBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>业务员</TD>
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:130">
                  <%=personBean.getList(otherPayableBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             </TR>
            <TR>
              <TD nowrap class="td">经办人</TD>
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="jbr" name="jbr" value='<%=otherPayableBean.getFixedQueryValue("jbr")%>' maxlength='16' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>审批人</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="sprid$xm" name="sprid$xm" value='<%=otherPayableBean.getFixedQueryValue("sprid$xm")%>' maxlength='16' onKeyDown="return getNextElement();">
              </TD>
            </tr>
            <TR>
            ...
            <TR>
              <TD colspan="4" nowrap class="td" align="center">
                <% String qu = "sumitFixedQuery("+otherPayableBean.FIXED_SEARCH+")";%>
                <INPUT class="button" onClick="sumitFixedQuery(<%=otherPayableBean.FIXED_SEARCH%>)" type="button" value="查询(K)" name="button" onKeyDown="return getNextElement();">
