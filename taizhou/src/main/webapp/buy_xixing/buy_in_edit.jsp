...
<%@ page import="engine.dataset.*,engine.project.Operate,java.math.BigDecimal,engine.html.*,java.util.ArrayList, java.util.Date,java.text.SimpleDateFormat" %><%!
...
%><%
  engine.erp.buy.xixing.Buy_In buyInBean = engine.erp.buy.xixing.Buy_In.getInstance(request);
  String pageCode = "buy_in";
...
function corpCodeSelect(obj)
{
  ProvideCodeChange(document.all['prod'], obj.form.name, 'srcVar=dwtxid&srcVar=dwdm&srcVar=dwmc',
                 'fieldVar=dwtxid&fieldVar=dwdm&fieldVar=dwmc', obj.value, 'sumitForm(<%=buyInBean.ONCHANGE%>)');
}
...
function Add()
{
  ...
  sumitForm(<%=buyInBean.MATERIAL_ON_CHANGE%>,i)
...
<%String retu = buyInBean.doService(request, response);
  ...
  EngineDataSet ds = buyInBean.getMaterTable();
  EngineDataSet list = buyInBean.getDetailTable();
  ...
  //if ( !SYS_CUST_NAME.equals("essen") ) bjfs = "1";//如果是essen的话,那么程序就一没有换算数量,一直用计量单位来报价
  boolean isHsbj = bjfs.equals("1");//如果等于1就以换算单位报价
  HtmlTableProducer masterProducer = buyInBean.masterProducer;
  HtmlTableProducer detailProducer = buyInBean.detailProducer;
  RowMap masterRow = buyInBean.getMasterRowinfo();
  RowMap[] detailRows= buyInBean.getDetailRowinfos();
  ...
  boolean isEnd = buyInBean.isRep || buyInBean.isApprove || (!buyInBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !buyInBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(buyInBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));

  String loginid= buyInBean.loginId;
  isEnd = isEnd||!czyid.equals(loginid);

  FieldInfo[] mBakFields = masterProducer.getBakFieldCodes();//主表用户的自定义字段
  String edClass = (isEnd || !isHasDeptLimit||!isCanAmend) ? "class=edline" : "class=edbox";
  ...
  boolean isEndSl = buyInBean.isRep || buyInBean.isApprove || (!buyInBean.masterIsAdd() && zt.equals("8"));//数量只有在完全入库后才不能修改
  isEndSl = isEndSl || !(buyInBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  String detailClass_rsl = (isEndSl || !isHasDeptLimit) ? "class=ednone_r" : "class=edFocused_r";
  String slreadonly = (isEndSl || !isHasDeptLimit) ? " readonly" : "";
  String masterReadonly = isCanAmend ? readonly : "readonly";
  //String needColor = isEnd ? "" : " style='color:#660000'";
  String title = zt.equals("1") ? ("已审核"/* 审核人:"+ds.getValue("shr")*/) : (zt.equals("8") ? "已入库" : (zt.equals("9") ? "审批中" : "未审核"));
  String head = djlx.equals("1") ? "采购进货单" : "采购退货单";
  boolean isAdd = buyInBean.isDetailAdd;
  boolean  isRead = buyInBean.isRep || buyInBean.isApprove || (!buyInBean.masterIsAdd() && (zt.equals("9") || zt.equals("8")));//表示在审批中或已入库不能修改
  isRead = isRead || !(buyInBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0">
                ...
                <tr>
                  ...
                  <td noWrap class="td">
                    <%String store_Change = "if(form1.storeid.value!='"+masterRow.get("storeid")+"')sumitForm("+buyInBean.STORE_CHANGE+")";%>
                  ...
                  <td noWrap class="td">
                    <%String onChange = "if(form1.deptid.value!='"+masterRow.get("deptid")+"')sumitForm("+buyInBean.DEPT_CHANGE+")";%>
                ...
                <tr>
                  ...
                  <%if(!isEnd && isCanAmend && isHasDeptLimit){%>
                  <img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=buyInBean.ONCHANGE%>)');"><img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';sumitForm(<%=buyInBean.ONCHANGE%>)">
                  <%}%>
                  </td>
                  <td noWrap class="tdTitle"><%=masterProducer.getFieldInfo("khlx").getFieldname()%></td>
                  <td noWrap class="td">
                    <%String khlx=masterRow.get("khlx");%>
                    <%if(isEnd || !isCanAmend || !isHasDeptLimit) out.print("<input type='text' value='"+masterRow.get("khlx")+"' style='width:90' class='edline' readonly>");
                    else {%>
                    <pc:select name="khlx" style="width:90" value='<%=khlx%>'>
               ...
               <%-- <td  noWrap class="tdTitle"><%=masterProducer.getFieldInfo("dwt_dwtxid").getFieldname()%></td>
                  ...
                  <td  noWrap class="td" colspan="3"><input type="text" name="dwt_dwdm" value='<%=corpCydwRow.get("dwdm")%>' style="width:60" <%=edClass%> onKeyDown="return getNextElement();" onchange="corpCydwCodeSelect(this);" <%=masterReadonly%>>
                    ...
                    <td noWrap class="td">
                    <%String fplx=masterRow.get("fplx").equals("1")?"增值税":masterRow.get("fplx").equals("2")?"企业普票":"一般收据";
                      String fplxk=masterRow.get("fplx");%>
                    <%if(isEnd || !isCanAmend || !isHasDeptLimit) out.print("<input type='text' value='"+fplx+"' style='width:50' class='edline' readonly>");
                    else {%>
                    <pc:select name="fplx" style="width:80" value='<%=fplxk%>'>
                      <pc:option value=''></pc:option>
                      <pc:option value='1'>增值税</pc:option>
                      <pc:option value='2'>企业普票</pc:option>
                    <pc:option value='3'>一般收据 </pc:option>
                      </pc:select>
                    <%}%>
                  </td>
                  ...
                  <td noWrap class="tdTitle">是否入库</td>
                    <td noWrap class="td">
                    <%String sfrk=masterRow.get("sfrk").equals("1")?"入库":masterRow.get("sfrk").equals("2")?"发料":"";
                      String sfrkk=masterRow.get("sfrk");%>
                    <%if(isEnd || !isCanAmend || !isHasDeptLimit) out.print("<input type='text' value='"+sfrk+"' style='width:50' class='edline' readonly>");
                    else {%>
                    <pc:select name="sfrk" style="width:90" value='<%=sfrkk%>'>
                      <pc:option value=''></pc:option>
                      <pc:option value='1'>入库</pc:option>
                      <pc:option value='2'>发料</pc:option>
                      </pc:select>
                    <%}%>
                  </td>
                    <td noWrap class="td">
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                      <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                        ...
                        <input class="edFocused_r"  name="tCopyNumber" value="<%=request.getParameter("tCopyNumber")==null?"1":request.getParameter("tCopyNumber")%>"  size="2" maxlength="2" onChange=" if ( isNaN(this.value) ) this.value=1">
                          ...
                          <td height='20' nowrap>实际入库换算数量</td>
                       <%--   <td nowrap><%=detailProducer.getFieldInfo("sjrkhsl").getFieldname()%></td>--%>
                        ...
                        <%
                          ...
                          BigDecimal t_sl = new BigDecimal(0), t_je = new BigDecimal(0), t_sjrkl = new BigDecimal(0),t_sjrkhsl = new BigDecimal(0), t_hssl = new BigDecimal(0);
                          int i=0;
                          RowMap detail = null;
                          for(; i<detailRows.length; i++)   {
                            ...
                            if(buyInBean.isDouble(sjrkl))
                              t_sjrkl = t_sjrkl.add(new BigDecimal(sjrkl));
                            String sjrkhsl = detail.get("sjrkhsl");
                            if(buyInBean.isDouble(sjrkhsl))
                              t_sjrkhsl = t_sjrkhsl.add(new BigDecimal(sjrkhsl));
                         ...
                         %>
                           <tr id="rowinfo_<%=i%>">
                           <td class="td" nowrap><%=i+1%>---<%=detail.get("cpid")%></td>
                           ...
                           <input name="image" class="img" type="image" title="复制当前行" onClick="if(form1.cpid_<%=i%>.value==''){alert('请输入产品');return;}sumitForm(<%=buyInBean.DETAIL_COPY%>,<%=i%>)" src="../images/copyadd.gif" border="0">
                        ...
                        <%--  <td class="td" nowrap><input type="text" <%=isHsbj ? "class=ednone_r" : detailClass_r%> onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=sl%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onchange="sl_onchange(<%=i%>, false)" <%=isHsbj ? "readonly" : readonly%>></td>--%>
                          <td class="td" nowrap><input type="text" <%=detailClass_rsl%> onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=sl%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onchange="sl_onchange(<%=i%>,false)" <%=slreadonly%>></td>
                          ...
                          <td class="td" nowrap><input type="text" <%=detailClass_r%> onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=hssl%>' maxlength='<%=list.getColumn("hssl").getPrecision()%>' onchange="hssl_onchange(<%=i%>,false)" <%=readonly%>></td>
                          ...
                          <td class="td" nowrap><input type="text" <%=detailClass_rsl%> onKeyDown="return getNextElement();" id="dj_<%=i%>" name="dj_<%=i%>" value='<%=detail.get("dj")%>' onchange="dj_onchange(<%=i%>, false)" <%=slreadonly%>></td>
                          ...
                          <td align="right" class="td" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="sjrkl_<%=i%>" name="sjrkl_<%=i%>" value='<%=detail.get("sjrkl")%>' maxlength='<%=list.getColumn("sjrkl").getPrecision()%>' readonly></td>
                          <td align="right" class="td" nowrap><input type="text" class=ednone_r onKeyDown="return getNextElement();" id="sjrkhsl_<%=i%>" name="sjrkhsl_<%=i%>" value='<%=detail.get("sjrkhsl")%>' maxlength='<%=list.getColumn("sjrkhsl").getPrecision()%>' readonly></td>
                      ...
                      %>
                        ...
                        <td class="td">&nbsp;</td>
                          ...
                          <td align="right" class="td"><input id="t_sjrkhsl" name="t_sjrkhsl" type="text" class="ednone_r" style="width:100%" value='<%=t_sjrkhsl%>' readonly></td>
                          <td class="td"></td>
                          <td class="td">&nbsp;</td>
                          <td class="td">&nbsp;</td>
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             <%if(!isEnd){%>
              ...
              <input name="btnback" class="button" type="button" value="引入合同(W)" style="width:100" onClick="if(form1.storeid.value==''){alert('请选择仓库');return;}OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=buyInBean.SINGLE_SELECT_ORDER%>)')">
                <pc:shortcut key="w" script="importOrder();"/>
              <input type="hidden" name="importOrder" value="引合同货物" onchange="sumitForm(<%=buyInBean.DETAIL_ORDER_ADD%>)">
              ...
              <input name="button2" type="button" class="button" title="删除数量为零行(ALT+X)" value="删除数量为零行(X)" style='width:120' onClick="sumitForm(<%=buyInBean.DELETE_BLANK%>);">
                <pc:shortcut key="x" script='<%="sumitForm("+buyInBean.DELETE_BLANK+")"%>'/>
              <input name="button2" type="button" class="button" onClick="sumitForm(<%=Operate.POST_CONTINUE%>);" value="保存添加(N)">
               ...
               <%}if((!isRead || !isHasDeptLimit)&&czyid.equals(loginid)){%>
              ...
              <%if(isCanDelete && isCanAmend && !buyInBean.isRep&&!isEnd){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);" value=" 删除(D)">
             <pc:shortcut key="d" script='delMaster();'/><%}%>
              ...
              <%if(!buyInBean.isApprove && !buyInBean.isRep){%><input name="btnback" type="button" class="button" onClick="backList();" value=" 返回(C)">
...
<%=buyInBean.adjustInputSize(new String[]{"cpbm","product", "sl", "dj", "je","sxz","sjrkl","bz","hssl","sjrkhsl","dhqk"}, "form1", detailRows.length)%>
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    function importOrder(){
     ...
     OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=buyInBean.SINGLE_SELECT_ORDER%>)')
     }
  ...
  //      }
   ...
   //  {
    ...
    function cal_tot(type)
    {
      ...
      else if(type == 'sjrkhsl')
        document.all['t_sjrkhsl'].value = formatSum(tot);
      else if(type == 'hssl')
    ...
    function OrderSingleSelect(frmName,srcVar,fieldVar,curID,storeid,methodName,notin)
    {
      ...
      paraStr = "../buy/order_single_select.jsp?operate=0&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+"&dwtxid="+curID+"&storeid="+storeid+"&djlx=1";
      if(methodName+'' != 'undefined')
        paraStr += "&method="+methodName;
      if(notin+'' != 'undefined')
        paraStr += "&notin="+notin;
...
<%if(buyInBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>