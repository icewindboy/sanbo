...
if(!loginBean.hasLimits(pageCode, request, response))
  ...
  engine.project.LookUp buyPriceBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_PRODUCT);//In_BuyBalanceBean.getBuyPriceBean(request);
...
{
    CustomerCodeChange('1',document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dz','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',obj.value,'sumitForm(<%=In_BuyBalanceBean.DWTXID_CHANGE%>,-1)');
}
function customerNameChange(obj)
{
    CustomerNameChange('1',document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dz','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',obj.value,'sumitForm(<%=In_BuyBalanceBean.DWTXID_CHANGE%>,-1)');
}
</script>
<%
              ...
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                <%
                 ...
                 <tr>
                  ...
                  <td noWrap class="td">
                 <% if(isEnd) out.print("<input name='wbid' type='hidden' value='"+masterRow.get("wbid")+"'style='width:110' class='edline' readonly><input  type='text' value='"+wbBean.getLookupName(masterRow.get("wbid"))+"'style='width:110' class='edline' readonly>");
                  else{%>
                  <pc:select name="wbid" style="width:110" addNull="1" onSelect="">
                 ...
                 <tr>
                  ...
                  <td noWrap class="td"><input type="text" name="hl" value='<%=masterRow.get("hl")%>' maxlength='<%=ds.getColumn("hl").getPrecision()%>' style="width:110" <%=edClass%> onKeyDown="return getNextElement();" <%=readonly%>></td>
                  <td noWrap class="tdTitle"><%=masterProducer.getFieldInfo("je").getFieldname()%></td>
                  <td noWrap class="td"><input type="text" name="ybje" value='<%=masterRow.get("ybje")%>' maxlength='<%=ds.getColumn("ybje").getPrecision()%>' style="width:110" <%=edClass%> onKeyDown="return getNextElement();" <%=readonly%>></td>
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                    <table id="tableview1" width="100%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <%if(!zt.equals("8")){%>
                        <input type="hidden" name="tdhwids" value="" onchange="sumitForm(<%=In_BuyBalanceBean.DETAIL_SALE_ADD%>,-1);">
                    ...
                    <%
                      ...
                      int[] widths = In_BuyBalanceBean.getMaxStyleWidth(detailRows, widthName, widthMin);
                      RowMap detail = null;
                      list.first();
      ...
      <td>
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            ...
            <%
            if(!In_BuyBalanceBean.isReport){
              if(!zt.equals("8")){
            String quto = "sumitForm("+In_BuyBalanceBean.AUTO_CANCER+");";
            String po = "sumitForm("+Operate.POST+");";
             %>
             <input name="btnback" class="button" type="button" value="自动核销(W)" onClick="sumitForm(<%=In_BuyBalanceBean.AUTO_CANCER%>);" border="0">
             <pc:shortcut key="w" script="<%=quto%>" />
             <input name="btnback" type="button" class="button" onClick="sumitForm(<%=In_BuyBalanceBean.MASTER_DETAIL_POST%>);" value="保存返回(S)">
...
<script language="javascript">
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    function TdhwMultiSelect(frmName,srcVar,fieldVar,curID,methodName,notin)
       {
         ...
         var winName= "MultiProdSelector";
           paraStr = "../finance_yuzhou/income_import_stocking_detail.jsp?operate=0&multi=1&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+curID;//
    ...
    function selctTDHWOfLading()
    {
      ...
      TdhwMultiSelect('form1','srcVar=tdhwids','fieldVar=jhdhwid','&dwtxid='+form1.dwtxid.value+'&personid='+form1.personid.value,"sumitForm(<%=In_BuyBalanceBean.DETAIL_SALE_ADD%>,-1)");
        }
    function OrderSingleSelect(frmName,srcVar,fieldVar,curID,methodName,notin)
       {
        var winopt = "location=no scrollbars=yes menubar=no status=no resizable=1 width=640 height=450  top=0 left=0";
        var winName= "SingleladingSelector";
        paraStr = "../finance_yuzhou/income_import_stocking_detail.jsp?operate=0&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+"&dwtxid="+curID;
        if(methodName+'' != 'undefined')
          paraStr += "&method="+methodName;
        if(notin+'' != 'undefined')
          paraStr += "&notin="+notin;
    ...
    function selctbilloflading()
       {
         ...
         OrderSingleSelect('form1','srcVar=selectedtdid','fieldVar=jhdhwid',form1.dwtxid.value,"sumitForm(<%=In_BuyBalanceBean.IMPORT_TD%>,-1)");
   }
</script>
<%
//&#$
