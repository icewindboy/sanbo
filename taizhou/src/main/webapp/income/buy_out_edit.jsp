...
function corpCodeSelect(obj)
{
  ProvideCodeChange(document.all['prod'], obj.form.name, 'srcVar=dwtxid&srcVar=dwdm&srcVar=dwmc',
                 'fieldVar=dwtxid&fieldVar=dwdm&fieldVar=dwmc', obj.value, 'sumitForm(<%=InbuyOutBean.ONCHANGE%>)');
}
...
<%String retu = InbuyOutBean.doService(request, response);
  ...
  engine.project.LookUp InbuyOrderBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_BUY_ORDER_GOODS);
  ...
  EngineDataSet ds = InbuyOutBean.getMaterTable();
  ...
  HtmlTableProducer masterProducer = InbuyOutBean.masterProducer;
  HtmlTableProducer detailProducer = InbuyOutBean.detailProducer;
  RowMap masterRow = InbuyOutBean.getMasterRowinfo();
  RowMap[] detailRows= InbuyOutBean.getDetailRowinfos();
  ...
  boolean isEnd = InbuyOutBean.isRep || InbuyOutBean.isApprove || (!InbuyOutBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !InbuyOutBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(InbuyOutBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  String head = djlx.equals("2") ? "进口货物进货单" : "进口货物退货单";
  boolean isAdd = InbuyOutBean.isDetailAdd;
  boolean  isRead = InbuyOutBean.isRep || InbuyOutBean.isApprove || (!InbuyOutBean.masterIsAdd() && (zt.equals("9") || zt.equals("2")));//表示在审批中或已入库不能修改
  isRead = isRead || !(InbuyOutBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0">
                ...
                <tr>
                  ...
                  <td noWrap class="td">
                    <%String store_Change = "if(form1.storeid.value!='"+masterRow.get("storeid")+"')sumitForm("+InbuyOutBean.STORE_CHANGE+")";%>
                  ...
                  <td noWrap class="td">
                    <%String onChange = "if(form1.deptid.value!='"+masterRow.get("deptid")+"')sumitForm("+InbuyOutBean.DEPT_CHANGE+")";%>
                ...
                <tr>
                  ...
                  <%if(!isEnd && isCanAmend && isHasDeptLimit){%>
                  <img style='cursor:hand' src='../images/view.gif' border=0 onClick="ProvideSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dwdm','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=dwdm',form1.dwtxid.value,'sumitForm(<%=InbuyOutBean.ONCHANGE%>)');"><img style='cursor:hand' src='../images/delete.gif' BORDER=0 ONCLICK="dwtxid.value='';dwmc.value='';sumitForm(<%=InbuyOutBean.ONCHANGE%>)">
                  <%}%>
                  </td>
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;overflow-y:auto;overflow-x:auto;">
                      <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                        ...
                        <%prodBean.regData(list,"cpid");
                          InbuyOrderBean.regData(list,"hthwid");
                          ...
                          for(; i<detailRows.length; i++){
                            ...
                            if(InbuyOutBean.isDouble(sjrkl))
                         ...
                         %>
                           ...
                           <input name="image" class="img" type="image" title="复制当前行" onClick="if(form1.cpid_<%=i%>.value==''){alert('请输入产品');return;}sumitForm(<%=InbuyOutBean.DETAIL_COPY%>,<%=i%>)" src="../images/copyadd.gif" border="0">
                         ...
                         %>
                          <%RowMap  buyOrderRow= InbuyOrderBean.getLookupRow(detail.get("hthwid"));%>
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             <%if(!isEnd){%>
              ...
              <input name="btnback" class="button" type="button" value="引入订单(W)" style="width:100" onClick="if(form1.storeid.value==''){alert('请选择仓库');return;}OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=InbuyOutBean.SINGLE_SELECT_ORDER%>)')">
                <pc:shortcut key="w" script="importOrder();"/>
              <input type="hidden" name="importOrder" value="引合同货物" onchange="sumitForm(<%=InbuyOutBean.DETAIL_ORDER_ADD%>)">
              ...
              <input name="button2" type="button" class="button" title="删除数量为零行(ALT+X)" value="删除数量为零行(X)" style='width:120' onClick="sumitForm(<%=InbuyOutBean.DELETE_BLANK%>);">
                <pc:shortcut key="x" script='<%="sumitForm("+InbuyOutBean.DELETE_BLANK+")"%>'/>
              ...
              <%if(isCanDelete && isCanAmend && !InbuyOutBean.isRep){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=Operate.DEL%>);" value=" 删除(D)">
             <pc:shortcut key="d" script='delMaster();'/><%}%>
              ...
              <%if(!InbuyOutBean.isApprove && !InbuyOutBean.isRep){%><input name="btnback" type="button" class="button" onClick="backList();" value=" 返回(C)">
...
<%=InbuyOutBean.adjustInputSize(new String[]{"cpbm","product", "sl", "dj", "je","sxz","sjrkl","bz"}, "form1", detailRows.length)%>
  ...
  function formatSum(srcStr){ return formatNumber(srcStr, '<%=loginBean.getSumFormat()%>');}
    ...
    function importOrder(){
     ...
     OrderSingleSelect('form1','srcVar=singleOrder','fieldVar=htid',form1.dwtxid.value,form1.storeid.value,'sumitForm(<%=InbuyOutBean.SINGLE_SELECT_ORDER%>)')
     }
   ...
   /*  else{
    ...
    function OrderSingleSelect(frmName,srcVar,fieldVar,curID,storeid,methodName,notin)
    {
      ...
      paraStr = "../income/inomeorder_single_select.jsp?operate=0&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+"&dwtxid="+curID+"&storeid="+storeid;
      if(methodName+'' != 'undefined')
        paraStr += "&method="+methodName;
      if(notin+'' != 'undefined')
        paraStr += "&notin="+notin;
...
<%if(InbuyOutBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>