...
<%String retu = saleOrderBean.doService(request, response);
  ...
  engine.project.LookUp deptBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_DEPT);//引用部门
  ...
  engine.project.LookUp prodBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_PRODUCT);//LookUp产品信息
  engine.project.LookUp wbBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_FOREIGN_CURRENCY);
  engine.project.LookUp balanceModeBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_BALANCE_MODE);//引用结算方式
  engine.project.LookUp sendModeBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_SEND_MODE);//发货方式
  engine.project.LookUp orderTypeBean = engine.project.OtherLookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_ORDER_TYPE);//合周类型
  ...
  boolean isCanDelete = !isEnd && !saleOrderBean.masterIsAdd() ;//没有结束,在修改状态,并有删除权限
  ...
  <table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="center" width="760">
    <tr valign="top">
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            <td class="activeVTab">采购计单(<%=title%>)</td>
          </tr>
        </table>
          ...
          <tr>
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" width="100%" bgcolor="#f0f0f0">
                <tr>
                  <td noWrap class="tdTitle">订单编号</td>
                  ...
                  <td noWrap class="td"><input type="text" name="htrq" value='<%=masterRow.get("htrq")%>' maxlength='10' style="width:85" <%=edClass%> onChange="checkDate(this)" onKeyDown="return getNextElement();"<%=readonly%>>
                    <%if(!isEnd){%>
                    <a href="#"><img align="absmiddle" src="<%=request.getContextPath() %>/images/seldate.gif" width="20" height="16" border="0" title="选择日期" onclick="select_Date('<%=request.getContextPath() %>',form1.htrq);"></a>
                    <%}%>
                  </td>
                  ...
                  <td  noWrap class="td">
                    ...
                    <%if(!isEnd){%>
                    <a href="#"><img align="absmiddle" src="<%=request.getContextPath() %>/images/seldate.gif" width="20" height="16" border="0" title="选择日期" onclick="select_Date('<%=request.getContextPath() %>',form1.ksrq);"></a>
                    <%}%></td>
                  <td noWrap>--</td>
                   <td noWrap>
                   <input type="text" name="jsrq" value='<%=masterRow.get("jsrq")%>' maxlength='10' style="width:85" <%=edClass%> onChange="checkDate(this)" onKeyDown="return getNextElement();"<%=readonly%>>
                    <%if(!isEnd){%>
                    <a href="#"><img align="absmiddle" src="<%=request.getContextPath() %>/images/seldate.gif" width="20" height="16" border="0" title="选择日期" onclick="select_Date('<%=request.getContextPath() %>',form1.jsrq);"></a>
                    <%}%>
                  </td>
                </tr>
                <tr>
                  <td noWrap class="tdTitle">购货单位</td><%--购货单位--%>
                  <td colspan="3" noWrap class="td">
                    <input type="hidden" name="dwtxid" value='<%=masterRow.get("dwtxid")%>'>
                     <input type="text" class="edline" name="dwmc"    value='<%=masterRow.get("dwmc")%>' style="width:255"   readonly>
                  </td>
                ...
                <tr>
                 ...
                 <tr>
                  <td noWrap class="tdTitle">签订地点</td>
                ...
                <tr>
                  <td  noWrap class="tdTitle">其他信息</td><%--其他信息--%>
...
function sl_onchange(i, isBigUnit)
    {
      ...
      unitConvert('<%=request.getContextPath() %>',document.all['prod_'+i], 'form1', 'srcVar=truebl_'+i, 'exp='+oldhsblObj.value, sxzObj.value, 'nsl_onchange('+i+','+isBigUnit+')');
     }
  ...
  function OrderMultiSelect(frmName, srcVar, methodName,notin)
 {
   ...
   paraStr = "<%=request.getContextPath() %>/sale/cust_prod_history_select.jsp?operate=0&srcFrm="+frmName+"&"+srcVar;
