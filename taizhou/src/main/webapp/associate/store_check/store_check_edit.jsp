                    ...
                    %>
                        <tr id="rowinfo_<%=i%>" onClick="showDetail()">
                         ...
                         %>
                          ...
                          <td class="td" nowrap align="right"> <input type="text" <%=detailClass_r%>  style="width:100%" onKeyDown="return getNextElement();" id="scsl_<%=i%>" name="scsl_<%=i%>" value='<%=detail.get("scsl")%>' onFocus="saveCurrentVariable(<%=i%>)"  onBlur="sl_onchange(<%=i%>, false)" maxlength='<%=list.getColumn("scsl").getPrecision()%>' <%=readonly%>>
                         ...
                         <td class="td" nowrap align="right"> <input type="text" <%=detailClass_r%>   onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=detail.get("hssl")%>' onFocus="saveCurrentVariable(<%=i%>)" onBlur="sl_onchange(<%=i%>, true)" maxlength='<%=list.getColumn("hssl").getPrecision()%>' <%=readonly%>>
                          </td>
                          <%--
        ...
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            ...
            <TR id="cpbmtext" style="display:block">
              <TD colspan="2" CLASS="td">
                ...
                宽度<input type="text" name="sxz_a" style="width:30" class="edbox">
                至
                <input type="text" name="sxz_b" style="width:30" class="edbox">
            </TD>
           </TR>
            <TR>
...
<SCRIPT language="javascript">
 ...
 {
    location.href='../pub/pdfprint.jsp?code=store_check_edit_bill&operate=<%=storeCheckBean.PRINT_BILL%>&a$pdid=<%=masterRow.get("pdid")%>&src=../store/store_check_edit.jsp';
 }
 //查看或生成损益单
 <%
   if ( !masterRow.get("pdhm").equals("") )
