...
<%String retu = corpBean.doService(request, response);
  ...
  String loginName = row.get("loginName");
%>
            ...
            <td>
              <table CELLSPACING="2" CELLPADDING="0" BORDER="0" width="100%" bgcolor="#f0f0f0">
                ...
                <tr>
                  ...
                  <input type="text" name="loginName" value='<%=row.get("loginName")%>' maxlength='<%=ds.getColumn("loginName").getPrecision()%>' style="width:160" class="edline" readOnly onKeyDown="return getNextElement();">
                  <%}else{ %>
                  <input type="text" name="loginName" value='<%=row.get("loginName")%>' maxlength='<%=ds.getColumn("loginName").getPrecision()%>' style="width:160" class="<%=rowClass%>"<%=readOnly%> onKeyDown="return getNextElement();">
                  <%}%>                  
                 <td noWrap class="tdTitle">密码</td>
                  <td noWrap class="td"><input type="text" name="loginPassword" value='<%=row.get("loginPassword")%>' maxlength='<%=ds.getColumn("loginPassword").getPrecision()%>' style="width:160" class="<%=rowClass%>"<%=readOnly%> onKeyDown="return getNextElement();"></td>
                </tr>                
                <tr>
