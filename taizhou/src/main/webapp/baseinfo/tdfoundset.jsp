...
<script LANGUAGE="javascript">
  ...
  function showInterFrame(oper, rownum){
    var url = "tdfoundsetedit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>包装类别设置</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = B_FoundsetBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
  ...
  <TABLE WIDTH="600" BORDER=0 CELLSPACING=0 CELLPADDING=0 align="center">
    <TR><TD align="right">
    <%if(loginBean.hasLimits(pageCode,op_add)) { String ret = "location.href='("+B_FoundsetBean.retuUrl+",-1)";%>
    <%if(B_FoundsetBean.retuUrl!=null)%>
    <input name="button2222232" type="button" align="Right" title = "返回" class="button" onClick="location.href='<%=B_FoundsetBean.retuUrl%>'" value=" 返回(C) "border="0"><pc:shortcut key="c" script='<%=ret%>'/><%}%>
   </TD></TR>
  </TABLE>
  <table id="tableview1" width="600" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(isCanEdit){%>onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)"<%}%> >
      ...
      <td class="td"><%=list.getValue("fundItemCode")%></td>
      <td class="td"><%=list.getValue("fundItemName")%></td>
    </tr>
    <%
      list.next();
      }
