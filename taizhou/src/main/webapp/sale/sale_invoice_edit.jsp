              ...
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                <%corpBean.regData(ds,"dwtxid");%>
                 ...
                 <tr>
                  ...
                  <td  class="td" colspan="5"><input type="text" align="left" <%=edClass%> name="bz" value='<%=masterRow.get("bz")%>' maxlength='<%=ds.getColumn("bz").getPrecision()%>' style="width:100%"  onKeyDown="return getNextElement();"<%=readonly%>>
                  </td>
                    ...
                    <%
                     ...
                     %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        RowMap  saleLadingRow= saleLadingBean.getLookupRow(detail.get("tdhwid"));
                        String isprops=prodRow.get("isprops");
                        String isdel=prodRow.get("isdelete");

                        %>
                        <td class="td" nowrap><%=saleLadingRow.get("tdbh")%></td>
                        <td class="td" nowrap><%=saleLadingRow.get("htbh")%></td>
                        <td class="td" nowrap><%=prodRow.get("cpbm")%></td><!--产品编码(存货代码)--->
                        <td class="td" nowrap><%=prodRow.get("product")%></td><!--品名 规格(存货名称与规格)--->

                        <td class="td" nowrap>
                        <input type="hidden"  id="dmsxid_<%=i%>"  name="dmsxid_<%=i%>"  value='<%=detail.get("dmsxid")%>' >
                        <%=propertyBean.getLookupName(detail.get("dmsxid"))%>
                        </td>
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
             <%if(!isEnd){%>
             <input name="btnback" class="button" type="button" value="引入提货单(W)" onClick="selctbilloflading()" border="0">
             <pc:shortcut key="w" script="selctbilloflading()" />
             <input type="hidden" name="tdhwids" value="" onchange='sumitForm(<%=b_SaleInvoiceBean.PRODUCT_ADD%>,-1)' >
             <input name="btnback" class="button" type="button" value="引入提货单货物(E)" onClick="selctHthwOfLading()" border="0">
...
<script language="javascript">
  ...
  function selctbilloflading()
  {
    ...
    OrderSingleSelect('form1','srcVar=selectedtdid','fieldVar=tdid',form1.dwtxid.value,"sumitForm(<%=b_SaleInvoiceBean.DETAIL_SALE_ADD%>,-1)");
   }
...
function selctHthwOfLading()
{
  ...
  TdhwMultiSelect('form1','srcVar=tdhwids','fieldVar=tdid',form1.dwtxid.value,"sumitForm(<%=b_SaleInvoiceBean.PRODUCT_ADD%>,-1)");
}
</script>
<%//&#$
