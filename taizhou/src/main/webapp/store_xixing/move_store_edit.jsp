...
<%String retu = moveStoreBean.doService(request, response);
  ...
  boolean isCanDelete = !isEnd && !moveStoreBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
...
<%=moveStoreBean.adjustInputSize(new String[]{"sl","dj","je", "bz", "cpbm", "sxz", "product", "jldw"},  "form1", (moveStoreBean.max-min+1), min)%>
    ...
    function nsl_onchange(i, isBigUnit)
    {
      ...
      if(isNaN(obj.value)){
       ...
       if(obj.value!="" && !isNaN(obj.value)){
         jeObj.value = formatQty(parseFloat(obj.value) * parseFloat(djObj.value));
       }
       else{
         jeObj.value='';
       }
...
//删除
 ...
 {
   location.href='../pub/pdfprint.jsp?code=move_store_edit_bill&operate=<%=moveStoreBean.PRINT_BILL%>&a$sfdjid=<%=masterRow.get("sfdjid")%>&src=../store_xixing/move_store_edit.jsp';
 }
 //新增
