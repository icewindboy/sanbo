...
<%String retu = planMakeTaskBean.doService(request, response);
  ...
  String SC_PRODUCE_UNIT_STYLE = planMakeTaskBean.SC_PRODUCE_UNIT_STYLE;
  ...
  EngineDataSet ds = planMakeTaskBean.getMaterTable();
  EngineDataSet list = planMakeTaskBean.getDetailTable();
  HtmlTableProducer masterProducer = planMakeTaskBean.masterProducer;
  HtmlTableProducer detailProducer = planMakeTaskBean.detailProducer;
  RowMap masterRow = planMakeTaskBean.getMasterRowinfo();
  RowMap[] detailRows= planMakeTaskBean.getDetailRowinfos();
  ...
  boolean isEnd = planMakeTaskBean.isApprove || (!planMakeTaskBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !planMakeTaskBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(planMakeTaskBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  String SYS_PRODUCT_SPEC_PROP =planMakeTaskBean.SYS_PRODUCT_SPEC_PROP;//生产用位换算的相关规格属性名称 得到的值为“宽度”……
%>
            ...
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" width="100%" bgcolor="#f0f0f0">
                 <%workShopBean.regData(ds,"deptid");%>
                  ...
                  <%String sumit = "if(form1.deptid.value!='"+masterRow.get("deptid")+"'){sumitForm("+planMakeTaskBean.ONCHANGE+");}";%>
                  <td noWrap class="td">
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      for(; i<detailRows.length; i++)   {
                        ...
                        if(planMakeTaskBean.isDouble(sl))
                          t_sl = t_sl.add(new BigDecimal(sl));
                        String scsl = detail.get("scsl");
                        if(planMakeTaskBean.isDouble(scsl))
                          t_scsl = t_scsl.add(new BigDecimal(scsl));
                        String yjgl = detail.get("yjgl");
                        if(planMakeTaskBean.isDouble(yjgl))
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
              ...
              <%if(!planMakeTaskBean.isApprove){%><input name="btnback" type="button" class="button" onClick="backList();" value=" 返回(C) ">
...
<%=planMakeTaskBean.adjustInputSize(new String[]{"cpbm", "product", "sxz", "csl","sl","scsl", "jgyq", "yjgl"}, "form1", detailRows.length)%>
  ...
  <%if(planMakeTaskBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</body>
</html>
