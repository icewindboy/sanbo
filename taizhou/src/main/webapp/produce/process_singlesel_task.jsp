...
%>
<%
  engine.erp.produce.B_Process_SingleSelTask singleSelTaskBean = engine.erp.produce.B_Process_SingleSelTask.getInstance(request);
...
<%
  if(inputName != null && fieldName != null)
  {
    ...
    for(int i=0; i< length; i++)
    {
      ...
      out.print(".value=");     out.println("obj."+ (fieldName[i].equals("rwdid")?"value;":"innerText;"));
    }
  }
  if(methodName != null)
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectProduce(<%=i%>);"<%}%>>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="rwdh_<%=i%>" class="td"><%=list.getValue("rwdh")%><input type="hidden" name="rwdid_<%=i%>" value="<%=list.getValue("rwdid")%>"></td>
