<%--生产加工单引入任务单界面--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%><%
  engine.erp.produce.ImportTask importTaskBean = engine.erp.produce.ImportTask.getInstance(request);
  String retu = importTaskBean.doService(request, response);
  ...
  EngineDataSet list = importTaskBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">生产任务单列表</TD>
  </TR></TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%RowMap prodRow = prodBean.getLookupRow(list.getValue("cpid"));%>
      <td nowrap align="center" class="td"><input type="checkbox" name="sel" value='<%=list.getValue("rwdmxid")%>' onKeyDown="return getNextElement();"></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="rwdh_<%=i%>" class="td"><%=list.getValue("rwdh")%><input type="hidden" id="rwdmxid_<%=i%>" value='<%=list.getValue("rwdmxid")%>'></td>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="wjgl_<%=i%>" class="td"><%=list.getValue("wjgl")%></td>
      <td nowrap align="center" id="jldw_<%=i%>" class="td" onClick="checkRadio(<%=i%>)"><%=prodRow.get("jldw")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="scsl_<%=i%>" class="td"><%=list.getValue("scsl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="wjgscl_<%=i%>" class="td"><%=list.getValue("wjgscl")%></td>
