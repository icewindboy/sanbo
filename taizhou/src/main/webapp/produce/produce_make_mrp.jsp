...
%><%
  engine.erp.produce.B_ProduceMakeMrp produceMakeMrpBean = engine.erp.produce.B_ProduceMakeMrp.getInstance(request);
  String pageCode = "mrp";
...
<script language="javascript">
   ...
   function propertyNameSelect(obj,cpid,i)
   {
     PropertyNameChange(document.all['prod'], obj.form.name, 'srcVar=dmsxid_'+i+'&srcVar=sxz_'+i,
                        'fieldVar=dmsxid&fieldVar=sxz', cpid, obj.value,'sumitForm(<%=produceMakeMrpBean.SUMIT_PROPERTY%>,'+i+')');
   }
   function productCodeSelect(obj, i)
  {
     ProdCodeChange(document.all['prod'], obj.form.name, 'srcVar=cpid_'+i+'&srcVar=cpbm_'+i+'&srcVar=product_'+i+'&srcVar=jldw_'+i,
                 'fieldVar=cpid&fieldVar=cpbm&fieldVar=product&fieldVar=jldw', obj.value,'sumitForm(<%=produceMakeMrpBean.PRODUCT_ONCHANGE%>,'+i+')');
  }
...
<%String retu = produceMakeMrpBean.doService(request, response);
  ...
  EngineDataSet ds = produceMakeMrpBean.getMaterTable();
  EngineDataSet list = produceMakeMrpBean.getDetailTable();
  HtmlTableProducer masterProducer = produceMakeMrpBean.masterProducer;
  HtmlTableProducer detailProducer = produceMakeMrpBean.detailProducer;
  RowMap masterRow = produceMakeMrpBean.getMasterRowinfo();
  RowMap[] detailRows= produceMakeMrpBean.getDetailRowinfos();
  ...
  boolean isEnd = (!produceMakeMrpBean.masterIsAdd() && !zt.equals("0"));//表示已经审核或已完成
  boolean isCanDelete = !isEnd && !produceMakeMrpBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);//没有结束,在修改状态,并有删除权限
  isEnd = isEnd || !(produceMakeMrpBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  //boolean isAdd = produceMakeMrpBean.isMasterAdd;
  String SYS_PRODUCT_SPEC_PROP =produceMakeMrpBean.SYS_PRODUCT_SPEC_PROP;//生产用位换算的相关规格属性名称 得到的值为“宽度”……
%>
                  ...
                  <td colspan="8" noWrap class="td"><div style="display:block;width:750;height=<%=width%>;overflow-y:auto;overflow-x:auto;">
                    <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <td height='20' align="center" nowrap>
                           <%if(!isEnd ){%>
                          <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=produceMakeMrpBean.DETAIL_ADD_BLANK%>)" src="../images/add_big.gif" border="0">
                          <pc:shortcut key="a" script='<%="sumitForm("+ produceMakeMrpBean.DETAIL_ADD_BLANK +",-1)"%>'/><%}%>
                        </td>
                    ...
                    <%prodBean.regData(list,"cpid");
                      ...
                      %>
                        ...
                        onClick="ProdSingleSelect('form1','srcVar=singleIdInput_<%=i%>','fieldVar=cpid','','sumitForm(<%=produceMakeMrpBean.SINGLE_PRODUCT_ADD%>,<%=i%>)')">
                        ...
                        <%}%>
                         ...
                         <input type="hidden" id="scdwgs_<%=i%>" name="scdwgs_<%=i%>" value="<%=prodRow.get("scdwgs")%>">
                          ...
                          <td class="td" align="right" nowrap><input type="text" class="ednone" id="jhkgl_<%=i%>" name="jhkgl_<%=i%>" value='<%=planUseAbleBean.getLookupName(new String[]{detail.get("cpid"), detail.get("dmsxid")})%>' readonly></td>
                         ...
                         <td class="td" nowrap><input type="text" class=ednone style="width:60" onKeyDown="return getNextElement();" id="jldw_<%=i%>" name="jldw_<%=i%>" value='<%=prodRow.get("jldw")%>' readonly></td>
                          ...
                          %>
                            ...
                            <%=produceMakeMrpBean.listToOption(lists, opkey.indexOf(detail.get("chxz")))%>
                          </pc:select>
                          </td><%}%>
        ...
        <table CELLSPACING=0 CELLPADDING=0 width="100%" align="center">
          ...
          <tr>
            <td colspan="3" noWrap class="tableTitle">
              <%if(!isEnd ){%>
              <%--input name="btnback" type="button" class="button" onClick="sumitForm(<%=produceMakeMrpBean.DETAIL_ADD%>);" value="生成物料需求"--%>
...
<%=produceMakeMrpBean.adjustInputSize(new String[]{"xql", "jlxql", "xgl","bz", "sxz", "jhkgl"}, "form1", detailRows.length)%>
