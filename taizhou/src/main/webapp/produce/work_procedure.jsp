...
<SCRIPT LANGUAGE="javascript">
  ...
  {
    var url = "work_procedureedit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>工序分段设置</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = b_WorkProcedureBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;"onKeyDown="return onInputKeyboard();">
...
</td> <TD align="right"><%if(b_WorkProcedureBean.retuUrl!=null){%><input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=b_WorkProcedureBean.retuUrl%>'" value=" 返回(C)"border="0">
  <% String back ="location.href='"+b_WorkProcedureBean.retuUrl+"'" ;%>
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      ...
      <%}%></td>
      <td class="td"><%=list.getValue("gdbh")%></td>
      <td class="td"><%=list.getValue("gdmc")%></td>
      <td class="td"><%=workshop.getLookupName(list.getValue("deptid"))%></td>
      <td class="td"><%=list.getValue("gdms")%></td>
    </tr>
    <%  list.next();
      }
