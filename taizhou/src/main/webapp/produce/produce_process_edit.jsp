...
<%String retu = produceProcessBean.doService(request, response);
  ...
  String scjhid = produceProcessBean.getPlanID();
%>
                     ...
                     <tr>
                      <td colspan="3" noWrap class="tableTitle">
                       ...
                       <input name="btnback" class="button" type="button" value="引入任务明细(E)" style="width:115" onClick="if(form1.deptid.value==''){alert('请选择车间'); return;}TaskGoodsSelect('form1','srcVar=mutitask&deptid='+form1.deptid.value)" border="0">
                        ...
                        <input name="button2" type="button" class="button" onClick="sumitForm(<%=produceProcessBean.COMMONPOST_CONTINUE%>);" value="保存添加(N)">
                           <pc:shortcut key="n" script='<%="sumitForm("+ produceProcessBean.COMMONPOST_CONTINUE +",-1)"%>'/>
                         <input name="btnback" type="button" class="button" onClick="sumitForm(<%=produceProcessBean.COMMON_POST%>);" value="保存返回(S)">
                        <pc:shortcut key="s" script='<%="sumitForm("+ produceProcessBean.COMMON_POST +",-1)"%>'/><%}%>
                        <%if(isCanDelete && isCanAmend){%><input name="button3" type="button" class="button" onClick="if(confirm('是否删除该记录？'))sumitForm(<%=produceProcessBean.COMMONDETAIL_DEL%>);" value=" 删除(D)">
