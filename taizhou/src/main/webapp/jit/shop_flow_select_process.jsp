<%--流转单引入任务单--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%>
<%
  engine.erp.jit.ShopFlowSelectProcessDetail shopFlowSelectProcessDetailBean = engine.erp.jit.ShopFlowSelectProcessDetail.getInstance(request);
  ...
  String retu = shopFlowSelectProcessDetailBean.doService(request, response);
  ...
  EngineDataSet list = shopFlowSelectProcessDetailBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr onClick="checkTr(<%=i%>)">
      <td class="td">
        <input type="checkbox" name="sel" value='<%=list.getValue("jgdid")%>' onKeyDown="return getNextElement();">
        <input type="hidden" name="jgdid_<%=i%>" value='<%=list.getValue("jgdid")%>'>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="deptidmc_<%=i%>" class="td"><%=deptBean.getLookupName(list.getValue("deptid"))%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="syzsl_<%=i%>" class="td"><%=list.getValue("syzsl")%></td>
