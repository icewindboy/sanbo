...
%>
<%
  engine.erp.jit.Select_ProcessCard Select_ProcessCardBean = engine.erp.jit.Select_ProcessCard.getInstance(request);
  String pageCode = "sale_invoice";
  //if(!loginBean.hasLimits(pageCode, request, response))
  //return;
  boolean hasSearchLimit = loginBean.hasLimits(pageCode, op_search);
  //引用结算方式
  engine.project.LookUp processcardBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_PROCESS_CARD_DETAIL);
  ...
  String retu = Select_ProcessCardBean.doService(request, response);
  ...
  EngineDataSet list = Select_ProcessCardBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%
      ...
      processcardBean.regData(list,"processmxid");
      ...
      for(; i<count; i++)   {
        ...
        RowMap cardrow  = processcardBean.getLookupRow(list.getValue("processmxid"));
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectCorp(<%=i%>);"<%}%>>
     ...
     <input type="hidden" name="processmxid_<%=i%>" value="<%=list.getValue("processmxid")%>">
     ...
     <input type="hidden" name="scjhid_<%=i%>" value="<%=list.getValue("scjhid")%>">
       </td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="processdm_<%=i%>" class="td"><%=cardrow.get("processdm")%></td>
      ...
      <td nowrap align="center"  class="td" onClick="checkRadio(<%=i%>)">
     <%=technicsNameBean.getLookupName(list.getValue("gymcid"))%>
       </td>
