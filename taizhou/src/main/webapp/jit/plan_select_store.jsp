...
<%@ page import="engine.dataset.*, engine.project.Operate, java.util.*,engine.html.*"%><%!
  private String srcFrm=null;           //传递的原form的名称
  private String multiIdInput = null;   //多选的ID组合串
%><%
  engine.erp.jit.PlanSelectStore planSelectStoreBean = engine.erp.jit.PlanSelectStore.getInstance(request);
  String pageCode = "produce_plan";
  if(!loginBean.hasLimits(pageCode, request, response))
    return;
  String retu = planSelectStoreBean.doService(request, response);
  engine.project.LookUp prodBean = engine.project.LookupBeanFacade.getInstance(request,engine.project.SysConstant.BEAN_PRODUCT);
  engine.project.LookUp wzlbidBean = engine.project.LookupBeanFacade.getInstance(request,engine.project.SysConstant.BEAN_PRODUCT_KIND);
  ...
  HtmlTableProducer searchTable = planSelectStoreBean.searchTable;
%>
<html>
<head>
<title></title>
     ...
     <TR>
        <TD>
          <TABLE cellspacing=3 cellpadding=0 border=0>
            ...
            %>
            <pc:select name="chxz" addNull="1" style="width:100%">
            <%=planSelectStoreBean.listToOption(lists, opkey.indexOf(planSelectStoreBean.getFixedQueryValue("chxz")))%>
            </pc:select>
            </td>
            <TD class="td" nowrap>物资类别</TD>
            <td>
              <pc:select name="wzlbid" addNull="1" style="width:100%">
            <%=wzlbidBean.getList(planSelectStoreBean.getFixedQueryValue("wzlbid"))%>
            </pc:select>
            </td>
            </TR>
            <%--searchTable.printWhereInfo(pageContext);
            --%>
            <TR>
...
<%
if("0".equals(request.getParameter("operate")))
  out.print(planSelectStoreBean.showJavaScript("showFixedQuery();"));
out.print(retu);
%>
</body>
</html>