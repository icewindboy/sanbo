<%--生产任务单单选物料需求计划--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%>
<%
  engine.erp.jit.SelectOtherBillCpid selectOtherBillCpidBean = engine.erp.jit.SelectOtherBillCpid.getInstance(request);
  ...
  String retu = selectOtherBillCpidBean.doService(request, response);
  ...
  EngineDataSet list = selectOtherBillCpidBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<%
  if(inputName != null && fieldName != null)
  {
    ...
    for(int i=0; i< length; i++)
    {
      ...
      out.print(".value=");     out.println("obj."+ (fieldName[i].equals("mxid")?"value;":"innerText;"));
    }
  }
  if(methodName != null)
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">单选相关单据产品列表</TD>
  </TR></TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectProduce(<%=i%>);"<%}%>>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)"  class="td"><%=list.getValue("djh")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)"  class="td"><%=prodRow.get("cpbm")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)"  class="td"><%=propertyBean.getLookupName(list.getValue("dmsxid"))%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sl_<%=i%>" class="td"><%=list.getValue("sl")%>
      <td nowrap onClick="checkRadio(<%=i%>)" id="scsl_<%=i%>" class="td"><%=list.getValue("scsl")%>
      <input type="hidden" id="djid_<%=i%>" name="djid_<%=i%>" value="<%=list.getValue("djid")%>">
      <input type="hidden" id="djkind_<%=i%>" name="djkind_<%=i%>" value="<%=list.getValue("djkind")%>">
      <input type="hidden" id="mxid_<%=i%>" name="mxid_<%=i%>" value="<%=list.getValue("mxid")%>">
      <input type="hidden" id="cpid_<%=i%>" name="cpid_<%=i%>" value="<%=list.getValue("cpid")%>">
      <input type="hidden" id="gxfdid_<%=i%>" name="gxfdid_<%=i%>" value="<%=list.getValue("gxfdid")%>">
    </tr>
    <%  list.next();
      }
      for(; i < iPage; i++){
