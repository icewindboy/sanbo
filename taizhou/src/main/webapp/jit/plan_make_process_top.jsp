...
%><%
  ...
  String SYS_APPROVE_ONLY_SELF =planMakeProcessBean.SYS_APPROVE_ONLY_SELF;//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
  boolean isApproveOnly = SYS_APPROVE_ONLY_SELF.equals("1") ? true : false;//true仅制单人可提交
  String SC_PLAN_ADD_STYLE =planMakeProcessBean.SC_PLAN_ADD_STYLE;//0=显示选择计划的对话框,1=直接生成通用计划,2=直接生成分切计划
...
function showDetail(masterRow){
  ...
  parent.bottom.location.href='plan_make_task_bottom.jsp?operate=<%=planMakeProcessBean.SHOW_DETAIL%>&rownum='+masterRow;
  unlockScreenWait();
}
...
<%String retu = planMakeProcessBean.doService(request, response);
  ...
  EngineDataSet list = planMakeProcessBean.getMaterTable();
  HtmlTableProducer table = planMakeProcessBean.masterProducer;
  String curUrl = request.getRequestURL().toString();
  String loginId = planMakeProcessBean.loginId;
%>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%BigDecimal t_zsl = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        if(planMakeProcessBean.isDouble(zsl))
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>)" onClick="showDetail(<%=list.getRow()%>)">
      ...
      <%if(isCancel){%><input name="image2" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=planMakeProcessBean.CANCEL_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%></td>
...
<%if(!planMakeProcessBean.masterIsAdd()){
    int row = planMakeProcessBean.getSelectedRow();
    if(row >= 0)
