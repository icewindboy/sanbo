...
%><%
  ...
  SC_PLAN_ADD_STYLE = "1";//dirctly generate common plan in JIT mode
...
function addExcute()
{
  if('<%=SC_PLAN_ADD_STYLE%>'=='')
    ...
    sumitForm(<%=mrpBean.SUBPLAN_MRP%>,-1);//直接生成分切计划物料
}
...
<%String retu = mrpBean.doService(request, response);
  ...
  engine.project.LookUp workShopBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_DEPT);//BEAN_WORKSHOP
