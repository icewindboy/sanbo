<%--采购申请单引入物料需求界面--%><%@ page contentType="text/html;charset=UTF-8"%><%@ include file="../pub/init.jsp"%>
...
%><%
  engine.erp.buy.Import_MrpGoods importMrpGoodsBean = engine.erp.buy.Import_MrpGoods.getInstance(request);
  String pageCode = "buy_apply";
  if(!loginBean.hasLimits(pageCode, request, response))
  return;
  String retu = importMrpGoodsBean.doService(request, response);
  ...
  EngineDataSet list = importMrpGoodsBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">物料需求明细列表</TD>
  </TR></TABLE>
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%RowMap prodRow = prodBean.getLookupRow(list.getValue("cpid"));%>
      ...
      <td nowrap onClick="checkRadio(<%=i%>)" id="xgl_<%=i%>" class="td" align="right"><%=list.getValue("xgl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="hssl_<%=i%>" class="td" align="right"><%=list.getValue("hssl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="wgl_<%=i%>" class="td" align="right"><%=list.getValue("wgl")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="dmsxid_<%=i%>" class="td"><%=list.getValue("dmsxid")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="xqrq_<%=i%>" class="td" align="left"><%=list.getValue("xqrq")%></td>
    </tr>
    <%  list.next();
      }
      for(; i < iPage; i++){
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="wlxqh" name="wlxqh" value='<%=importMrpGoodsBean.getFixedQueryValue("wlxqh")%>' onKeyDown="return getNextElement();"></TD>
              <TD align="center" nowrap class="td">产品名称</TD>
              <td nowrap class="td"><input class="EDLine" style="WIDTH:130px" name="product" value='<%=prodBean.getLookupName(importMrpGoodsBean.getFixedQueryValue("cpid"))%>' onKeyDown="return getNextElement();"readonly>
                <INPUT TYPE="HIDDEN" NAME="cpid" value="<%=importMrpGoodsBean.getFixedQueryValue("cpid")%>">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="rq$a" value='<%=importMrpGoodsBean.getFixedQueryValue("rq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="rq$b" style="WIDTH: 130px" name="rq$b" value='<%=importMrpGoodsBean.getFixedQueryValue("rq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
                <A href="#"><IMG title=选择日期 onClick="selectDate(rq$b);" height=20 src="../images/seldate.gif" width=20 align=absMiddle border=0></A></TD>
            </TR>
          </TABLE>
      <TR>
        <TD colspan="4" nowrap class="td" align="center"><INPUT class="button" onClick="sumitFixedQuery(<%=importMrpGoodsBean.FIXED_SEARCH%>)" type="button" value=" 查询(F)" name="button" onKeyDown="return getNextElement();">
        <pc:shortcut key="f" script='<%="sumitFixedQuery("+ importMrpGoodsBean.FIXED_SEARCH +",-1)"%>'/>
