...
%><%
  engine.erp.buy.ImportApply importApplyBean = engine.erp.buy.ImportApply.getInstance(request);
  String retu = importApplyBean.doService(request, response);
  ...
  EngineDataSet list = importApplyBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();">
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <%RowMap prodRow = prodBean.getLookupRow(list.getValue("cpid"));%>
      <td nowrap align="center" class="td"><input type="checkbox" name="sel" value='<%=list.getValue("cgsqdhwid")%>' onKeyDown="return getNextElement();"></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="dwmc_<%=i%>" class="td"><%=list.getValue("dwmc")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sqbh_<%=i%>" class="td"><%=list.getValue("sqbh")%> <input type="hidden" id="cgsqdhwid_<%=i%>" value='<%=list.getValue("cgsqdhwid")%>'></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sqrq_<%=i%>" class="td"><%=list.getValue("sqrq")%></td>
    ...
    <TABLE cellspacing=3 cellpadding=0 border=0>
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:130" id="wlxqh" name="sqbh" value='<%=importApplyBean.getFixedQueryValue("sqbh")%>' onKeyDown="return getNextElement();"></TD>
              <TD align="center" nowrap class="td">产品名称</TD>
              <td nowrap class="td"><input class="EDLine" style="WIDTH:130px" name="product" value='<%=prodBean.getLookupName(importApplyBean.getFixedQueryValue("cpid"))%>' onKeyDown="return getNextElement();"readonly>
                <INPUT TYPE="HIDDEN" NAME="cpid" value="<%=importApplyBean.getFixedQueryValue("cpid")%>">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="sqrq$a" value='<%=importApplyBean.getFixedQueryValue("sqrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" id="rq$b" style="WIDTH: 130px" name="sqrq$b" value='<%=importApplyBean.getFixedQueryValue("sqrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
                <A href="#"><IMG title=选择日期 onClick="selectDate(sqrq$b);" height=20 src="../images/seldate.gif" width=20 align=absMiddle border=0></A></TD>
            </TR>
          </TABLE>
      <TR>
        <TD colspan="4" nowrap class="td" align="center"><INPUT class="button" onClick="sumitFixedQuery(<%=importApplyBean.FIXED_SEARCH%>)" type="button" value=" 查询(F)" name="button" onKeyDown="return getNextElement();">
          <pc:shortcut key="f" script='<%="sumitFixedQuery("+ importApplyBean.FIXED_SEARCH +",-1)"%>'/>
