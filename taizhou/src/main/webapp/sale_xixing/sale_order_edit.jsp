                   ...
                   <div style="display:block;width:900;overflow-y:auto;overflow-x:auto;">
                    ...
                    %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap>
                        <input <%=(!isEnd&&!iscanedit)?detailClass:"class=ednone"%> id="sxz_<%=i%>" name="sxz_<%=i%>" value='<%=propertyBean.getLookupName(detail.get("dmsxid"))%>' onchange="if(form1.cpid_<%=i%>.value==''){alert('请先输入产品');return;}propertyNameSelect(this,form1.cpid_<%=i%>.value,<%=i%>)" onKeyDown="return getNextElement();" <%=(!isEnd&&!iscanedit)?readonly:"readonly"%>>
                        ...
                        <td class="td" nowrap align="right"><input type="text"  class="ednone"  onKeyDown="return getNextElement();" id="xsj_<%=i%>" name="xsj_<%=i%>" value='<%=detail.get("xsj")%>'  readonly></td>
                        <td class="td" nowrap align="right"><input type="text" <%=detailClass_r%>  onKeyDown="return getNextElement();" id="zk_<%=i%>" name="zk_<%=i%>" value='<%=detail.get("zk")%>' maxlength='<%=list.getColumn("zk").getPrecision()%>' onchange="dj_onchange(<%=i%>, true)" <%=readonly%>></td>
                        <td class="td" nowrap align="right"><input type="text" <%=detailClass_r%>  onKeyDown="return getNextElement();" id="dj_<%=i%>" name="dj_<%=i%>" value='<%=detail.get("dj")%>' maxlength='<%=list.getColumn("dj").getPrecision()%>' onchange="dj_onchange(<%=i%>, false)" <%=readonly%>></td>
                        <td><input type="text" class='ednone_r' size=16 onKeyDown="return getNextElement();" id="jje_<%=i%>" name="jje_<%=i%>" value='<%=detail.get("jje")%>' readonly></td>
