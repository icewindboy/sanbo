...
<%
  ...
  boolean hasSearchLimit = loginBean.hasLimits(pageCode, op_search);
    synchronized(b_StrikeBalanceBean){
...
function toDetail(){
  // 转到主从明细
  location.href='strike_balance_edit.jsp';
  return;
}
function toTihd(){
  // 转到提货单
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar"><TR>
    <TD NOWRAP align="center">销售冲帐单</TD>
  </TR>
</TABLE>
<%
  ...
  EngineDataSet list = b_StrikeBalanceBean.getMaterListTable();//主表数据集
  HtmlTableProducer table = b_StrikeBalanceBean.masterListProducer;//主表数据的表格打印
     ...
     <pc:dbNavigator dataSet="<%=key%>" pageSize="<%=pageSize%>"/>
      ...
      <td class="td" nowrap align="right">
        ...
        <%
            ...
            //if(b_StrikeBalanceBean.retuUrl!=null){
            String s = "location.href='"+b_StrikeBalanceBean.retuUrl+"'";
            //String s = "location.href='../pub/main.jsp'";//+b_StrikeBalanceBean.retuUrl+"'";
         %>
  ...
  <table id="tableview1" width="95%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    BigDecimal t_zsl = new BigDecimal(0), t_zje = new BigDecimal(0);
      ...
      for(; i<count; i++)   {
        ...
        if(b_StrikeBalanceBean.isDouble(zsl))
          t_zsl = t_zsl.add(new BigDecimal(zsl));
        String zje = list.getValue("zje");
        if(b_StrikeBalanceBean.isDouble(zje))
        ...
        boolean hasReferenced = b_StrikeBalanceBean.hasReferenced(list.getValue("tdid"));
        ...
        boolean isCancer=rowzt.equals("1") && loginId.equals(sprid)&&b_StrikeBalanceBean.isCanCancer(list.getValue("tdid"));
        boolean isCanOver = b_StrikeBalanceBean.isCanOver(list.getValue("tdid"));

        boolean cansubmit=false;
        boolean submitType = b_StrikeBalanceBean.submitType;//用于判断true=仅制定人可提交,false=有权限人可提交
    ...
    <tr id="tr_<%=list.getRow()%>" onDblClick="sumitForm(<%=Operate.EDIT%>,<%=list.getRow()%>);" onClick="selectRow();">
      <td <%=rowClass%> align="center" nowrap>
       ...
       <%if(isCancer){%>
           <input name="image3" class="img" type="image"  title='取消审批' onClick="if(confirm('确实要取消审批吗？'))sumitForm(<%=b_StrikeBalanceBean.CANCER_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
     <%if((rowzt.equals("1")||rowzt.equals("0"))&&loginBean.hasLimits(pageCode, op_cancel)){%>
         <input name="image3" class="img" type="image"   title='作废' onClick="if(confirm('确实要做废吗？'))sumitForm(<%=b_StrikeBalanceBean.LADDING_CANCER%>,<%=list.getRow()%>)" src="../images/close.gif" border="0"><%}%>
       <%if((rowzt.equals("1")&&loginBean.hasLimits(pageCode, op_over))){%>
         <input name="image3" class="img" type="image" title='完成' onClick="if(confirm('确认完成吗？'))sumitForm(<%=b_StrikeBalanceBean.LADING_OUT%>,<%=list.getRow()%>)" src="../images/ok.gif" border="0"><%}%>
     </td>
    ...
    <%  list.next();
      ...
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
       <td align="right" nowrap class="td"><input type="text" class="ednone_r" style="width:100%" value='<%=engine.util.Format.formatNumber(b_StrikeBalanceBean.getZje(),"#0.00")%>' readonly></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
      <td class="td" nowrap></td>
...
<%
  if(!b_StrikeBalanceBean.masterIsAdd()){
    int row = b_StrikeBalanceBean.getSelectedRow();//得到所选择的行号
    if(row >= 0)
      ...
      <TR>
        <TD> <TABLE cellspacing=3 cellpadding=0 border=0>
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH:160" id="tdbh" name="tdbh" value='<%=b_StrikeBalanceBean.getFixedQueryValue("tdbh")%>' maxlength='16' onKeyDown="return getNextElement();"></TD>
              <TD class="td" nowrap>制单人</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="czy" name="czy" value='<%=b_StrikeBalanceBean.getFixedQueryValue("czy")%>' maxlength='16' onKeyDown="return getNextElement();">
              </TD>
            </tr>
            <tr>
              <TD align="center" nowrap class="td">往来单位</TD>
              <td nowrap class="td" colspan=3>
              <input type="text" class="edbox" style="width:70" onKeyDown="return getNextElement();" name="dwdm" value='<%=b_StrikeBalanceBean.dwdm%>' onchange="customerCodeSelect(this)" >
              <input type="text" name="dwmc"  style="width:260" value='<%=b_StrikeBalanceBean.dwmc%>' class="edbox"  onchange="customerNameSelect(this)" >
              <INPUT TYPE="HIDDEN" NAME="dwtxid" value='<%=b_StrikeBalanceBean.getFixedQueryValue("dwtxid")%>'>
            ...
            <TR>
              ...
              <TD nowrap class="td"><INPUT class="edbox" style="WIDTH: 130px" name="czrq$a" value='<%=b_StrikeBalanceBean.getFixedQueryValue("czrq$a")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
              ...
              <TD class="td" nowrap><INPUT class="edbox" style="WIDTH: 130px" name="czrq$b" value='<%=b_StrikeBalanceBean.getFixedQueryValue("czrq$b")%>' maxlength='10' onChange="checkDate(this)" onKeyDown="return getNextElement();">
            ...
            <TR>
              ...
              <%=deptBean.getList(b_StrikeBalanceBean.getFixedQueryValue("deptid"))%></pc:select>
              </TD>
              <TD class="td" nowrap>业务员</TD>
              <TD class="td" nowrap>
                <pc:select name="personid" addNull="1" style="width:130">
                  <%=personBean.getList(b_StrikeBalanceBean.getFixedQueryValue("personid"))%>
                </pc:select>
             </TD>
             </TR>
             <tr>
              <TD class="td" nowrap>业务员模糊</TD>
              <TD class="td" nowrap>
              <INPUT class="edbox" style="WIDTH:160" id="personid$xm" name="personid$xm" value='<%=b_StrikeBalanceBean.getFixedQueryValue("personid$xm")%>' maxlength='16' onKeyDown="return getNextElement();">
             </TD>
             </tr>
            <TR>
              ...
              <TD colspan="3" nowrap class="td">
                <%
                  String [] zt = b_StrikeBalanceBean.zt;
                  String zt0="";
                  String zt9="";
                  String zt1="";
                  String zt4="";
