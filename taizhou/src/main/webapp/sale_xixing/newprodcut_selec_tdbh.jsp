...
%>
<%
  engine.erp.sale.xixing.B_SingleSelectProduct b_SingleSelectProductBean = engine.erp.sale.xixing.B_SingleSelectProduct.getInstance(request);
  ...
  String retu = b_SingleSelectProductBean.doService(request, response);
  ...
  EngineDataSet list = b_SingleSelectProductBean.getOneTable();
%>
<html>
<head>
<title></title>
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
  ...
  <table id="tableview1" width="90%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    ...
    <tr <%if(!isMultiSelect){%>onDblClick="checkRadio(<%=i%>);selectCorp(<%=i%>);"<%}%>>
      ...
      <input type="hidden" name="tdhwid_<%=i%>" value="<%=list.getValue("tdhwid")%>">
      </td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="tdbh_<%=i%>" class="td"><%=list.getValue("tdbh")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="cpbm_<%=i%>" class="td"><%=productRow.get("cpbm")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="product_<%=i%>" class="td"><%=productRow.get("product")%></td>
      <td nowrap onClick="checkRadio(<%=i%>)" id="sxz_<%=i%>" class="td"><%=propertyRow.get("sxz")%></td>
