<%--广告促销单--%><%@ page contentType="text/html; charset=UTF-8" %><%@ include file="../pub/init.jsp"%>
...
%>
<%
  engine.erp.sale.xixing.B_SalePromotion b_SalePromotionBean = engine.erp.sale.xixing.B_SalePromotion.getInstance(request);
...
function backList()
{
  location.href='sale_promotion.jsp';
}
//打印
...
function customerCodeSelect(obj)
{
  ...
  CustCodeChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dz','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',obj.value,'sumitForm(<%=b_SalePromotionBean.DWTXID_CHANGE%>,-1)');
}
//选择购货单位
...
CustSingleSelect('form1','srcVar=dwtxid&srcVar=dwmc&srcVar=dz','fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',form1.dwtxid.value,'sumitForm(<%=b_SalePromotionBean.DWTXID_CHANGE%>,-1)');
}
//输入购货单位名称
function customerNameSelect(obj)
{
  ...
  CustNameChange(document.all['prod'], obj.form.name,'srcVar=dwdm&srcVar=dwtxid&srcVar=dwmc&srcVar=dz','fieldVar=dwdm&fieldVar=dwtxid&fieldVar=dwmc&fieldVar=addr',obj.value,'sumitForm(<%=b_SalePromotionBean.DWTXID_CHANGE%>,-1)');
}
//运输单位
...
<%
  String retu = b_SalePromotionBean.doService(request, response);
  ...
  EngineDataSet ds = b_SalePromotionBean.getMaterTable();
  EngineDataSet list = b_SalePromotionBean.getDetailTable();
  EngineDataSet cyqktable = b_SalePromotionBean.getCyqkTable();
  EngineDataSet qtfytable = b_SalePromotionBean.getQtfyTable();


  HtmlTableProducer masterProducer = b_SalePromotionBean.masterProducer;
  HtmlTableProducer detailProducer = b_SalePromotionBean.detailProducer;


  RowMap masterRow = b_SalePromotionBean.getMasterRowinfo();//主表一行
  RowMap[] detailRows= b_SalePromotionBean.getDetailRowinfos();//从表多行
  RowMap[] cyqkRows= b_SalePromotionBean.getTdcyqkRowinfos();//从表多行
  RowMap[] qtfyRows= b_SalePromotionBean.getTdqtfyRowinfos();//从表多行


  creditBean.regData(ds,"dwtxid");
  if(b_SalePromotionBean.isApprove)
  {
  ...
  boolean isEnd =  b_SalePromotionBean.isReport||b_SalePromotionBean.isApprove || (!b_SalePromotionBean.masterIsAdd() && !zt.equals("0"));
  boolean isCanDelete = !isEnd && !b_SalePromotionBean.masterIsAdd() && loginBean.hasLimits(pageCode, op_delete);
  isEnd = isEnd || !(b_SalePromotionBean.masterIsAdd() ? loginBean.hasLimits(pageCode, op_add) : loginBean.hasLimits(pageCode, op_edit));
  ...
  boolean mustConversion = b_SalePromotionBean.conversion;//是否需要强制转换
          ...
          <tr>
            <td>
              <table CELLSPACING="1" CELLPADDING="1" BORDER="0" bgcolor="#f0f0f0" width="100%">
                 <tr>
                    <td  noWrap class="tdTitle">促销单编号</td>
              ...
              <tr>
                ...
                <td  noWrap class="td">
                  ...
                  <td noWrap class="td">
                    <input type="text" <%=dwtxClass%> name="thr" onKeyDown="return getNextElement();" value='<%=masterRow.get("thr")%>' style="width:110"   <%=dwtxRead%>>
                  </td>
                    ...
                    <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                      <tr class="tableTitle">
                        ...
                        <td height='20' align="center" nowrap>
                          <%if(!isEnd){%><%if(b_SalePromotionBean.canOperate){%>
                          <input type="hidden" name="cpids" value="" onchange='sumitForm(<%=b_SalePromotionBean.PRODUCT_ADD%>,-1)' >
                          <input name="image" class="img" type="image" title="引入促销产品(A)" onClick="selctHthwOfLading()" src="../images/add_big.gif" border="0">
                          <pc:shortcut key="a" script="selctHthwOfLading()" /><%}%>
                          <%}%>
                        </td>
                        <td height='20' nowrap>产品代码</td>
                        <td height='20' nowrap>产品名称</td>
                     ...
                     %>
                      <tr id="rowinfo_<%=i%>">
                        ...
                        <td class="td" nowrap>
                           ...
                           <input type="HIDDEN"   id="xsj_<%=i%>" name="xsj_<%=i%>" value='<%=detail.get("xsj")%>' readonly>
                           <input type="HIDDEN"   id="zk_<%=i%>" name="zk_<%=i%>" value='<%=detail.get("zk")%>' readonly>
                        ...
                        <input type="text" class="ednone"  onKeyDown="return getNextElement();" name="cpbm_<%=i%>" value='<%=productRow.get("cpbm")%>'  readonly></td><!--产品编码(存货代码)--->
                        <td class="td" nowrap><input type="text" class="ednone"  onKeyDown="return getNextElement();" id="product_<%=i%>" name="product_<%=i%>" value='<%=productRow.get("product")%>'     readonly></td>
                        ...
                        <td class="td" nowrap><input type="text" <%=djandjjeClass%>  onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=detail.get("sl")%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' onblur="nsl_onchange(<%=i%>, false)" <%=canedt?"":" readonly"%>></td><!--数量-->
                  ...
                  %>
                    ...
                    <SCRIPT LANGUAGE="javascript">rowinfo = new RowControl();
                      ...
                      initDefaultTableRow('tableview1',1);
                       <%=b_SalePromotionBean.adjustInputSize(new String[]{"kdsl","sl","dj","bz"}, "form1", detailRows.length)%>
                    </SCRIPT>
                  </td>
                </tr>
              </table>
...
<table id="tableview2" width="745" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
   <tr class="tableTitle">
     ...
     <td height='20' align="center" nowrap width=30>
      <%
       ...
       %>
      <%if(b_SalePromotionBean.canOperate){%>
      <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=b_SalePromotionBean.CYQK_ADD%>)" src="../images/add_big.gif" border="0">
      <%}%>
      <%}%>
     </td>
     <td nowrap>类型</td>
   ...
   <tr id="rowinfo_<%=k%>">
     ...
     <%if(cyrowcount>0&&!cyrow.get("dwtxid").equals("")){%><input name="image32" class="img" type="image" title="打印承运单据" onClick="location.href='../pub/pdfprint.jsp?code=shyu_sendlist_ty_print&operate=<%=Operate.PRINT_BILL%>&a$tdcyqkid=<%=cyrow.get("tdcyqkid")%>&src=../sale_xixing/sale_promotion_edit.jsp'" src="../images/print.gif" border="0"><%}%>
     <input name="image" class="img" type="image" title="删除" onClick="if(confirm('是否删除该记录？')) sumitForm(<%=b_SalePromotionBean.CYQK_DEL%>,<%=k%>)" src="../images/delete.gif" border="0">
     </td>
     <td class="td" nowrap>
     <%
 ...
 <center>
   <table id="tableview3" width="745" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
     ...
     <td height='20' align="center" nowrap width=30>
       ...
       if(b_SalePromotionBean.canOperate){
      %>
      <input name="image" class="img" type="image" title="新增(A)" onClick="sumitForm(<%=b_SalePromotionBean.QTFY_ADD%>)" src="../images/add_big.gif" border="0">
   ...
   %>
     <tr id="rowinfo_<%=n%>">
       <td class="td"><%=n+1%></td>
       <td class="td" nowrap><input name="image" class="img" type="image" title="删除" onClick="if(confirm('是否删除该记录？')) sumitForm(<%=b_SalePromotionBean.QTFY_DEL%>,<%=n%>)" src="../images/delete.gif" border="0"></td>
            ...
            <td colspan="3" noWrap class="tableTitle">
             <%
              ...
              <%
               ...
               if(zt.equals("0"))
               {
                 if(!b_SalePromotionBean.isReport){
              ...
              <pc:shortcut key="s" script="<%=post%>" />
               <%}
                 ...
                 if(!b_SalePromotionBean.isReport){
                   String appost ="sumitForm("+b_SalePromotionBean.APPROVED_MASTER_ADD+");";
             %>
              <input name="btnback" style="width:80" type="button" class="button" onClick="sumitForm(<%=b_SalePromotionBean.APPROVED_MASTER_ADD%>);" value="保存(S)">
              <pc:shortcut key="s" script="<%=appost%>" />
              <%
              }}if(isCanDelete&&!b_SalePromotionBean.isReport){
               String del = "if(confirm('是否删除该记录？'))sumitForm("+Operate.DEL+");";
               String detNull = "sumitForm("+b_SalePromotionBean.DEL_NULL+");";
              %>
              ...
              <input type="button" class="button" value="打包单(P)" onclick="location.href='../pub/pdfprint.jsp?code=shyu_sale_promotiont_package_print&operate=<%=Operate.PRINT_BILL%>&a$tdid=<%=masterRow.get("tdid")%>&src=../sale_xixing/send_list_edit.jsp'">
              <%}%>
              <%if(zt.equals("8")){%>
              <input type="button" class="button" value="打印(P)" onclick="location.href='../pub/pdfprint.jsp?code=shyu_sale_promotiont_print&operate=<%=Operate.PRINT_BILL%>&a$tdid=<%=masterRow.get("tdid")%>&src=../sale_xixing/sale_promotion_edit.jsp'">
              <%if(cyrowcount>0&&!cyrow.get("dwtxid").equals("")){%><input type="button" class="button" value="托运单打印(P)" onclick="location.href='../pub/pdfprint.jsp?code=shyu_sendlist_ty_print&operate=<%=Operate.PRINT_BILL%>&a$tdid=<%=masterRow.get("tdid")%>&src=../sale_xixing/send_list_edit.jsp'"> <%}%>
              <%}%>
...
<script language="javascript">
  ...
  <%=b_SalePromotionBean.adjustInputSize(new String[]{"cpbm","product","sxz","sl","dj","bz"}, "form1", detailRows.length)%>
...
function selctHthwOfLading()
{
  ...
  TdhwMultiSelect('form1','srcVar=cpids','fieldVar=cpid&storeid='+form1.storeid.value,form1.dwtxid.value,"sumitForm(<%=b_SalePromotionBean.PRODUCT_ADD%>,-1)");
}
function TdhwMultiSelect(frmName,srcVar,fieldVar,curID,methodName,notin)
{
     ...
     paraStr = "../sale_xixing/import_promotion_product.jsp?operate=0&multi=1&srcFrm="+frmName+"&"+srcVar+"&"+fieldVar+"&dwtxid="+curID;
     if(methodName+'' != 'undefined')
       paraStr += "&method="+methodName;
     if(notin+'' != 'undefined')
       paraStr += "&notin="+notin;
...
<%//&#$
if(b_SalePromotionBean.isApprove){%><jsp:include page="../pub/approve.jsp" flush="true"/><%}%>
<%out.print(retu);%>
</BODY>
</Html>