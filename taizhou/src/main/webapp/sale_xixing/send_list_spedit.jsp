                     ...
                     %>
                      ...
                      <%--  <%if(canedt){%>--%>
                       <%if(!isEnd&&!isprops.equals("0")){%>
                        ...
                        <td class="td" nowrap><input type="text"  <%=djandjjeClass%>  onKeyDown="return getNextElement();" id="sl_<%=i%>" name="sl_<%=i%>" value='<%=b_SendBillBean.isyrdd==true?(wkdsl=="0"?detail.get("sl"):wkdsl):detail.get("sl")%>'  onchange="sl_onchange(<%=i%>, false)" <%=canedt?"":" readonly"%>></td><!--数量-->
                       ...
                       <%-- <td class="td" nowrap><input type="text"  <%=djandjjeClass%>  onKeyDown="return getNextElement();" id="hssl_<%=i%>" name="hssl_<%=i%>" value='<%=detail.get("hssl")%>' maxlength='<%=list.getColumn("sl").getPrecision()%>' size='<%=list.getColumn("hssl").getPrecision()-6%>' onchange="sl_onchange(<%=i%>, true)"  <%=canedt?"":" readonly"%>></td><!--换算数量-->
                        ...
                        <input type="hidden" class='ednone_r'    onKeyDown="return getNextElement();" id="xsj_<%=i%>" name="xsj_<%=i%>" value='<%=detail.get("xsj")%>' readonly>
                        <!--销售价-->
                        <input type="hidden" <%=detailClass_r%>   onKeyDown="return getNextElement();" id="zk_<%=i%>" name="zk_<%=i%>" value='<%=detail.get("zk")%>'  <%=readonly%>>
                       <!--折扣-->
                        <input type="hidden" <%=detailClass_r%>   onKeyDown="return getNextElement();" id="dj_<%=i%>" name="dj_<%=i%>" value='<%=detail.get("dj")%>' <%=readonly%> >
                       <!--单价-->
                        <input type="hidden" class='ednone_r'  onKeyDown="return getNextElement();" id="jje_<%=i%>" name="jje_<%=i%>" value='<%=detail.get("jje")%>'  readonly>
                        <!--净金额-->
