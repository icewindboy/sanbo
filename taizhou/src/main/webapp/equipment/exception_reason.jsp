...
<script LANGUAGE="javascript">
  ...
  function showInterFrame(oper, rownum){
    var url = "exception_reasonedit.jsp?operate="+oper+"&rownum="+rownum;
...
<%
  EngineDataSet list = B_ExceptionReasonBean.getOneTable();
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;">
  ...
  <TABLE WIDTH="600" BORDER=0 CELLSPACING=0 CELLPADDING=0 align="center">
    <TR><TD align="right">
    <%if(loginBean.hasLimits(pageCode,op_add)) { String ret = "location.href='("+B_ExceptionReasonBean.retuUrl+",-1)";%>
    <%if(B_ExceptionReasonBean.retuUrl!=null)%>
    <input name="button2222232" type="button" align="Right" title = "返回" class="button" onClick="location.href='<%=B_ExceptionReasonBean.retuUrl%>'" value=" 返回(C) "border="0"><pc:shortcut key="c" script='<%=ret%>'/><%}%>
   </TD></TR>
  </TABLE>
