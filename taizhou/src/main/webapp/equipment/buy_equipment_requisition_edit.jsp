...
%>
<%
  if(!loginBean.hasLimits("buy_equipment_requisition", request, response))
    return;
  ...
  b_EquipmentBuyApplyBean.apply_type="1";
%>
<html>
<head>
<title></title>
...
function backList()
{
  location.href="buy_equipment_requisition.jsp";
}
  ...
  <table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="center" width="750">
    <tr valign="top">
      <td><table border=0 CELLSPACING=0 CELLPADDING=0 class="table">
        <tr>
            <td width="160" class="activeVTab" id='lx'>生产设备购置申请(<%=title%>)</td>
          </tr>
        </table>
                      ...
                      <table id="tableview1" width="750" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
                        ...
                        <tr class="tableTitle">
                          ...
                          <%
                            ...
                            detail = detailRows[i];
                            RowMap equipmentRow = equipmentBean.getLookupRow(detail.get("equipmentid"));
                         %>
                          <tr id="rowinfo_<%=i%>">
                          <%if(!isEnd){%><td class="td"><div align="center">
                          <img name="image2" type="image" class="img" title="选择设备" onClick="equipmentSingleSelect('form1','srcVar=equipmentID_<%=i%>&srcVar=equipment_code_<%=i%>&srcVar=equipment_name_<%=i%>&srcVar=standard_gg_<%=i%>&srcVar=manufacturer_<%=i%>','fieldVar=equipmentid&fieldVar=equipment_code&fieldVar=equipment_name&fieldVar=standard_gg&fieldVar=manufacturer','','','')" src="../images/select_prod.gif" width="15" height="15" border="0">
                          ...
                          <td class="td">
                          <input type="hidden" name="equipmentID_<%=i%>" value='<%=detail.get("equipmentID")%>'>
                          <input type="text" name="equipment_code_<%=i%>" value='<%=detail.get("equipment_code")%>' maxlength='32' style="width:100%" <%=noneClass%> <%=readonly%>></td>
                          <td class="td">
                          <input type="text" name="equipment_name_<%=i%>" value='<%=detail.get("equipment_name")%>' maxlength='32' style="width:100%" <%=noneClass%> <%=readonly%>></td>
                          <td class="td">
                          <input type="text" name="standard_gg_<%=i%>" value='<%=detail.get("standard_gg")%>' maxlength='32' style="width:100%" <%=noneClass%> <%=readonly%>></td>
                          <td class="td">
                          <input type="text" name="manufacturer_<%=i%>" value='<%=detail.get("manufacturer")%>' maxlength='32' style="width:100%" <%=noneClass%> <%=readonly%>>
                          </td>
                          <td class="td">
   ...
   <td colspan="=8">
    <table CELLSPACING=0 CELLPADDING=0 width="760" align="center">
                <tr>
                  <td width="57" class="td"><div align="center"><strong>主要功能</strong></div></td>
                  <td width="701" class="td"><textarea name="main_function" rows="2" onKeyDown="return getNextElement();" style="width: 690; height: 65" cols="20"><%=masterRow.get("main_function")%></textarea></td>
                </tr>
              </table>
   </td>
     </tr>
