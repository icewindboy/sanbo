<!--生产设备购置申请--><%@page contentType="text/html; charset=UTF-8" %><%@ include file="../pub/init.jsp"%>
...
<script language="javascript">
  function toDetail(){
  // 转到主从明细
  parent.location.href='buy_equipment_requisition_edit.jsp';
}
function showDetail(mastereditRow){
  //单击主表的一行激活的操作
  selectRow();
  lockScreenToWait("处理中, 请稍候！");
   parent.bottom.location.href='buy_equipment_requisition_bottom.jsp?operate=<%=b_EquipmentBuyApplyBean.SHOW_DETAIL%>&rownum='+mastereditRow;
     unlockScreenWait();
}
...
<TD NOWRAP align="center">生产设备购置申请</TD>
</TR>
</TABLE>
<%
...
<form name="form1" method="post" action="<%=curUrl%>" onsubmit="return false;" onKeyDown="return onInputKeyboard();" >
 ...
 <%if(isCancelApprove){%><input name="image3" class="img" type="image" title='取消审批' onClick="if(confirm('是否取消审批该纪录？'))sumitForm(<%=b_EquipmentBuyApplyBean.CANCLE_APPROVE%>,<%=list.getRow()%>)" src="../images/clear.gif" border="0"><%}%>
   ...
   <td nowrap <%=rowClass%>><a href='javascript:'onClick="openUrlOpt1('../pub/approve_result.jsp?operate=0&project=buy_equipment_requisition&id=<%=list.getValue("buy_applyid")%>')">
   <%=stateval%></a></td><!--状态-->
