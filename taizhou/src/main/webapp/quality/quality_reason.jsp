...
<%if(!loginBean.hasLimits("quality_reason", request, response))
    return;
  //engine.erp.quality.B_CheckStandard ReasonBean=engine.erp.quality.B_CheckStandard.getInstance(request);
...
<SCRIPT LANGUAGE="javascript">
  ...
  {
    var url = "quality_reason_edit.jsp?operate="+oper+"&rownum="+rownum;
...
<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 CLASS="headbar">
  <TR>
    <TD colspan="6" align="center" NOWRAP>质量原因</TD>
  </TR>
</TABLE>
<%
  EngineDataSet list = ReasonBean.getOneTable();
...
</td> <TD align="right"><%if(ReasonBean.retuUrl!=null){%><input name="button2222232" type="button" align="Right"
  class="button" onClick="location.href='<%=ReasonBean.retuUrl%>'" value=" 返回 "border="0"><%}%></TD>
    </TR>
  </TABLE>
  <table id="tableview1" width="60%" border="0" cellspacing="1" cellpadding="1" class="table" align="center">
    <tr class="tableTitle">
      <td width="242" nowarp>原因编码</td>
      <td width="400" nowarp>原因名称</td>
      <td nowarp width="70">
    ...
    <tr onDblClick="showInterFrame(<%=Operate.EDIT%>,<%=list.getRow()%>)">
      <td class="td" nowarp>
      &nbsp;<%
       String reasonCode  = list.getValue("reasonCode");
          out.print(reasonCode );
      %>
      </td>
      <td class="td" nowarp>&nbsp;<%=list.getValue("reasonName")%></td>
      <td class="td" nowarp>
...
<div class="queryPop" id="detailDiv" name="detailDiv">
  ...
  <TABLE cellspacing=0 cellpadding=0 border=0>
    <TR>
      <TD><iframe id="interframe1" src="" width="210" height="160" marginWidth="0" marginHeight="0" frameBorder="0" noResize scrolling="no"></iframe>
      </TD>
    </TR>
  </TABLE>
</div>
