...
<%String retu = certifiedBean.doService(request, response);
  ...
  String creatorID=row.get("creatorID");
  boolean hasid=!creatorID.equals("");
  boolean isCanAdd =loginBean.hasLimits(pageCode, op_edit)||loginBean.hasLimits(pageCode,op_add);//权限
  if(hasid)
   isCanAdd=isCanAdd&&loginBean.hasLimits(pageCode, op_edit);
  boolean CanAdd =loginBean.hasLimits(pageCode,op_add);
