...
public final class B_BugGrade extends BaseAction implements Operate
{
  /**
   * 提取收发单据列表所有信息的SQL语句
   */
  private static final String Quality_SQL = "SELECT * FROM zl_bugGrade";//
   ...
   * @return 返回收发单据信息的实例
   */
  public static B_BugGrade getInstance(HttpServletRequest request)
  {
    B_BugGrade b_BugBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_BugBean";
      b_BugBean = (B_BugGrade)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(b_BugBean == null)
      {
        b_BugBean = new B_BugGrade();
        session.setAttribute(beanName, b_BugBean);
      }
    }
    return b_BugBean;
  }
  ...
  protected void jbInit() throws Exception
  {
    ...
    dsWorkStandard.setSort(new SortDescriptor("", new String[]{"bugID"}, new boolean[]{false}, null, 0));
        dsWorkStandard.setSequence(new SequenceDescriptor(new String[]{"bugID"}, new String[]{"S_zl_bugGrade"}));
    //添加操作的触发对象
  ...
  class B_WorkProcedure_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String bugCode = rowInfo.get("bugCode");
      String bugName = rowInfo.get("bugName");
      //if(bugCode.equals("")){
      ...
      ds.setValue("bugCode", bugCode);
      ds.setValue("bugName", bugName);
      ds.post();
      ds.saveChanges();
