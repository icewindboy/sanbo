 ...
 * @version 1.0
 */
public final class Select_DXPromotion_Product extends BaseAction implements Operate
{
   ...
   * 析构函数
   * */
  public static Select_DXPromotion_Product getInstance(HttpServletRequest request)
  {
    Select_DXPromotion_Product Select_DXPromotion_ProductBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "Select_DXPromotion_ProductBean";
      Select_DXPromotion_ProductBean = (Select_DXPromotion_Product)session.getAttribute(beanName);
      if(Select_DXPromotion_ProductBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        Select_DXPromotion_ProductBean = new Select_DXPromotion_Product();
        Select_DXPromotion_ProductBean.fgsid = loginBean.getFirstDeptID();
        session.setAttribute(beanName, Select_DXPromotion_ProductBean);
      }
    }
    return Select_DXPromotion_ProductBean;
  }
  /**
   * 构造函数
   */
  private Select_DXPromotion_Product()
  {
    try {
      jbInit();
    }
