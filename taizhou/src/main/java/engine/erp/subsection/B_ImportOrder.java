 ...
 *提货单引入合同
 * */
public final class B_ImportOrder extends BaseAction implements Operate
{
  public  static final String SHOW_DETAIL = "9001";

  private static final String MASTER_STRUT_SQL = "SELECT  * FROM xs_ht WHERE 1<>1 ";
  private static final String MASTER_SQL = "SELECT  * FROM xs_ht WHERE htid in(SELECT DISTINCT htid FROM VW_SALE_HTHW WHERE zt=1 and abs(nvl(skdsl,0))<abs(nvl(sl,0)) AND ? AND fgsid=? ? ) ";
  //"SELECT * FROM VW_SALE_HTHW WHERE zt=1 and abs(nvl(skdsl,0))<abs(nvl(sl,0)) AND ? AND fgsid=? ? ";
  ...
  /*这样会出现引合同的次数超过一次时，无论是否引用完，都会出现在top页面上，需要在top页面上使用lookupBean，engine.project.LookUp ykslBean = engine.project.LookupBeanFacade.getInstance(request, engine.project.SysConstant.BEAN_TDHW_YKTDSL); //已开提单数量 2004.12.03 wlc
      = "SELECT * FROM xs_ht WHERE htid IN ( "
      + " SELECT b.htid FROM xs_ht a, xs_hthw b, xs_tdhw c, vw_xs_wzdj d "
      + " WHERE a.htid=b.htid AND b.hthwid=c.hthwid(+) AND b.wzdjid=d.wzdjid "
      + " AND a.zt=1 AND ? AND a.fgsid=? ? "
      + " GROUP BY b.htid, b.sl "
...
*/
  ...
  private static final String DETAIL_SQL
      = "select * From VW_SALE_HTHW  where  zt=1 and abs(nvl(skdsl,0))<abs(nvl(sl,0)) and htid= ";
  ...
  public static B_ImportOrder getInstance(HttpServletRequest request)
  {
    ...
    {
      String beanName = "b_DxImportOrderBean";
 ...
 //       SQL = SQL+" AND a.sendmodeid='"+sendmodeid+"' ";
