...
public final class B_SaleExchange extends BaseAction implements Operate
{
  ...
  public  static final String D_DETAIL_MULTI_ADD    = "369258";
...
//  private engine.erp.subsection.B_ImportOrder b_ImportOrderBean = null; //提货单引用销售合同
//  private engine.erp.subsection.ImportOrderProduct importOrderProductBean = null;
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    addObactioner(String.valueOf(D_DETAIL_MULTI_ADD), new D_Detail_Multi_Add());
        /*
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("dj",formatNumber( rowInfo.get("ddj_"+i),priceFormat));//单价
      detailRow.put("zk", rowInfo.get("dzk_"+i));//折扣
...
public RowMap getkhtyzk(String xydj,String cplx)
{
 ...
 try{
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<dsDetailTable.getRowCount(); i++)
      {
        ...
        zje =zje+sl*dj;
        dsDetailTable.setValue("xsj", detailrow.get("xsj"));//销售价
        dsDetailTable.setValue("sl", detailrow.get("sl"));//数量
        dsDetailTable.setValue("dj", detailrow.get("dj"));//单价
        dsDetailTable.setValue("zk", detailrow.get("zk"));//折扣
      ...
      for(int i=0; i<dsThTable.getRowCount(); i++)
      {
        ...
        double dj = detailrow.get("dj").length() > 0 ? Double.parseDouble(detailrow.get("dj")) : 0;//单价
        zje =zje-sl*dj;
        dsThTable.setValue("xsj",detailrow.get("xsj"));//销售价
        dsThTable.setValue("sl", detailrow.get("sl"));//数量
        dsThTable.setValue("zk", detailrow.get("zk"));//数量
  ...
  /***物资多选**/
  class D_Detail_Multi_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      HttpServletRequest req = data.getRequest();
      putDetailInfo(data.getRequest());
      String importOrder = m_RowInfo.get("dwzdjid");
      ...
      EngineRow locateGoodsRow = new EngineRow(dsThTable, "wzdjid");
      ...
      for(int i=0; i < tmp.getRowCount(); i++)
      {
        ...
        if(!dsThTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          if(cnn!=null&&!cnn.equals("0"))
        {
          ...
          dsThTable.setValue("tdhwid", "-1");
          dsThTable.setValue("tdid", tdid);
          dsThTable.setValue("wzdjid", wzdjid);
          if(xsj==null||xsj.equals(""))
            continue;
          if(mrzk==null)
            mrzk="";
          dsThTable.setValue("xsj", xsj);
          dsThTable.setValue("zk", mrzk);
          dsThTable.setValue("cpid", cpid);
          dsThTable.post();
          //创建一个与用户相对应的行
          detailrow = new RowMap(dsThTable);
          d_TdcyqkRowInfos.add(detailrow);
        }
        tmp.next();
      }
  ...
  class Detail_Change implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        String sl=detailRow.get("sl");
        String oldje=detailRow.get("jje");
        if(oldje.equals(""))
        oldje="0";
        if(rownum==i)
        {
          ...
          double ddj =  Double.parseDouble(zk)*Double.parseDouble(xsj)/100;
          dj=String.valueOf(ddj);
          }
          else
          {
          ...
          double ddj =  Double.parseDouble(mrzk)*Double.parseDouble(xsj)/100;
          dj=String.valueOf(ddj);
          zk=mrzk;
          }
          }
          ...
          if(!sl.equals(""))
          {
           je = Double.parseDouble(sl)*Double.parseDouble(dj);
          }
          tje=je-Double.parseDouble(oldje);
          detailRow.put("jje", String.valueOf(je));
          detailRow.put("xsj", xsj);
          detailRow.put("dj", dj);
          detailRow.put("zk", zk);
          zje=zje+tje;
          ...
          zzje=String.valueOf(Double.parseDouble(tzje)+zje);
          m_RowInfo.put("zje",zzje);
    }
  }
  /**
   *  从表更新
  ...
  class Detail_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(String.valueOf(DETAIL_DEL).equals(action))
      {
        ...
        zzje=String.valueOf(Double.parseDouble(zje)-Double.parseDouble(jje));
        m_RowInfo.put("zje",zzje);
      }
      else if(String.valueOf(CYQK_DEL).equals(action))
      {
        RowMap detailRow = (RowMap)d_TdcyqkRowInfos.get(rownum);
        String zje=m_RowInfo.get("zje");
        if(zje.equals(""))
          zje="0";
        String jje=detailRow.get("jje");
