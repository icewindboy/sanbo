...
public final class B_DxReceivableAccount extends BaseAction implements Operate
{
  public static final String FIXED_SEARCH = "1009";
  public static final String OVER= "1001";
  public static final String copy= "1002";
  public static final String DETAIL_CHANGE = "1003";
  public static final String BATCH_ADD = "1004";

  private static final String RECEIVEABLE_ACCOUNT_SAVE_STRUCT_SQL = "select * from xs_ysk_fb where 1<>1";
  private static final String RECEIVEABLE_ACCOUNT_SAVE_SQL = "select * from VW_XS_YSK_FB where fgsid=? ? AND ? ";//

  private static final String SUM_SQL = "select SUM(nvl(ysk,0))zysk from VW_XS_YSK_FB where fgsid=? ? and ? ";//

  private static final String BATCH_IMPORT_SQL = "SELECT a.dwtxid,a.dqh,a.dwdm,a.fgsid,a.personid "
                                               + "FROM dwtx a,dwtx_lx b,dwdq c,bm d "
                                               + "WHERE a.deptid=d.deptid AND d.ismember=1 AND a.dwtxid=b.dwtxid AND b.ywlx=2 AND a.dqh=c.dqh AND a.Isdelete='0' AND a.? ";

  private static final String B_QUERY_SQL = "select * from VW_XS_YSK_FB t  WHERE t.fgsid=? ? AND t.? ";//
  private static final String B_update_SQL = "update xs_ysk_fb set rq=";//
//  private static final String B_update = "UPDATE systemparam t SET t.value='1' WHERE t.code='XS_INIT_OVER'";
  private static final String xs_init_over = "select count(*) from VW_XS_YSK_FB where fgsid=? and ? and isinit=1 ";
  /**
   *本地数据集
   * 保存物资销售单价信息的数据集
   */
  ...
  public  HtmlTableProducer table = new HtmlTableProducer(dsB_ReceivableAccount_save, "xs_ysk_fb", "xs_ysk_fb");
  /**
   * 点击返回按钮的URL
   */
  ...
  public static B_DxReceivableAccount getInstance(HttpServletRequest request)
  {
    B_DxReceivableAccount b_DxReceivableAccountBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_DxReceivableAccountBean";
      b_DxReceivableAccountBean = (B_DxReceivableAccount)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      //b_employeecardnoBean = (B_EmployeeCardNo)session.getAttribute(beanName);
      if(b_DxReceivableAccountBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        fgsid = loginBean.getFirstDeptID();
        b_DxReceivableAccountBean = new B_DxReceivableAccount(fgsid);
        b_DxReceivableAccountBean.loginName = loginBean.getUserName();
        b_DxReceivableAccountBean.user = loginBean.getUser();
//        b_DxReceivableAccountBean.XS_INIT_OVER = loginBean.getSystemParam("XS_INIT_OVER");
        session.setAttribute(beanName,b_DxReceivableAccountBean);
      }
    }
     return b_DxReceivableAccountBean;
  }
  /**
   * 构造函数(实例变量:分公司ID为初始化参数)
   */
  private B_DxReceivableAccount(String fgsid)
 {  this.fgsid = fgsid;
   try {
     jbInit();
   }
  ...
  protected void jbInit() throws Exception
  {
    ...
    dsB_ReceivableAccount_save.setTableName("xs_ysk_fb");
  ...
  {
    try{
      String opearate = request.getParameter("operate");
 ...
 public final String getZysk()
 {
  ...
  class dsB_ReceivableAccount_save_Init implements Obactioner
  {
    ...
    {
      XS_INIT_OVER = dataSetProvider.getSequence(combineSQL(xs_init_over,"?",new String[]{fgsid,user.getHandleDeptValue()}));
      retuUrl = data.getParameter("src");
      retuUrl = retuUrl!= null ? retuUrl.trim() : retuUrl;
      table.getWhereInfo().clearWhereValues();
      String SQL = combineSQL(RECEIVEABLE_ACCOUNT_SAVE_SQL,"?",new String[]{fgsid,"",user.getHandleDeptValue()});
      //SUM_SQL
      YSSQL =  combineSQL(SUM_SQL,"?",new String[]{fgsid,"",user.getHandleDeptValue()});
 ...
 class FIXED_SEARCH implements Obactioner
 {
   ...
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     YSSQL =  combineSQL(SUM_SQL,"?",new String[]{fgsid, SQL,user.getHandleDeptValue()});
     EngineDataSet dssl = new EngineDataSet();
     setDataSetProperty(dssl,YSSQL);
     dssl.open();
     dssl.first();
     ...
     SQL = combineSQL(B_QUERY_SQL, "?", new String[]{fgsid, SQL,user.getHandleDeptValue()});
  ...
  class dsB_ReceivableAccount_save_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        String count=null;
        if(khlx.equals(""))
          count=dataSetProvider.getSequence("SELECT count(*) FROM xs_ysk_fb WHERE dwtxid='"+dwtxid+"' AND fgsid='"+fgsid+"'");
        else
          count=dataSetProvider.getSequence("SELECT count(*) FROM xs_ysk_fb WHERE dwtxid='"+dwtxid+"' AND fgsid='"+fgsid+"' AND khlx='"+khlx+"'");
        ...
        if(isOver)
        dsB_ReceivableAccount_save.setValue("isinit", "1");
...
//      XS_INIT_OVER = dataSetProvider.getSequence("select value from systemparam where code='XS_INIT_OVER' ");
    }
  }
  /**
   *  从表更新
  ...
  class Batch_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      SQL = combineSQL(SQL, "?", new String[]{user.getHandleDeptValue(),SQL});
