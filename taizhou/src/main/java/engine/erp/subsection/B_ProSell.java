...
/**
 * <p>Title: 销售管理--促销发货单--</p>
 * <p>Description: 销售管理--促销发货单--<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author engine
 * @version 1.0
 */
public final class B_ProSell extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM xs_td WHERE djlx=14 AND ? AND fgsid=? ?  order by tdbh desc";
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=14 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL    = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=14 AND ? AND fgsid=? ?  ";
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td.14");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_ProSell getInstance(HttpServletRequest request)
  {
    B_ProSell B_ProSellBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_ProSellBean";
      B_ProSellBean = (B_ProSell)session.getAttribute(beanName);
      if(B_ProSellBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        B_ProSellBean = new B_ProSell();
        B_ProSellBean.qtyFormat = loginBean.getQtyFormat();
        B_ProSellBean.priceFormat = loginBean.getPriceFormat();
        B_ProSellBean.sumFormat = loginBean.getSumFormat();
        B_ProSellBean.fgsid = loginBean.getFirstDeptID();
        B_ProSellBean.loginId = loginBean.getUserID();
        B_ProSellBean.loginName = loginBean.getUserName();
        B_ProSellBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          B_ProSellBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          B_ProSellBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        B_ProSellBean.dsMasterTable.setColumnFormat("zje", B_ProSellBean.sumFormat);
        B_ProSellBean.dsMasterTable.setColumnFormat("zsl", B_ProSellBean.qtyFormat);

        B_ProSellBean.dsDetailTable.setColumnFormat("sl", B_ProSellBean.qtyFormat);
        B_ProSellBean.dsDetailTable.setColumnFormat("hssl", B_ProSellBean.qtyFormat);
        B_ProSellBean.dsDetailTable.setColumnFormat("xsj", B_ProSellBean.priceFormat);
        B_ProSellBean.dsDetailTable.setColumnFormat("dj", B_ProSellBean.priceFormat);
        B_ProSellBean.dsDetailTable.setColumnFormat("jje", B_ProSellBean.priceFormat);
        B_ProSellBean.dsDetailTable.setColumnFormat("xsje", B_ProSellBean.priceFormat);

        session.setAttribute(beanName, B_ProSellBean);
      }
    }
    return B_ProSellBean;
  }
  /**
   * 构造函数
   */
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.14','tdbh') from dual"}));
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.14','tdbh') from dual");
 ...
 */
   class Add_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       String content = dsMasterTable.getValue("tdbh");
       String deptid = dsMasterTable.getValue("deptid");
       approve.putAproveList(dsMasterTable, dsMasterTable.getRow(),"prosell", content,deptid);
     }
  }
  ...
  */
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(),"prosell");
    }
  }
  /**
   * 作废
  ...
  */
  ...
  class Detail_Change implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     Select_DXPromotion_Product Select_DXPromotion_ProductBean =Select_DXPromotion_Product.getInstance(req);
     for(int i=0; i < cpids.length; i++)
     {
       ...
       {
         RowMap saleRow =null;
         saleRow = Select_DXPromotion_ProductBean.getLookupRow(cpids[i]);
