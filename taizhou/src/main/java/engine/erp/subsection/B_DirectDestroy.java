 ...
 * 2004-4-20 18:08 将原来的djxz=1改成现在的djxz=17
...
public final class B_DirectDestroy extends BaseAction implements Operate
{
  ...
  public  static final String PRODUCE_CHANGE         = "173928";//选择产品触发的事件

  private static final String MASTER_STRUT_SQL = "SELECT * FROM kc_sfdj WHERE 1<>1";
  private static String MASTER_SQL       = "SELECT * FROM kc_sfdj WHERE ? AND djxz=? and fgsid=? ? ORDER BY sfrq DESC";
  ...
  private static final String DETAIL_STRUT_SQL = "SELECT kc_sfdjmx.* FROM kc_sfdjmx WHERE 1<>1";
  //02.28 23:15  修改 将下面此句的sql,用了'?'来代替原来的sfdjid=.
  //解决了当sdfdjid是空的时候会页面上(主要是contract_instore_bottom.jsp上)会出现sql错误的问题.yjg
  private static final String DETAIL_SQL         = "SELECT kc_sfdjmx.* FROM kc_sfdjmx kc_sfdjmx, kc_dm b WHERE kc_sfdjmx.cpid = b.cpid and sfdjid='?' order by b.cpbm ";//
  //用于审批时候提取一条记录
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "kc_sfdj.15");
   ...
   * @return 返回入库单列表的实例
   */
  public static B_DirectDestroy getInstance(HttpServletRequest request)
  {
    B_DirectDestroy B_directdestroyBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_directdestroyBean";
      B_directdestroyBean = (B_DirectDestroy)session.getAttribute(beanName);
      if(B_directdestroyBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        B_directdestroyBean = new B_DirectDestroy();
        B_directdestroyBean.qtyFormat = loginBean.getQtyFormat();

        B_directdestroyBean.fgsid = loginBean.getFirstDeptID();
        B_directdestroyBean.loginId = loginBean.getUserID();
        B_directdestroyBean.loginName = loginBean.getUserName();
        B_directdestroyBean.loginDept = loginBean.getDeptID();
        B_directdestroyBean.user = loginBean.getUser();
        //B_directdestroyBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");
        //设置格式化的字段
        B_directdestroyBean.dsDetailTable.setColumnFormat("sl", B_directdestroyBean.qtyFormat);
        B_directdestroyBean.dsDetailTable.setColumnFormat("hssl", B_directdestroyBean.qtyFormat);
        B_directdestroyBean.dsMasterTable.setColumnFormat("zsl", B_directdestroyBean.qtyFormat);
        if (SYS_CUST_NAME.equals("essen"))
          MASTER_SQL = "SELECT * FROM kc_sfdj WHERE ? AND djxz=? and fgsid=? ? ORDER BY sfdjdh DESC, sfrq DESC";
        session.setAttribute(beanName, B_directdestroyBean);
      }
    }
    return B_directdestroyBean;
  }

  /**
   * 构造函数
   */
  private B_DirectDestroy()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"sfdjdh"}, new String[]{"SELECT pck_base.billNextCode('kc_sfdj','sfdjdh','17') from dual"}));
    ...
    addObactioner(String.valueOf(PRODUCE_CHANGE), new produce_Change());//选择产品
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=min; i<=max; i++)
    {
      ...
      detailRow.put("je", formatNumber(rowInfo.get("je_"+i), priceFormat));//
...
/*打开从表*/
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //03.16 15:04 修改 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
      //以实现:根据下达的部门，进行提交审批；
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "direct_destroy_list", content, dsMasterTable.getValue("deptid"));
    }
  }
  /**
   * 取消审批触发操作
  ...
  class Cancel_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(), "direct_destroy_list");
    }
  }
  /**
  * 提交仓库
 ...
 * 选择产品变化
 */
class produce_Change implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    for(int i=0; i<d_RowInfos.size(); i++)
    {
      ...
      if(rownum==i)
      {
        ...
        String msj = dataSetProvider.getSequence("select t.msj from xs_wzdj t WHERE t.cpid='"+cpid+"'");

        if(msj==null||msj.equals(""))
        {
          data.setMessage(showJavaScript("alert('该产品没定义门市价!')"));
          detailRow.put("sl", "");
          detailRow.put("dj", "");
          return;
        }

        detailRow.put("dj", msj);
 ...
 class Master_Post implements Obactioner
 {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        detail.setValue("je", detailrow.get("je"));
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("sfdjid"), "kc_sfdjmx", "sfdjid", "cpid", null, "="),//从表规格
