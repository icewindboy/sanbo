 ...
 * @version 1.0
 */
public final class B_SaleInstep extends BaseAction implements Operate
{
  ...
  public  static final String MATERIAL_ON_CHANGE = "21564";//改变产品代码
  ...
  public  static final String DETAIL_MULTI_ADD    = "258369"; //物资多选
  ...
  private static final String MASTER_SQL           = "SELECT * FROM xs_td WHERE djlx=11 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_EDIT_SQL       = "SELECT * FROM xs_td WHERE tdid='?' ";
  private static final String MASTER_SUM_SQL       = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=11 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL        = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=11 AND ? AND fgsid=? ?  ";
  ...
  private EngineDataSet productStockData = new EngineDataSet();//库存量
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td.11");
  public  HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "xs_td.11");
...
//  private RowMap[] p_RowInfos = null; //与从表象对应的库存量表
  ...
  public String zkl = "";
  public boolean isyrdd=false;//判断用户是否引入订单,而使edit页面显示订单页数量还是已审数量
  /**
   * 销售合同列表的实例
   * @param request jsp请求
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_SaleInstep getInstance(HttpServletRequest request)
  {
    B_SaleInstep b_SaleInstepBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_SaleInstepBean";
      b_SaleInstepBean = (B_SaleInstep)session.getAttribute(beanName);
      if(b_SaleInstepBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_SaleInstepBean = new B_SaleInstep();
        b_SaleInstepBean.qtyFormat = loginBean.getQtyFormat();
        b_SaleInstepBean.priceFormat = loginBean.getPriceFormat();
        b_SaleInstepBean.sumFormat = loginBean.getSumFormat();
        b_SaleInstepBean.fgsid = loginBean.getFirstDeptID();
        b_SaleInstepBean.loginId = loginBean.getUserID();
        b_SaleInstepBean.loginName = loginBean.getUserName();
        b_SaleInstepBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          b_SaleInstepBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          b_SaleInstepBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        b_SaleInstepBean.dsMasterTable.setColumnFormat("zje", b_SaleInstepBean.sumFormat);
        b_SaleInstepBean.dsMasterTable.setColumnFormat("zsl", b_SaleInstepBean.qtyFormat);

        b_SaleInstepBean.dsDetailTable.setColumnFormat("sl", b_SaleInstepBean.qtyFormat);
        b_SaleInstepBean.dsDetailTable.setColumnFormat("hssl", b_SaleInstepBean.qtyFormat);
        b_SaleInstepBean.dsDetailTable.setColumnFormat("xsj", b_SaleInstepBean.priceFormat);
        b_SaleInstepBean.dsDetailTable.setColumnFormat("dj", b_SaleInstepBean.priceFormat);
        b_SaleInstepBean.dsDetailTable.setColumnFormat("jje", b_SaleInstepBean.priceFormat);
        b_SaleInstepBean.dsDetailTable.setColumnFormat("xsje", b_SaleInstepBean.priceFormat);

        session.setAttribute(beanName, b_SaleInstepBean);
      }
    }
    return b_SaleInstepBean;
  }
  /**
   * 构造函数
   */
  private B_SaleInstep()
  {
    try {
      jbInit();
    }
...
//    PorductStockTableInit(); //库存量
    ...
    setDataSetProperty(dsAnotherDetailTable, DETAIL_STRUT_SQL);// 得到与从表一样的表，主要是用于sale_order_top.jsp的状态描述
    dsDetailTable.setTableName("xs_tdhw");
    setDataSetProperty(dsSearchTable, combineSQL(SEARCH_SQL,"?",new String[]{""}));
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.11','tdbh') from dual"}));
    ...
    addObactioner(String.valueOf(MATERIAL_ON_CHANGE), new Detail_Change()); //改变原料的到库存量
    //&#$//审核部分
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
//        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.11','tdbh') from dual");
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("sqdhwid", rowInfo.get("sqdhwid_"+i));
      //保存用户自定义的字段
          /*
  ...
  /* 得到与从表一样的表，主要是用于sale_order_top.jsp的状态描述*/
  ...
  public final RowMap[] getTdqtfyRowinfos() {
    ...
    /* 得到与从表一样的表，主要是用于send_list_top.jsp的状态描述*/
 ...
 */
  public class Add_Approve  implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterList, dsMasterList.getRow(),"instep", content,deptid, this);
    }
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterList,dsMasterList.getRow(),"instep");
      dsMasterList.readyRefresh();
    }
  }
  /**
 ...
 dsMasterTable.setValue("zt","8");
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ds.setValue("kc__storeid", rowInfo.get("kc__storeid"));
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("kc__storeid"), null, null, null, null, "="),
  ...
  class Detail_Copy_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String sqdhwid = newadd.get("sqdhwid");
      ...
      for(int i=0;i<copynumber;i++)
      {
        ...
        dsDetailTable.setValue("sqdhwid", sqdhwid);
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          if(dwtxid.equals(""))
          {
           ...
           RowMap corRow = corpBean.getLookupRow(dwtxid);
             String kc__storeid = dataSetProvider.getSequence("SELECT a.storeid FROM kc_ck a,dwtx b WHERE a.deptid=b.deptid_equal and dwtxid='"+dwtxid+"'");
             ...
             m_RowInfo.put("kc__storeid",kc__storeid);
...
/*      HttpServletRequest req = data.getRequest();
      ...
      for(int i=0; i < SQDhwIDs.length; i++)
      {
        ...
        if(!isMasterAdd)
          dsMasterTable.goToInternalRow(masterRow);
        String sqdhwid = SQDhwIDs[i];
        importOrderProductBean =get_OrderProductBean(req);     //提货
        String tdid = dsMasterTable.getValue("tdid");
        RowMap detailrow = null;
        locateGoodsRow.setValue(0, sqdhwid);
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          RowMap saleRow =null;
          saleRow = importOrderProductBean.getLookupRow(sqdhwid);
          dsDetailTable.insertRow(false);
          dsDetailTable.setValue("tdhwid", "-1");
          dsDetailTable.setValue("tdid", tdid);
