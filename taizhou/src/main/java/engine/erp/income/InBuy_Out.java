...
/**
 * <p>Title: 采购--采购退货单列表</p>
 * <p>Description: 采购--采购退货单列表<br>
...
public final class InBuy_Out extends BaseAction implements Operate
{
  ...
  public  static final String STORE_CHANGE = "10021";//
  ...
  public  static final String DELETE_BLANK = "2007";//删除数量为空行触发事件
  private static final String MASTER_STRUT_SQL = "SELECT * FROM cg_htjhd WHERE djlx=-2 AND 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM cg_htjhd WHERE djlx=-2 AND ? AND fgsid=? ? ORDER BY jhdbm DESC";
  ...
  private static final String DETAIL_SQL    = "SELECT a.* FROM VW_WM_JHDHW a WHERE jhdid='?' ORDER BY hthwid,cpid";//
  //判断进货单是否被入库单引用
  private static final String DETAIL_IS_REFERENCE = "SELECT count(*) FROM cg_htjhdhw WHERE nvl(sjrkl,0)>0 AND jhdid= ";

  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM cg_htjhd WHERE djlx=-2 AND jhdid='?'";
  //通过合同ID得到合同货物信息
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cg_htjhd.2");
  public  HtmlTableProducer detailProducer = new HtmlTableProducer(dsDetailTable, "cg_htjhdhw.1");
  ...
  private In_SingleSelectOrder InsingleOrderBean = null;//采购合同主表bean的引用
   ...
   * @return 返回销售合同列表的实例
   */
  public static InBuy_Out getInstance(HttpServletRequest request)
  {
    InBuy_Out InbuyOutBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "InbuyOutBean";
      InbuyOutBean = (InBuy_Out)session.getAttribute(beanName);
      if(InbuyOutBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        InbuyOutBean = new InBuy_Out();
        InbuyOutBean.qtyFormat = loginBean.getQtyFormat();
        InbuyOutBean.priceFormat = loginBean.getPriceFormat();
        InbuyOutBean.sumFormat = loginBean.getSumFormat();
        InbuyOutBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");

        InbuyOutBean.fgsid = loginBean.getFirstDeptID();
        InbuyOutBean.loginId = loginBean.getUserID();
        InbuyOutBean.loginName = loginBean.getUserName();
        InbuyOutBean.loginDept = loginBean.getDeptID();
        InbuyOutBean.user = loginBean.getUser();
        InbuyOutBean.SYS_APPROVE_ONLY_SELF = loginBean.getSystemParam("SYS_APPROVE_ONLY_SELF");//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
        SYS_CUST_NAME = loginBean.getSystemParam("SYS_CUST_NAME");//客户名称,用于客制化程序
        //InbuyOutBean.deptLimit = loginBean.getUser().getHandleDeptValue();
        //设置格式化的字段
        InbuyOutBean.dsDetailTable.setColumnFormat("sl", InbuyOutBean.qtyFormat);
        //InbuyOutBean.dsDetailTable.setColumnFormat("hssl", InbuyOutBean.qtyFormat);
        InbuyOutBean.dsDetailTable.setColumnFormat("dj", InbuyOutBean.priceFormat);
        InbuyOutBean.dsDetailTable.setColumnFormat("je", InbuyOutBean.sumFormat);
        InbuyOutBean.dsDetailTable.setColumnFormat("sjrkl", InbuyOutBean.sumFormat);
        InbuyOutBean.dsMasterTable.setColumnFormat("zsl", InbuyOutBean.sumFormat);
        InbuyOutBean.dsMasterTable.setColumnFormat("zje", InbuyOutBean.sumFormat);
        session.setAttribute(beanName, InbuyOutBean);
      }
    }
    return InbuyOutBean;
  }
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      //if ( SYS_CUST_NAME.equals("essen") ) detailRow.put("hssl", formatNumber(rowInfo.get("hssl_"+i), qtyFormat));//换算数量
  ...
  /**
   ...
   class Instore_Confirm implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       String SQL =  combineSQL(DETAIL_SQL,"?",new String[]{jhdid});
       openDetailTable(false);
       //boolean isCanConfirm = true;
       dsDetailTable.first();
       BigDecimal sl=new BigDecimal(0), stsl = new BigDecimal(0);
       for(int i=0;i<dsDetailTable.getRowCount(); i++)
       {
         sl = dsDetailTable.getBigDecimal("sl");
         stsl = dsDetailTable.getBigDecimal("stsl");//实际入库数量
         if(stsl.compareTo(sl)<0)
         {
           data.setMessage(showJavaScript("alert('还有未完全出库的物资，不能出库确认')"));
           return;
         }
         dsDetailTable.next();
       }
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterTable, row, "income_out", content,deptid);
  ...
  class Cancle_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(), "income_out");
    }
  }
  /**
  * 输入产品编码把产品iD存入RorMap
 ...
 class Single_Select_Order implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     for(int i=0; i < processData.getRowCount(); i++)
     {
       ...
       {
         double sjjhl=0, sl=0, wjhl=0,dj=0,hssl=0;//实际入库量， 合同数量， 未进货量,金额
         ...
         hssl  = processData.getValue("sl").length() > 0 ? Double.parseDouble(processData.getValue("sl")) : 0;//换算数量
         ...
         if(bjfs.equals("1"))
         dsDetailTable.setValue("hssl", processData.getValue("sl"));
         else
 ...
 class Master_Post implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        detail.setValue("hssl", detailrow.get("sl"));//保存换算数量
        //if ( SYS_CUST_NAME.equals("essen") ) detail.setValue("hssl", detailrow.get("hssl"));//保存换算数量
        detail.setValue("je", bjfs.equals("1") ? String.valueOf(hssl * dj) : String.valueOf(1*sl * dj));//金额
  ...
  class Complete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0;i<dsDetailTable.getRowCount();i++)
      {
        ...
        {
          data.setMessage("alert('还有款项未结清,不能完成!')");
          return;
        }
        dsDetailTable.next();
      }
  ...
  class Master_Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(SQL.length() > 0)
        SQL = " AND "+SQL;
      String []zt = data.getRequest().getParameterValues("zt");
  if(!(zt==null))
  {
 ...
 class Wb_Onchange implements Obactioner
 {
   ...
   */
  public In_SingleSelectOrder getOrderMasterBean(HttpServletRequest req)
  {
    if(InsingleOrderBean == null)
      InsingleOrderBean = In_SingleSelectOrder.getInstance(req);
    return InsingleOrderBean;
  }
  /**
   * 得到外币信息的bean
   * @param req WEB的请求
