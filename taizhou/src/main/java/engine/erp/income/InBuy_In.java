...
public final class InBuy_In extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM cg_htjhd WHERE ? AND fgsid=? ? and djlx=2 ORDER BY jhdbm DESC";
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cg_htjhd.3");
   ...
   * @return 返回销售合同列表的实例
   */
  public static InBuy_In getInstance(HttpServletRequest request)
  {
    InBuy_In inbuyInBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "inbuyInBean";
      inbuyInBean = (InBuy_In)session.getAttribute(beanName);
      if(inbuyInBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        inbuyInBean = new InBuy_In();
        inbuyInBean.qtyFormat = loginBean.getQtyFormat();
        inbuyInBean.priceFormat = loginBean.getPriceFormat();
        inbuyInBean.sumFormat = loginBean.getSumFormat();
        inbuyInBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");

        inbuyInBean.fgsid = loginBean.getFirstDeptID();
        inbuyInBean.loginId = loginBean.getUserID();
        inbuyInBean.loginName = loginBean.getUserName();
        inbuyInBean.loginDept = loginBean.getDeptID();
        inbuyInBean.user = loginBean.getUser();
        inbuyInBean.SYS_APPROVE_ONLY_SELF = loginBean.getSystemParam("SYS_APPROVE_ONLY_SELF");//提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
        SYS_CUST_NAME = loginBean.getSystemParam("SYS_CUST_NAME");//客户名称,用于客制化程序
        //inbuyInBean.deptLimit = loginBean.getUser().getHandleDeptValue();
        //设置格式化的字段
        inbuyInBean.dsDetailTable.setColumnFormat("sl", inbuyInBean.qtyFormat);
        //inbuyInBean.dsDetailTable.setColumnFormat("hssl", inbuyInBean.qtyFormat);
        inbuyInBean.dsDetailTable.setColumnFormat("dj", inbuyInBean.priceFormat);
        inbuyInBean.dsDetailTable.setColumnFormat("je", inbuyInBean.sumFormat);
        inbuyInBean.dsDetailTable.setColumnFormat("sjrkl", inbuyInBean.sumFormat);
        inbuyInBean.dsMasterTable.setColumnFormat("zsl", inbuyInBean.sumFormat);
        inbuyInBean.dsMasterTable.setColumnFormat("zje", inbuyInBean.sumFormat);
        session.setAttribute(beanName, inbuyInBean);
      }
    }
    return inbuyInBean;
  }
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(billType.equals("2"))
        approve.putAproveList(dsMasterTable, row, "income_in", content,deptid);
      //else
  ...
  class Cancle_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(), "income_in");
    }
  }
  /**
  * 输入产品编码把产品iD存入RorMap
   ...
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
     ...
     // ds.setValue("khlx", rowInfo.get("khlx"));//客户类型
      ...
      ds.post();
         dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"jhdbm"}, new String[]{"SELECT pck_base.billNextCode('cg_htjhd.j','jhdbm') from dual"}));
  ...
  class Complete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0;i<dsDetailTable.getRowCount();i++)
      {
        ...
        {
          data.setMessage("alert('还有款项未付完,不能完成!')");
          return;
        }
        dsDetailTable.next();
      }
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("zt"), null, null, null, null, "in")
      });
      isInitQuery = true;
    }
  }
