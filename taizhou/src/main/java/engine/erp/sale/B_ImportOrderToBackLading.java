...
public final class B_ImportOrderToBackLading extends BaseAction implements Operate
{
  ...
  private static final String DETAIL_SQL    = "select * From VW_SALE_HTHW  where isrefer=1 AND zt<>4  and htid= ";
  private static final String HTHW_SQL    = "select * From VW_SALE_HTHW where isrefer=1 AND zt<>4  AND ? AND fgsid=? ? ";
