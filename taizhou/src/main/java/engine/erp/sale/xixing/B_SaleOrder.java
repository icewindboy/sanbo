 ...
 * <p>Company: </p>
 * @version 1.0
 * 2004.12.08 wlc 新增 在主页面上显示状态描述“出库”，“下单” 对应dsAnotherDetailTable
...
public final class B_SaleOrder extends BaseAction implements Operate
{
  ...
  //public  static final String SUPL_ADD            = "1022";           //辅料新增
  //public  static final String SUPL_DEL            = "1024";           //辅料删除
  //public  static final String SUPL_DETAIL_CHANGE  = "1025";
  ...
  private static final String DETAIL_SQL         = "SELECT * FROM xs_hthw WHERE ISSUPL=0 and  htid=";
  private static final String CAN_OVER_SQL       = "select count(*) from xs_hthw a where a.ISSUPL=0 and  nvl(a.sl,0)>nvl(a.stsl,0) and a.htid=";
  private static final String CAN_GEN_SQL        = "SELECT COUNT(*) FROM xs_hthw a, xs_ht b WHERE a.ISSUPL=0 and  a.htid=b.htid AND b.zt=1 AND abs(nvl(a.sl,0))>abs(nvl(a.skdsl,0)) AND a.htid=";//合同数>实开单数且合同没作废和完成
  ...
  //private static final String XS_SUPL_STRUT_SQL   = "SELECT * FROM xs_hthw WHERE ISSUPL=1 and 1<>1 ";//其他费用
  //private static final String XS_SUPL_SQL         = "SELECT * FROM xs_hthw WHERE ISSUPL=1 and  htid= ";//
  ...
  private EngineDataSet regDataTable  = new EngineDataSet();
  private EngineDataSet dsCancel = new EngineDataSet();        //用
  //private EngineDataSet dsSuplTable  = new EngineDataSet();//辅料
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    //setDataSetProperty(dsSuplTable, XS_SUPL_STRUT_SQL);
    ...
    setDataSetProperty(regDataTable, null);  //由于edit页有用到注册数据ds list中都没有的字段prodBean.getLookupRow(detail.get("cpid"));salePriceBean.getLookupRow(detail.get("wzdjid"))，而数据集用于注册数据的数据集2004.12.08 wlc
    regDataTable.open();
      regDataTable.addColumn("cpid",com.borland.dx.dataset.Variant.STRING);
      regDataTable.addColumn("wzdjid",com.borland.dx.dataset.Variant.STRING);
    dsDetailTable.setSequence(new SequenceDescriptor(new String[]{"hthwid"}, new String[]{"s_xs_hthw"}));
    //dsSuplTable.setSequence(new SequenceDescriptor(new String[]{"hthwid"}, new String[]{"s_xs_hthw"}));
    ...
    //addObactioner(String.valueOf(SUPL_DETAIL_CHANGE), new Supl_Detail_Change());
    ...
    //addObactioner(String.valueOf(SUPL_ADD), new Detail_Add());
    addObactioner(String.valueOf(DETAIL_DEL), new Detail_Delete());
    //addObactioner(String.valueOf(SUPL_DEL), new Detail_Delete());
  }
  /**
   *引入已审核合同未发货金额
   * */
...
/*得到总金额*/
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    else
    {
      ...
      if(d_TdqtfyRowInfos == null)
        d_TdqtfyRowInfos = new ArrayList(dsSuplTable.getRowCount());
      else if(isInit)
        d_TdqtfyRowInfos.clear();
      dsSuplTable.first();
      for(int i=0; i<dsSuplTable.getRowCount(); i++)
      {
        RowMap row = new RowMap(dsSuplTable);
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("wzdjid", rowInfo.get("swzdjid_"+i));
      detailRow.put("cpid", rowInfo.get("scpid_"+i));
      detailRow.put("dmsxid", rowInfo.get("sdmsxid_"+i));
      detailRow.put("hssl", formatNumber(rowInfo.get("shssl_"+i), qtyFormat));//
      detailRow.put("sl", formatNumber(rowInfo.get("ssl_"+i), qtyFormat));//
      detailRow.put("zk", formatNumber(rowInfo.get("szk_"+i), priceFormat));//
      detailRow.put("dj", formatNumber(rowInfo.get("sdj_"+i), priceFormat));//含税单价

      detailRow.put("xsje", formatNumber(rowInfo.get("sxsje_"+i), sumFormat));
      detailRow.put("jje", formatNumber(rowInfo.get("sjje_"+i), sumFormat));//无税单价
      detailRow.put("jzj", formatNumber(rowInfo.get("sjzj_"+i), sumFormat));//基准价
      detailRow.put("cjtcl", formatNumber(rowInfo.get("scjtcl_"+i), sumFormat));//差价提成率
      detailRow.put("jxts", formatNumber(rowInfo.get("sjxts_"+i), qtyFormat));//计息天数
      detailRow.put("hlts", formatNumber(rowInfo.get("shlts_"+i), qtyFormat));//回笼天数
      detailRow.put("hltcl", formatNumber(rowInfo.get("shltcl_"+i), sumFormat));//回笼提成率
      detailRow.put("wbje", formatNumber(rowInfo.get("swbje_"+i), sumFormat));
      String xs_jzj = rowInfo.get("sxs_jzj_"+i);
      detailRow.put("xs_jzj", formatNumber(rowInfo.get("sxs_jzj_"+i), qtyFormat));//引入的基准价
      detailRow.put("bz", rowInfo.get("sbz_"+i));//备注
      detailRow.put("jhrq", rowInfo.get("sjhrq_"+i));//
      detailRow.put("xsj", rowInfo.get("sxsj_"+i));//
      detailRow.put("bz", rowInfo.get("sbz_"+i));
    }
    */
  }
  /*得到表对象*/
  ...
  public final EngineDataSet getRegDataTable(){
  if(!regDataTable.isOpen())
    regDataTable.open();
  return regDataTable;
  }
  /*打开从表*/
  ...
  {
    String SQL =  DETAIL_SQL+(isMasterAdd ? "-1" : htid);
    ...
    if(dsDetailTable.isOpen())
      ...
      dsDetailTable.open();
/*
    SQL = XS_SUPL_SQL + (isMasterAdd ? "-1" : htid);
    dsSuplTable.setQueryString(SQL);
    if(!dsSuplTable.isOpen())
    {
      dsSuplTable.open();
    }
    else
...
{
    String htidd=dsMasterList.getValue("htid");
    dsAnotherDetailTable.setQueryString(DETAIL_SQL + (isMasterAdd ? "-1" : htidd));
  if(dsAnotherDetailTable.isOpen())
  ...
  */
  ...
  class Over implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(String.valueOf(SALE_OVER).equals(action))
      {
        if(!count.equals("0"))
        {
          data.setMessage(showJavaScript("if(confirm('还未完全出库,要强制完成吗?')) sumitForm("+FORCE_OVER+",-1);"));//强制完成
          return;
        }
      }
      //openDetailTable(false);
  ...
  class Cancer implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      htid = dsMasterList.getValue("htid");
      String count = dataSetProvider.getSequence("select count(*) from xs_hthw a,xs_tdhw b where b.hthwid=a.hthwid and a.htid="+htid);
     if(!count.equals("0"))
     {
       data.setMessage(showJavaScript("alert('该单据已被发货通知单或销售退货单引用，不能作废，您可以删除发货通知单或销售退货单中的相应货物后再作废!')"));
        return;
      }
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
     ...
     if(!count.equals("0"))
     {
       data.setMessage(showJavaScript("alert('该单据已被发货通知单或销售退货单引用，不能取消审批，您可以删除发货通知单或销售退货单中的相应货物后再取消审批!')"));
        return;
      }
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        if(conversion)
        {
          ...
          detail.setValue("jje", engine.util.Format.formatNumber(String.valueOf(sl * detail.getBigDecimal("dj").doubleValue()),"#.00") );
        ...
        else
        {
          ...
          detail.setValue("jje", engine.util.Format.formatNumber(detailrow.get("jje"),"#.00") );
          detail.setValue("hssl", detailrow.get("hssl"));
        }
...
/*
      ...
      for(int i=0; i<dsSuplTable.getRowCount(); i++)
      {
        detailrow = (RowMap)d_TdqtfyRowInfos.get(i);
        //新添的记录
        dsSuplTable.setValue("htid", htid);
        dsSuplTable.setValue("issupl", "1");
        //新添的记录
        dsSuplTable.setValue("wzdjid", detailrow.get("wzdjid"));
        dsSuplTable.setValue("dmsxid", detailrow.get("dmsxid"));
        dsSuplTable.setValue("bz", detailrow.get("bz"));
        dsSuplTable.setValue("htid", htid);
        double xsj = detailrow.get("xsj").length() > 0 ? Double.parseDouble(detailrow.get("xsj")) : 0;//销售单价
        //double hsbl = detailrow.get("hsbl").length() > 0 ? Double.parseDouble(detailrow.get("hsbl")) : 0;//换算比例
        double sl = detailrow.get("sl").length() > 0 ? Double.parseDouble(detailrow.get("sl")) : 0;
        double dj = detailrow.get("dj").length() > 0 ? Double.parseDouble(detailrow.get("dj")) : 0;
        String bxsj = detailrow.get("xsj");
        dsSuplTable.setValue("xsj", detailrow.get("xsj"));//
        dsSuplTable.setValue("sl", detailrow.get("sl"));
        if(conversion)
        {
          dsSuplTable.setValue("zk", String.valueOf(dj/xsj*100));
          dsSuplTable.setValue("xsje", String.valueOf(sl * xsj));
          dsSuplTable.setValue("dj", String.valueOf(dj));
          dsSuplTable.setValue("jje", engine.util.Format.formatNumber(String.valueOf(sl * detail.getBigDecimal("dj").doubleValue()),"#.00") );
          dsSuplTable.setValue("hssl", detailrow.get("hssl"));
        }
        else
        {
          dsSuplTable.setValue("zk", detailrow.get("zk"));
          dsSuplTable.setValue("xsje", detailrow.get("xsje"));
          dsSuplTable.setValue("dj", detailrow.get("dj"));
          dsSuplTable.setValue("jje", engine.util.Format.formatNumber(detailrow.get("jje"),"#.00") );
          dsSuplTable.setValue("hssl", detailrow.get("hssl"));
        }
        dsSuplTable.post();
        dsSuplTable.next();
    ...
    private String checkDetailInfo()
    {
      ...
      for(int i=0; i<d_TdqtfyRowInfos.size(); i++)
      {
        ...
        if(wzdjid.equals(""))
          return showJavaScript("alert('包装物不能空!');");
        String sl = detailrow.get("sl");
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("htid"), "vw_xs_hthw", "htid", "cpbm", "cpbm", "left_like"),//
        new QueryColumn(master.getColumn("htid"), "vw_xs_hthw", "htid", "product", "product", "like"),//
        new QueryColumn(master.getColumn("htid"), "vw_xs_hthw", "htid", "sxz","sxz","like")//属性值
  ...
  class Detail_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      */
      ...
      {
        /*
        String wzdjid = data.getParameter("wzdjid");
        ...
        String xsj = dataSetProvider.getSequence("select t."+mrjg+" from xs_wzdj t WHERE t.wzdjid='"+wzdjid+"'");
        if(xsj==null||xsj.equals(""))
        {
          data.setMessage(showJavaScript("alert('该产品没定义默认价格!')"));
          return;
        }
        if(mrzk==null||mrzk.equals(""))
          mrzk="100";
         */
        dsDetailTable.insertRow(false);
        dsDetailTable.setValue("hthwid", "-1");
        //dsDetailTable.setValue("xsj", xsj);
        //dsDetailTable.setValue("dj", xsj);
      ...
      /*
      else if(String.valueOf(SUPL_ADD).equals(action))
      {
        dsSuplTable.insertRow(false);
        dsSuplTable.setValue("hthwid", "-1");
        dsSuplTable.setValue("htid", htid);
        dsSuplTable.post();
        RowMap detailrow = new RowMap(dsSuplTable);
  ...
  /*
  class Supl_Detail_Change implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      HttpServletRequest req = data.getRequest();
      ...
      data.setMessage(showJavaScript("document.form1.ssl_"+rownum+".focus();"));
    }
  }
  */
  /**
  ...
  class Detail_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      /*
      else if(String.valueOf(SUPL_DEL).equals(action))
      {
        d_TdqtfyRowInfos.remove(rownum);
        dsSuplTable.goToClosestRow(rownum);
        dsSuplTable.deleteRow();
      }
      */
    }
  }
