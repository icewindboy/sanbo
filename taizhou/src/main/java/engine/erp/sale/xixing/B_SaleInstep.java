...
public final class B_SaleInstep extends BaseAction implements Operate
{
  ...
  public  static final String SHOW_DETAIL          = "12500";
  public  static final String REPORT               = "2000";
  public  static final String NEXT                 = "9999";
  public  static final String PRIOR                = "9998";
  public  static final String RECODE_ACCOUNT       = "9996";
  public  static final String NEW_TRANSFERSCAN     = "10062";
  public  static final String TRANSFERSCAN         = "10061";
  public  static final String TURNPAGE             = "9995";
  public  static final String DETAIL_ADD_BLANK     = "9994";
  public  static final String DELETE_BLANK         = "11581";
  public  static final String DETAIL_CHANGE        = "1009";//从表操作
  public  static final String OVER                 = "1010";

  private static final String MASTER_STRUT_SQL = "SELECT * FROM kc_sfdj WHERE 1<>1";
  private static final String MASTER_SQL    = "SELECT * FROM kc_sfdj WHERE ? AND djxz=11 and fgsid=? ? ORDER BY sfrq DESC";

  private static final String DETAIL_STRUT_SQL = "SELECT * FROM kc_sfdjmx WHERE 1<>1";
  private static final String DETAIL_SQL = "SELECT a.* FROM kc_sfdjmx a,kc_dm b WHERE a.cpid=b.cpid and sfdjid='?' order by b.cpbm ";//

  private static final String MASTER_APPROVE_SQL = "SELECT * FROM kc_sfdj WHERE djxz=11 and sfdjid='?'";
  private static final String RECODE_DATASQL     = " SELECT COUNT(*) FROM kc_sfdj a WHERE a.zdrid='?' AND  a.djxz =11 AND a.zt=1";
  private static final String RECODE             = "UPDATE kc_sfdj a SET a.zt=2 WHERE a.zdrid='?' AND  a.djxz =11 AND a.zt=1";
  private static final String TOTALZSL_SQL  = "SELECT SUM(nvl(zsl,0)) tzsl FROM kc_sfdj a WHERE ? AND djxz=11 AND fgsid=? ? ORDER BY sfdjdh DESC";
  ...
  public  HtmlTableProducer masterProducer    = new HtmlTableProducer(dsMasterTable, "kc_sfdj.11");
  public  HtmlTableProducer detailProducer    = new HtmlTableProducer(dsDetailTable, "kc_sfdjmx.11");
  ...
  public int min = 0;
  public int max = 0;
  public String isRepeat = "0";
  private double childcount = 0;
  private ArrayList locateImportMaterial = new ArrayList();
  private ImportMaterail importMaterailBean = null;
  ...
  public static B_SaleInstep getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(b_SaleInstepBean == null)
      {
        ...
        b_SaleInstepBean.loginDept = loginBean.getDeptID();
        ...
        b_SaleInstepBean.dsMasterTable.setColumnFormat("drsl", b_SaleInstepBean.qtyFormat);
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsDetailTable.setTableName("kc_sfdjmx");
    setDataSetProperty(dsTotalZsl, null);
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"sfdjdh"}, new String[]{"SELECT pck_base.billNextCode('kc_sfdj.11','sfdjdh') from dual"}));
    ...
    addObactioner(String.valueOf(OVER), new Lading_Over());
 ...
 /*打开从表*/
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      //以实现:根据下达的部门，进行提交审批；
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "instep", content, dsMasterTable.getValue("deptid"));
    }
  }
  /**
   * 取消审批的操作类
  ...
  class Cancel_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable, dsMasterTable.getRow(), "instep");
    }
  }
  /**
  * 提交仓库
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        //double sl = detailrow.get("sl").length() > 0 ? Double.parseDouble(detailrow.get("sl")) : 0;//换算数量
        detail.setValue("sl", detailrow.get("sl"));//保存数量
        detail.setValue("hssl", detailrow.get("hssl"));//保存数量String.valueOf(hsbl==0 ? 0 : sl/hsbl)
        //detail.setValue("sl", String.valueOf(sl));//保存数量String.valueOf(hsbl==0 ? 0 : sl/hsbl)
  ...
  class Detail_Change implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if(rownum==i)
        {
          String mrjg = dataSetProvider.getSequence("select t.mrjg from xs_wzdj t WHERE t.cpid='"+cpid+"'");
          if(mrjg==null||mrjg.equals(""))
          {
            data.setMessage(showJavaScript("alert('该产品没定义默认价格!')"));
            return;
          }
          String xsj = dataSetProvider.getSequence("select t."+mrjg+" from xs_wzdj t WHERE t.cpid='"+cpid+"'");
          if(xsj==null)
            xsj = "";
 ...
 class Detail_Add implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     for(int i=0; i < wzmxID.length; i++)
     {
       ...
       String wzdjid = materailRow.get("wzdjid");
       String mrjg = materailRow.get("mrjg");
       ...
       dsDetailTable.setValue("wzmxid","1");
...
/**
  ...
  class Recode_Account implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
     //是否有符合记帐的数据，有几条
     String SQL = combineSQL(RECODE_DATASQL, "?", new String[]{loginId, djxz});
     String UPDATE_SQL = combineSQL(RECODE, "?", new String[]{loginId, djxz});
