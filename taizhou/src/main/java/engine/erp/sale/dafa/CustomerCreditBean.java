...
public final class CustomerCreditBean extends BaseAction implements Operate
{
  /**
   *对应修改按钮
   * */
   ...
   * 以便取出所有往来单位信息
   * */
  private static final String CUSTOMER_CREDIT_SQL = "SELECT * FROM VW_XS_NEW_XYED WHERE fgsid=? ? ORDER BY areacode,dwdm ";//
  /**
   * 客户信誉额度表
   * */
  ...
  * 定义固定查询类
   ...
   * @return 返回报价资料信息的实例
   */
  public static CustomerCreditBean getInstance(HttpServletRequest request)
  {
    CustomerCreditBean xs_khxyedbean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "xs_khxyedbean";
      xs_khxyedbean = (CustomerCreditBean)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(xs_khxyedbean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        String fgsid = loginBean.getFirstDeptID();
        xs_khxyedbean = new CustomerCreditBean(fgsid);
        xs_khxyedbean.loginName = loginBean.getUserName();
        //设置格式化的字段
        //xs_khxyedbean.dsxs_khxyed.setColumnFormat("xyed", loginBean.getPriceFormat());
        session.setAttribute(beanName, xs_khxyedbean);
      }
    }
    return xs_khxyedbean;
  }
  /**
   * 构造函数
   */
  private CustomerCreditBean(String fgsid)
  {
    this.fgsid = fgsid;
    try {
      jbInit();
  ...
  protected void jbInit() throws Exception
  {
    ...
    //添加操作的触发对象
    addObactioner(String.valueOf(INIT), new CustomerCreditBean_Init());
    addObactioner(String.valueOf(DETAIL_DEL), new CustomerCreditBean_Delete());
    addObactioner(String.valueOf(POST), new CustomerCreditBean_Post());
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("hlts", rowInfo.get("hlts_"+i));
   ...
   * 初始化操作的触发类
   */
  class CustomerCreditBean_Init implements Obactioner
  {
  ...
  class FIXED_SEARCH implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("xydj"), null, null, null, null, "="),
   ...
   * 保存操作的触发类
   */
  class CustomerCreditBean_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      RowMap detailrow = null;
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        String hlts=detailrow.get("hlts");
         ...
         dsxs_khxyed_salve.setValue("hlts", hlts);
   ...
   * 删除操作的触发类
   */
  class CustomerCreditBean_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
