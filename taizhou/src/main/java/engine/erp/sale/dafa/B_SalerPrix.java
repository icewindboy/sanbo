 ...
 *
 */
public final class B_SalerPrix extends BaseAction implements Operate
{
  ...
  private static final String LX = "1";

  private static final String MASTER_STRUT_SQL = "SELECT * FROM xs_jjzb where lx=1 and  1<>1";//业务员奖金主表结构
  private static final String MASTER_SQL    = "SELECT * FROM xs_jjzb ";//
  private static final String SEARCH_SQL = "SELECT * from VW_SALER_JJDETAIL WHERE lx=1 ?  ";
  ...
  private static final String TD_YCYE_SQL=
    ...
    +" SELECT SUM(nvl(x.ysk,0))ysk FROM xs_ysk x WHERE x.personid=? group by x.personid "
      ...
      +") df    "
      +" FROM dual )";//按提单月初余额
  private static final String SALER_LX_SQL = "select a.tdrq,sum(nvl(b.jje,0))jje from xs_td a,xs_tdhw b where a.tdid=b.tdid and personid =? and a.tdrq >= to_date('?','YYYY-MM-DD') AND a.tdrq<to_date('?','YYYY-MM-DD') and a.zt in(2,3,8) group by a.tdrq";//销售利息
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_SalerPrix getInstance(HttpServletRequest request)
  {
    B_SalerPrix b_SalerPrixBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_SalerPrixBean";
      b_SalerPrixBean = (B_SalerPrix)session.getAttribute(beanName);
      if(b_SalerPrixBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_SalerPrixBean = new B_SalerPrix();
        //b_SalerPrixBean.qtyFormat = loginBean.getQtyFormat();
        b_SalerPrixBean.priceFormat = loginBean.getPriceFormat();
        //b_SalerPrixBean.sumFormat = loginBean.getSumFormat();
        b_SalerPrixBean.fgsid = loginBean.getFirstDeptID();
        b_SalerPrixBean.loginId = loginBean.getUserID();
        b_SalerPrixBean.loginName = loginBean.getUserName();
        //设置格式化的字段
        b_SalerPrixBean.dsDetailTable.setColumnFormat("cgj", b_SalerPrixBean.priceFormat);
        b_SalerPrixBean.dsDetailTable.setColumnFormat("jj", b_SalerPrixBean.priceFormat);
        session.setAttribute(beanName, b_SalerPrixBean);
      }
    }
    return b_SalerPrixBean;
  }
  /**
   * 构造函数
   */
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    setDataSetProperty(dsSearchTable,combineSQL(SEARCH_SQL,"?",new String[]{" and 1<>1"}));
  ...
  class MonthOnchange implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String sql = MASTER_SQL+" WHERE lx=1 and nf="+year + " and yf="+smonth;//查主表
...
class Master_Over implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    dsMasterTable.updateQuery(new String[]{" update xs_jjzb set sfjz=1 where  jjzbid='"+jjzbid+"'"});
    //dsMasterTable.saveChanges();
  }
}
/**
...
class Master_Search implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    SQL=MASTER_SQL+" WHERE lx=1 and jjzbid="+jjzbid;
    if(dsMasterTable.isOpen())
      dsMasterTable.close();
    setDataSetProperty(dsMasterTable,SQL);
    dsMasterTable.open();
...
class Detail_Calculate implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    for(int i=0;i<drows.length;i++)
    {
      ...
      String xsa = dssl.getValue("XSA").equals("")?"0":dssl.getValue("XSA");
      ...
      //计算销售回笼利息xsb
      //(当月的总天数-当天的日期)*结算金额
      SQL = "select a.rq,nvl(a.je,0)je from cw_xsjs a where a.zt=8  and  a.personid="+personid+" AND  a.rq>=to_date('"+startDate+"','YYYY-MM-DD') AND a.rq<to_date('"+endDate+"','YYYY-MM-DD')";
