     ...
     * @version 1.0
     */
    public final class B_SalerPrixDetail extends BaseAction implements Operate
    {
      ...
      private static final String SALER_CJJ_SQL = "select *  from vw_saler_cjj a  WHERE 1=1 and personid=";//差价奖
      private static final String SALER_TCJ_SQL ="select *  from vw_saler_tcj a  WHERE 1=1 and personid=";
      private static final String SALER_TCJ_INIT_SQL ="select *  from vw_saler_tcj_init a  WHERE 1=1 and personid=";
       ...
       * @return 返回销售合同列表的实例
       */
      public static B_SalerPrixDetail getInstance(HttpServletRequest request)
      {
        B_SalerPrixDetail b_SalerPrixDetailBean = null;
        HttpSession session = request.getSession(true);
        synchronized (session)
        {
          String beanName = "b_SalerPrixDetailBean";
          b_SalerPrixDetailBean = (B_SalerPrixDetail)session.getAttribute(beanName);
          if(b_SalerPrixDetailBean == null)
          {
            //引用LoginBean
            LoginBean loginBean = LoginBean.getInstance(request);
            b_SalerPrixDetailBean = new B_SalerPrixDetail();
            //b_SalerPrixDetailBean.qtyFormat = loginBean.getQtyFormat();
            b_SalerPrixDetailBean.priceFormat = loginBean.getPriceFormat();
            //b_SalerPrixDetailBean.sumFormat = loginBean.getSumFormat();
            b_SalerPrixDetailBean.fgsid = loginBean.getFirstDeptID();
            b_SalerPrixDetailBean.loginId = loginBean.getUserID();
            b_SalerPrixDetailBean.loginName = loginBean.getUserName();
            //设置格式化的字段
           session.setAttribute(beanName, b_SalerPrixDetailBean);
          }
        }
        return b_SalerPrixDetailBean;
      }
      /**
       * 构造函数
       */
      private B_SalerPrixDetail()
      {
        try {
          jbInit();
        }
      ...
      class Init implements Obactioner
      {
        public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
        {
          ...
          //lxRow.put("XSA",formatNumber(dssl.getValue("XSA").equals("")?"0":dssl.getValue("XSA"), priceFormat));
          lxRow.put("XSA",xsa);
          ...
          */
          String xsb = String.valueOf(zhllx);
          ...
          */
          String xsd = String.valueOf(dxslx);
