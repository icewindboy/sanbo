     ...
     * @version 1.0
     */
    public final class B_SaleFlz extends BaseAction implements Operate
    {
       ...
       * @return 返回销售合同列表的实例
       */
      public static B_SaleFlz getInstance(HttpServletRequest request)
      {
        B_SaleFlz b_SaleFlzBean = null;
        HttpSession session = request.getSession(true);
        synchronized (session)
        {
          String beanName = "b_SaleFlzBean";
          b_SaleFlzBean = (B_SaleFlz)session.getAttribute(beanName);
          if(b_SaleFlzBean == null)
          {
            //引用LoginBean
            LoginBean loginBean = LoginBean.getInstance(request);
            b_SaleFlzBean = new B_SaleFlz();
            session.setAttribute(beanName, b_SaleFlzBean);
          }
        }
        return b_SaleFlzBean;
      }
