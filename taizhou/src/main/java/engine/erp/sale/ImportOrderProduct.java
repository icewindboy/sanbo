...
public final class ImportOrderProduct extends BaseAction implements Operate
{
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(!request.getParameter("isnet").trim().equals(""))
        isnet =  request.getParameter("isnet").trim();
      if(dwtxid.equals("")||storeid.equals("")||djlx.equals("")||personid.equals("")||khlx.equals("")||jsfsid.equals(""))
        return;
      ...
      if(djlx.equals("-1"))
        SQL=combineSQL(IMPORT_SALE_ORDER_SQL, "?", new String[]{" AND jsfsid='"+jsfsid+"' AND personid='"+personid+"' AND isnet='"+isnet+"' AND khlx='"+khlx+"' AND fgsid='"+fgsid+"' AND dwtxid='"+dwtxid+"' AND (storeid is null OR storeid='"+storeid+"') AND isrefer=1 AND zt<>4 AND zt<>8 AND lb=2  "});
      else
        SQL=combineSQL(IMPORT_SALE_ORDER_SQL, "?", new String[]{" AND jsfsid='"+jsfsid+"' AND personid='"+personid+"' AND isnet='"+isnet+"' AND khlx='"+khlx+"' AND fgsid='"+fgsid+"' AND dwtxid='"+dwtxid+"' AND (storeid is null OR storeid='"+storeid+"') AND isrefer=1 AND zt<>4 AND zt<>8 AND lb=1 and sl>0 "});
  ...
  class Master_Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      SQL=SQL+(djlx.equals("-1")?" AND lb=1 ":" AND lb=2 ")+" AND jsfsid='"+jsfsid+"' AND personid='"+personid+"' AND khlx='"+khlx+"' AND dwtxid='"+dwtxid+"'  AND fgsid='"+fgsid+"' AND (storeid is null or storeid='"+storeid+"') and isnet="+isnet;
