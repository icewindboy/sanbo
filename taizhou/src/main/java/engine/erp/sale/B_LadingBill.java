 ...
 * @version 1.0
 */
public final class B_LadingBill extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE 1=1 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL     = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE 1=1 AND ? AND fgsid=? ?  ";
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_LadingBill getInstance(HttpServletRequest request)
  {
    B_LadingBill b_ladingbillBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_ladingbillBean";
      b_ladingbillBean = (B_LadingBill)session.getAttribute(beanName);
      if(b_ladingbillBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_ladingbillBean = new B_LadingBill();
 ...
 class Master_Post implements Obactioner
 {
   ...
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     for(int i=0; i<detail.getRowCount(); i++)
     {
       ...
       detail.setValue("jje",String.valueOf(sl * dj));
