...
public final class B_ImportOrder extends BaseAction implements Operate
{
  ...
  private static final String DETAIL_SQL    = "select * From VW_SALE_HTHW  where isrefer=1 AND abs(nvl(skdsl,0))<abs(nvl(sl,0)) AND zt<>4 AND zt<>8 and htid= ";
  private static final String HTHW_SQL    = "select * From VW_SALE_HTHW where isrefer=1 AND abs(nvl(skdsl,0))<abs(nvl(sl,0)) AND zt<>4 AND zt<>8 AND ? AND fgsid=? ? ";
