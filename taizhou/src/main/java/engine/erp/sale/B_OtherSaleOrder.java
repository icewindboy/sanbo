...
import engine.common.OtherApproveFacade;
...
import engine.project.OtherLookupBeanFacade;
 ...
 * @version 1.0
 */
public final class B_OtherSaleOrder extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SUM_SQL     = "SELECT SUM(nvl(zsl,0))zsl FROM xs_ht WHERE 1=1  ?   ?  ";
  private static final String MASTER_JE_SQL      = "SELECT SUM(nvl(zje,0))zje FROM xs_ht WHERE 1=1  ?   ?  ";
...
//  public  HtmlTableProducer detailProducer = new HtmlTableProducer(dsDetailTable, "xs_hthw");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_OtherSaleOrder getInstance(HttpServletRequest request)
  {
    B_OtherSaleOrder otherSaleOrderBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "otherSaleOrderBean";
      otherSaleOrderBean = (B_OtherSaleOrder)session.getAttribute(beanName);
      if(otherSaleOrderBean == null)
      {
        //引用LoginBean
    	  OtherLoginBean loginBean = OtherLoginBean.getInstance(request);

        otherSaleOrderBean = new B_OtherSaleOrder();
        otherSaleOrderBean.qtyFormat = loginBean.getQtyFormat();
        otherSaleOrderBean.priceFormat = loginBean.getPriceFormat();
        otherSaleOrderBean.sumFormat = loginBean.getSumFormat();

        otherSaleOrderBean.fgsid = loginBean.getFirstDeptID();
        otherSaleOrderBean.loginId = loginBean.getUserID();
        otherSaleOrderBean.loginName = loginBean.getUserName();
        if(loginBean.getSystemParam("XS_ORDER_SHOW_CREDIT").equals("1"))
            otherSaleOrderBean.appear=true;
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          otherSaleOrderBean.conversion = true;
//        if(loginBean.getSystemParam("XS_ORDER_SHOW_ADD_FIELD").equals("1"))
//          otherSaleOrderBean.showable = true;//显示 差价提成率(%) 计息天数 回笼天数 回笼提成率(%)
        //设置格式化的字段
        otherSaleOrderBean.dsDetailTable.setColumnFormat("sl", otherSaleOrderBean.qtyFormat);
        otherSaleOrderBean.dsDetailTable.setColumnFormat("hssl", otherSaleOrderBean.qtyFormat);
        otherSaleOrderBean.dsDetailTable.setColumnFormat("xsj", otherSaleOrderBean.priceFormat);
        otherSaleOrderBean.dsDetailTable.setColumnFormat("dj", otherSaleOrderBean.priceFormat);
        otherSaleOrderBean.dsDetailTable.setColumnFormat("xsje", otherSaleOrderBean.sumFormat);
        otherSaleOrderBean.dsDetailTable.setColumnFormat("jje", otherSaleOrderBean.sumFormat);

        otherSaleOrderBean.dsMasterTable.setColumnFormat("zsl", otherSaleOrderBean.qtyFormat);
        otherSaleOrderBean.dsMasterTable.setColumnFormat("zje", otherSaleOrderBean.sumFormat);
        otherSaleOrderBean.dsMasterTable.setColumnFormat("wbje", otherSaleOrderBean.sumFormat);

        otherSaleOrderBean.user = loginBean.getUser();
        session.setAttribute(beanName, otherSaleOrderBean);
      }
    }
    return otherSaleOrderBean;
  }

  /**
   * 构造函数
   */
  private B_OtherSaleOrder()
  {
    try {
      jbInit();
    }
...
/**
 ...
 public String getTotalJsje(String dwtxid)
 {
   ...
   if(dwtxid.equals(""))
     return "0";
   try{
     wsje =dataSetProvider.getSequence("select sum(nvl(b.jje,0)-nvl(b.ssje,0))wsje from xs_td a,xs_tdhw b where a.tdid=b.tdid and a.zt<>4 and a.hkrq<=sysdate and a.dwtxid="+dwtxid);
     if(wsje==null)
       wsje="";
   }catch(Exception e){}
   return wsje;
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("dwtxid",user.getDwtxId());
        m_RowInfo.put("dwmc",user.getDwmc());
        m_RowInfo.put("deptid",user.getDeptId());
...
//      detailProducer.init(request, loginId);
      ...
      String MSQL =  combineSQL(MASTER_SQL, "?", new String[]{" and dwtxid="+user.getDwtxId(), SQL});
      ...
      SLSQL =  combineSQL(MASTER_SUM_SQL, "?", new String[]{" and dwtxid="+user.getDwtxId(),  SQL});

      JESQL = combineSQL(MASTER_JE_SQL, "?", new String[]{" and dwtxid="+user.getDwtxId(),  SQL});
 ...
 public class Add_Approve implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      openDetailTable(false);
      OtherApproveFacade approve = OtherApproveFacade.getInstance(data.getRequest());
...
//        if((temp = checkNumber(jzj, detailProducer.getFieldInfo("jzj").getFieldname())) != null)
//          return temp;
...
//          confirmString = "该合同需要进行优惠价审批!";
  ...
  class Master_Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String MSQL = combineSQL(MASTER_SQL, "?", new String[]{" and dwtxid="+user.getDwtxId(), SQL});//组装SQL语句
      if(!dsMasterList.getQueryString().equals(MSQL))
      {
        dsMasterList.setQueryString(MSQL);
        dsMasterList.setRowMax(null);//以便dbNavigator刷新数据集
      }
      String SLSQL =  combineSQL(MASTER_SUM_SQL, "?", new String[]{" and dwtxid="+user.getDwtxId(), SQL});

      String JESQL = combineSQL(MASTER_JE_SQL, "?", new String[]{" and dwtxid="+user.getDwtxId(), SQL});
  ...
  class Detail_Product_Add implements Obactioner
  {
    ...
    {
      HttpServletRequest req = data.getRequest();
      engine.project.LookUp salePriceBean = engine.project.OtherLookupBeanFacade.getInstance(req, engine.project.SysConstant.BEAN_SALE_PRICE);
      //保存输入的明细信息
      putDetailInfo(data.getRequest());
...
//          dsDetailTable.setValue("jzj", saleRow.get("xsjzj"));
  ...
  class Dwtxid_Change implements Obactioner
  {
    ...
    {
      HttpServletRequest req = data.getRequest();
      engine.project.LookUp corpBean = engine.project.OtherLookupBeanFacade.getInstance(req, engine.project.SysConstant.BEAN_CORP);
  ...
  public LookUp getSalePriceBean(HttpServletRequest req)
  {
    if(salePriceBean == null)
      salePriceBean = OtherLookupBeanFacade.getInstance(req, SysConstant.BEAN_SALE_PRICE);
    return salePriceBean;
  }
  public LookUp getForeignBean(HttpServletRequest req)
  {
    engine.project.LookUp wbBean = engine.project.OtherLookupBeanFacade.getInstance(req, engine.project.SysConstant.BEAN_FOREIGN_CURRENCY);
    return wbBean;
  }
}