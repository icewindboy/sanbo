...
/**
 * <p>Title: 销售管理--销售货物管理--销售赠送单--</p>
 * <p>Description: 销售管理--销售货物管理--销售赠送单<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author engine
 * @version 1.0
 */
public final class B_SaleLargess extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM xs_td WHERE djlx=3 AND ? AND fgsid=? ?  order by djlx desc,tdbh desc";
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=3 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL    = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=3 AND ? AND fgsid=? ?  ";
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td.3");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_SaleLargess getInstance(HttpServletRequest request)
  {
    B_SaleLargess b_SaleLargessBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_SaleLargessBean";
      b_SaleLargessBean = (B_SaleLargess)session.getAttribute(beanName);
      if(b_SaleLargessBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_SaleLargessBean = new B_SaleLargess();
        b_SaleLargessBean.qtyFormat = loginBean.getQtyFormat();
        b_SaleLargessBean.priceFormat = loginBean.getPriceFormat();
        b_SaleLargessBean.sumFormat = loginBean.getSumFormat();
        b_SaleLargessBean.fgsid = loginBean.getFirstDeptID();
        b_SaleLargessBean.loginId = loginBean.getUserID();
        b_SaleLargessBean.loginName = loginBean.getUserName();
        b_SaleLargessBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          b_SaleLargessBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          b_SaleLargessBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        b_SaleLargessBean.dsMasterTable.setColumnFormat("zje", b_SaleLargessBean.sumFormat);
        b_SaleLargessBean.dsMasterTable.setColumnFormat("zsl", b_SaleLargessBean.qtyFormat);

        b_SaleLargessBean.dsDetailTable.setColumnFormat("sl", b_SaleLargessBean.qtyFormat);
        b_SaleLargessBean.dsDetailTable.setColumnFormat("hssl", b_SaleLargessBean.qtyFormat);
        b_SaleLargessBean.dsDetailTable.setColumnFormat("xsj", b_SaleLargessBean.priceFormat);
        b_SaleLargessBean.dsDetailTable.setColumnFormat("dj", b_SaleLargessBean.priceFormat);
        b_SaleLargessBean.dsDetailTable.setColumnFormat("jje", b_SaleLargessBean.priceFormat);
        b_SaleLargessBean.dsDetailTable.setColumnFormat("xsje", b_SaleLargessBean.priceFormat);

        session.setAttribute(beanName, b_SaleLargessBean);
      }
    }
    return b_SaleLargessBean;
  }
  /**
   * 构造函数
   */
  private B_SaleLargess()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.3','tdbh') from dual"}));
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.3','tdbh') from dual");
  ...
  class Add_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(),"sale_largess", content,deptid);
    }
  }
  /**
   * 取消审批
  ...
  class Cancer_Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
      approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(),"sale_largess");
    }
  }
  /**
   * 作废
    ...
    for(int i=0;i<dsDetailTable.getRowCount();i++)
    {
     ...
     if(counta>0&&!String.valueOf(LADING_OUT_TWO).equals(action))
     {
      ...
      for(int i=0;i<dsDetailTable.getRowCount();i++)
      {
        ...
        String stsl = dsDetailTable.getValue("stsl");//出库回填
        double dsl = stsl.length() > 0 ? Double.parseDouble(stsl) : 0;  //数量
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        detail.setValue("zk", String.valueOf(zk));//折扣
    ...
    private String checkDetailInfo()
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if(Double.parseDouble(dj) == 0)
          return showJavaScript("alert('单价不能等于零!');");
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        if(rownum==i)
        {
          ...
          detailRow.put("xsj", xsj);
          detailRow.put("dj", xsj);//销售赠送单直接引默认价格
          detailRow.put("zk", "100");//因业务规则变更，　现在规定引用产品的价格是产品的默认价格。注：不打折。
        }
      }
    }
  }
