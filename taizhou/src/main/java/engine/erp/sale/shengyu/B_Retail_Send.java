...
/**
 * <p>Title: 销售管理--零售发货单--</p>
 * <p>Description: 销售管理--零售发货单--<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author engine
 * @version 1.0
 */
public final class B_Retail_Send extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM xs_td WHERE djlx=7 AND ? AND fgsid=? ?  order by tdbh desc";
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(zsl,0))zsl FROM xs_td WHERE djlx=7 AND ? AND fgsid=? ?  ";
  private static final String MASTER_JE_SQL    = "SELECT SUM(nvl(zje,0))zje FROM xs_td WHERE djlx=7 AND ? AND fgsid=? ?  ";
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "xs_td.ls");
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_Retail_Send getInstance(HttpServletRequest request)
  {
    B_Retail_Send B_Retail_SendBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_Retail_SendBean";
      B_Retail_SendBean = (B_Retail_Send)session.getAttribute(beanName);
      if(B_Retail_SendBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        B_Retail_SendBean = new B_Retail_Send();
        B_Retail_SendBean.qtyFormat = loginBean.getQtyFormat();
        B_Retail_SendBean.priceFormat = loginBean.getPriceFormat();
        B_Retail_SendBean.sumFormat = loginBean.getSumFormat();
        B_Retail_SendBean.fgsid = loginBean.getFirstDeptID();
        B_Retail_SendBean.loginId = loginBean.getUserID();
        B_Retail_SendBean.loginName = loginBean.getUserName();
        B_Retail_SendBean.user = loginBean.getUser();
        if(loginBean.getSystemParam("SC_STORE_UNIT_STYLE").equals("1"))
          B_Retail_SendBean.conversion = true;
        if(loginBean.getSystemParam("XS_LADINGBILL_HANDWORK").equals("1"))
          B_Retail_SendBean.canOperate = true;//是否可以手工开提单
        //设置格式化的字段

        B_Retail_SendBean.dsMasterTable.setColumnFormat("zje", B_Retail_SendBean.sumFormat);
        B_Retail_SendBean.dsMasterTable.setColumnFormat("zsl", B_Retail_SendBean.qtyFormat);

        B_Retail_SendBean.dsDetailTable.setColumnFormat("sl", B_Retail_SendBean.qtyFormat);
        B_Retail_SendBean.dsDetailTable.setColumnFormat("hssl", B_Retail_SendBean.qtyFormat);
        B_Retail_SendBean.dsDetailTable.setColumnFormat("xsj", B_Retail_SendBean.priceFormat);
        B_Retail_SendBean.dsDetailTable.setColumnFormat("dj", B_Retail_SendBean.priceFormat);
        B_Retail_SendBean.dsDetailTable.setColumnFormat("jje", B_Retail_SendBean.priceFormat);
        B_Retail_SendBean.dsDetailTable.setColumnFormat("xsje", B_Retail_SendBean.priceFormat);

        session.setAttribute(beanName, B_Retail_SendBean);
      }
    }
    return B_Retail_SendBean;
  }
  /**
   * 构造函数
   */
  private B_Retail_Send()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"tdbh"}, new String[]{"SELECT pck_base.billNextCode('xs_td.ls','tdbh') from dual"}));
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        m_RowInfo.put("zt", "0");
        tdbh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('xs_td.ls','tdbh') from dual");
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      detailRow.put("dj", formatNumber(rowInfo.get("dj_"+i),sumFormat));//单价
 ...
 try{
  ...
  public class Add_Approve  implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(),"retail_send", content,deptid, this);
    }
    public void processApprove(ApproveResponse[] reponses) throws Exception
    {
      ...
      if(TmpRow.length==1)
      {
       ...
       dsDetailTable.next();
        ...
        if(reponses[i].getProjectFlowCode().equals("sale_special_retail_send")&&dzk>=zkl)
         {
           reponses[i].add();
         }
         else
      ...
      else if(TmpRow.length==2)
      {
       ...
       for(int j=0;j<dsDetailTable.getRowCount();j++)
       {
        ...
        if(reponses[i].getProjectFlowCode().equals("sale_special_retail_send")&&dzk>=zk1&&dzk<zk2)
        {
        reponses[i].add();
        }
        else
