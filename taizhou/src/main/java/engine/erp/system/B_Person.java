...
public class B_Person extends BaseAction implements Operate
{
  ...
  private static final String PERSON_SQL
      = "SELECT personid, bm, xm, username, userpass, deptid, phone, email, isuse, isdelete FROM emp WHERE isdelete=0 AND deptid IN (SELECT deptid FROM bm WHERE ismember=0 AND isdelete=0) ? ORDER BY bm";
