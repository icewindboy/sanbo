...
public final class B_EmpTry extends BaseAction implements Operate
{
  private static final String MASTER_STRUT_SQL = "SELECT * FROM rl_emp_try WHERE 1<>1";   //取主表的结构
  private static final String MASTER_SQL    = "SELECT * FROM rl_emp_try where 1=1 ? ";    //对主表的参数化查询
...
//操作
   ...
   * @return 返回试用情况的实例
   */
  public static B_EmpTry getInstance(HttpServletRequest request)
  {
    B_EmpTry b_EmpTryBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_EmpTryBean";
      b_EmpTryBean = (B_EmpTry)session.getAttribute(beanName);
      if(b_EmpTryBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_EmpTryBean = new B_EmpTry();
        session.setAttribute(beanName, b_EmpTryBean);
      }
    }
    return b_EmpTryBean;
  }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"try_ID"}, new String[]{"S_RL_EMP_TRY"}));
    dsMasterTable.setSort(new SortDescriptor("", new String[]{"try_ID"}, new boolean[]{false}, null, 0));
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ds.insertRow(false);
        try_ID = isMasterAdd ? dataSetProvider.getSequence("S_RL_EMP_TRY") : ds.getValue("try_ID");

        ds.setValue("try_ID", try_ID);
      }

      //保存主表数据
      String aa = rowInfo.get("deptid");
      //String bb = rowInfo.get("personid");
      ds.setValue("emp_name", rowInfo.get("emp_name"));//试用人姓名
      ds.setValue("deptid", rowInfo.get("deptid"));//反馈人所在部门
      ds.setValue("emp_sex", rowInfo.get("emp_sex"));//培训项目
      ds.setValue("try_job", rowInfo.get("try_job"));//试用职位
      ds.setValue("emp_polity", rowInfo.get("emp_polity"));//政治面貌
      ds.setValue("emp_study", rowInfo.get("emp_study"));//学历
      ds.setValue("try_state", rowInfo.get("try_state"));//试用情况
      ds.setValue("memo", rowInfo.get("memo"));//备注
      ds.setValue("emp_nation", rowInfo.get("emp_nation"));//民族
      ds.setValue("emp_native", rowInfo.get("emp_native"));//籍贯
      ds.setValue("emp_ic", rowInfo.get("emp_ic"));//身份证号
      ds.setValue("emp_phone", rowInfo.get("emp_phone"));//电话
      ds.setValue("date_born", rowInfo.get("date_born"));//出生日期
      ds.setValue("date_in", rowInfo.get("date_in"));//入厂日期
      ds.setValue("isDirect", rowInfo.get("isDirect"));//能否转正
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        new QueryColumn(master.getColumn("emp_name"), null, null, null, null, "like"),
        new QueryColumn(master.getColumn("try_job"), null, null, null, null, "try_job"),
        //new QueryColumn(master.getColumn("try_ID"), "VW_PERSON_PXFK", "try_ID", "xm", "xm", "like"),
  ...
  class Rl_Pxkc_adddel  implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      if(action.equals(PXKC_ADD))
      {
        if(!isMasterAdd)
          dsMasterTable.goToInternalRow(masterRow);
        String try_ID = dsMasterTable.getValue("try_ID");
      }
