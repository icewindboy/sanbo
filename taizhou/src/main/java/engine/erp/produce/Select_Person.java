 ...
 * @version 1.0
 */
public final class Select_Person extends BaseAction implements Operate
 {
   private static final String IMPORT_PERSON_SQL = "SELECT * FROM emp WHERE 1=1 ? order by deptid ";//分公司; 往来单位
    ...
    * 析构函数
    * */
   public static Select_Person getInstance(HttpServletRequest request)
   {
     Select_Person Select_PersonBean = null;
     HttpSession session = request.getSession(true);
     synchronized (session)
     {
       String beanName = "Select_PersonBean";
       Select_PersonBean = (Select_Person)session.getAttribute(beanName);
       if(Select_PersonBean == null)
       {
         LoginBean loginBean = LoginBean.getInstance(request);
         Select_PersonBean = new Select_Person();
         Select_PersonBean.fgsid = loginBean.getFirstDeptID();
         session.setAttribute(beanName, Select_PersonBean);
       }
     }
     return Select_PersonBean;
   }
   /**
    * 构造函数
    */
   private Select_Person()
   {
     try {
       jbInit();
     }
