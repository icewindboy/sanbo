...
{
  private static final String IMPORT_ORDER_SQL = "SELECT * FROM VW_PRODUCEPLAN_SEL_SALE WHERE zt<>8 AND fgsid='?' ? ";
  private static final String LOOK_ORDER_SQL = "SELECT * FROM VW_PRODUCEPLAN_SEL_SALE WHERE fgsid='?' ?";

  //查询出来的物料类别属性
  private static final String PROPERTY_SQL ="SELECT DISTINCT t.sxmc,t.sxlx FROM kc_dmlbsx t WHERE t.wzlbid IN(?)";
  private static final String PROPERTY_SQL2 ="SELECT DISTINCT t.sxmc,t.sxlx FROM kc_dmlbsx t WHERE 1<>1";
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(jhlx.equals("0"))
        SQL = combineSQL(IMPORT_ORDER_SQL, "?", new String[]{fgsid});
