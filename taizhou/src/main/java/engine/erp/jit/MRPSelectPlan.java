...
public final class MRPSelectPlan extends BaseAction implements Operate
{
   ...
   * @return 返回生产计划信息的实例
   */
  public static MRPSelectPlan getInstance(HttpServletRequest request)
  {
    MRPSelectPlan mrpSelectPlan = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "mrpSelectPlan";
      mrpSelectPlan = (MRPSelectPlan)session.getAttribute(beanName);
      if(mrpSelectPlan == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        mrpSelectPlan = new MRPSelectPlan();
        mrpSelectPlan.qtyFormat =loginBean.getQtyFormat();
        mrpSelectPlan.fgsid = loginBean.getFirstDeptID();

        mrpSelectPlan.dsProduce.setColumnFormat("zsl", mrpSelectPlan.qtyFormat);
        session.setAttribute(beanName, mrpSelectPlan);
      }
    }
    return mrpSelectPlan;
  }

  /**
   * 构造函数
   */
  private MRPSelectPlan()
  {
    try {
      jbInit();
    }
