...
public final class PlanSelectSaleMaster extends BaseAction implements Operate
{
   ...
   * @return 返回销售合同信息的实例
   */
  public static PlanSelectSaleMaster getInstance(HttpServletRequest request)
  {
    PlanSelectSaleMaster singleSelectSaleOrderBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "singleSelectSaleOrderBean";
      singleSelectSaleOrderBean = (PlanSelectSaleMaster)session.getAttribute(beanName);
      if(singleSelectSaleOrderBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        singleSelectSaleOrderBean = new PlanSelectSaleMaster();
