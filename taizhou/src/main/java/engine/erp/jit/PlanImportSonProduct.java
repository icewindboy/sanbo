 ...
 * @version 1.0
 */
public final class PlanImportSonProduct extends BaseAction implements Operate
{
   ...
   *
   * */
  public static PlanImportSonProduct getInstance(HttpServletRequest request)
  {
    PlanImportSonProduct planImportSonProductBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "planImportSonProductBean";
      planImportSonProductBean = (PlanImportSonProduct)session.getAttribute(beanName);
      if(planImportSonProductBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        planImportSonProductBean = new PlanImportSonProduct();
        planImportSonProductBean.fgsid = loginBean.getFirstDeptID();
        session.setAttribute(beanName, planImportSonProductBean);
      }
    }
    return planImportSonProductBean;
  }
  /**
   * 构造函数
   */
  private PlanImportSonProduct()
  {
    try {
      jbInit();
    }
