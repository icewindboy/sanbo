...
public final class ProduceProcess extends BaseAction implements Operate
{
  ...
  private ProcessSelectPlan processSelectPlanBean = null; //生产任务单主表信息BEAN的引用
   ...
   * @return 返回生产加工单列表的实例
   */
  public static ProduceProcess getInstance(HttpServletRequest request)
  {
    ProduceProcess produceProcessBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "produceProcessBean";
      produceProcessBean = (ProduceProcess)session.getAttribute(beanName);
      if(produceProcessBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        produceProcessBean = new ProduceProcess();
 ...
 class CommonMaterail_Edit implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     if(wlxqjhmxid.equals("") || scjhid.equals("")  || scjhid==null){
       ...
       String  SQl = combineSQL(MATERAIL_LIST_SQL, "?", new String[]{wlxqjhmxid, scjhid,sjcpid, temp});
       dsFactBomMaterail.setQueryString(SQl);
         ...
         for(int i=0;i<dsFactBomMaterail.getRowCount();i++){
           ...
           dsDrawMaterail.setValue("jgdmxid", jgdmxid);
           dsDrawMaterail.setValue("jgdid",isMasterAdd ? "-1" : jgdid);
           dsDrawMaterail.setValue("cpid", dsDrawMaterail.getValue("cpid"));
           dsDrawMaterail.setValue("dmsxid", dsDrawMaterail.getValue("dmsxid"));
           dsDrawMaterail.setValue("sl",dsDrawMaterail.getValue("xql"));
           dsDrawMaterail.setValue("scsl",dsDrawMaterail.getValue("scxql"));
           dsDrawMaterail.post();
  ...
  class Common_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      */
      ...
      ds.setValue("sfwjg", sfwjg);//是否外加工
      ds.setValue("dwtxid", rowInfo.get("dwtxid"));
      ds.setValue("gylxid", rowInfo.get("gylxid"));//工艺类型
      ds.setValue("gxfdid", rowInfo.get("gxfdid"));//工序分段
      //保存用户自定义的字段
      FieldInfo[] fields = masterProducer.getBakFieldCodes();
      for(int j=0; j<fields.length; j++)
      {
 ...
 class Detail_Select_Task implements Obactioner
 {
   ...
   *  选择任务单主表，引入从表所有未加工信息
   *
   *  jit 不引任务单改成引入物料需求 04.10.12 yjg
   */
 class Single_Select_Task implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
   {
     ...
     if(!isMasterAdd)
       dsMasterTable.goToInternalRow(masterRow);
     String jgdid = dsMasterTable.getValue("jgdid");
     dsMasterTable.setValue("scjhid",scjhid);
     ...
     for(int i=0; i<tempTaskData.getRowCount(); i++)
     {
       ...
       String wlxqjhmxid = tempTaskData.getValue("wlxqjhmxid");
       ...
       if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
       {
         ...
         if(i==tempTaskData.getRowCount()-1){
             rwdmxidBuf = rwdmxidBuf.append("'").append(wlxqjhmxid).append("'");
  ...
  * @return 返回用于查找生产任务单主表的一条信息的bean
   */
 public ProcessSelectPlan getTaskMasterBean(HttpServletRequest req)
 {
   if(processSelectPlanBean == null)
     processSelectPlanBean = ProcessSelectPlan.getInstance(req);
   return processSelectPlanBean;
 }
