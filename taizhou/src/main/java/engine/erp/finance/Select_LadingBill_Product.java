 ...
 * @version 1.0
 */
public final class Select_LadingBill_Product extends BaseAction implements Operate
 {
   private static final String IMPORT_SALE_SQL = "SELECT * FROM VW_SALE_INVOICE_IMPORT_TDDETAL WHERE fgsid=? ? order by tdrq ";//分公司; 往来单位
   private static final String MASTER_SQL    = "SELECT * FROM VW_SALE_INVOICE_IMPORT_TDDETAL WHERE  fgsid=? ?  order by tdrq ";
    ...
    * 析构函数
    * */
   public static Select_LadingBill_Product getInstance(HttpServletRequest request)
   {
     Select_LadingBill_Product select_LadingBill_ProductBean = null;
     HttpSession session = request.getSession(true);
     synchronized (session)
     {
       String beanName = "select_LadingBill_ProductBean";
       select_LadingBill_ProductBean = (Select_LadingBill_Product)session.getAttribute(beanName);
       if(select_LadingBill_ProductBean == null)
       {
         LoginBean loginBean = LoginBean.getInstance(request);
         select_LadingBill_ProductBean = new Select_LadingBill_Product();
         select_LadingBill_ProductBean.fgsid = loginBean.getFirstDeptID();
         session.setAttribute(beanName, select_LadingBill_ProductBean);
       }
     }
    ...
    * 构造函数
    */
   private Select_LadingBill_Product()
   {
     try {
       jbInit();
     }
