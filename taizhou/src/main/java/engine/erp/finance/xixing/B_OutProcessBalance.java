...
import engine.erp.finance.xixing.B_wjgdhwList;
/**
...
public final class B_OutProcessBalance extends BaseAction implements Operate
{
  ...
  private static final String AUTO_CANCER_SQL = "SELECT * FROM VW_SC_WJG_BALANCE where 1=1 and dwtxid=? ";
  ...
  private B_wjgdhwList ImportLadingProductBean = null; //进货单货物引用
 ...
 class Master_Add_Edit implements Obactioner
 {
  ...
  class Detail_Auto_Cancer implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0;i<dstmp.getRowCount();i++)
      {
        ...
        //dsDetailTable.setValue("hkje", dstmp.getValue("wjsje"));
        //dsDetailTable.setValue("ce", dstmp.getValue("0"));
        zjsje +=jsje;
        if(zjsje>=sjje)
        {
          dsDetailTable.setValue("jsje", String.valueOf(jsje-zjsje+sjje));
          //dsDetailTable.setValue("ce", String.valueOf(zjsje-sjje));
  ...
  class Detail_SALE_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      B_wjgdhwList ImportLadingProductBean =getsaleOrderBean(req);
      for(int i=0; i < jgdmxid.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          dsDetailTable.setValue("jsje", saleRow.get("jgje"));//净金额与实收金额的差额
          //dsDetailTable.setValue("tcl", saleRow.get("hktcl").length()>0?saleRow.get("hktcl"):"0");

          double jsje = saleRow.get("jgje").length() > 0 ? Double.parseDouble(saleRow.get("jgje")) : 0;//结算金额
   ...
   * @return 返回用于查找合同编号的bean
     */
  public B_wjgdhwList getsaleOrderBean(HttpServletRequest req)
  {
    if(ImportLadingProductBean == null)
      ImportLadingProductBean = B_wjgdhwList.getInstance(req);
