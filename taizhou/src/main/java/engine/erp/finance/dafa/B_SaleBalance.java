package engine.erp.finance.dafa;
...
import engine.erp.finance.ImportLadingProduct;
/**
 * <p>Title: 销售管理--销售结算管理--</p>
 * <p>Description: 销售管理--销售结算管理--<br>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: </p>
 * @author engine
 *  * @version 1.0
 */
public final class B_SaleBalance extends BaseAction implements Operate
{
  ...
  private static final String MASTER_STRUT_SQL = "SELECT * FROM cw_xsjs WHERE 1<>1 ";
  private static final String MASTER_SEARCH_SQL    = "SELECT * FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ?  ?  order by djh desc";
  private static final String MASTER_SQL    = "SELECT cw_xsjs.* FROM cw_xsjs,dwtx WHERE cw_xsjs.dwtxid=dwtx.dwtxid  ? AND cw_xsjs.fgsid =? ?  ? ";
  private static final String JE_SQL    = "select SUM(nvl(je,0))zje FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String HXJE_SQL    = "select SUM(nvl(hxje,0))zhxje FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String WHXJE_SQL    = "select SUM(nvl(whxje,0))zwhxje FROM cw_xsjs WHERE 1=1 AND ? AND fgsid =? ? ORDER BY djh DESC ";
  private static final String COLLECT_SQL = "SELECT sum(nvl(a.je,0))zje,sum(nvl(a.hxje,0))zhxje,sum(nvl(a.je,0)-nvl(a.hxje,0))totalWhxje FROM cw_xsjs a WHERE a.dwtxid='?' and a.personid='?' and a.khlx='?' GROUP BY a.dwtxid";//汇总

  private static final String DETAIL_STRUT_SQL = "SELECT * FROM cw_xsjshx WHERE 1<>1 ";
  private static final String DETAIL_SQL    = "SELECT * FROM cw_xsjshx WHERE xsjsID= ";//
  private static final String TDMX_SQL      ="SELECT * FROM VW_SALE_BALANCE_IMPORT_TDDETAL WHERE tdid= ";//
  //用于审批时候提取一条记录
  private static final String MASTER_APPROVE_SQL = "SELECT * FROM cw_xsjs WHERE xsjsID='?'";
  private static final String AUTO_CANCER_SQL = "SELECT * FROM vw_sale_autobalance where 1=1 and dwtxid=? and personid=? and khlx='?' and isnet='?' ORDER BY px, tdrq";
  private EngineDataSet dsMasterTable  = new EngineDataSet();//主表
  private EngineDataSet dsDetailTable  = new EngineDataSet();//从表
  private EngineDataSet tdMxTable      = new EngineDataSet();//提单明细

  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cw_xsjs","cw_xsjs");
  public  HtmlTableProducer detailProducer = new HtmlTableProducer(dsDetailTable, "cw_xsjshx");
  public  boolean isApprove = false;     //是否在审批状态
  private boolean isMasterAdd = true;    //是否在添加状态
  private long    masterRow = -1;         //保存主表修改操作的行记录指针
  private RowMap  m_RowInfo    = new RowMap(); //主表添加行或修改行的引用
  private ArrayList d_RowInfos = null; //从表多行记录的引用
  private ImportLadingProduct ImportLadingProductBean = null; //提单货物引用
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_SaleBalance getInstance(HttpServletRequest request)
  {
    B_SaleBalance b_SaleBalanceBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "b_SaleBalanceBean";
      b_SaleBalanceBean = (B_SaleBalance)session.getAttribute(beanName);
      if(b_SaleBalanceBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);
        b_SaleBalanceBean = new B_SaleBalance();
        b_SaleBalanceBean.qtyFormat = loginBean.getQtyFormat();
        b_SaleBalanceBean.priceFormat = loginBean.getPriceFormat();
        b_SaleBalanceBean.sumFormat = loginBean.getSumFormat();
        b_SaleBalanceBean.fgsid = loginBean.getFirstDeptID();
        b_SaleBalanceBean.loginid = loginBean.getUserID();
        b_SaleBalanceBean.loginName = loginBean.getUserName();
        //设置格式化的字段
        b_SaleBalanceBean.user = loginBean.getUser();
        b_SaleBalanceBean.dsMasterTable.setColumnFormat("je", b_SaleBalanceBean.sumFormat);
        b_SaleBalanceBean.dsMasterTable.setColumnFormat("hxje", b_SaleBalanceBean.sumFormat);
        b_SaleBalanceBean.dsMasterTable.setColumnFormat("whxje", b_SaleBalanceBean.sumFormat);

        b_SaleBalanceBean.dsDetailTable.setColumnFormat("tcl", b_SaleBalanceBean.qtyFormat);
        b_SaleBalanceBean.dsDetailTable.setColumnFormat("tcj", b_SaleBalanceBean.sumFormat);
        b_SaleBalanceBean.dsDetailTable.setColumnFormat("jsje", b_SaleBalanceBean.sumFormat);
        session.setAttribute(beanName, b_SaleBalanceBean);
      }
    }
    return b_SaleBalanceBean;
  }
  /**
   * 构造函数
   */
  private B_SaleBalance()
  {
    try {
      jbInit();
    }
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    dsMasterTable.setSequence(new SequenceDescriptor(new String[]{"djh"}, new String[]{"SELECT pck_base.billNextCode('cw_xsjs','djh') from dual"}));
    //dsMasterTable.setSort(new SortDescriptor("", new String[]{"djh"}, new boolean[]{true}, null, 0));
    dsMasterTable.setTableName("cw_xsjs");
    dsDetailTable.setSequence(new SequenceDescriptor(new String[]{"xsjshxID"}, new String[]{"s_cw_xsjshx"}));
  ...
  private final void initRowInfo(boolean isMaster, boolean isAdd, boolean isInit) throws java.lang.Exception
  {
    ...
    if(isMaster){
      ...
      else
      {
        ...
        String djh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('cw_xsjs','djh') from dual");
 ...
 public final String getZwhxje()
 {
  ...
  {
    String SQL = DETAIL_SQL + (isMasterAdd ? "-1" : xsjsid);
  ...
  //&#$
   ...
   class Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       //打开从表
       xsjsid = dsMasterTable.getValue("xsjsid");
  ...
  class Balance_Over implements Obactioner
  {
   ...
   class Cancer_Approve implements Obactioner
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.cancelAprove(dsMasterTable,dsMasterTable.getRow(),"sale_balance");
     }
  }
   //&#$
   /**
   ...
   public class Add_Approve  implements Obactioner,ApproveListener
   {
     public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
     {
       ...
       approve.putAproveList(dsMasterTable, dsMasterTable.getRow(),"sale_balance", content,deptid, this);
     }
     public void processApprove(ApproveResponse[] reponses) throws Exception
     {
       ...
       for(int i=0;i<reponses.length;i++)
       {
         ...
         if(reponses[i].getProjectFlowCode().equals("sale_special_balance")&&khlx.equalsIgnoreCase(reponses[i].getProjectFlowValue()))
         {
           reponses[i].add();
         }
         else
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      zt = new String[]{""};
      isMasterAdd = true;
      String SQL = " AND cw_xsjs.zt<>8 AND cw_xsjs.zt<>4 ";
      String deptvalue = user.getHandleDeptValue("deptid");
      if(deptvalue.indexOf(">")>0)
        deptvalue = " 1<>1 ";
      else
        deptvalue = " and cw_xsjs."+user.getHandleDeptValue("deptid");
      String MSQL =  combineSQL(MASTER_SQL, "?", new String[]{deptvalue, fgsid, SQL," order by cw_xsjs.djh desc"});
 ...
 class Master_Add_Edit implements Obactioner
 {
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isMasterAdd){
        ds.insertRow(false);
        xsjsid = dataSetProvider.getSequence("s_cw_xsjs");
        String djh = rowInfo.get("djh");
        String count = dataSetProvider.getSequence("select count(*) from cw_xsjs t where t.djh='"+djh+"'");
        if(!count.equals("0"))
          djh = dataSetProvider.getSequence("SELECT pck_base.billNextCode('cw_xsjs','djh') from dual");
        ds.setValue("djh", djh);//

        ds.setValue("xsjsid", xsjsid);//主健
        ds.setValue("zt","0");
  ...
  class Master_Detail_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ds.setValue("isnet",rowInfo.get("isnet"));
  ...
  public final int getSelectedRow()
  {
    ...
    private String checkDetailInfo()
    {
      ...
      //if(zwhje.compareTo(zhxje)<0&&(zwhje.compareTo(new BigDecimal(0))!=0))
  ...
  class Master_Delete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      xsjsid = ds.getValue("xsjsID");
      String zt = dataSetProvider.getSequence("SELECT a.zt FROM cw_xsjs a WHERE a.xsjsid='"+xsjsid+"'");
  ...
  final class Orderby implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      SQL = combineSQL(MASTER_SQL, "?", new String[]{
                       ...
                       fgsid+" and ",
                       deptvalue,
                       orderBy.length() > 0 ? "ORDER BY "+orderBy+" desc " : " ORDER BY cw_xsjs.djh desc"
                       });
      dsMasterTable.setQueryString(SQL);
      dsMasterTable.readyRefresh();
    }
  ...
  class Master_Search implements Obactioner
  {
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      fixedQuery.addShowColumn("", new QueryColumn[]{
        ...
        new QueryColumn(master.getColumn("jsfsid"), null, null, null, null, "="),
  ...
  class Detail_Auto_Cancer implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      putDetailInfo(data.getRequest());
      RowMap rowInfo = getMasterRowinfo();
      String isnet = rowInfo.get("isnet");
      ...
      if(dwtxid.equals("")||personid.equals("")||khlx.equals(""))
        return;

      EngineDataSet dscollect = new EngineDataSet();
      setDataSetProperty(dscollect,combineSQL(COLLECT_SQL,"?",new String[]{dwtxid,personid,khlx}));
      dscollect.open();
      dscollect.first();
      ...
      String SQL = combineSQL(AUTO_CANCER_SQL,"?",new String[]{dwtxid,personid,khlx,isnet});
      ...
      for(int i=0;i<dstmp.getRowCount();i++)
      {
        ...
        dsDetailTable.setValue("xsjshxID", "-1");
        dsDetailTable.setValue("xsjsid", xsjsid);

        dsDetailTable.setValue("cpid", dstmp.getValue("cpid"));

        dsDetailTable.setValue("tdhwId", dstmp.getValue("tdhwId"));

        dsDetailTable.setValue("tcl", dstmp.getValue("hktcl").length()>0?dstmp.getValue("hktcl"):"0");

        double jsje = dstmp.getValue("wjsje").length() > 0 ? Double.parseDouble(dstmp.getValue("wjsje")) : 0;//结算金额
        double tcl = dstmp.getValue("hktcl").length() > 0 ? Double.parseDouble(dstmp.getValue("hktcl")) : 0;  //提成率

        dsDetailTable.setValue("tcj", String.valueOf(jsje * tcl*0.01));//提成奖
        dsDetailTable.setValue("yhrq", dstmp.getValue("yhrq"));
        ...
        if(zjsje>=sjje)
        {
          ...
          dsDetailTable.setValue("tcj", String.valueOf((jsje-zjsje+sjje)*tcl*0.01));//提成奖
          dsDetailTable.post();
          break;
        }
        dsDetailTable.post();
  ...
  class Detail_SALE_Add implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      ImportLadingProduct ImportLadingProductBean =getsaleOrderBean(req);
      for(int i=0; i < tdhwid.length; i++)
      {
        ...
        if(!isMasterAdd)
        dsMasterTable.goToInternalRow(masterRow);
        String xsjsid = dsMasterTable.getValue("xsjsid");
        RowMap detailrow = null;
        locateGoodsRow.setValue(0, tdhwid[i]);
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          RowMap saleRow = ImportLadingProductBean.getLookupRow(tdhwid[i]);
          ...
          dsDetailTable.setValue("xsjshxID", "-1");
          dsDetailTable.setValue("xsjsid", xsjsid);
          dsDetailTable.setValue("tdhwId", tdhwid[i]);
   ...
   */
  public ImportLadingProduct getsaleOrderBean(HttpServletRequest req)
  {
    if(ImportLadingProductBean == null)
      ImportLadingProductBean = ImportLadingProduct.getInstance(req);
