...
public final class ImportApply extends BaseAction implements Operate
{
   ...
   * @return 返回申请单信息的实例
   */
  public static ImportApply getInstance(HttpServletRequest request)
  {
    ImportApply importApplyBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "importApplyBean";
      importApplyBean = (ImportApply)session.getAttribute(beanName);
      if(importApplyBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        importApplyBean = new ImportApply();
        importApplyBean.fgsid = loginBean.getFirstDeptID();
        session.setAttribute(beanName, importApplyBean);
      }
    }
    return importApplyBean;
  }
  ...
  class Init implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String SQL = combineSQL(IMPORT_APPLY_SQL, "?", new String[]{fgsid,dwtxid});
      //if(!dwtxid.equals(""))
  ...
  class Search implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(SQL.length() > 0)
        SQL = " AND "+SQL;
      SQL = combineSQL(IMPORT_APPLY_SQL, "?", new String[]{fgsid, dwtxid, SQL});
  ...
  *得到一行信息
  ...
  public final RowMap getLookupRow(String cgsqdhwid)
  {
    ...
    if(cgsqdhwid == null || cgsqdhwid.equals(""))
      return row;
    EngineRow locateRow = new EngineRow(dsBuyApplyProduct, "cgsqdhwid");
    if(locateRow == null)
      locateRow = new EngineRow(getOneTable(), "cgsqdhwid");
