...
public final class B_BuyOrder extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM cg_ht ORDER BY htbh DESC";
  ...
  public  HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "cg_ht");
  ...
  public static B_BuyOrder getInstance(HttpServletRequest request)
  {
    ...
    synchronized (session)
    {
      ...
      if(buyOrderBean == null)
      {
        ...
        buyOrderBean.dsDetailTable.setColumnFormat("je", buyOrderBean.priceFormat);
 ...
 class Approve implements Obactioner
 {
  ...
  public class Add_Approve  implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(),"buy_order", content,deptid, this);
    }
    public void processApprove(ApproveResponse[] reponses) throws Exception
    {
      ...
      for(int i=0;i<reponses.length;i++)
      {
        ...
        if(reponses[i].getProjectFlowCode().equals("buy_special_order")&&khlx.equalsIgnoreCase(reponses[i].getProjectFlowValue()))
        {
          reponses[i].add();
        }
        else
