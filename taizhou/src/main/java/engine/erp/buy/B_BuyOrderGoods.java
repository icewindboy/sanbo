...
public final class B_BuyOrderGoods extends BaseAction implements Operate
{
  ...
  private static final String MASTER_SQL    = "SELECT * FROM cg_htjhd WHERE ? AND fgsid=? ? ORDER BY djlx DESC, jhdbm DESC";
   ...
   * @return 返回销售合同列表的实例
   */
  public static B_BuyOrderGoods getInstance(HttpServletRequest request)
  {
    B_BuyOrderGoods buyOrderGoodsBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "buyOrderGoodsBean";
      buyOrderGoodsBean = (B_BuyOrderGoods)session.getAttribute(beanName);
      if(buyOrderGoodsBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        buyOrderGoodsBean = new B_BuyOrderGoods();
