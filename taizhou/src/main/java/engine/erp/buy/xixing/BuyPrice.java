...
public final class BuyPrice extends BaseAction implements Operate
{
  ...
  private static final String BUYPRICE_SQL  //"SELECT * FROM cg_bj where fgsid=? ? ORDER BY cgbjID DESC";//
      = "SELECT a.* FROM cg_bj a, kc_dm b, dwtx c WHERE a.cpid = b.cpid AND a.dwtxid = c.dwtxid "
      + "AND a.fgsid=? AND (c.deptid IS NULL OR (c.?)) ? ORDER BY b.cpbm ";
  private static final String MASTER_STRUT_SQL = "SELECT * FROM cg_bj WHERE 1<>1";
  ...
  public  HtmlTableProducer table = new HtmlTableProducer(dsBuyPrice, "cg_bj", "a");
  /**
   * 用于定位数据集
   */
  ...
  * 定义固定查询类
   ...
   * @return 返回报价资料信息的实例
   */
  public static BuyPrice getInstance(HttpServletRequest request)
  {
    BuyPrice buyPriceBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "buyPriceBean";
      buyPriceBean = (BuyPrice)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(buyPriceBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        buyPriceBean = new BuyPrice();
  ...
  class BuyPrice_Init implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      dsBuyPrice.setQueryString(MASTER_STRUT_SQL);
  ...
  class FIXED_SEARCH implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(SQL.length() > 0)
        SQL = " AND "+SQL;
      SQL = combineSQL(BUYPRICE_SQL, "?", new String[]{fgsid,user.getHandleDeptValue(),SQL});
    ...
    private void initQueryItem(HttpServletRequest request)
    {
      ...
      EngineDataSet master = dsBuyPrice;
      if(!master.isOpen())
        master.open();
  ...
  class BuyPrice_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String dwtxid = rowInfo.get("dwtxid");  //单位通信id
      String dmsxid = rowInfo.get("dmsxid");  //规格属性
      String cpid  = rowInfo.get("cpid");    //产品
      String bj  = rowInfo.get("bj");        //报价
      String cgbjid = rowInfo.get("cgbjid");  //采购报价id
      String yhtj  = rowInfo.get("yhtj");    //优惠条件
      String ksrq  = rowInfo.get("ksrq");    //开始日期
      String jsrq  = rowInfo.get("jsrq");    //结束日期
      String jbzlqk = rowInfo.get("jbzlqk");
      String hyf = "";
      String hp = "";
      String hg = "";
      String[] c = data.getRequest().getParameterValues("CH");
      if(c==null)
      {
        hyf ="";
        hp ="";
      ...
      String gyslh  = rowInfo.get("gyslh");   //供应商料号
      ...
      if(bj.length()>0 && (temp = checkNumber(bj, "报价", false))!=null)
      {
      ...
      String count2= "0";
      String count3= "0";
      if(dmsxid.equals("")&&!sflsbj.equals("1")&&isAdd)
      {
      {
        count2  = dataSetProvider.getSequence("SELECT COUNT(*)  FROM cg_bj t WHERE t.cpid='"+cpid+"' AND t.dmsxid is null AND t.dwtxid='"+dwtxid+"' AND t.sflsbj=0 AND t.jsrq=to_date('"+jsrq+"','yyyy-mm-dd')" );
      }
      count3  = dataSetProvider.getSequence("SELECT COUNT(*)  FROM cg_bj t WHERE t.cpid='"+cpid+"' AND t.dmsxid='"+dmsxid+"' AND t.dwtxid='"+dwtxid+"' AND t.sflsbj=0 AND t.jsrq=to_date('"+jsrq+"','yyyy-mm-dd')");
      if(!count2.equals("0")||!count3.equals("0")){
        data.setMessage(showJavaScript("alert('该种报价已存在！');"));
        return;
      }
      }
      if(dmsxid.equals("")&&!sflsbj.equals("1"))
      {
      {
        count2  = dataSetProvider.getSequence("SELECT COUNT(*)  FROM cg_bj t WHERE t.cpid='"+cpid+"' AND t.dmsxid is null AND t.dwtxid='"+dwtxid+"' AND t.sflsbj=0 AND t.jsrq=to_date('"+jsrq+"','yyyy-mm-dd') AND t.cgbjid<>'"+cgbjid+"'" );
      }
      count3  = dataSetProvider.getSequence("SELECT COUNT(*)  FROM cg_bj t WHERE t.cpid='"+cpid+"' AND t.dmsxid='"+dmsxid+"' AND t.dwtxid='"+dwtxid+"' AND t.sflsbj=0 AND t.jsrq=to_date('"+jsrq+"','yyyy-mm-dd') AND t.cgbjid<>'"+cgbjid+"'");
      if(!count2.equals("0")||!count3.equals("0")){
        data.setMessage(showJavaScript("alert('该种报价已存在！');"));
        return;
      }
