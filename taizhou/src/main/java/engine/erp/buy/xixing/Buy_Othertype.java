...
public final class Buy_Othertype extends BaseAction implements Operate
{
  ...
  {
    Buy_Othertype billTypeBean2 = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "billTypeBean2";
      billTypeBean2 = (Buy_Othertype)session.getAttribute(beanName);
      //判断该session是否有该bean的实例
      if(billTypeBean2 == null)
      {
        billTypeBean2 = new Buy_Othertype();
        session.setAttribute(beanName, billTypeBean2);//加入到session中
      }
    }
