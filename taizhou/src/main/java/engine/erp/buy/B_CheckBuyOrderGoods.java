...
public final class B_CheckBuyOrderGoods extends BaseAction implements Operate {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1009129479286042835L;
	...
	private static final String MASTER_SQL = "SELECT * FROM cg_htjhd WHERE ? AND djlx=1  AND fgsid=? ? ORDER BY djlx DESC, jhdbm DESC";
	...
	// 通过合同ID得到合同货物信息
	private static final String ORDER_DETAIL_SQL = "SELECT a.* FROM cg_hthw a, kc_dm b WHERE a.cpid=b.cpid AND nvl(a.sl,0)>nvl(a.sjjhl,0) AND (b.storeid IS NULL OR b.storeid=? ) AND htid=? ";//
	 ...
	 * @return 返回销售合同列表的实例
	 */
	public static B_CheckBuyOrderGoods getInstance(HttpServletRequest request) {
		B_CheckBuyOrderGoods checkBuyOrderGoodsBean = null;
		HttpSession session = request.getSession(true);
		synchronized (session) {
			String beanName = "checkBuyOrderGoodsBean";
			checkBuyOrderGoodsBean = (B_CheckBuyOrderGoods) session.getAttribute(beanName);
			if (checkBuyOrderGoodsBean == null) {
				// 引用LoginBean
				LoginBean loginBean = LoginBean.getInstance(request);

				checkBuyOrderGoodsBean = new B_CheckBuyOrderGoods();
				checkBuyOrderGoodsBean.qtyFormat = loginBean.getQtyFormat();
				checkBuyOrderGoodsBean.priceFormat = loginBean.getPriceFormat();
				checkBuyOrderGoodsBean.sumFormat = loginBean.getSumFormat();
				checkBuyOrderGoodsBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");

				checkBuyOrderGoodsBean.fgsid = loginBean.getFirstDeptID();
				checkBuyOrderGoodsBean.loginId = loginBean.getUserID();
				checkBuyOrderGoodsBean.loginName = loginBean.getUserName();
				checkBuyOrderGoodsBean.loginDept = loginBean.getDeptID();
				checkBuyOrderGoodsBean.user = loginBean.getUser();
				checkBuyOrderGoodsBean.SYS_APPROVE_ONLY_SELF = loginBean.getSystemParam("SYS_APPROVE_ONLY_SELF");// 提交审批是否只有制单人可以提交,1=仅制单人可提交,0=有权限人可提交
				SYS_CUST_NAME = loginBean.getSystemParam("SYS_CUST_NAME");// 客户名称,用于客制化程序
				// checkBuyOrderGoodsBean.deptLimit =
				// loginBean.getUser().getHandleDeptValue();
				// 设置格式化的字段
				checkBuyOrderGoodsBean.dsDetailTable.setColumnFormat("sl", checkBuyOrderGoodsBean.qtyFormat);
				checkBuyOrderGoodsBean.dsDetailTable.setColumnFormat("hssl", checkBuyOrderGoodsBean.qtyFormat);
				// checkBuyOrderGoodsBean.dsDetailTable.setColumnFormat("dj",
				// checkBuyOrderGoodsBean.priceFormat);
				// checkBuyOrderGoodsBean.dsDetailTable.setColumnFormat("je",
				// checkBuyOrderGoodsBean.sumFormat);
				// checkBuyOrderGoodsBean.dsDetailTable.setColumnFormat("sjrkl",
				// checkBuyOrderGoodsBean.sumFormat);
				checkBuyOrderGoodsBean.dsMasterTable.setColumnFormat("zsl", checkBuyOrderGoodsBean.sumFormat);
				checkBuyOrderGoodsBean.dsMasterTable.setColumnFormat("zje", checkBuyOrderGoodsBean.sumFormat);
				session.setAttribute(beanName, checkBuyOrderGoodsBean);
			}
		}
		return checkBuyOrderGoodsBean;
	}

	/**
	 * 构造函数
	 */
	private B_CheckBuyOrderGoods() {
		try {
			jbInit();
		} catch (Exception ex) {
			log.error("jbInit", ex);
	...
	private final void jbInit() throws java.lang.Exception {
		...
		// dsDetailTable.setSort(new SortDescriptor("", new String[]{"cpbm"},
		// new boolean[]{false}, null, 0));
		dsDetailTable.setSort(new SortDescriptor("", new String[] { "hthwid", "cpid" }, new boolean[] { false, false },
				null, 0));
		...
		// addObactioner(String.valueOf(SINGLE_PRODUCT_ADD), new
		// Single_Product_Add());
	...
	private final void putDetailInfo(HttpServletRequest request) {
		...
		for (int i = 0; i < rownum; i++) {
			...
			// detailRow.put("sl", formatNumber(rowInfo.get("sl_"+i),
			// qtyFormat));//数量
			// if ( SYS_CUST_NAME.equals("essen") ) detailRow.put("hssl",
			// formatNumber(rowInfo.get("hssl_"+i), qtyFormat));//换算数量
			// detailRow.put("dj", formatNumber(rowInfo.get("dj_"+i),
			// priceFormat));//单价
			// detailRow.put("je", formatNumber(rowInfo.get("je_"+i),
			// sumFormat));//金额
			...
			detailRow.put("check_result", rowInfo.get("check_result_" + i));
			detailRow.put("check_number", rowInfo.get("check_number_" + i));
			detailRow.put("check_explain", rowInfo.get("check_explain_" + i));
			detailRow.put("check_permit", rowInfo.get("check_permit_" + i));
	...
	/**
	 * 根据参数判断改进货单是否被采购入库单引用 如果被引用返回true
	 * 
	 * @return 是否在添加状态
	 */
	...
	class Add_Approve implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			if (billType.equals("1"))
				approve.putAproveList(dsMasterTable, row, "buy_ordergoods", content, deptid);
			else
				approve.putAproveList(dsMasterTable, row, "unbuy_ordergoods", content, deptid);
		}
	}
	 ...
	 * 从表增加操作（单选产品）
	 * 
	 * class Single_Product_Add implements Obactioner { public void
	 * execute(String action, Obationable o, RunData data, Object arg) throws
	 * Exception { HttpServletRequest req = data.getRequest(); //保存输入的明细信息
	 * putDetailInfo(data.getRequest()); int row =
	 * Integer.parseInt(data.getParameter("rownum")); String singleIdInput =
	 * m_RowInfo.get("singleIdInput_"+row); if(singleIdInput.equals("")) return;
	 * 
	 * //实例化查找数据集的类 String cpid = singleIdInput; if(!isMasterAdd)
	 * dsMasterTable.goToInternalRow(masterRow); String jhdid =
	 * dsMasterTable.getValue("jhdid"); dsDetailTable.goToRow(row); RowMap
	 * detailrow = null; detailrow = (RowMap)d_RowInfos.get(row);
	 * detailrow.put("jhdhwid", "-1"); detailrow.put("cpid", cpid);
		...
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int i = 0; i < processData.getRowCount(); i++) {
				...
				if (!dsDetailTable.locate(locateGoodsRow, Locate.FIRST)) {
					double sjjhl = 0, sl = 0, wjhl = 0, dj = 0;// 实际入库量， 合同数量，
																// 未进货量,金额
					sjjhl = processData.getValue("sjjhl").length() > 0 ? Double.parseDouble(processData
							.getValue("sjjhl")) : 0;// 实际进货数量
					...
					// double hsbl = productRow.get("hsbl").length() > 0 ?
					// Double.parseDouble(productRow.get("hsbl")) : 0;//数量
					...
					if (djlx.equals("1")) {
						...
						/**
						 * dsDetailTable.setValue("sl",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : wjhl*hsbl) :
						 * String.valueOf(wjhl), qtyFormat));
						 * dsDetailTable.setValue
						 * ("hssl",formatNumber(bjfs.equals("1") ?
						 * String.valueOf(wjhl) : String.valueOf(hsbl==0 ? 0 :
						 * wjhl/hsbl), qtyFormat)); dsDetailTable.setValue("je",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : wjhl*hsbl*dj) :
						 * String.valueOf(wjhl*dj),sumFormat) );
						 * */
					} else {
						if (bjfs.equals("1"))
						 ...
						 * dsDetailTable.setValue("sl",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : wjhl*hsbl*-1) :
						 * String.valueOf(wjhl*-1), qtyFormat));
						 * dsDetailTable.setValue
						 * ("hssl",formatNumber(bjfs.equals("1") ?
						 * String.valueOf(wjhl*-1) : String.valueOf(hsbl==0 ? 0
						 * : -1*wjhl/hsbl), qtyFormat));
						 * dsDetailTable.setValue("je",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : -1*wjhl*hsbl*dj) :
						 * String.valueOf(-1*wjhl*dj),sumFormat) );
						 * */
					}
					...
					dsDetailTable.setValue("check_result", processData.getValue("check_result"));
					dsDetailTable.setValue("check_number", processData.getValue("check_number"));
					dsDetailTable.setValue("check_explain", processData.getValue("check_explain"));
					dsDetailTable.setValue("check_permit", processData.getValue("check_permit"));
	...
	class Master_Post implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int i = 0; i < d_RowInfos.size(); i++) {
				...
				if (djlx.equals("1"))// 单据为进货单时保存正数即页面信息
					...
					// 单据为退货单时保存负数即页面数据乘以-1
					detail.setValue("je",
							bjfs.equals("1") ? String.valueOf(-1 * hssl * dj) : String.valueOf(-1 * sl * dj));// 金额
				...
				detail.setValue("ybje", detailrow.get("ybje"));

				dsDetailTable.setValue("check_result", detailrow.get("check_result"));
				dsDetailTable.setValue("check_number", detailrow.get("check_number"));
				dsDetailTable.setValue("check_explain", detailrow.get("check_explain"));
				dsDetailTable.setValue("check_permit", detailrow.get("check_permit"));
			...
			ds.setValue("shdz", rowInfo.get("shdz"));
			// 保存用户自定义的字段
			...
			ds.post();
			if (djlx.equals("1"))
				dsMasterTable.setSequence(new SequenceDescriptor(new String[] { "jhdbm" },
						new String[] { "SELECT pck_base.billNextCode('cg_htjhd','jhdbm') from dual" }));
			if (djlx.equals("-1"))
				dsMasterTable.setSequence(new SequenceDescriptor(new String[] { "jhdbm" },
						new String[] { "SELECT pck_base.billNextCode('cg_htjhd','jhdbm','t') from dual" }));
		...
		private String checkDetailInfo() {
			...
			for (int i = 0; i < d_RowInfos.size(); i++) {
				...
				// double b_hssl = hssl.length()>0 ? Double.parseDouble(hssl) :
				// 0;
				// if(bjfs.equals("1")){
	...
	class Master_Search implements Obactioner {
		...
		private void initQueryItem(HttpServletRequest request) {
			...
			fixedQuery.addShowColumn("",
					new QueryColumn[] { new QueryColumn(master.getColumn("jhdbm"), null, null, null),
							...
							new QueryColumn(master.getColumn("jhdid"), "VW_CG_HTJHDHW", "jhdid", "product", "product",
									"like"),// 从表产品
							new QueryColumn(master.getColumn("jhdid"), "VW_CG_HTJHDHW", "jhdid", "cpbm", "cpbm",
									"left_like"),// 从表产品
			...
			for (int i = 0; i < hthwIDs.length; i++) {
				...
				if (!dsDetailTable.locate(locateGoodsRow, Locate.FIRST)) {
					...
					if (djlx.equals("1")) {
						 ...
						 * dsDetailTable.setValue("sl",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : sl*hsbl) :
						 * String.valueOf(sl), qtyFormat));
						 * dsDetailTable.setValue
						 * ("hssl",formatNumber(bjfs.equals("1") ?
						 * String.valueOf(sl) : String.valueOf(hsbl==0 ? 0 :
						 * sl/hsbl), qtyFormat)); dsDetailTable.setValue("je",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : sl*hsbl*dj) :
						 * String.valueOf(sl*dj),sumFormat) );
						 * */
					} else {
						if (bjfs.equals("1"))
						 ...
						 * dsDetailTable.setValue("sl",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : sl*hsbl*-1) :
						 * String.valueOf(sl*-1), qtyFormat));
						 * dsDetailTable.setValue
						 * ("hssl",formatNumber(bjfs.equals("1") ?
						 * String.valueOf(sl*-1) : String.valueOf(hsbl==0 ? 0 :
						 * -1*sl/hsbl), qtyFormat));
						 * dsDetailTable.setValue("je",
						 * formatNumber(bjfs.equals("1") ?
						 * String.valueOf(hsbl==0 ? 0 : -1*sl*hsbl*dj) :
						 * String.valueOf(-1*sl*dj),sumFormat) );
						 * */
					}
