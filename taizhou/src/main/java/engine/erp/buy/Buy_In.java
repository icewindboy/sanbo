...
public final class Buy_In extends BaseAction implements Operate
{
  ...
  public  static final String CPONCHANGE = "2008001";//选择产品触发事件
  ...
  private static final String MASTER_SQL    = "SELECT * FROM cg_htjhd WHERE ? AND fgsid=? ? AND djlx=1 ORDER BY jhdbm DESC";
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    addObactioner(String.valueOf(CPONCHANGE), new Cp_Onchange());//删除数量为空白行触发事件
  ...
  public final void openDetailTable(boolean isMasterAdd)
  {
    ...
    int c=dsDetailTable.getRowCount();
    int d=dsDetailTable.getRowCount();
  }
  ...
  class Add_Approve implements Obactioner
  {
    ...
    {
      dsMasterTable.goToRow(Integer.parseInt(data.getParameter("rownum")));
//      String billType = dsMasterTable.getValue("1");
 ...
 class Master_Post implements Obactioner
 {
   public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
     int a=d_RowInfos.size();
      putDetailInfo(data.getRequest());

      EngineDataSet ds = getMaterTable();
      RowMap rowInfo = getMasterRowinfo();
      String asldfj=rowInfo.get("khlx");
      //校验表单数据
    ...
    private String checkDetailInfo()
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        String gyszyh = detailrow.get("gyszyh");

        StringBuffer buf = new StringBuffer().append(cpid).append(",").append(dmsxid).append(",").append(hthwid).append(",").append(gyszyh);
  ...
  class Complete implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0;i<dsDetailTable.getRowCount();i++)
      {
        ...
        {
          data.setMessage(showJavaScript("alert('还有款项未付完,不能完成!')"));
          return;
        }
        dsDetailTable.next();
      }
...
*选择产品触发事件
*/
class Cp_Onchange implements Obactioner
{
 public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
 {
   HttpServletRequest req = data.getRequest();
   ...
   RowMap ProductRow = getProductBean(req).getLookupRow(cpid);
   String dmsxid = ProductRow.get("dmsxid");
   rowInfo.put("cpid",cpid);
   rowInfo.put("dmsxid",dmsxid);
 }
 }
  /**
    * 从表增加操作
