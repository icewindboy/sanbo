...
/**
 * 2004-6-19 16:10 新增 客户名称 用于给不同的客户定制程序.现在第一次新增是因为essen有关排序的要求 yjg 2004-4-30
 * 12:52 新增 以改正:复制功能复制的时候,除页面上最后一笔以外的数据都不能保留在复制前输入的数量(还未保存)值. yjg 2004-4-29
 * 16:59 修改 如果dmsxidArray尺寸小于2那么就continue因为在size<2的情况下dmsxidArray[1]会出错.
 * size<2默认为编码不合格.同时cpids也要删除刚刚做此操作cpids.add(cpid);add进来的这笔cpid. 2004-4-29 10:35
 * 修改 判断是不是一个要分到一组中的条件是:主从表的cpbm,宽度*长度相等 yjg 2004-4-23 21:36 修改 销售出库单经手人默认为空 yjg
 * 2004-4-21 17:45 修改 引提单的时候不把部门引过来. yjg 2004-4-17 17:48 新增 记帐功能 yjg 2004-4-13
 * 16:54 新增 用于添加从表信息的SQL，根据条件和汇总表得到信息.提供给盘点机使用. yjg 2004-3-31 14:48 修改
 * 为防止排序出错而改成多加一句put("InternalRow")这样的.以及相关改动 yjg 2004-3-31 12:28 修改
 * 重新将原来单价,金额的注释去掉,起用单价及金额.同时加上根据系统参数决定是否将提单单价也引过来 yjg 2004-3-31 10:47 修改
 * 根据系统参数将提单单价也引过来 yjg 2004-3-30 14:53 新增 如果是保存按钮行为完成后定位数据集 yjg 2004-3-29 23:08
 * 新增 判断当是保存按钮的时候改变isMasterAdd为false.
 * 因为现在要求的保存行为是:在最初进来新增的那笔资料的基础上做修改的操作,而不是再新增一笔.yjg 2004-3-27 11:46 修改
 * 注释掉下面这个返回bachList()js函数的功能.以实现保存返回功能改成保存功能的要求. yjg 2004-3-27 10:24 新增
 * 新增删除空白行功能 yjg 2004-3-27 10:24 修改 去掉当复制当前行的时候连同当前行的sl,hssl也复制过来的行为. yjg 03:26
 * 21:38 新增 当配批号的时候是否引库存量过来.1=以库存数量为准,0=没有变化 yjg 03:26 21:15 新增
 * 当配批号的时候也要把配批号页面上的库存量引过来 yjg 03:26 15:25 新增 当引入提单时同时也把结算方式带过来给销售出库单主表.
 * VW_SALEOUTSTORE_SINGLE_LADDING也加入了jsfsid yjg 03:23 17:07 修改
 * 修改下面这条sql.将原来的nvl(b.sl,0))>nvl(b.stsl,0)条件加上取绝对值的函数. yjg 03.16 16:03
 * 加入部门不能为空的判断. yjg 03.16 15:25 修改 在Master_Search中去掉判断旧的SQL和新产生的SQL是否相同的if语句.
 * yjg 03.16 15:04 修改
 * 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
 * 以实现:mantis上库存管理中0000158bug描述的:根据下达的部门，进行提交审批；
 */

public final class OtherSaleOutStore extends BaseAction implements Operate {
	public static final String DETAIL_LADINGGOODS_ADD = "10801";
	public static final String SINGLELADDING_DETAIL_LADINGGOODS_ADD = "10802";
	...
	public static final String REPORT = "2000";// 02.23 11:26 新增
												// 为配合小李的报表追踪调用事件而加 yjg
	...
	private static String MASTER_SQL = "SELECT * FROM kc_other_sfdj WHERE dwtxid=? AND djxz=? and fgsid=? ? ORDER BY sfdjdh DESC";
	private static String MASTER_EDIT_SQL = "select * from kc_other_sfdj where sfdjid = '?' ";
	...
	private static final String TOTALHSSL_DETAIL_SQL = "SELECT SUM(hssl) sumHssl FROM kc_other_sfdjmx WHERE sfdjid='?'";//
	// 用于审批时候提取一条记录
	...
	private static final String STOCK_MATERIAL_LIST = "SELECT b.cpid, b.dmsxid, b.ph, SUM(nvl(b.zl,0)) kcsl, SUM(nvl(b.hszl,0)) hszl " + "FROM   kc_wzmx b, vw_kc_dm_exist a "
			+ "WHERE  b.cpid = a.cpid @ AND b.storeid = @ " + "GROUP BY b.storeid, b.cpid, b.dmsxid, b.ph";
	private static final String SINGLELADDING_DETAIL_SQL = "SELECT * FROM VW_SALE_OUTSTORE WHERE tdhwid in ( ? ) ";//
	...
	public HtmlTableProducer masterProducer = new HtmlTableProducer(dsMasterTable, "kc_other_sfdj.2");
	public HtmlTableProducer masterListProducer = new HtmlTableProducer(dsMasterList, "kc_other_sfdj.2");
	public HtmlTableProducer detailProducer = new HtmlTableProducer(dsDetailTable, "kc_other_sfdjmx.2");

	private EngineDataSet dsBillWrapperMaster = new EngineDataSet();// 2004-4-2
																	// 21:23 新增
																	// 专门为套打的主表数据集.
	private EngineDataSet dsBillWrapperDetail = new EngineDataSet();// 2004-4-2
																	// 21:23 新增
																	// 专门为套打的从表数据集.
	...
	private ImportSaleOrderGoods saleLadingBean = null; // 销售提单货物的bean的引用,
														// 用于提取销售提单货物信息
	...
	private String sfdjid = "";
	private String qtyFormat = null, priceFormat = null, sumFormat = null;
	private String fgsid = null; // 分公司ID
	private double childcount = 0; // 判断仓库是否含有库位
	private OtherUser user = null; // 定义一个存放用户信息的实例
	private static boolean isKC_OUT_STORE_STYLE = false;// 03:26 21:38 新增
														// 当配批号的时候是否引库存量过来.1=以库存数量为准,0=没有变化
														// yjg
	public static String KC_OUT_SHOW_PRICE = "1";// 销售出库是否显示单价金额.1=显示,0=不显示
	 ...
	 * @return 返回入库单列表的实例
	 */
	public static OtherSaleOutStore getInstance(HttpServletRequest request) {
		OtherSaleOutStore saleOutStoreBean = null;
		HttpSession session = request.getSession(true);
		synchronized (session) {
			String beanName = "otherSaleOutStore";
			saleOutStoreBean = (OtherSaleOutStore) session.getAttribute(beanName);
			if (saleOutStoreBean == null) {
				// 引用LoginBean
...
//				KC_OUT_SHOW_PRICE = loginBean.getSystemParam("KC_OUT_SHOW_PRICE");// 销售出库单1=显示单价,0=不显示单价
				...
				saleOutStoreBean = new OtherSaleOutStore();
				saleOutStoreBean.qtyFormat = loginBean.getQtyFormat();
				saleOutStoreBean.priceFormat = loginBean.getPriceFormat();
				saleOutStoreBean.sumFormat = loginBean.getSumFormat();

				saleOutStoreBean.fgsid = loginBean.getFirstDeptID();
				saleOutStoreBean.loginId = loginBean.getUserID();
				saleOutStoreBean.loginName = loginBean.getUserName();
//				saleOutStoreBean.loginDept = loginBean.getDeptID();
				saleOutStoreBean.user = loginBean.getUser();
				saleOutStoreBean.widthSyskey = loginBean.getSystemParam("SYS_PRODUCT_SPEC_PROP");
				saleOutStoreBean.lengthSyskey = loginBean.getSystemParam("SYS_PRODUCT_SPEC_LENGTH");
				// 设置格式化的字段
				saleOutStoreBean.dsDetailTable.setColumnFormat("sl", saleOutStoreBean.qtyFormat);
				saleOutStoreBean.dsDetailTable.setColumnFormat("hssl", saleOutStoreBean.qtyFormat);
				saleOutStoreBean.dsDetailTable.setColumnFormat("dj", saleOutStoreBean.priceFormat);
				saleOutStoreBean.dsDetailTable.setColumnFormat("je", saleOutStoreBean.sumFormat);
				saleOutStoreBean.dsMasterTable.setColumnFormat("zsl", saleOutStoreBean.qtyFormat);
				// saleOutStoreBean.dsMasterTable.setColumnFormat("zje",
				// saleOutStoreBean.qtyFormat);
				if (SYS_CUST_NAME.equals("essen"))
					MASTER_SQL = "SELECT * FROM kc_other_sfdj WHERE dwtxid=? AND djxz=? and fgsid=? ? ORDER BY sfdjdh DESC, sfrq DESC";
				session.setAttribute(beanName, saleOutStoreBean);
			}
		}
		return saleOutStoreBean;
	}

	/**
	 * 构造函数
	 */
	private OtherSaleOutStore() {
		try {
			jbInit();
		} catch (Exception ex) {
			log.error("jbInit", ex);
	...
	private final void jbInit() throws java.lang.Exception {
		...
		dsMasterTable.setSequence(new SequenceDescriptor(new String[] { "sfdjdh" }, new String[] { "SELECT pck_base.billNextCode('kc_other_sfdj','sfdjdh','b') from dual" }));
		...
		addObactioner(String.valueOf(SINGLELADDING_DETAIL_LADINGGOODS_ADD), new SingleLadding_Detail_LadingGoods_Add());
		...
		addObactioner(SHOW_DETAIL, new Show_Detail());// 02.17 新增
														// 新增查看从表明细资料事件发生时的触发操作类.
														// yjg
		addObactioner(String.valueOf(REPORT), new Approve());// 2.14 新增报表追此事件
																// yjg
	...
	private final void putDetailInfo(HttpServletRequest request) {
		...
		for (int i = min; i <= max; i++) {
			...
			// detailRow.put("wjid", rowInfo.get("wjid_"+i));//
			...
			detailRow.put("wzmxid", rowInfo.get("wzmxid_" + i));//
			// 2004-3-31 12:28 修改
			// 重新将原来单价,金额的注释去掉,起用单价及金额.同时加上根据系统参数决定是否将提单单价也引过来 yjg
	...
	public final void openDetailTable() {
		// String sfdjid = dsMasterList.getValue("sfdjid");
		// 02.28 23:15 修改
	...
	class Show_Detail implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			sfdjid = dsMasterList.getValue("sfdjid");
			// 打开从表
			openDetailTable();
		}
	}
	...
	class Master_Add_Edit implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			if (!isMasterAdd) {
				...
				masterRow = dsMasterList.getInternalRow();
				sfdjid = dsMasterList.getValue("sfdjid");
				String SQL = combineSQL(MASTER_EDIT_SQL, "?", new String[] { sfdjid });
	...
	class transferScan implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int out = 0; out < total; out++) {
				...
				for (int j = 0; j < 10; j++) {
					...
					String cpbm = cpbms[i].length() > 6 ? cpbms[i].substring(0, 7) : cpbms[i];// 2004-4-2
																								// 23:12
																								// 修改
																								// 暂时修改取产品编码前七位
																								// yjg
					...
					if (isNew) {
						...
						// 2004-4-29 16:59 修改
						// 如果dmsxidArray尺寸小于2那么就continue因为在size<2的情况下dmsxidArray[1]会出错.
						// size<2默认为编码不合格
			...
			for (i = 0; i < cpids.size(); i++) {
				...
				for (int m = 0; m < tmpcpids.size(); m++) {
					...
					// 用dsDetailTable中每笔记录的sxz中的宽度去由扫描机扫进来的dmsxid而得到的sxz宽度indexof
						 ...
						 * tempDsDetailViewTable.refresh(); else
						 * tempDsDetailViewTable.open();
						 */
						...
						for (int f = 0; f < tempDsDetailViewTable.getRowCount(); f++) {
							...
							if (tmpDetailCpid.equals(cpid)) {
								...
								if (scanSxz.indexOf(detailSxz) < 0) {
									if (rejectScanDataMessage.length() <= 0)
										rejectScanDataMessage.append("以下盘点机读进来的数据\n因其品名规格和规格属性\n在本单据没有找到与之对应的数据,未能取用\n");
							 ...
							 * //如果cpid能定位到那么考虑尝试sxz中的宽度是否匹配. if
							 * (dsDetailTable.locate(locateCpidGoodsRow,
							 * Locate.FIRST)) {
							 * //循环取出dsDetailTable中每笔的dmsxid然后得到sxz
							 * ,用这个sxz去当前盘点机扫进来的这个dmsxid的sxz去indexof
							 * //一旦找到一笔就.不再找下面的了 }
							 */
						}
					}/*
					 * else //如果没有定位到那么.记录下来此笔读进来资料的cpid, dmsxid { if (
					 * rejectScanDataMessage.length()<=0 )
					 * rejectScanDataMessage.
					 * append("以下盘点机读进来的数据\n因其品名规格和规格属性\n在本单据没有找到与之对应的数据,未能取用\n"
					 * ); rejectScanDataMessage.append(cpbm+dmsxid+ph+"\n"); }
					 */
				}
		...
		public String getDmsxId(String cpid, String cpbm) throws Exception {
			String tempGgSx = cpbm.length() > 7 ? cpbm.substring(7) : cpbm;// 2004-4-2
																			// 23:12
																			// 修改
																			// 暂时修改取产品编码前七位
																			// yjg
	...
	/**
	 * 提交仓库 class Store_Onchange implements Obactioner { public void
	 * execute(String action, Obationable o, RunData data, Object arg) throws
	 * Exception { HttpServletRequest request = data.getRequest();
	...
	class Add_Approve implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			dsMasterList.goToRow(Integer.parseInt(data.getParameter("rownum")));
			String sfdjid = dsMasterList.getValue("sfdjid");
			String zt = dataSetProvider.getSequence("SELECT a.zt FROM kc_other_sfdj a WHERE a.sfdjid='" + sfdjid + "'");
			if (zt != null && !zt.equals("0")) {
				dsMasterList.readyRefresh();
				return;
			}
			ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
			String content = dsMasterList.getValue("sfdjdh");
			String deptid = dsMasterList.getValue("deptid");
			// 03.16 15:04 修改
			// 将下面的approve.putAproveList()方法再新增一个传入参数.:dsMasterTable.getValue("deptid").
			// 以实现:mantis上库存管理中0000158bug描述的:根据下达的部门，进行提交审批；
			approve.putAproveList(dsMasterList, dsMasterList.getRow(), "outputlist", content, deptid);
		}
	}
	...
	class Cancel_Approve implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			ApproveFacade approve = ApproveFacade.getInstance(data.getRequest());
			approve.cancelAprove(dsMasterList, dsMasterList.getRow(), "outputlist");
			dsMasterList.readyRefresh();
		}
	}
	...
	class Master_Post implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int i = 0; i < detail.getRowCount(); i++) {
				detailrow = (RowMap) d_RowInfos.get(i);
				// 2004-3-31 14:48 修改 为防止排序出错而改成多加一句put("InternalRow")这样的.以及相关改动
				// yjg
				...
				double dj = 0.0;
				// if (KC_OUT_SHOW_PRICE.equals("1")) //1=显示,0=不显示
				dj = detailrow.get("dj").length() > 0 ? Double.parseDouble(detailrow.get("dj")) : 0;// 单价
				...
				// detail.setValue("hssl", String.valueOf(hsbl==0 ? 0 :
				// sl/hsbl));//保存换算数量
				// else
				detail.setValue("hssl", String.valueOf(hssl));// 保存换算数量
				// 2004-3-31 12:28 修改
				// 紧接着的下面两行去掉注释.目地是为:重新将原来单价,金额的注释去掉,起用单价及金额.同时加上根据系统参数决定是否将提单单价也引过来
				// yjg
				// if (KC_OUT_SHOW_PRICE.equals("1")) //1=显示,0=不显示
				// {
				...
				detail.setValue("wzmxid", detailrow.get("wzmxid"));// 备注
			...
			if (KC_OUT_SHOW_PRICE.equals("1")) // 1=显示,0=不显示
			{
				ds.setValue("zje", String.valueOf(totalSum));// 金额
			}
			...
			// 2004-3-29 23:08 新增 判断当是保存按钮的时候改变isMasterAdd为false.入下面的
...
//				return showJavaScript("alert('请选择部门！');");
	...
	class Master_Search implements Obactioner {
		...
		private void initQueryItem(HttpServletRequest request) {
			...
			fixedQuery.addShowColumn("",
					new QueryColumn[] { new QueryColumn(master.getColumn("sfdjdh"), null, null, null), new QueryColumn(master.getColumn("khlx"), null, null, null),
							...
							new QueryColumn(master.getColumn("sfdjid"), "kc_other_sfdjmx", "sfdjid", "cpid", null, "="),// 从表产品
							...
							new QueryColumn(master.getColumn("zt"), null, null, null, null, "="), new QueryColumn(master.getColumn("jsr"), null, null, null, null, "like") });
			isInitQuery = true;
		}
	}
	/**
	 ...
	 * 从表增加操作
	 * 
	 * class Detail_Add implements Obactioner { public void execute(String
	 * action, Obationable o, RunData data, Object arg) throws Exception {
	 * HttpServletRequest req = data.getRequest(); //保存输入的明细信息
	 * putDetailInfo(data.getRequest());
	 * 
	 * String multiIdInput = m_RowInfo.get("multiIdInput");
	 * if(multiIdInput.length() == 0) return;
	 * 
	 * //实例化查找数据集的类 EngineRow locateGoodsRow = new EngineRow(dsDetailTable,
	 * "cpid"); if(!isMasterAdd) dsMasterTable.goToInternalRow(masterRow);
	 * String sfdjid = dsMasterTable.getValue("sfdjid"); String[] cpID =
	 * parseString(multiIdInput,","); for(int i=0; i < cpID.length; i++) {
	 * if(cpID[i].equals("-1")) continue; RowMap detailrow = null;
	 * locateGoodsRow.setValue(0, cpID[i]);
	 * if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST)) {
	 * dsDetailTable.insertRow(false); dsDetailTable.setValue("rkdmxid", "-1");
	 * dsDetailTable.setValue("cpid", cpID[i]); dsDetailTable.setValue("sfdjid",
	 * isMasterAdd ? "" : sfdjid); dsDetailTable.post(); //创建一个与用户相对应的行
	 * detailrow = new RowMap(dsDetailTable); d_RowInfos.add(detailrow); } } } }
	 */
	/**
	 * 从表引入销售提单货物操作
	 */
		...
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int i = 0; i < tddhwIDs.length; i++) {
				...
				if (!dsDetailTable.locate(locateGoodsRow, Locate.FIRST)) {
					...
					// double djlx = saleLadingRow.get("djlx").length() > 0 ?
					// Double.parseDouble(saleLadingRow.get("djlx")) : 0;
					String djlx = saleLadingRow.get("djlx");
					double jje = saleLadingRow.get("jje").length() > 0 ? Double.parseDouble(saleLadingRow.get("jje")) : 0;
					jje = djlx.equals("-1") ? -1 * jje : jje;
					// djlx=-1表示退库
					double wckhssl = djlx.equals("-1") ? -1 * sjhssl : sjhssl;
					double wcksl = djlx.equals("-1") ? -1 * sjsl : sjsl;
					dsDetailTable.insertRow(false);
					...
					dsDetailTable.setValue("je", formatNumber(String.valueOf(jje), sumFormat));
					// }
					dsDetailTable.post();
					// 创建一个与用户相对应的行
					detailrow = new RowMap(dsDetailTable);
	...
	/**
	 * 引销售提单弹出页面的下方框架页面从表引入销售提单货物操作
	 */
	class SingleLadding_Detail_LadingGoods_Add implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			HttpServletRequest req = data.getRequest();
			// 保存输入的明细信息
			putDetailInfo(data.getRequest());
			RowMap rowinfo = getMasterRowinfo();
			String signleimportLadingGoods = m_RowInfo.get("signleimportLadingGoods");

			// 实例化查找数据集的类
			EngineDataSet dsSingleLaddingDetail = new EngineDataSet();// 物资明细表的数据集

			String SQL = combineSQL(SINGLELADDING_DETAIL_SQL, "?", new String[] { signleimportLadingGoods });
			setDataSetProperty(dsSingleLaddingDetail, SQL);
			dsSingleLaddingDetail.open();
			if (dsSingleLaddingDetail.getRowCount() < 1)
				return;
			dsSingleLaddingDetail.first();
			// RowMap singleLaddingRow =
			// getSingleLaddingBean(req).getLookupRow(singleSelectLadding);//采购合同主表一行信息
			rowinfo.put("storeid", dsSingleLaddingDetail.getValue("storeid"));// 仓库
			rowinfo.put("deptid", dsSingleLaddingDetail.getValue("deptid"));// 仓库
			rowinfo.put("dwtxid", dsSingleLaddingDetail.getValue("dwtxid"));
			// 2004-4-22 10:45 新增 当引入提单时同时也把客户类型带过来给销售出库单主表 yjg
			rowinfo.put("khlx", dsSingleLaddingDetail.getValue("khlx"));
			...
			rowinfo.put("jsfsid", dsSingleLaddingDetail.getValue("jsfsid"));
			String djlx = dsSingleLaddingDetail.getValue("djlx");
			// 实例化查找数据集的类
			EngineRow locateGoodsRow = new EngineRow(dsDetailTable, "wjid");
			if (!isMasterAdd)
				dsMasterTable.goToInternalRow(masterRow);
			String sfdjid = dsMasterTable.getValue("sfdjid");
			for (int i = 0; i < dsSingleLaddingDetail.getRowCount(); i++) {
				// laddingGoodsData.goToRow(i);
				String tdhwid = dsSingleLaddingDetail.getValue("tdhwid");
				locateGoodsRow.setValue(0, tdhwid);
				if (!dsDetailTable.locate(locateGoodsRow, Locate.FIRST)) {
					double sjhssl = dsSingleLaddingDetail.getValue("sjhssl").length() > 0 ? Double.parseDouble(dsSingleLaddingDetail.getValue("sjhssl")) : 0;// 实际换算数量
					double sjsl = dsSingleLaddingDetail.getValue("sjsl").length() > 0 ? Double.parseDouble(dsSingleLaddingDetail.getValue("sjsl")) : 0;// 实际数量
					djlx = dsSingleLaddingDetail.getValue("djlx");
					double jje = dsSingleLaddingDetail.getValue("jje").length() > 0 ? Double.parseDouble(dsSingleLaddingDetail.getValue("jje")) : 0;
					jje = djlx.equals("-1") ? -1 * jje : jje;
					// djlx=-1表示退库
					double wckhssl = djlx.equals("-1") ? -1 * sjhssl : sjhssl;
					double wcksl = djlx.equals("-1") ? -1 * sjsl : sjsl;
					...
					dsDetailTable.setValue("sfdjid", isMasterAdd ? "-1" : sfdjid);
					dsDetailTable.setValue("wjid", tdhwid);
					dsDetailTable.setValue("cpid", dsSingleLaddingDetail.getValue("cpid"));
					dsDetailTable.setValue("sl", formatNumber(String.valueOf(wcksl), qtyFormat));
					dsDetailTable.setValue("hssl", formatNumber(String.valueOf(wckhssl), qtyFormat));
					dsDetailTable.setValue("bz", dsSingleLaddingDetail.getValue("bz"));
					dsDetailTable.setValue("dmsxid", dsSingleLaddingDetail.getValue("dmsxid"));
					// dsMasterTable.setValue("dwtxid",
					// dsSingleLaddingDetail.getValue("dwtxid"));
					// 2004-3-31 10:47 新增 根据系统参数将提单单价也引过来 yjg
					// if (KC_OUT_SHOW_PRICE.equals("1")) //1=显示,0=不显示
					// {
					dsDetailTable.setValue("dj", dsSingleLaddingDetail.getValue("dj"));
					dsDetailTable.setValue("je", formatNumber(String.valueOf(jje), sumFormat));
					// }
					dsDetailTable.post();
					// 创建一个与用户相对应的行
	...
	class Single_Ladding implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			String djlx = laddingGoodsData.getValue("djlx");
			// 实例化查找数据集的类
			EngineRow locateGoodsRow = new EngineRow(dsDetailTable, "wjid");
			if (!isMasterAdd)
				dsMasterTable.goToInternalRow(masterRow);
			...
			for (int i = 0; i < laddingGoodsData.getRowCount(); i++) {
				...
				if (!dsDetailTable.locate(locateGoodsRow, Locate.FIRST)) {
					...
					// double djlx = laddingGoodsData.getValue("djlx").length()
					// > 0 ?
					// Double.parseDouble(laddingGoodsData.getValue("djlx")) :
					// 0;//实提数量
					double jje = laddingGoodsData.getValue("jje").length() > 0 ? Double.parseDouble(laddingGoodsData.getValue("jje")) : 0;
					double wthsl = djlx.equals("-1") ? -1 * (hssl - sthssl) : hssl - sthssl;// 未提换算数量.-1表:销售退货单
					double wtsl = djlx.equals("-1") ? -1 * (sl - stsl) : sl - stsl;// 未提数量
					jje = djlx.equals("-1") ? -1 * jje : jje;
					if (wtsl == 0)
						continue;
					dsDetailTable.insertRow(false);
	...
	class Copy_CurrentRow implements Obactioner {
		/**
		 * 取得网页上明细资料表格中当前行的资料,然后再复制指定数量的此行的资料.加入到网页数据集中,显示出来在网页上. 1.
		 * 从request中得到页面上当前光标所停留在的行数是下方明细表格中的那一行.设为i行. 2.
		 * 接着要从保存网页资料的RowMap集中取出此i行的数据. 3. 将此i行的数据加入到保存网页资料的RowMap集中,连续加入指定数量次.
		 */
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			// 取得网页上光标所停留在的那一行的行数.
			...
			for (int i = 0; i < d_RowInfos.size(); i++) {
				...
				// 2004-4-30 12:52 新增
				// 以改正:复制功能复制的时候,除页面上最后一笔以外的数据都不能保留在复制前输入的数量(还未保存)值. yjg
				detail.goToInternalRow(swapdetailRow);
				// double hsbl = rowinfo.get("hsbl").length() > 0 ?
				// Double.parseDouble(rowinfo.get("hsbl")) : 0;//换算比例
				// 2004-3-31 12:28 修改
				// 重新将原来单价,金额的注释去掉,起用单价及金额.同时加上根据系统参数决定是否将提单单价也引过来 yjg
				double dj = 0.0;
				if (KC_OUT_SHOW_PRICE.equals("1")) // 1=显示,0=不显示
	...
	/**
	 * 2004-3-27 10:24 新增 新增删除空白行功能 yjg 删除数量为空白的操作
	 */
	class Delete_Blank implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			for (int i = 0; i < d_RowInfos.size(); i++) {
				...
				detail.goToInternalRow(internalRow);
				String sl = detailrow.get("sl");
				// 02.25 11:14 修改 修改删除一行库存量为空的一笔记录的条件改为 电脑库存数量, 实际库存数量全部都是零的才删除
				// yjg
	...
	class Recode_Account implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			if (count.equals("0")) {
				...
				dsMasterList.updateQuery(new String[] { UPDATE_SQL });
				dsMasterList.readyRefresh();
				dsMasterList.refresh();
			}
		}
	}

	/**
	 * 套打 提供套打数据 主从表的数据集分别提供,放入在RowMap中
	 */
	class Wrapper_Print implements Obactioner {
		public void execute(String action, Obationable o, RunData data, Object arg) throws Exception {
			...
			// int cureentPageNo = 0;
			/*
			 * 1.mrows中保存的是从sql语句中查询出来原始的主要是由cpbm,dmsxid分组的记录. 2.但是可能会有cpbm,
			 * 长度*宽度 也相同的记录.因为是用cpbm,dmsxid分组,而不是cpbm ,sxz分组
			 * 3.而打印的本意是:cpbm,sxz=长度*宽度相同的统一归到一笔主表记录下面.所以,主表中记录要求: 3.1
			 * cpbm,sxz=长度*宽度组合的数据在主表数据集中不允许有相同的
			...
			/*
			 * RowMap[] mastRows = (RowMap[])mrows.values().toArray(new
			 * RowMap[mrows.size()]); for (int j=0; j<mastRows.length;j++) {
			 * //用来记录每一个与主表数据集的一笔数据相对应的明细资料分组中被包含进去的明细资料的记录个数. int
			 * perGroupNumber = 0; ArrayList dprintrow = new ArrayList(); RowMap
			 * tempMrm = mastRows[j]; String cpbm = tempMrm.get("cpbm"); String
			 * dmsxid = tempMrm.get("dmsxid"); String mgg = tempMrm.get("gg");
			 * //ArrayList dArray = new ArrayList(); int m = 0; RowMap rtmp =
			 * new RowMap(); BigDecimal alBtotal = new BigDecimal(0); for (int
			 * l=0;l<al.size();l++) { //对从表 RowMap tempDrm = (RowMap)al.get(l);
			 * String dcpbm = tempDrm.get("cpbm"); String ddmsxid =
			 * tempDrm.get("dmsxid"); String dgg = tempDrm.get("gg"); String ph
			 * = tempDrm.get("ph"); String sl = tempDrm.get("sl"); String hssl =
			 * tempDrm.get("hssl"); //2004-4-29 10:35 修改
			 * 判断是不是一个要分到一组中的条件是:主从表的cpbm,宽度*长度相等 yjg if (dcpbm.equals(cpbm) &&
			 * dgg.equals(mgg)) { perGroupNumber++; rtmp.put("ph"+m,ph);
			 * rtmp.put("sl"+m,sl); rtmp.put("hssl"+m,hssl); //对cpbm,
			 * dmsxid相同的数据(即:打印出来的时候会打印在同一张纸上)合计sl alBtotal = alBtotal.add(new
			 * BigDecimal(sl.equals("")?"0":sl)); m=m+1; if(m==3) { //从表的记录数多于3行
			 * m=0; dprintrow.add(rtmp); rtmp = new RowMap(); }
			 * //dArray.add(tempDrm);//取出子表对应主表一行的数据. } } //形成 合计:xxx.xx供打印
			 * tempMrm.put("total", "合计:"+alBtotal.toString()); if(m<3)
			 * dprintrow.add(rtmp);// //如果dprintrow的大小大于一页9行的数量.那么就要分页来打印了.
			 * //里面记录了.属于同一个明细资料长*宽分组的明细数据. //循环几次.即按每组9行.要分拆成几组 if
			 * (dprintrow.size()>9) { int total = dprintrow.size()/9 +
			 * (dprintrow.size()%9 > 0 ? 1 : 0); //tempMrm.put("pageNo", "第" +
			 * String.valueOf(j+1)+ "页" + "/"+ "共" +
			 * String.valueOf(mrows.size()+total-1) + "页"); for (int
			 * i=0;i<total;i++) { //确定这一页会有几条记录 int tempSize =
			 * dprintrow.size()-(i*9)>9?9:dprintrow.size()-(i*9); RowMap[] drs=
			 * new RowMap[tempSize]; //取从表打印数据数据集中的第几笔到第几笔组成一页. String sl0 =
			 * "0"; String sl1 = "0"; String sl2 = "0"; String hssl0 = "0";
			 * String hssl1 = "0"; String hssl2 = "0"; boolean isPaper =
			 * tempMrm.get("isPaper").equals("true"); tempMrm.put("total", "0");
			 * tempMrm.put("hssl", ""); BigDecimal btotal = new BigDecimal(0);
			 * BigDecimal hsslBtotal = new BigDecimal(0); for (int v=i*9,w=0;
			 * v<(i+1)*9&&v<dprintrow.size();v++,w++) { RowMap rp =
			 * (RowMap)dprintrow.get(v); drs[w] = (RowMap)dprintrow.get(v); sl0
			 * = rp.get("sl0").equals("")?"0":rp.get("sl0"); sl1 =
			 * rp.get("sl1").equals("")?"0":rp.get("sl1"); sl2 =
			 * rp.get("sl2").equals("")?"0":rp.get("sl2"); //是平装纸则求它的每页面的换算数量页数
			 * if (isPaper) { hssl0 =
			 * rp.get("hssl0").equals("")?"0":rp.get("hssl0"); hssl1 =
			 * rp.get("hssl1").equals("")?"0":rp.get("hssl1"); hssl2 =
			 * rp.get("hssl2").equals("")?"0":rp.get("hssl2"); //求每页换算数量和数量的和
			 * hsslBtotal=hsslBtotal.add(new
			 * BigDecimal(hssl0.equals("")?"0":hssl0).add(new
			 * BigDecimal(hssl1.equals("")?"0":hssl1).add(new
			 * BigDecimal(hssl2.equals("")?"0":hssl2)))); }
			 * btotal=btotal.add(new BigDecimal(sl0.equals("")?"0":sl0).add(new
			 * BigDecimal(sl1.equals("")?"0":sl1).add(new
			 * BigDecimal(sl2.equals("")?"0":sl2))));
			 * 
			 * //float tempTotal=
			 * tempMrm.get("total").equals("")?0:Float.parseFloat
			 * (tempMrm.get("total")); //float tempSl = Float.parseFloat(sl0) +
			 * Float.parseFloat(sl1) + Float.parseFloat(sl2);
			 ...
			 * totalPageNos++; //同一组的明细资料分成几页,那么主表也要相应的有几个主表资料头 RowMap
			 * tempNewMrm = (RowMap)tempMrm.clone();
			 * mprint.add(tempNewMrm);//放的是RowMap;
			 * dprint.add(drs);//放的是arrayList } } else { RowMap[] drs= new
			 * RowMap[dprintrow.size()]; for(int q=0;q<dprintrow.size();q++) {
			 * RowMap rp =(RowMap)dprintrow.get(q); drs[q] =
			 * (RowMap)dprintrow.get(q); String sl = rp.get("sl0"); }
			 * totalPageNos++; mprint.add(tempMrm);//放的是RowMap
			 * dprint.add(drs);//放的是arrayList }
			 * //如果明细资料中按长度*宽度分组后被分出来的数据条数和本来sql抽取到的明细资料表数据条数一样
			 * .那说明只存在一种分组.只须打印一张. //不须要再对主表数据集mrows进行循环了. if ( perGroupNumber
			 * == al.size() ) break; }
			 */
			/*
			 * 对数据集进行循环,对货物按产品编码cpbm进行分组,相同的货物每三个一行, 不足三个的占一行.多余三个的分成几行
			 * dsBillWrapperDetail.first(); for(int
			 * i=0;i<dsBillWrapperDetail.getRowCount();i++) { String cpbm =
			 * dsBillWrapperDetail.getValue("cpbm"); if(contains.contains(cpbm))
			 * { //已经分组的货物不再分组 dsBillWrapperDetail.next(); continue; } rm = new
			 * RowMap(); int h=0; String count = dataSetProvider.getSequence(
			 * "SELECT count(*) from VW_OUTPUTLIST_DTL_BILL_WRAPPER where sfdjid='"
			 * +sfdjid+"'"); int totalcount =
			 * Integer.parseInt(count);//对应同类货物(不同规格属性
			 * )总数,有totalcount/3+Integer.parseInt(((totalcount%3)>0)?"1":"0")行
			 * for(int j=0;j<al.size();j++) { //al里包含了数据集里的数据 RowMap rt
			 * =(RowMap)al.get(j); String ph = rt.get("ph"); //String cpbm =
			 * rt.get("cpbm"); if(rt.get("cpbm").equals(cpbm)) { //提取出对应tdhwid
			 * String sl = rt.get("sl"); String sn = "s"+(h%3); rm.put(sn,sl);
			 * h=h+1; if(totalcount<3&&h==totalcount) { //总数小于3且循环到结束.
			 * rm.put("ph",ph); drows.add(rm);//循环完 contains.add(cpbm); } else
			 * if((h%3)==0) { // rm.put("ph",ph); drows.add(rm);
			 * contains.add(cpbm); } else if(totalcount>3&&h==totalcount) { //
			 * rm.put("ph",ph); drows.add(rm);//循环完 contains.add(cpbm); } }
			 * }//////// dsBillWrapperDetail.next(); }
			 */
		}
	 ...
	 * 
	 *         public final int getSelectedRow() { if(masterRow < 0) return -1;
	 *         dsMasterTable.goToInternalRow(masterRow); return
	 *         dsMasterTable.getRow(); }
	 */
	/**
	 * 得到选中的行的行数
	 * 
	...
	private String checkNumRigtt(int tempmin, int tempmax) throws Exception {
		...
		for (int i = tempmin; i <= tempmax; i++) {
			 ...
			 * showJavaScript("alert('第"+row+"行产品重复');"); else list.add(cpid);
			 */
			String sl = detailrow.get("sl");
			String hssl = detailrow.get("hssl");
			String dj = detailrow.get("dj");
			...
			// 新增 :如果cpid, ph, dmsxid的组合有相同的则是不允许的.盘点点中不允许有这样的情况. 02.14 21:19
			// yjg
