...
public final class B_StoreBill extends BaseAction implements Operate
{
  ...
  private final void putDetailInfo(HttpServletRequest request)
  {
    ...
    for(int i=0; i<rownum; i++)
    {
      ...
      String s= rowInfo.get("dhsl_"+i);
      detailRow.put("dhsl", rowInfo.get("dhsl_"+i));
      detailRow.put("gfhl", rowInfo.get("gfhl_"+i));//
      detailRow.put("gfph", rowInfo.get("gfph_"+i));
      detailRow.put("gfzbsl", rowInfo.get("gfzbsl_"+i));
  ...
  class Approve implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(isReport){
        ...
        isApprove = true;//审批操作
        isMasterAdd = false;//当是审批的时候则肯定不是新增的状态
   ...
   * 添加到审核列表的操作类
   */
  class Add_Approve implements Obactioner,ApproveListener
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      HttpServletRequest req = data.getRequest();
      ...
      approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "contract_instore_list", content, dsMasterTable.getValue("deptid"), this);
    }
    public void processApprove(ApproveResponse[] reponses) throws Exception
    {
      openDetailTable(false);
      prodBean.regData(dsDetailTable,"cpid");
      if(reponses==null||reponses.length==0)
        return;
      for(int i=0;i<reponses.length;i++)
      {
        if(reponses[i].getProjectFlowCode().equals("constract_instore_editapprove")){//含量检验
          dsDetailTable.first();
          String specialValue = reponses[i].getProjectFlowValue();
          for (int j=0;j<dsDetailTable.getRowCount();j++)
          {
            ...
            String chmc = prodRow.get("chmc");
            if(chmc.equals(specialValue))
              reponses[i].add();
            else
              reponses[i].skip();
            dsDetailTable.next();
  ...
  class Master_Post implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i<detail.getRowCount(); i++)
      {
        ...
        detail.setValue("dhsl", detailrow.get("dhsl"));//保存换算数量
        ...
        detail.setValue("gfhl", detailrow.get("gfhl"));//
        detail.setValue("gfph", detailrow.get("gfph"));
        detail.setValue("gfzbsl", detailrow.get("gfzbsl"));
    ...
    private String checkDetailInfo() throws Exception
    {
      ...
      for(int i=0; i<d_RowInfos.size(); i++)
      {
        ...
        String dhsl = detailrow.get("dhsl");
        //String dhsl = detailrow.get("dhsl");
        String gfhl = detailrow.get("gfhl");
        String gfph = detailrow.get("gfph");
        String gfzbsl = detailrow.get("gfzbsl");
        ...
        */
        if((temp = checkNumber(dhsl, "第"+row+"行到货数量")) != null)
          return temp;
        if(dhsl.length()>0 && dhsl.equals("0"))
          return showJavaScript("alert('第"+row+"行到货数量不能为零！');");

        if((temp = checkNumber(gfhl, "第"+row+"行供方含量")) != null)
          return temp;
        if(gfhl.length()>0 && gfhl.equals("0"))
          return showJavaScript("alert('第"+row+"行供方含量不能为零！');");

        if((temp = checkNumber(gfzbsl, "第"+row+"行供方折百数量")) != null)
          return temp;
        if(gfzbsl.length()>0 && gfzbsl.equals("0"))
          return showJavaScript("alert('第"+row+"行供方折百数量不能为零！');");

        /*if((temp = checkNumber(jyhl, "第"+row+"行数量")) != null)
          return temp;
        if(jyhl.length()>0 && jyhl.equals("0"))
          return showJavaScript("alert('第"+row+"行数量不能为零！');");

        if((temp = checkNumber(jyph, "第"+row+"行数量")) != null)
          return temp;
        if(jyph.length()>0 && jyph.equals("0"))
          return showJavaScript("alert('第"+row+"行数量不能为零！');");
        */
       ...
       //如果收货单明细中产品相同批号也相同，并且该产品设置为批号跟踪则不能保存。设置是否批号跟踪在物资信息中设置
        if(!ph.equals("") && isbatchno.equals("1")){
          if(list.contains(ph))
     ...
     for(int t=0; t<d_RowInfos.size(); t++)
     {
       ...
       if(!ph.equals("") && isbatchno.equals("1")){
         String count ="0";
         if(isMasterAdd)
           count  = dataSetProvider.getSequence("SELECT COUNT(*) FROM vw_kc_storebill t WHERE t.djxz IN(1,3,5,7,9) AND t.ph='"+ph+"' AND t.cpid='"+cpid+"'");
         else{
           String sfdjid = dsMasterTable.getValue("sfdjid");
           count  = dataSetProvider.getSequence("SELECT COUNT(*) FROM vw_kc_storebill t WHERE t.djxz IN(1,3,5,7,9) AND t.ph='"+ph+"' AND t.cpid='"+cpid+"' AND t.sfdjid<>'"+sfdjid+"'");
         }
         if(!count.equals("0"))
           return showJavaScript("alert('第"+row+"行批号"+ph+"在库存中已存在');");
       }
      }
      return null;
    }
  ...
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    for(int i=0; i < buyOrderGoodsData.getRowCount(); i++)
    {
      ...
      if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
      {
        ...
        dsDetailTable.setValue("cpid", cpid);
        dsDetailTable.setValue("hssl", formatNumber(String.valueOf(wrksl), qtyFormat));//hssl字段现在当成到货数量
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < jhdhwIDs.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          //dsDetailTable.setValue("sl", bjfs.equals("1") ? formatNumber(String.valueOf(hsbl==0 ? 0 : wrksl*hsbl), qtyFormat) : String.valueOf(wrksl));
          //hssl现在当成了到货数量来用
          dsDetailTable.setValue("hssl",formatNumber(wrksl, qtyFormat));
