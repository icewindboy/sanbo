 ...
 /*打开从表*/
  ...
  class Add_Approve implements Obactioner,ApproveListener
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      isout = dsMasterTable.getValue("isout");
      ...
      if(!isout.equals("1"))
        approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "self_gain_list", content, dsMasterTable.getValue("deptid"), this);
      else
        approve.putAproveList(dsMasterTable, dsMasterTable.getRow(), "process_instore_list", content, dsMasterTable.getValue("deptid"), this);
    }
    public void processApprove(ApproveResponse[] reponses) throws Exception
    {
      openDetailTable(false);
      prodBean.regData(dsDetailTable,"cpid");
      if(reponses==null||reponses.length==0||isout.equals("1"))
        return;
      for(int i=0;i<reponses.length;i++)
      {
        if(reponses[i].getProjectFlowCode().equals("self_gain_editapprove")){
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < processData.getRowCount(); i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          dsDetailTable.setValue("batchNo",processData.getValue("scph"));
