...
{
  private static final String IMPORT_MATERAIL_SQL = "SELECT * FROM VW_KC_WZMX WHERE zl<>0 and fgsid=? and (storeid is null or storeid=? ) ?  order by cpbm";
  ...
  * 定义固定查询类
  ...
  public  HtmlTableProducer searchTable = new HtmlTableProducer(dsMaterail, "kc_dm_select", "");//查询得到数据库中配置的字段
  /**
   * 得到库存物资信息的实例
   * @param request jsp请求
