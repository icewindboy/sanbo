...
/**
 * <p>Title: 库存管理-仓库产成品明细帐</p>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: ENGINE</p>
 * @version 1.0
 */
public final class B_StocksProd extends BaseAction implements Operate
{
  public static final String FIXED_SEARCH = "1009";
  private static final String KC_SRMX_SQL = "SELECT * FROM REP_PRODUCT_DETAIL WHERE 1=1 and  ? ";
  private static final String KC_CQ_SQL = "SELECT sum(nvl(sl,0))sl,sum(nvl(je,0))je FROM REP_PRODUCT_DETAIL  where 1=1 ? ";
   ...
   */
  public static B_StocksProd getInstance(HttpServletRequest request)
  {
    B_StocksProd B_StoreProdBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_StoreProdBean";//名称-值对应
      B_StoreProdBean = (B_StocksProd)session.getAttribute(beanName);
      if(B_StoreProdBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        String fgsID = loginBean.getFirstDeptID();
        B_StoreProdBean = new B_StocksProd(fgsID);//调用构造函数
        B_StoreProdBean.loginName = loginBean.getUserName();
        B_StoreProdBean.personid=loginBean.getUserID();
        session.setAttribute(beanName,B_StoreProdBean);
      }
    }
    return B_StoreProdBean;
  }
  /**
   * 构造函数(实例变量:分公司ID为初始化参数)
   */
  private B_StocksProd(String fgsid)
  {
    this.fgsid = fgsid;
    try {
      jbInit();
