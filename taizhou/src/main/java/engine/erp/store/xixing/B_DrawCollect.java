...
/**
 * <p>Title: 辅助材料发出汇总分配表</p>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: ENGINE</p>
 * @version 1.0
 */
public final class B_DrawCollect extends BaseAction implements Operate
{
  public static final String FIXED_SEARCH = "1009";
  private static final String KC_SRMX_SQL = "SELECT * FROM vw_draw_fc WHERE 1=1 and  ? ";
  private static final String KC_CQ_SQL = "SELECT sum(nvl(sl,0))sl,sum(nvl(je,0))je FROM vw_draw_fc  where 1=1 ? ";
  private static final String KC_DM_SQL =" select cpid,deptid from vw_draw_fc where 1=1 ? ";
   ...
   */
  public static B_DrawCollect getInstance(HttpServletRequest request)
  {
    B_DrawCollect B_DrawCollectBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "B_DrawCollectBean";//名称-值对应
      B_DrawCollectBean = (B_DrawCollect)session.getAttribute(beanName);
      if(B_DrawCollectBean == null)
      {
        LoginBean loginBean = LoginBean.getInstance(request);
        String fgsID = loginBean.getFirstDeptID();
        B_DrawCollectBean = new B_DrawCollect(fgsID);//调用构造函数
        B_DrawCollectBean.loginName = loginBean.getUserName();
        B_DrawCollectBean.personid=loginBean.getUserID();
        session.setAttribute(beanName,B_DrawCollectBean);
      }
    }
    return B_DrawCollectBean;
  }
  /**
   * 构造函数(实例变量:分公司ID为初始化参数)
   */
  private B_DrawCollect(String fgsid)
  {
    this.fgsid = fgsid;
    try {
      jbInit();
  ...
  {
    addObactioner(String.valueOf(INIT), new B_DrawCollect_Init());//初始化
   ...
   * 初始化操作的触发类
   */
  class B_DrawCollect_Init implements Obactioner
  {
  ...
  class FIXED_SEARCH implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      String cksql = combineSQL(KC_DM_SQL, "?", new String[]{" and djlb=-1 and sfrq>=to_date('"+ksrq+"','yyyy-mm-dd') and  sfrq<=to_date('"+jsrq+"','yyyy-mm-dd') and storeid='"+storeid+"' group by cpid,deptid"}) ;
      ...
      for(int i=0;i<tdscqck.getRowCount();i++)
      {
        ...
        String deptid = tdscqck.getValue("deptid");
        getData(cpid,ksrq,jsrq,storeid,deptid);
        tdscqck.next();
      }
    ...
    public void getData(String cpid,String ksrq,String jsrq,String storeid,String deptid)
    {
      ...
      ////////*************计算期初*********************/////////
        ...
        String tcksql = combineSQL(KC_CQ_SQL, "?", new String[]{"  and cpid="+cpid+"  and  storeid="+storeid+"  and sfrq<to_date('"+jsrq+"','yyyy-mm-dd') group by cpid"}) ;
      ...
      String srcsql = combineSQL(KC_SRMX_SQL, "?", new String[]{"   cpid="+cpid+" and djlb=-1 and deptid="+deptid+" and  storeid="+storeid+" and sfrq>=to_date('"+ksrq+"','yyyy-mm-dd') and  sfrq<=to_date('"+jsrq+"','yyyy-mm-dd')"}) ;
