...
public final class B_SelfGain extends BaseAction implements Operate
{
  ...
  public  static final String SELF_SEL_SHOP = "10802";//引入车间产能表接口常量
  ...
  private static final String MASTER_SQL    = "SELECT * FROM sc_receiveprod WHERE ? AND ? AND filialeID='?' AND isout='?' ? ORDER BY receivedate DESC,receiveCode DESC";
  private static final String DETAIL_STRUT_SQL = "SELECT * FROM sc_receiveproddetail WHERE 1<>1";
  private static final String DETAIL_SQL = "SELECT * FROM sc_receiveproddetail WHERE receiveid='?' ORDER BY jgdwlid,cpid";//

  //查询过后总数量
  private static final String MASTER_SUM_SQL    = "SELECT SUM(nvl(totalnum,0)) zsl FROM sc_receiveprod WHERE ? AND ? AND filialeid='?' AND isout='?' ?  ";
  ...
  private static final String SELF_SINGLE_SHOP_SQL = "SELECT * FROM VW_STOREPROCESS_SEL_SHOP WHERE cjcnid= ";//自制收货单引入车间产能表明细信息
  ...
  private final void jbInit() throws java.lang.Exception
  {
    ...
    addObactioner(String.valueOf(SELF_SEL_SHOP), new Self_Sel_Shop());//自制收货单引入车间产能表从表增加操作
 ...
 /**
 *  自制收货单从表增加操作
 *  显示的车间产能表必须是该加工单至少有一条明细还未入库完，并且存放仓库和所选仓库要相同的数据才引过来，空的也过来
 *  引入的数据来源于车间产能表主表
 *  单选加工单主表，通过主表得到这张车间产能表的信息并插入生产领料单从表
 */

class Self_Sel_Shop implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    HttpServletRequest req = data.getRequest();
    ...
    String importShop = rowinfo.get("importShop");
    if(importShop.equals(""))
      return;
    String SQL = SELF_SINGLE_SHOP_SQL + importShop;
    EngineDataSet processData = null;
    if(processData==null)
    {
      processData = new EngineDataSet();
    ...
    rowinfo.put("deptid", processData.getValue("deptid"));
    //实例化查找数据集的类
    EngineRow locateGoodsRow = new EngineRow(dsDetailTable, "cjcnmxid");
    if(!isMasterAdd)
      dsMasterTable.goToInternalRow(masterRow);
    String receiveID = dsMasterTable.getValue("receiveID");
    processData.first();
    ...
    {
      processData.goToRow(i);
      locateGoodsRow.setValue(0, processData.getValue("cjcnmxid"));
      if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
      {
        dsDetailTable.insertRow(false);
        dsDetailTable.setValue("receiveDetailID","-1");
        dsDetailTable.setValue("cjcnmxid",processData.getValue("cjcnmxid"));
        String cpid = processData.getValue("cpid");
        String drawnum = processData.getValue("drawnum");
        dsDetailTable.setValue("cpid",processData.getValue("cpid"));
