...
public final class B_StoreBill extends BaseAction implements Operate
{
  ...
  //通过进货单id得到进货单货物的数据 2004.11.30 改为未审批之前就更改
  //private static final String BUY_ORDERGOODS_SQL = "SELECT a.djlx, a.jsfsid, b.* FROM cg_htjhd a, cg_htjhdhw b WHERE a.jhdid = b.jhdid and b.jhdid= ";//cpid,b.hssl,b.sl,b.dj,b.je,b.bz,b.dmsxid FROM cg_htjhd a, cg_htjhdhw b,kc_sfdjmx c "
                                                // + "WHERE a.jhdid = b.jhdid AND b.jhdid= ";//
  private static final String BUY_ORDERGOODS_SQL
      = "SELECT a.djlx, a.jsfsid, b.jhdhwid,b.cpid,b.jhdid,b.hthwid,b.gyszyh, "
      + "(nvl(b.sl,0)-nvl((SELECT SUM(nvl(k.sl,0)) FROM kc_sfdjmx k WHERE k.wjid = b.jhdhwid AND k.djxz=1), 0)) sl, "
      + "(nvl(b.hssl,0)-nvl((SELECT SUM(nvl(k.hssl,0)) FROM kc_sfdjmx k WHERE k.wjid = b.jhdhwid AND k.djxz=1), 0)) hssl, "
      + "b.dj,b.je,b.bz,b.sjrkl,b.sjrkhsl,b.skpsl,b.skphsl,b.dmsxid,b.sfje,b.jhrq,b.ybje,b.dhqk "
      + "FROM cg_htjhd a, cg_htjhdhw b WHERE a.jhdid = b.jhdid AND b.jhdid= ";
  //查询数据库是否有记账的单据
  ...
  class Onchange implements Obactioner
  {
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      if(!oldDwtxid.equals(dwtxid)  || !oldsfdjlbid.equals(sfdjlbid)){
        EngineDataSet detail = getDetailTable();
        detail.first();
        while(detail.inBounds())
        {
        String wjid = detail.getValue("wjid");
        if(!wjid.equals("")&&detail.getRowCount()!=0)
        {
  ...
  class Master_Post implements Obactioner
  {
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
        ...
        for(int i=0; i<d_RowInfos.size(); i++)
      {
         ...
         if(!row.get("wjid").equals(""))
         {
           ...
           String count = dataSetProvider.getSequence("select count(*) from cg_htjhdhw b,cg_htjhd c where  c.jhdid=b.jhdid and  c.zt=1 and b.jhdhwid="+wjid);
           if(count.equals("0"))
           {
             data.setMessage(showJavaScript("alert('入库单明细中第"+(i+1)+"行所引到货单被取消审批，请核对审批后再保存!');"));
             return;
           }
         }
    ...
    private void uniteUniformRow(){
      ...
      for(int i=0; i<d_RowInfos.size(); i++){
        ...
        buf.append(dmsxid).append(",");
        combinStr = buf.toString();
        if(list.contains(combinStr)){
          int j=list.indexOf(combinStr);
          RowMap tempRow = (RowMap)d_RowInfos.get(j);
          double tempSl = detailrow.get("sl").length() > 0 ? Double.parseDouble(detailrow.get("sl")) : 0;
          double tempHssl = detailrow.get("hssl").length() > 0 ? Double.parseDouble(detailrow.get("hssl")) : 0;//换算数量
          String tSl = String.valueOf(tempSl+Double.parseDouble(tempRow.get("sl")));
          String tHssl = String.valueOf(tempHssl+Double.parseDouble(tempRow.get("hssl")));
          tempRow.put("sl",tSl);
          tempRow.put("hssl",tHssl);
          d_RowInfos.remove(i);
          detail.goToRow(i);
          detail.deleteRow();
        }else{
...
class Single_Select_Order implements Obactioner
{
  public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
  {
    ...
    putDetailInfo(data.getRequest());
    RowMap rowinfo = getMasterRowinfo();
    String rowdjlx = rowinfo.get("srlx");
    ...
    if (rowdjlx.equals("1"))//进货单
    {
      if(bjfs.equals("1"))
        SQL = SQL + " AND abs(nvl(b.hssl,0))>abs(nvl((SELECT SUM(nvl(k.hssl,0)) FROM kc_sfdjmx k WHERE k.wjid = b.jhdhwid AND k.djxz=1),0)) ";//如果以换算单位报价SQL加个条件换算数量大于实际入库换算数量
      else
        SQL = SQL + " AND abs(nvl(b.sl,0))>abs(nvl((SELECT SUM(nvl(k.sl,0)) FROM kc_sfdjmx k WHERE k.wjid = b.jhdhwid AND k.djxz=1),0)) ";//如果以计量单位报价SQL加个条件数量大于实际入库数量
    }
    else if (rowdjlx.equals("-1"))//退货单
    {
      if(bjfs.equals("1"))
        SQL = SQL + " AND abs(nvl(b.hssl,0))>abs(nvl((SELECT SUM(nvl(k.hssl,0)) FROM kc_sfdjmx k WHERE k.wjid = b.jhdhwid AND k.djxz=1),0))";//如果以换算单位报价SQL加个条件换算数量大于实际入库换算数量
      else
    ...
    //rowinfo.put("dj", singleRow.get("khlx"));
    ...
    for(int i=0; i < buyOrderGoodsData.getRowCount(); i++)
    {
      ...
      if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
      {
        ...
        double  dj  = buyOrderGoodsData.getValue("dj").length() > 0 ? Double.parseDouble(buyOrderGoodsData.getValue("dj")) : 0;//单价
        ...
        //String  djlx = buyOrderGoodsData.getValue("djlx").length() > 0 ? buyOrderGoodsData.getValue("djlx") : "1";
        String djlx = buyOrderGoodsData.getValue("djlx");//-1表示采购退货
        double wrkhsl = djlx.equals("-1")?-1*hssl:hssl;//未入库换算数量
        double wrksl = djlx.equals("-1")?-1*sl:sl;//未入库数量
        double je = buyOrderGoodsData.getValue("je").length() > 0 ? Double.parseDouble(buyOrderGoodsData.getValue("je")) : 0;//实际数量
        je = djlx.equals("-1")?-1*je:je;
        dsDetailTable.insertRow(false);
        ...
        dsDetailTable.setValue("je", formatNumber(String.valueOf(je), sumFormat));
    ...
    public void execute(String action, Obationable o, RunData data, Object arg) throws Exception
    {
      ...
      for(int i=0; i < jhdhwIDs.length; i++)
      {
        ...
        if(!dsDetailTable.locate(locateGoodsRow, Locate.FIRST))
        {
          ...
          dsDetailTable.setValue("hssl",String.valueOf(wrkhsl));
