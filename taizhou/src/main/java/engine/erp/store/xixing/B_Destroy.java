...
public final class B_Destroy extends BaseAction implements Operate
{
  ...
  private static String MASTER_SQL       = "SELECT * FROM kc_sfdj WHERE ? AND ? AND djxz=? and fgsid=? ? ORDER BY sfrq DESC";
   ...
   * @return 返回入库单列表的实例
   */
  public static B_Destroy getInstance(HttpServletRequest request)
  {
    B_Destroy destroyBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "destroyBean";
      destroyBean = (B_Destroy)session.getAttribute(beanName);
      if(destroyBean == null)
      {
        //引用LoginBean
        LoginBean loginBean = LoginBean.getInstance(request);

        destroyBean = new B_Destroy();
        ...
        destroyBean.loginDept = loginBean.getDeptID();
        destroyBean.user = loginBean.getUser();
        //destroyBean.bjfs = loginBean.getSystemParam("BUY_PRICLE_METHOD");
        //设置格式化的字段
