...
/**
 * <p>Title: 库存管理—销售出库单引入提单主表</p>
 * <p>Description: 库存管理—销售出库单引入提单主表</p>
 ...
 */
public final class B_SingleLadding extends BaseAction implements Operate
{
  //2004-4-23 17:37 修改 VW_SALEOUTSTORE_SINGLE_LADDING加入了isrefer条件 yjg
  private static final String SINGLE_LADDING_SQL = "SELECT * FROM VW_SALEOUTSTORE_SINGLE_LADDING WHERE fgsid= ";
  private static final String DETAIL_STRUT_SQL   = "SELECT * FROM VW_SALE_OUTSTORE WHERE 1<>1";
  private static final String DETAIL_SQL         = "SELECT * FROM VW_SALE_OUTSTORE WHERE tdId ='?'";//
   ...
   */
  public static B_SingleLadding getInstance(HttpServletRequest request)
  {
    B_SingleLadding singleLaddingBean = null;
    HttpSession session = request.getSession(true);
    synchronized (session)
    {
      String beanName = "singleLaddingBean";
      singleLaddingBean = (B_SingleLadding)session.getAttribute(beanName);
      if(singleLaddingBean == null)
      {
        singleLaddingBean = new B_SingleLadding();
        LoginBean loginBean = LoginBean.getInstance(request);
        singleLaddingBean.loginId = loginBean.getUserID();
        singleLaddingBean.loginName = loginBean.getUserName();
        singleLaddingBean.loginDept = loginBean.getDeptID();

        singleLaddingBean.qtyFormat = loginBean.getQtyFormat();
        singleLaddingBean.sumFormat = loginBean.getSumFormat();

        singleLaddingBean.fgsid = loginBean.getFirstDeptID();
        singleLaddingBean.dsLadding.setColumnFormat("zsl", singleLaddingBean.qtyFormat);
        singleLaddingBean.dsLadding.setColumnFormat("zje", singleLaddingBean.sumFormat);
        session.setAttribute(beanName, singleLaddingBean);
      }
    }
    return singleLaddingBean;
  }

  /**
   * 构造函数
   */
  private B_SingleLadding()
  {
    try {
      jbInit();
    }
