...
/**
 * Title: 自定义的Beans Description: 
 * 1.客户端TableDatSet的Provide和Resolve
 * 2.服务端更新提交的数据到数据库 
 * 
...
public abstract class CommonClass implements HttpSessionBindingListener, java.io.Serializable {
	/**
	 * 
	 */
	 ...
	 * @return 返回格式化过的字符串
	 */
	public synchronized static String formatNumber(double d, String pattern)// throws
																			// Exception
	{
	 ...
	 * @return 包含option的字符串
	 */
	public synchronized static String dataSetToOption(DataSet ds, String idColumn, String capColumn, String selectIdValue, String existColumn, String existValue) {
		...
		for (int i = 0; i < ds.getRowCount(); i++) {
			...
			String id = EngineDataSet.getValue(ds, ds.getColumn(idColumn).getOrdinal());
			...
			buf.append(EngineDataSet.getValue(ds, ds.getColumn(capColumn).getOrdinal()));
			buf.append("</option>");
			ds.next();
		}
		return buf.toString();
	...
	public synchronized static String showMessage(String value, boolean isError) {
		 ...
		 * <tr><td><fieldset> <legend align='center'>信息</legend>
		 * <div align='left' style='color: red'>&nbsp;123&nbsp;</div>
		 * </fieldset> </td></tr></table>
		 */
		String table = "<table align='center' cellspacing='1' cellpadding='0'><tr><td><fieldset><legend align='center'>" + (isError ? "错误" : "信息") + "</legend><div align='left'"
	 ...
	 * @return 若返回null表示是数字，非null为javasrcip语句
	 */
	public static String checkNumber(String value, String caption, boolean canZero) {
		if (value.equals(""))
			return showJavaScript("alert('" + caption + " 不能为空！');");
		try {
	 ...
	 * @return 若返回null表示是数字，非null为javasrcip语句
	 */
	public static String checkNumber(String value, String caption, double min, double max) {
		if (value.equals(""))
			return showJavaScript("alert('" + caption + " 不能为空！');");
		double dValue;
		try {
